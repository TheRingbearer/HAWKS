...
import org.apache.ode.dao.fc.jpa.FCManagementDAOImpl;
...
public class BPELDAOConnectionImpl implements BpelDAOConnection {
	...
	public ProcessInstanceDAO getInstance(Long iid) {
		ProcessInstanceDAOImpl instance = _em.find(
				ProcessInstanceDAOImpl.class, iid);
		return instance;
	}
		...
		if (criteria != null) {
			...
			if (criteria.getPidFilter() != null) {
				...
				for (int m = 0; m < pids.size(); m++) {
					filters.append(" pi._process._processId = '")
							.append(pids.get(m)).append("'");
					if (m < pids.size() - 1)
						filters.append(" or");
				}
				clauses.add(" (" + filters + ")");
			...
			if (criteria.getPropertyValuesFilter() != null) {
				...
				for (String propKey : props.keySet()) {
					...
					clauses.add(" csp" + i + ".propertyKey = '" + propKey
							...
							// spaces have to be escaped, might be better
							// handled in InstanceFilter
	...
	public Map<Long, Collection<CorrelationSetDAO>> getCorrelationSets(
		...
		Collection<CorrelationSetDAOImpl> csets = _em
				.createNamedQuery(
						CorrelationSetDAOImpl.SELECT_CORRELATION_SETS_BY_INSTANCES)
				.setParameter("instances", iids).getResultList();
	...
	public Collection<CorrelationSetDAO> getActiveCorrelationSets() {
		return _em.createNamedQuery(CorrelationSetDAOImpl.SELECT_ACTIVE_SETS)
				.setParameter("state", ProcessState.STATE_ACTIVE)
				.getResultList();
	}
	...
	public FCManagementDAO getFCManagement() {
		return new FCManagementDAOImpl(_em);
	}
	// AH: end
}
