...
public class CorrelatorDAOImpl extends OpenJPADAO implements CorrelatorDAO {
	...
	@OneToMany(targetEntity = MessageRouteDAOImpl.class, mappedBy = "_correlator", fetch = FetchType.EAGER, cascade = {
			CascadeType.MERGE, CascadeType.PERSIST, CascadeType.REFRESH })
	private Collection<MessageRouteDAOImpl> _routes = new ArrayList<MessageRouteDAOImpl>();
	@OneToMany(targetEntity = MessageExchangeDAOImpl.class, mappedBy = "_correlator", fetch = FetchType.LAZY, cascade = {
			CascadeType.MERGE, CascadeType.PERSIST, CascadeType.REFRESH })
	...
	public void addRoute(String routeGroupId, ProcessInstanceDAO target,
			int index, CorrelationKeySet correlationKeySet, String routePolicy) {
		if (__log.isDebugEnabled()) {
			__log.debug("addRoute " + routeGroupId + " " + target + " " + index
					+ " " + correlationKeySet + " " + routePolicy);
		}
	...
	public MessageExchangeDAO dequeueMessage(CorrelationKeySet correlationKeySet) {
		...
		// correlator???
		for (Iterator<MessageExchangeDAOImpl> itr = _exchanges.iterator(); itr
				.hasNext();) {
	...
	public List<MessageRouteDAO> findRoute(CorrelationKeySet correlationKeySet) {
		...
		Query qry = getEM().createQuery(
				generateSelectorQuery(ROUTE_BY_CKEY_HEADER, subSets));
		...
		List<MessageRouteDAO> candidateRoutes = (List<MessageRouteDAO>) qry
				.getResultList();
	...
	void removeLocalRoutes(String routeGroupId, ProcessInstanceDAO target) {
		...
		boolean flush = false;
		for (Iterator<MessageRouteDAOImpl> itr = _routes.iterator(); itr
				.hasNext();) {
