...
public interface BpelServer {
	...
	/**
	 * Configure the {@BpelEngine} with a message-exchange context.
	 * BPEL engine uses this context to initiate communication with external
	 * services.
	 * 
	 * @see MessageExchangeContext
	 * @param mexContext
	 *            {@link MessageExchangeContext} implementation
	 */
	...
	/**
	 * Configure the {@BpelEngine} with an endpoint-reference (EPR)
	 * context. BPEL engine uses this context to EPRs.
	 * 
	 * @see EndpointReferenceContext
	 * @param eprContext
	 *            {@link EndpointReferenceContext} implementation
	 */
	void setEndpointReferenceContext(EndpointReferenceContext eprContext)
			throws BpelEngineException;

	/**
	 * Configure the {@BpelEngine} with a binding context. The BPEL
	 * engine uses this context to register the services that it exposes and
	 * obtain communication links to partner services.
	 * 
	 * @see BindingContext
	...
	/**
	 * Initialize the BPEL engine. The various contexts needed by the engine
	 * must be configured before this method is called.
	 */
	void init() throws BpelEngineException;

	/**
	 * Start the BPEL engine. The BPEL engine will not execute process instances
	 * until it is started.
	 */
	...
	// @hahnml
	/**
	 * Updates the process instance meta data after migration. For example this
	 * method changes the deprecated process instance ID to the correct new one.
	 * 
	 * @param oldInstanceID
	 * @param currentVersion
	 * 
	 */
	void migrateProcessInstanceMetaData(QName newProcessID, Long instanceID,
			Long currentVersion);

	// @hahnml
	/**
	 * Provides the OProcess object for the given instanceID.
	 * 
	 * @param instanceID
	 *            The id of the instance to get the process model of.
	 * 
	 * @return The OProcess of the given instance.
	 */
	OProcess getProcessModel(Long instanceID);
