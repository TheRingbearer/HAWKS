 ...
 * </ul>
 * 
 * The first example shows an empty correlation key set. The second shows a set
 * with one correlation key inside. The third shows a set with two keys inside.
 * The correlation keys are sorted by the correlation set ids.
 * 
 * @author sean
 * 
 */
public class CorrelationKeySet implements Serializable {
	...
	private final Set<CorrelationKey> correlationKeys = new TreeSet<CorrelationKey>(
			new CorrelationKeyComparator());
	...
	/**
	 * Restores the state by parsing the given canonical form of correlation key
	 * set.
	 * 
	 * @param canonicalForm
	 *            canonical form of correlation key set
	 */
	...
	/**
	 * Adds a correlation key to this correlation key set. If a correlation key
	 * with the same correlation set id already exists, the old one is replaced
	...
	/**
	 * Checks if this correlation key set contains the opaque correlation key as
	 * the only key in this correlation key set.
	 * 
	 * @return returns true if the correlation key set is opaque
	 */
	public boolean isOpaque() {
		return correlationKeys.size() == 1
				&& correlationKeys.iterator().next().getCorrelationSetName()
						.equals("-1");
	}

	/**
	 * Checks if an incoming message with this correlation key set can be
	 * accepted by the given correlation key set.
	 * 
	 * @param candidateKeySet
	 *            a correlation key set stored in a route
	 * @param isAllRoute
	...
	/**
	 * Checks if this correlation key set contains all correlation keys from the
	 * given correlation key set.
	 * 
	 * @param c
	...
	/**
	 * Returns true if this correlation key set contains the give correlation
	 * key.
	 * 
	...
	/**
	 * Returns an iterator on the correlation keys that this correlation key set
	 * contains.
	 * 
	 * @return an iterator
	 */
	...
	public List<CorrelationKeySet> findSubSets() {
		...
		// if the key set contains a opaque key and at least one non-opaque key,
		// take out the opaque key
		...
		// we are generating (2 powered by the number of correlation keys)
		// number of sub-sets
	...
	public String toCanonicalString() {
		...
		for (CorrelationKey ckey : correlationKeys) {
			...
			buf.append("[")
					.append(escapeRightBracket(ckey.toCanonicalString()))
					.append("]");
		}
	...
	// parses a canonical form of correlation key set through an automata
	// subsystem(FSM)
	private void parseCanonicalForm(String canonicalForm) {
		...
		for (int i = 0; i < canonicalForm.length(); i++) {
			...
			if (state == ParserState.INITIAL) {
				...
				} else if (ch == '?') { // this is only a convenient feature for
					// testing
					if (buf.toString().trim().length() != 0) {
	...
	private class CorrelationKeyComparator implements Serializable,
		...
		public int compare(CorrelationKey o1, CorrelationKey o2) {
			...
			// used only in sorting the correlation keys in the
			// CorrelationKeySet; does not matter with the values
			return o1.getCorrelationSetName().compareTo(
