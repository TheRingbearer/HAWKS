...
public class InstanceFilter extends Filter<InstanceFilter.Criteria> implements
	...
	/**
	 * If set, will filter on the instance id (IID) and select all matching
	 * instances
	 */
	private List<String> iids;

	/**
	 * If set, will filter on the process id (PID) and select all matching
	 * process definitions
	 */
	...
	enum Criteria {
		IID {
			void process(InstanceFilter filter, String key, String op,
					String value) {
		...
		PROPERTY {
			void process(InstanceFilter filter, String key, String op,
				...
				filter.propertyValuesFilter.put(key.substring(1, key.length()),
						value);
			}
		};
	...
	public InstanceFilter(String filter, String orderKeys, int limit) {
		...
		if (orderKeys != null && orderKeys.length() > 0) {
			...
			for (StringTokenizer orderKeysTok = new StringTokenizer(orderKeys,
				...
				try {
					...
					OrderKeys.valueOf(justKey.replaceAll("-", "_")
							.toUpperCase());
					orders.add(orderKey);
				} catch (IllegalArgumentException e) {
					throw new InvalidRequestException(
		...
		if (limit < 0) {
			throw new IllegalArgumentException(
					"Limit should be greater or equal to 0.");
		}
		this.limit = limit;
	}
	...
	public List<Short> convertFilterState() {
		...
		for (String status : statusFilter) {
			...
			} else if (StatusKeys.TERMINATED.toString()
	...
	public static void main(String[] args) {
		InstanceFilter instf = new InstanceFilter(
				"name = dtc* namespace=http://www.intalio.com* "
						...
						+ "${http://ode.org/}order-id= 12 $shipping-id=aa45fz",
				"name started", 50);
