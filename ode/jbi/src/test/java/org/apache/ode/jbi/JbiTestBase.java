...
public class JbiTestBase extends SpringTestSupport {
	...
	protected AbstractXmlApplicationContext createBeanFactory() {
		return new ClassPathXmlApplicationContext(new String[] {
				"/smx-base.xml", "/" + getTestName() + "/smx.xml" });
	}
	...
	protected void setUp() throws Exception {
		...
		ComponentContextImpl cc = new ComponentContextImpl(jbiContainer,
				new ComponentNameSpace(jbiContainer.getName(), "ODE"));
		ActivationSpec activationSpec = new ActivationSpec();
		activationSpec.setComponent(odeComponent);
		activationSpec.setComponentName("ODE");
		jbiContainer.activateComponent(
				new File("target/test/smx/ode").getAbsoluteFile(),
				odeComponent, "", cc, activationSpec, true, false, false, null);
	...
	protected void enableProcess(String resource, boolean enable)
			throws Exception {
		...
		try {
			...
			do {
				...
				if (request != null && request.startsWith("@")) {
					request = inputStreamToString(getClass()
							.getResourceAsStream(
									"/" + getTestName() + "/"
											+ request.substring(1)));
				}
				...
				}
				{
					...
					if (httpUrl != null && request != null) {
						...
						URLConnection connection = new URL(httpUrl)
								.openConnection();
						...
						wt.close();
						// Read the response.
						String result = inputStreamToString(connection
								.getInputStream());
				...
				}
				{
					if (testProperties.getProperty(prefix + "nmr.service") != null
						...
						io.setService(QName.valueOf(testProperties
								.getProperty(prefix + "nmr.service")));
						io.setOperation(QName.valueOf(testProperties
								.getProperty(prefix + "nmr.operation")));
						...
						if (io.getStatus() == ExchangeStatus.ACTIVE) {
							...
							String result = new SourceTransformer()
	...
	protected void matchResponse(String expectedResponse, String result,
		...
		if (succeeded) {
			assertTrue("Response doesn't match expected regex.\nExpected: "
					+ expectedResponse + "\nReceived: " + result, Pattern
					.compile(expectedResponse, Pattern.DOTALL).matcher(result)
					.matches());
		} else {
