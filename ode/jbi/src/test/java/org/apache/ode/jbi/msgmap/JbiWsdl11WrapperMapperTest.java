...
public class JbiWsdl11WrapperMapperTest extends junit.framework.TestCase {
	private static Log __log = LogFactory
			.getLog(JbiWsdl11WrapperMapperTest.class);
		...
		public Source getContent() {
			try {
				return new DOMSource(
						DOMUtils.stringToDOM(IOUtils.toString(getClass()
	...
	public void testFault() throws Exception {
		...
		Definition w = javax.wsdl.factory.WSDLFactory.newInstance()
				.newWSDLReader()
				.readWSDL(getClass().getResource("/test.wsdl").getFile());
		{
			Fault f = new MockJbiFault("/fault.xml");
			javax.wsdl.Fault k = m
					.toFaultType(
							f,
							((Operation) w
									.getPortType(
											QName.valueOf("{http://www.example.org/test/}test"))
									.getOperations().get(0)).getFaults()
									.values());
