...
public class OdeComponent implements Component {
	...
	public Document getServiceDescription(ServiceEndpoint ref) {
		return _lifeCycle.getOdeContext().getServiceDescription(
				ref.getServiceName());
	}
	 ...
	 * interact with the the provider completely. Ths provider is described by
	 * the given capabilities, and JBI has already ensure that a fit exists
	 ...
	 * consists of simple set matching based on capability names only. <br>
	 * <br>
	 * Note that JBI assures matches on capability names only; it is the
	 * responsibility of this method to examine capability values to ensure a
	 ...
	 * consumer. This is called by JBI when it is attempting to resolve the
	 * given endpoint reference on behalf of a component.
	 * 
	 * @param epr
