...
public class ServiceMixMapper extends BaseXmlMapper implements Mapper {
	...
	public Recognized isRecognized(NormalizedMessage nmsMsg, Operation op) {
		...
		if (op.getInput().getMessage().getParts().size() == 1) {
			Part part = (Part) op.getInput().getMessage().getParts().values()
					.iterator().next();
			QName elementName = part.getElementName();
			if (elementName != null
					&& elementName.getLocalPart().equals(msg.getLocalName())
					&& elementName.getNamespaceURI().equals(
							msg.getNamespaceURI())) {
		...
		for (String pname : ((Set<String>) op.getInput().getMessage()
				.getParts().keySet())) {
			...
			if (part.getElementName() != null) {
				...
				__log.debug("Part " + part.getName() + " has element content "
						+ part.getElementName()
						+ ". It's not allowed for RPC style.");
				return Recognized.FALSE;
			}

			// with RPC semantic the body is wrapped by a partName which is same
			// as bodyElementName
			Element pdata = DOMUtils.findChildByName(msg,
	...
	public void toNMS(NormalizedMessage nmsMsg, Message odeMsg,
		...
		if (headers != null) {
			for (String header : headers.keySet()) {
				...
				Map<QName, DocumentFragment> headers2 = (Map<QName, DocumentFragment>) nmsMsg
						.getProperty("org.apache.servicemix.soap.headers");
				if (headers2 == null) {
				...
				DocumentFragment f = v.getOwnerDocument()
		...
		Element firstPartEl = part == null ? null : DOMUtils
				.getFirstChildElement(part);

		if (fault != null) {
			// We treat faults seperately as there are some assumption we can
			// make, mainly that there is
			// a single part and it is an element part.

			if (msgdef.getParts().size() != 1)
				throw new MessageTranslationException("Message for fault \""
						+ fault
						+ "\" does not contain exactly one part! Cannot map!");

			Part partDef = (Part) msgdef.getParts().values().iterator().next();
			if (partDef.getElementName() == null)
				throw new MessageTranslationException("Message for fault \""
						+ fault + "\" does not contain an element part.");

			if (firstPartEl == null) {
				// Oooops, our assumption did not pan out; we'll do our best
				// i.e. create empty content.
				__log.warn("Proceessing fault \"" + fault
						+ "\" with empty content (check your BPEL).");

				Document doc = newDocument();
				Element content = doc.createElementNS(partDef.getElementName()
						.getNamespaceURI(), partDef.getElementName()
						.getLocalPart());
				doc.appendChild(content);
				if (__log.isDebugEnabled())
					__log.debug("toNMS() ode message (fault, BS): "
							+ prettyPrint(content));
				nmsMsg.setContent(new DOMSource(doc));
			} else {
				if (__log.isDebugEnabled())
					__log.debug("toNMS() ode message (fault): "
							+ prettyPrint(firstPartEl));
		...
		if (msgdef.getParts().size() == 0) {
			...
			return;
		} else if (msgdef.getParts().size() != 1
				|| ((Part) msgdef.getParts().values().iterator().next())
						.getElementName() == null) {
			// If we have more than one part, or a single non-element part, then
			// we can't use the standard
			// NMS doc-lit like convention. Instead we place the entire message
			// on the bus and hope for the
			// best.
			if (__log.isDebugEnabled())
				__log.debug("toNMS() ode message (rpc-like): "
						+ prettyPrint(ode));
		...
		if (__log.isDebugEnabled())
			__log.debug("toNMS() normalized message (doc-like):"
					+ prettyPrint(firstPartEl));
		nmsMsg.setContent(new DOMSource(firstPartEl));
	}
	...
	public void toODE(Message odeMsg, NormalizedMessage nmsMsg,
		...
		for (String pname : ((Set<String>) msgdef.getParts().keySet())) {
			...
			// servicemix-http has a (bad) habit of placing the SOAP body
			// content directly in the normalized message
			QName elementName = part.getElementName();
			if (elementName != null
					&& elementName.getLocalPart().equals(nms.getLocalName())
					&& elementName.getNamespaceURI().equals(
							nms.getNamespaceURI())) {
				docLit = true;
				break;
		...
		Map<QName, DocumentFragment> headers = (Map<QName, DocumentFragment>) nmsMsg
				.getProperty("org.apache.servicemix.soap.headers");
		if (headers != null) {
			for (QName header : headers.keySet()) {
				...
				try {
					odeMsg.setHeaderPart(header.getLocalPart(), DOMUtils
							.stringToDOM(DOMUtils.domToString(headers
									.get(header))));
				} catch (Exception e) {
