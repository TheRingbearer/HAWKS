...
abstract class OdeConsumer extends ServiceBridge implements
	...
	private static final long DEFAULT_RESPONSE_TIMEOUT = Long.getLong(
			"org.apache.ode.jbi.timeout", 2 * 60 * 1000L);
	...
	public void invokePartner(final PartnerRoleMessageExchange odeMex)
		...
		MessageExchangeFactory mexf = _ode.getChannel().createExchangeFactory(
				se);
		final MessageExchange jbiMex;
		try {
			jbiMex = mexf
					.createExchange(isTwoWay ? MessageExchangePattern.IN_OUT
			...
			odeMex.replyWithFailure(FailureType.COMMUNICATION_ERROR, errmsg,
					null);
			return;
		}
		...
		try {
			if (!isTwoWay) {
				...
				mapper.toNMS(nmsg, odeMex.getRequest(), odeMex.getOperation()
						.getInput().getMessage(), null);
	...
	protected abstract void doSendOneWay(PartnerRoleMessageExchange odeMex,
			InOnly inonly);
	...
	public void onJbiMessageExchange(MessageExchange jbiMex)
		...
		if (!jbiMex.getPattern().equals(MessageExchangePattern.IN_ONLY)
			...
			__log.error("JBI MessageExchange " + jbiMex.getExchangeId()
		...
		if (jbiMex.getStatus() == ExchangeStatus.ACTIVE) {
			...
			__log.error("Unexpected status " + jbiMex.getStatus()
					+ " for JBI message exchange: " + jbiMex.getExchangeId());
		}
	}

	private void outFailure(final InOut jbiMex) {
		final PartnerRoleMessageExchange pmex = _outstandingExchanges
				.remove(jbiMex.getExchangeId());
		if (pmex == null) {
		...
		try {
			_ode._scheduler.execTransaction(new Callable<Boolean>() {
				public Boolean call() throws Exception {
					pmex.replyWithFailure(FailureType.OTHER,
							"Error: " + jbiMex.getError(), null);
					return null;
				}
			});
		} catch (Exception ex) {
	...
	private void outResponse(final InOut jbiMex) {
		final PartnerRoleMessageExchange outstanding = _outstandingExchanges
				.remove(jbiMex.getExchangeId());
		if (outstanding == null) {
			__log.warn("Received a response for unknown JBI message exchange "
					+ jbiMex.getExchangeId());
				...
				public Boolean call() throws Exception {
					...
					PartnerRoleMessageExchange pmex = (PartnerRoleMessageExchange) _ode._server
							.getEngine().getMessageExchange(
									outstanding.getMessageExchangeId());
					if (pmex == null) {
					...
					Mapper mapper = mapperName == null ? _ode
							.getDefaultMapper() : _ode.getMapper(mapperName);
					if (mapper == null) {
						String errmsg = "Mapper not found.";
						__log.error(errmsg);
						pmex.replyWithFailure(FailureType.FORMAT_ERROR, errmsg,
								null);
					} else {
						try {
							Fault jbiFlt = jbiMex.getFault();
							if (jbiFlt != null) {
								javax.wsdl.Fault wsdlFlt = mapper.toFaultType(
										jbiFlt,
										(Collection<javax.wsdl.Fault>) pmex
												.getOperation().getFaults()
												.values());
								if (wsdlFlt == null) {
									pmex.replyWithFailure(
											FailureType.FORMAT_ERROR,
											"Unrecognized fault message.", null);
								} else {
									if (wsdlFlt.getMessage() != null) {
										Message faultResponse = pmex
												.createMessage(wsdlFlt
														.getMessage()
														.getQName());
										mapper.toODE(faultResponse, jbiFlt,
												wsdlFlt.getMessage());
										pmex.replyWithFault(
												new QName(pmex.getPortType()
														.getQName()
														.getNamespaceURI(),
														wsdlFlt.getName()),
												faultResponse);
										...
										__log.fatal("Internal Error: fault found without a message type: "
												+ wsdlFlt);
										pmex.replyWithFailure(
												FailureType.FORMAT_ERROR,
												"Fault has no message: "
														+ wsdlFlt.getName(),
												null);
									}
								}
								...
								Message response = pmex.createMessage(pmex
										...
										.getMessage().getQName());
								mapper.toODE(response, jbiMex.getOutMessage(),
										pmex.getOperation().getOutput()
												.getMessage());
								pmex.reply(response);
							}
						} catch (MessageTranslationException mte) {
							__log.error("Error translating message.", mte);
							pmex.replyWithFailure(FailureType.FORMAT_ERROR,
									mte.getMessage(), null);
						}
					}
					return null;
