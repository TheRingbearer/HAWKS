...
class OdeServiceUnit {
	...
	/** Ctor. */
	OdeServiceUnit(OdeContext ode, String serviceUnitID,
	...
	public void deploy() throws DeploymentException {
		...
		try {
			...
			String errmsg = __msgs.msgOdeProcessDeploymentFailed(
					_serviceUnitRootPath, _serviceUnitID);
	...
	public void undeploy() throws Exception {
		try {
			Collection<QName> undeployed = _ode._store
					.undeploy(_serviceUnitRootPath);
	...
	public void start() throws Exception {
		List<QName> pids = _ode._store.listProcesses(_serviceUnitRootPath
				.getName());
		if (pids == null) {
			__log.error(_serviceUnitRootPath.getName()
					+ " not found in process store. ");
	...
	public ClassLoader getConfigurationClassLoader() throws DeploymentException {
		return new URLClassLoader(getDefaultLocations(), getClass()
				.getClassLoader());
	}

	protected URL[] getDefaultLocations() throws DeploymentException {
		try {
			File[] jars = new File(_serviceUnitRootPath, LIB_DIR)
			...
			if (__log.isDebugEnabled()) {
				for (URL u : urls) {
					__log.debug("in classpath for " + _serviceUnitID
							+ " using url: " + u);
				}
			}
			return urls;
