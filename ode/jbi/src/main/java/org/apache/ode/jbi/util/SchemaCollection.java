...
public class SchemaCollection {
	...
	public void read(String location, URI baseUri) throws Exception {
		if (log.isDebugEnabled()) {
			log.debug("Reading schema at '" + location + "' with baseUri '"
					+ baseUri + "'");
		}
		if (baseUri == null) {
		...
		if (baseUri != null) {
			...
			if (!loc.isAbsolute()) {
				throw new IllegalArgumentException("Unable to resolve '"
						+ loc.toString() + "' relative to '" + baseUri + "'");
			}
		} else {
			...
			if (!loc.isAbsolute()) {
				throw new IllegalArgumentException("Location '"
						+ loc.toString()
						+ "' is not absolute and no baseUri specified");
			}
		}
	...
	public void read(InputSource inputSource) throws Exception {
		DocumentBuilderFactory docFac = XMLParserUtils
				.getDocumentBuilderFactory(); // don't trust system provided
		// parser!
		docFac.setNamespaceAware(true);
		DocumentBuilder builder = docFac.newDocumentBuilder();
		Document doc = builder.parse(inputSource);
		read(doc.getDocumentElement(),
				inputSource.getSystemId() != null ? new URI(inputSource
						.getSystemId()) : null);
	}
	...
	protected void handleImports(Schema schema) throws Exception {
		...
		for (int i = 0; i < children.getLength(); i++) {
			...
			if (child instanceof Element) {
				...
				if ("http://www.w3.org/2001/XMLSchema".equals(ce
						.getNamespaceURI())
