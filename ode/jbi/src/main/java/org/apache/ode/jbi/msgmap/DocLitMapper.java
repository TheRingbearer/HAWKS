...
public class DocLitMapper extends BaseXmlMapper implements Mapper {
	...
	public Recognized isRecognized(NormalizedMessage nmsMsg, Operation op) {
		...
		Message out = op.getOutput() == null ? null : op.getOutput()
				.getMessage();
		...
		return inpart.getElementName().equals(elName) ? Recognized.TRUE
				: Recognized.FALSE;
	}
	...
	public void toNMS(NormalizedMessage nmsMsg,
		...
		if (msgdef == null && fault != null) {
			...
			Element content = doc.createElementNS(fault.getNamespaceURI(),
					fault.getLocalPart());
		...
		Element part = DOMUtils.findChildByName(odeMsg.getMessage(), new QName(
				null, partdef.getName()));
		if (part == null) {
			String errmsg = "ODE message did not contain expected part: "
					+ partdef.getName();
			__log.debug(errmsg);
			throw new MessageTranslationException(errmsg);
		}

		Element content = DOMUtils.findChildByName(part,
				partdef.getElementName());
		if (content == null) {
			String errmsg = "ODE message did not contain element "
					+ partdef.getElementName() + " in part: "
					+ partdef.getName();
			__log.debug(errmsg);
			throw new MessageTranslationException(errmsg);
		}
	...
	public Fault toFaultType(javax.jbi.messaging.Fault jbiFlt,
		...
		for (Fault f : faults) {
			if (f.getMessage() == null || f.getMessage().getParts().size() != 1)
				continue;
			javax.wsdl.Part pdef = (Part) f.getMessage().getParts().values()
					.iterator().next();
