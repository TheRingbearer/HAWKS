 ...
 * {@link org.apache.ode.bpel.iapi.EndpointReferenceContext} interface used by
 * the BPEL engine to convert XML descriptions of endpoint references (EPRs)
 * into Java object representations. In the JBI context all endpoint references
 * are considered to be JBI {@link javax.jbi.servicedesc.ServiceEndpoint}s are
...
public class EndpointReferenceContextImpl implements EndpointReferenceContext {
	private static final Log __log = LogFactory
			.getLog(EndpointReferenceContextImpl.class);

	private final OdeContext _ode;

	static final QName JBI_EPR = new QName(
	...
	public EndpointReference resolveEndpointReference(Element epr) {
		...
		if (!elname.equals(EndpointReference.SERVICE_REF_QNAME))
			throw new IllegalArgumentException("EPR root element " + elname
					+ " should be " + EndpointReference.SERVICE_REF_QNAME);
		...
		ServiceEndpoint se = _ode.getContext().resolveEndpointReference(
				fragment);
		if (se == null)
			return null;
	...
	public Map getConfigLookup(EndpointReference epr) {
		Map m = new HashMap();
		m.put("service", ((JbiEndpointReference) epr).getServiceEndpoint()
				.getServiceName());
		return m;
	}
}
