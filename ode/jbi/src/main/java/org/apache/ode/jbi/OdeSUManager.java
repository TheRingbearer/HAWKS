...
public class OdeSUManager implements ServiceUnitManager {
	...
	public synchronized void start(String serviceUnitID)
		...
		try {
			Thread.currentThread().setContextClassLoader(
					su.getConfigurationClassLoader());
			su.start();
		} catch (Exception ex) {
	 ...
	 * mention how lame this is? If not, let me remind the reader: this is just
	 * about the lamest "clever idea" I have ever seen.
	 * 
	 * @param task
	...
	private String makeStatusMessage(String task, String status) {
		 ...
		 * <component-name>BC1</component-name> <component-task-result-details
		 * xmlns="http://java.sun.com/xml/ns/jbi/management-
		 * <task-result-details> <task-id>deploy</task-id>
		 * <task-result>SUCCESS</task-result> </task-result-details>
		...
		Document doc;
		try {
			// Note that we are using our own choice of factory (xerces), not
			// the
