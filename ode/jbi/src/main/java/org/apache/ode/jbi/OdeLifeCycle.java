...
public class OdeLifeCycle implements ComponentLifeCycle {
	...
	public void init(ComponentContext context) throws JBIException {
		try {
			...
			// Use system property to determine if DeliveryChannel.sendSync or
			// DeliveryChannel.send is used.
			...
			// @hahnml: Bugfix 302, see
			// https://issues.apache.org/jira/secure/attachment/12384082/ODE302.patch.txt
			...
			case INTERNAL:
				try {
					...
					throw new RuntimeException(
							"Unable to create Geronimo Transaction Manager", e);
				}
				break;
			default:
				break;
		...
		for (String className : mappers) {
			try {
				...
				String errmsg = __msgs
						.msgOdeInitMapperClassLoadFailed(className);
				__log.error(errmsg);
			...
			try {
				...
				String errmsg = __msgs
						.msgOdeInitMapperInstantiationFailed(className);
				__log.error(errmsg);
				throw new JBIException(errmsg, t);
			}
		}
	...
	private void initProperties() throws JBIException {
		OdeConfigProperties config = new OdeConfigProperties(new File(_ode
		...
		try {
			...
			String errmsg = __msgs.msgOdeInstallErrorCfgReadError(config
					.getFile());
			throw new JBIException(errmsg, ex);
		}
		_ode._config = config;
	...
	private void initBpelServer() throws JBIException {
		...
		if (_ode._config.getThreadPoolMaxSize() == 0)
			...
			_ode._executorService = Executors.newFixedThreadPool(_ode._config
					.getThreadPoolMaxSize());
		_ode._scheduler = new SimpleScheduler(new GUID().toString(),
				new JdbcDelegate(_ode._dataSource),
				_ode._config.getProperties());
		_ode._scheduler.setJobProcessor(_ode._server);
		_ode._scheduler.setExecutorService(_ode._executorService);

		// @hahnml: Bugfix 302, see
		// https://issues.apache.org/jira/secure/attachment/12384082/ODE302.patch.txt
		_ode._scheduler.setTransactionManager(_ode.getTransactionManager());
		...
		gencon.setActiveMQURL(_ode._config.getActiveMQURL());
		gencon.start(_ode._store);

		IncomingMessageHandler incMess = IncomingMessageHandler.getInstance();
		incMess.setScheduler(_ode._scheduler);

		_ode._store.loadAll();

		_ode._server
				.setInMemDaoConnectionFactory(new org.apache.ode.bpel.memdao.BpelDAOConnectionFactoryImpl(
	...
	private void initDao() throws JBIException {
		...
		try {
			...
			String errmsg = __msgs.msgDAOInstantiationFailed(_ode._config
	...
	private void initConnector() throws JBIException {
		...
		if (port == 0) {
			...
			try {
				...
				__log.debug(
						"Failed to initialize JCA connector (check security manager configuration)",
						e);
	...
	private void registerMBean() throws JBIException {
		ProcessAndInstanceManagementMBean pmapi = new ProcessAndInstanceManagementMBean(
				_ode._server, _ode._store);
		MBeanServer server = _ode.getContext().getMBeanServer();
		try {
			if (server != null) {
				_mbeanName = _ode.getContext().getMBeanNames()
						.createCustomComponentMBeanName("Management");
	...
	public synchronized void start() throws JBIException {
		...
		try {
			...
			if (_ode.getChannel() == null) {
				throw (new JBIException("No channel!",
