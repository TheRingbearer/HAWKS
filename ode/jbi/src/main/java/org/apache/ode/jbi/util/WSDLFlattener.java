...
public class WSDLFlattener {
		...
		if (!_initialized) {
			if (_schemas == null) {
				this._schemas = new SchemaCollection(
						getUri(_definition.getDocumentBaseURI()));
			}
	 ...
	 * 
	 * @param portType
	 *            the port type to create a flat definition for
	...
	private Definition flattenDefinition(QName name) throws Exception {
		...
		for (Iterator itOper = defPort.getOperations().iterator(); itOper
				.hasNext();) {
			...
			if (defOper.getInput() != null) {
				...
				if (defOper.getInput().getMessage() != null) {
					Message flatInputMsg = copyMessage(defOper.getInput()
							.getMessage(), flat);
			...
			if (defOper.getOutput() != null) {
				...
				if (defOper.getOutput().getMessage() != null) {
					Message flatOutputMsg = copyMessage(defOper.getOutput()
							.getMessage(), flat);
			...
			for (Iterator itFault = defOper.getFaults().values().iterator(); itFault
					.hasNext();) {
				...
				if (defFault.getMessage() != null) {
					Message flatFaultMsg = copyMessage(defFault.getMessage(),
							flat);
	...
	private void parseSchemas(Definition def) throws Exception {
		if (def.getTypes() != null
				&& def.getTypes().getExtensibilityElements() != null) {
			for (Iterator iter = def.getTypes().getExtensibilityElements()
		...
		if (def.getImports() != null) {
			for (Iterator itImp = def.getImports().values().iterator(); itImp
					.hasNext();) {
				Collection imps = (Collection) itImp.next();
	...
	private void addNamespaces(Definition flat, Definition def) {
		for (Iterator itImport = def.getImports().values().iterator(); itImport
				.hasNext();) {
		...
		for (Iterator itNs = def.getNamespaces().keySet().iterator(); itNs
				.hasNext();) {
	...
	private Message copyMessage(Message defMessage, Definition flat) {
		...
		if (defMessage.getQName() != null) {
			flatMsg.setQName(new QName(flat.getTargetNamespace(), defMessage
					.getQName().getLocalPart()));
		}
		for (Iterator itPart = defMessage.getParts().values().iterator(); itPart
				.hasNext();) {
