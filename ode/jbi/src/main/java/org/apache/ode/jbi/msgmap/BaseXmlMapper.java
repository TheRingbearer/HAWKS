...
public abstract class BaseXmlMapper {
	...
	private static Map<Source, Document> __parsed = Collections
			.synchronizedMap(new WeakHashMap<Source, Document>());
	...
	protected Element parse(Source content) throws MessageTranslationException {
		...
		// so that we are thread safe, although it is possible for the parse to
		// happen twice.
