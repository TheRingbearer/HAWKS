...
/**
 * JBI message exchange processor that uses {@link DynamicService} for
 * reflection-based method dispatch.
 */
public class DynamicMessageExchangeProcessor<T> extends
	...
	public NormalizedMessage invoke(InOut mex) throws MessagingException {
		try {
			DynamicService<T> service = new DynamicService<T>(_service);
			OMElement payload = OMUtils.toOM(mex.getInMessage().getContent());
			OMElement response = service.invoke(mex.getOperation()
