...
/**
 * JBI-based implementation of the ODE
 * {@link org.apache.ode.bpel.iapi.EndpointReference} interface. This is
 * basically a wrapper around the {@link javax.jbi.servicedesc.ServiceEndpoint}
 * interface.
 */
class JbiEndpointReference implements EndpointReference {
	...
	public Document toXML() {
		...
		Element root = doc.createElementNS(SERVICE_REF_QNAME.getNamespaceURI(),
				SERVICE_REF_QNAME.getLocalPart());
		try {
			DocumentFragment fragment = getServiceEndpoint().getAsReference(
					_type);
			if (fragment != null) {
	...
	public boolean equals(Object other) {
		if (other instanceof JbiEndpointReference)
			return getServiceEndpoint().getServiceName().equals(
					((JbiEndpointReference) other).getServiceEndpoint()
							.getServiceName());
		return false;
	}
	...
	public String toString() {
		if (_se != null) {
			...
			return "JbiEndpointReference["
					+ _initialPartnerEndpoint.serviceName + ":"
	...
	ServiceEndpoint getServiceEndpoint() {
		if (_se == null) {
			if (_initialPartnerEndpoint != null) {
				_se = _ode.getContext().getEndpoint(
						_initialPartnerEndpoint.serviceName,
						_initialPartnerEndpoint.portName);
				if (_se == null) {
