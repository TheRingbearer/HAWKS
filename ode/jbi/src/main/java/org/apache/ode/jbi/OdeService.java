...
public class OdeService extends ServiceBridge implements
	...
	public void activate() throws JBIException {
		...
		if (count != 1)
			return;
		if (_serviceref == null) {
			ServiceEndpoint[] candidates = _ode.getContext()
					.getExternalEndpointsForService(_endpoint.serviceName);
	...
	public void onJbiMessageExchange(javax.jbi.messaging.MessageExchange jbiMex)
		...
		if (jbiMex.getOperation() == null) {
			throw new IllegalArgumentException(
					"Null operation in JBI message exchange id="
							+ jbiMex.getExchangeId() + " endpoint=" + _endpoint);
		}

		if (jbiMex.getPattern().equals(
				org.apache.ode.jbi.MessageExchangePattern.IN_ONLY)) {
		...
		} else if (jbiMex.getPattern().equals(
				org.apache.ode.jbi.MessageExchangePattern.IN_OUT)) {
			boolean success = false;
			Exception err = null;
			try {
	...
	public void onResponse(MyRoleMessageExchange mex) {
		...
		__log.debug("Processing MEX tracker mexId: " + mexId + " clientId: "
				+ clientId);
		final javax.jbi.messaging.MessageExchange jbiMex = _jbiMexTracker
				.peek(clientId);
		if (jbiMex == null) {
		...
		try {
			...
			_ode._scheduler.registerSynchronizer(new Synchronizer() {
				public void afterCompletion(boolean success) {
					if (success) {
						// Deliver reply to external world only if ODE
						// scheduler's job has completed successfully
						try {
							_ode.getChannel().send(jbiMex);
							...
							__log.error(
									"Error delivering response from ODE to JBI mexId: "
											+ mexId + " clientId: " + clientId,
									e);
						}
					}
				}
	...
	private void invokeOde(javax.jbi.messaging.MessageExchange jbiMex,
		...
		try {
			...
			odeMex = _ode._server.getEngine().createMessageExchange(
					jbiMex.getExchangeId(), _endpoint.serviceName,
					jbiMex.getOperation().getLocalPart());

			if (odeMex.getOperation() != null) {
				copyMexProperties(odeMex, jbiMex);
				javax.wsdl.Message msgdef = odeMex.getOperation().getInput()
						.getMessage();
				...
				odeMex.setProperty(Mapper.class.getName(), mapper.getClass()
						.getName());
				...
				setError(jbiMex, new IllegalArgumentException(
						"Unroutable invocation."));
			}
			...
			if (jbiMex.getPattern().equals(
				...
				__log.debug("Consuming non Req/Res MEX tracker "
						+ jbiMex.getExchangeId() + " with pattern "
						+ jbiMex.getPattern());
				_jbiMexTracker.consume(jbiMex.getExchangeId());
			}
	...
	private void outResponse(MyRoleMessageExchange mex,
		...
		try {
			...
			if (mapper == null) {
				String errmsg = "Message-mapper " + mapperName
						+ " used in ODE MEX " + mex.getMessageExchangeId()
						+ " is no longer available.";
				__log.error(errmsg);
				throw new MessageTranslationException(errmsg);
			}

			mapper.toNMS(nmsg, mex.getResponse(), mex.getOperation()
					.getOutput().getMessage(), null);
	...
	private void outResponseFault(MyRoleMessageExchange mex,
		...
		try {
			...
			javax.wsdl.Fault wsdlFault = mex.getOperation().getFault(
					fault.getLocalPart());
			if (wsdlFault == null) {
				setError(jbiMex,
						new MessageTranslationException("Unmapped Fault : "
			...
			__log.error(
					"Error translating ODE fault message "
	...
	private static class JbiMexTracker {
		...
		synchronized boolean track(javax.jbi.messaging.MessageExchange jbiMex) {
			boolean found = _outstandingJbiExchanges.containsKey(jbiMex
					.getExchangeId());
