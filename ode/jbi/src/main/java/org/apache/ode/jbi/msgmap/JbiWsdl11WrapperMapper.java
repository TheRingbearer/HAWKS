...
public class JbiWsdl11WrapperMapper extends BaseXmlMapper implements Mapper {
	...
	public Recognized isRecognized(NormalizedMessage nmsMsg, Operation op) {
		...
		return WSDL11_W_MESSAGE.equals(srcName) ? Recognized.TRUE
				: Recognized.FALSE;
	}
	...
	public void toNMS(NormalizedMessage nmsMsg, Message odeMsg,
		...
		dstMsgEl.setAttribute("type", "msgns:"
				+ odeMsg.getType().getLocalPart());
		...
		while (n != null) {
			if (n.getNodeType() == Node.ELEMENT_NODE) {
				...
				if (pdata == null)
					...
					eltypes.add(new QName(pdata.getNamespaceURI(), pdata
							.getLocalName()));
			}
			n = n.getNextSibling();
		}

		// See if it matches what we expect the faults to look like (first one
		// wins!)
