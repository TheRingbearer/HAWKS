...
public class WSDL20Endpoint implements MutableEndpoint {
	...
	public String getSessionId() {
		Element endpointElmt = (Element) _serviceElmt.getElementsByTagNameNS(
				Namespaces.WSDL_20, "endpoint").item(0);
		NodeList idNodes = endpointElmt.getElementsByTagNameNS(
				Namespaces.ODE_SESSION_NS, "session");
		if (idNodes.getLength() > 0) {
			return idNodes.item(0).getTextContent();
		} else {
			// try the same with the intalio header
			idNodes = endpointElmt.getElementsByTagNameNS(
	...
	public void setSessionId(String sessionId) {
		Element endpointElmt = (Element) _serviceElmt.getElementsByTagNameNS(
				Namespaces.WSDL_20, "endpoint").item(0);
		NodeList idList = endpointElmt.getElementsByTagNameNS(
				Namespaces.ODE_SESSION_NS, "session");
		if (idList.getLength() > 0)
			idList.item(0).setTextContent(sessionId);
		else {
			Element sessElmt = endpointElmt.getOwnerDocument().createElementNS(
					Namespaces.ODE_SESSION_NS, "session");
		...
		idList = endpointElmt.getElementsByTagNameNS(
				Namespaces.INTALIO_SESSION_NS, "session");
		if (idList.getLength() > 0)
			idList.item(0).setTextContent(sessionId);
		else {
	...
	public String getUrl() {
		return ((Element) _serviceElmt.getElementsByTagNameNS(
				Namespaces.WSDL_20, "endpoint").item(0))
	...
	public void setUrl(String url) {
		Element endpointElmt = (Element) _serviceElmt.getElementsByTagNameNS(
				Namespaces.WSDL_20, "endpoint").item(0);
		NodeList addrList = endpointElmt.getElementsByTagNameNS(
				Namespaces.WSDL_20, "address");
		if (addrList.getLength() > 0)
			addrList.item(0).setTextContent(url);
		else {
			Element addrElmt = _serviceElmt.getOwnerDocument().createElementNS(
					Namespaces.WSDL_20, "address");
	...
	public void fromMap(Map eprMap) {
		...
		if (eprMap.get(SESSION) != null) {
			...
			session = doc.createElementNS(Namespaces.INTALIO_SESSION_NS,
					"session");
			session.setTextContent((String) eprMap.get(SESSION));
			endpoint.appendChild(session);
		}
