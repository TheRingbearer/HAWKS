...
/**
 * Dynamic invocation handler for XML-based service; uses RPC message format
 * conventions to dispatch invocation based on top-level element name (e.g.
 * method name) and sub-elements (e.g. parameters).
 */
public class DynamicService<T> {
	...
	// AH: needed for invokation of the FragmentCompositionAPI
	public void invokeOneWay(String operation, OMElement payload) {
		...
		try {
			...
			invokedMethod.invoke(_service, params);
			...
			throw new RuntimeException("Invocation of method " + methodName
					+ " in management interface failed: "
					+ e.getTargetException().getMessage(),
					e.getTargetException());
		}
	}

	public OMElement convertResponse(String operation, String namespace,
			Object result) {
		OMElement response = null;
		QName elementName;
		if (namespace == null) {
			elementName = new QName("", operation + "Response");
		} else {
			elementName = new QName(namespace, operation + "Response");
		}
		response = OM.createOMElement(elementName);
	...
	public OMElement invoke(String operation, OMElement payload) {
		...
		try {
			...
			if (result != null) {
				...
				response = OM.createOMElement(new QName(
						(payload.getNamespace() == null ? "" : payload
								.getNamespace().getNamespaceURI()), methodName
								+ "Response"));
			...
			if (__log.isDebugEnabled()) {
				__log.debug("Response: operation " + operation + " on "
						+ _clazz + ":\n" + response);
			}
			return response;
		} catch (IllegalAccessException e) {
	...
	private Object[] extractParams(Method method, OMElement omElmt) {
		...
		if (omElmt != null) {
			// AH: end
			Iterator<OMElement> omChildren = (Iterator<OMElement>) omElmt
					.getChildElements();
			int paramIdx = 0;
			for (Class<?> paramClass : paramTypes) {
				OMElement omchild = (OMElement) omChildren.next();
				__log.debug("Extracting param " + paramClass + " from "
						+ omchild);
	...
	private static Object convertFromOM(Class<?> clazz, OMElement elmt) {
		...
		if (elmt == null || elmt.getText().length() == 0
				&& !elmt.getChildElements().hasNext() && !clazz.isArray())
			return null;
		else if (clazz.equals(String.class)) {
			return elmt.getText();
		} else if (clazz.equals(Boolean.class) || clazz.equals(Boolean.TYPE)) {
			return (elmt.getText().equals("true") || elmt.getText().equals(
					"yes")) ? Boolean.TRUE : Boolean.FALSE;
			...
			// AH: parse Mapping
			/*
			 * } else if (clazz.equals(Mapping.class)){
			 * 
			 * Iterator iter =
			 * elmt.getChildrenWithNamespaceURI(FCConstants.FC_TARGET_NAMESPACE
			 * ); String fromVar = ""; String toVar = ""; while
			 * (iter.hasNext()){ OMElement element = (OMElement)iter.next(); if
			 * (element.getLocalName().equals(FCConstants.FC_MAP_FROM_VAR_FIELD.
			 * getLocalPart())){ fromVar = element.getText(); } else if
			 * (element.
			 * getLocalName().equals(FCConstants.FC_MAP_TO_VAR_FIELD.getLocalPart
			 * ())){ toVar = element.getText(); } }
			 * 
			 * return new Mapping(fromVar, toVar);
			 * 
			 * // AH: end
			 */
			...
			try {
				...
				throw new RuntimeException("Couldn't find class "
						...
						+ ".Factory to instantiate xml bean", e);
			} catch (IllegalAccessException e) {
				throw new RuntimeException("Couldn't access class "
						+ clazz.getCanonicalName()
						+ ".Factory to instantiate xml bean", e);
			} catch (InvocationTargetException e) {
				throw new RuntimeException(
						"Couldn't access xml bean parse method on class "
								+ clazz.getCanonicalName() + ".Factory "
								+ "to instantiate xml bean", e);
			} catch (NoSuchMethodException e) {
				throw new RuntimeException(
						"Couldn't find xml bean parse method on class "
								+ clazz.getCanonicalName() + ".Factory "
								+ "to instantiate xml bean", e);
			}
		} else
			throw new RuntimeException("Couldn't use element " + elmt
					+ " to obtain a management method parameter.");
	}
	...
	private static OMElement convertToOM(Object obj) {
		if (obj instanceof XmlObject) {
			try {
				return new StAXOMBuilder(((XmlObject) obj).newInputStream())
						.getDocumentElement();
			} catch (XMLStreamException e) {
				throw new RuntimeException(
						"Couldn't serialize result to an outgoing messages.", e);
			}
			...
			OMElement resultElmt = OM.createOMElement("return", null);
			resultElmt.setText(String.valueOf((Boolean) obj));
			return resultElmt;
		} else if (obj instanceof byte[]) {
			OMElement resultElmt = OM.createOMElement("base64", null);
			resultElmt.setText(new String((byte[]) obj));
			return resultElmt;
			// AH: end
		} else
			throw new RuntimeException("Couldn't convert object " + obj
					+ " into a response element.");
	}

	@SuppressWarnings("unchecked")
	private static OMElement stripNamespace(OMElement element) {
		OMElement parent = OM.createOMElement(new QName("", element
				.getLocalName()));
		// AH:
		parent.setText(element.getText());
		// AH: end
		Iterator<OMElement> iter = (Iterator<OMElement>) element
				.getChildElements();
		while (iter.hasNext()) {
	...
	private Method findMethod(String methodName) {
		...
		throw new RuntimeException("Couldn't find any method named "
				+ methodName + " in interface " + _clazz.getName());
	}
