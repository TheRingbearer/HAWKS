 ...
 */
// @stmz: may not work, as ode doesn't like the extensions in the runtime for
// the bpel-test stuff
// and we did't modify the MockScheduler
public class MockScheduler implements Scheduler {
	...
	public String schedulePersistedJob(final JobDetails detail, final Date date)
	...
	public String scheduleVolatileJob(final boolean transacted,
			final JobDetails detail, final Date date) throws ContextException {
		if (date != null) {
	...
	private void exec(boolean transacted, final JobDetails detail) {
		final Boolean bool = (Boolean) detail.getDetailsExt().get("bool");
		try {
			if (transacted) {
			...
			throw new ContextException(
					"Failure when scheduling a new volatile job.", e);
		}
	}

	public String scheduleMapSerializableRunnable(
			final MapSerializableRunnable runnable, final Date when)
			throws ContextException {
		if (when != null) {
			registerSynchronizer(new Synchronizer() {
				public void afterCompletion(boolean success) {
	...
	public void registerSynchronizer(final Synchronizer synch)
		...
		if (_txm != null) {
			try {
				_txm.getTransaction().registerSynchronization(
						new Synchronization() {
							public void beforeCompletion() {
