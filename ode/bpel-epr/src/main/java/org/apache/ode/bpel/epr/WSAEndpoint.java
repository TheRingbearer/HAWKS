...
public class WSAEndpoint implements MutableEndpoint {
	...
	public WSAEndpoint(WSAEndpoint prototype) {
		_eprElmt = (Element) DOMUtils.newDocument().importNode(
				prototype._eprElmt, true);
	}
	...
	public String getSessionId() {
		NodeList idNodes = _eprElmt.getElementsByTagNameNS(
				Namespaces.ODE_SESSION_NS, "session");
		if (idNodes.getLength() > 0) {
			return idNodes.item(0).getTextContent();
		} else {
			// try the same with the intalio header
			idNodes = _eprElmt.getElementsByTagNameNS(
					Namespaces.INTALIO_SESSION_NS, "session");
			if (idNodes.getLength() > 0) {
				return idNodes.item(0).getTextContent();
			}
			return null;
	...
	public void setSessionId(String sessionId) {
		NodeList idList = _eprElmt.getElementsByTagNameNS(
				Namespaces.ODE_SESSION_NS, "session");
		if (idList.getLength() > 0)
			idList.item(0).setTextContent(sessionId);
		else {
			Element sessElmt = _eprElmt.getOwnerDocument().createElementNS(
					Namespaces.ODE_SESSION_NS, "session");
		...
		idList = _eprElmt.getElementsByTagNameNS(Namespaces.INTALIO_SESSION_NS,
				"session");
		if (idList.getLength() > 0)
			idList.item(0).setTextContent(sessionId);
		else {
	...
	public void setUrl(String url) {
		NodeList addrList = _eprElmt.getElementsByTagNameNS(
				Namespaces.WS_ADDRESSING_NS, "Address");
	...
	public QName getServiceName() {
		NodeList metadataList = _eprElmt.getElementsByTagNameNS(
				Namespaces.WS_ADDRESSING_NS, "Metadata");
		if (metadataList.getLength() > 0) {
			...
			String serviceNS = _eprElmt.getOwnerDocument().lookupNamespaceURI(
					prefix);
			...
			QName result = new QName(serviceNS, serviceTextQName.substring(
					twoDotsIdx + 1, serviceTextQName.length()));
	...
	public void set(Node node) {
		...
		if (__log.isDebugEnabled())
			__log.debug("Setting a WSAEndpoint value: "
					+ DOMUtils.domToString(_eprElmt));
	}
	...
	public Map toMap() {
		...
		if (metadataList.getLength() > 0) {
			...
			try {
				serviceNS = _eprElmt.getOwnerDocument().lookupNamespaceURI(
						prefix);
			} catch (Exception e) {
				e.printStackTrace();
			}
			...
			result.put(
					SERVICE_QNAME,
					new QName(serviceNS, serviceTextQName.substring(
							twoDotsIdx + 1, serviceTextQName.length())));
			result.put(PORT_NAME, service.getAttribute("EndpointName"));
			if (__log.isDebugEnabled()) {
				__log.debug("Filled transfo map with service: "
						+ result.get(SERVICE_QNAME));
				__log.debug("Filled transfo map with port: "
						+ result.get(PORT_NAME));
			}
		}
		return result;
	}
	...
	public void fromMap(Map eprMap) {
		...
		if (eprMap.get(SESSION) != null) {
			...
			_eprElmt.appendChild(sessElmt);
			// and the same for the (deprecated) intalio namespace for backward
			// compatibility
		...
		if (eprMap.get(SERVICE_QNAME) != null) {
			Element metadataElmt = doc.createElementNS(
					Namespaces.WS_ADDRESSING_NS, "Metadata");
			_eprElmt.appendChild(metadataElmt);
			Element serviceElmt = doc.createElementNS(
					Namespaces.WS_ADDRESSING_WSDL_NS, "ServiceName");
			metadataElmt.appendChild(serviceElmt);
			QName serviceQName = (QName) eprMap.get(SERVICE_QNAME);
			serviceElmt.setAttribute("xmlns:servicens",
					serviceQName.getNamespaceURI());
			serviceElmt.setTextContent("servicens:"
					+ serviceQName.getLocalPart());
			serviceElmt.setAttribute("EndpointName",
					(String) eprMap.get(PORT_NAME));
		}
		_eprElmt.appendChild(addrElmt);
		if (__log.isDebugEnabled())
			__log.debug("Constructed a new WSAEndpoint: "
					+ DOMUtils.domToString(_eprElmt));
	}
}
