...
public class OMUtils {
	...
	public static Element toDOM(OMElement element, Document doc, boolean deepNS) {
		final Element domElement = doc.createElementNS(element.getQName()
				.getNamespaceURI(), element.getQName().getLocalPart());
		...
		if (deepNS) {
			...
			if (element.getAllDeclaredNamespaces() != null) {
				for (Iterator<OMNamespace> i = element
						.getAllDeclaredNamespaces(); i.hasNext();) {
					OMNamespace omns = i.next();
					if (omns.getPrefix().equals(""))
						domElement.setAttributeNS(DOMUtils.NS_URI_XMLNS,
								"xmlns", omns.getNamespaceURI() == null ? ""
										: omns.getNamespaceURI());
					else
						domElement.setAttributeNS(DOMUtils.NS_URI_XMLNS,
								"xmlns:" + omns.getPrefix(),
								omns.getNamespaceURI());
				}
		...
		for (Iterator i = element.getAllAttributes(); i.hasNext();) {
			...
			if (attr.getNamespace() != null)
				newAttr = doc.createAttributeNS(attr.getNamespace()
						.getNamespaceURI(), attr.getLocalName());
			else
			...
			if (colonIdx > 0) {
				OMNamespace attrValNs = element.findNamespaceURI(attr
						.getAttributeValue().substring(0, colonIdx));
				if (attrValNs != null)
		...
		for (Iterator<OMNode> i = element.getChildren(); i.hasNext();) {
			...
			case OMNode.CDATA_SECTION_NODE:
				domElement.appendChild(doc.createCDATASection(((OMText) omn)
						.getText()));
				break;
			case OMNode.TEXT_NODE:
				domElement.appendChild(doc.createTextNode(((OMText) omn)
						.getText()));
				break;
	...
	private static void buildNScontext(NSContext nscontext, OMElement element) {
		...
		if (element.getAllDeclaredNamespaces() != null)
			for (Iterator<OMNamespace> i = element.getAllDeclaredNamespaces(); i
					.hasNext();) {
		...
		if (element.getDefaultNamespace() != null)
			nscontext.register("", element.getDefaultNamespace()
					.getNamespaceURI());
	}
	...
	public static OMElement toOM(Element src, OMFactory omf, OMContainer parent) {
		OMElement omElement = parent == null ? omf.createOMElement(
				src.getLocalName(), null) : omf.createOMElement(
				src.getLocalName(), null, parent);
		if (src.getNamespaceURI() != null) {
			if (src.getPrefix() != null)
				omElement.setNamespace(omf.createOMNamespace(
						src.getNamespaceURI(), src.getPrefix()));
			else
		...
		for (int i = 0; i < attrs.getLength(); ++i) {
			...
			// @hahnml: Fixed NullPointerException, if attr.getLocalName() returns null value the result of attr.getName() is used
			if ((attr.getLocalName() != null && attr.getLocalName().equals("xmlns")) || attr.getName().equals("xmlns")
					|| (attr.getNamespaceURI() != null && attr
							.getNamespaceURI().equals(DOMUtils.NS_URI_XMLNS)))
				continue;
		...
		for (int i = 0; i < children.getLength(); ++i) {
			...
			case Node.CDATA_SECTION_NODE:
				omElement.addChild(omf.createOMText(
						((CDATASection) n).getTextContent(),
