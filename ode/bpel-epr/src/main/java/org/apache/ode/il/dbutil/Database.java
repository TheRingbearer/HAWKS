...
/**
 * Does the dirty work of setting up / obtaining a DataSource based on the
 * configuration in the {@link OdeConfigProperties} object.
 * 
 */
public class Database {
	...
	public synchronized void shutdown() {
		...
		if (_connectionManager != null)
			try {
				...
				__log.warn("Exception while stopping connection manager: "
						+ t.getMessage());
			} finally {
				_connectionManager = null;
			}
		...
		if (_needDerbyShutdown) {
			...
			try {
				...
				if (ex.getErrorCode() != 45000)
					__log.error(
							"Error shutting down Derby: " + ex.getErrorCode(),
							ex);
	...
	public DataSource getDataSource() {
		...
		return __logSql.isDebugEnabled() ? LoggingInterceptor.createLoggingDS(
				_datasource, __logSql) : _datasource;
	}
	...
	private void initExternalDb() throws DatabaseConfigException {
		try {
			_datasource = (DataSource) lookupInJndi(_odeConfig
					.getDbDataSource());
			__log.info(__msgs.msgOdeUsingExternalDb(_odeConfig
					.getDbDataSource()));
		} catch (Exception ex) {
			String msg = __msgs.msgOdeInitExternalDbFailed(_odeConfig
					.getDbDataSource());
			__log.error(msg, ex);
	...
	private void initInternalDb() throws DatabaseConfigException {
		__log.info(__msgs.msgOdeUsingInternalDb(
				_odeConfig.getDbIntenralJdbcUrl(),
	...
	private void initInternalDb(String url, String driverClass,
			String username, String password) throws DatabaseConfigException {
		...
		PoolingSupport poolingSupport = new SinglePool(
				_odeConfig.getPoolMaxSize(), _odeConfig.getPoolMinSize(),
				CONNECTION_MAX_WAIT_MILLIS, CONNECTION_MAX_IDLE_MINUTES, true, // match
				// one
				false, // match all
				false); // select one assume match

		_connectionManager = new GenericConnectionManager(transactionSupport,
				poolingSupport, null, connectionTracker,
				(RecoverableTransactionManager) _txm, getClass().getName(),
		...
		try {
			...
			if (mcfClass != null) {
				Properties dbInternalMCFProps = _odeConfig
						.getDbInternalMCFProperties();
				if (__log.isDebugEnabled()) {
					__log.debug("Using internal DB MCF " + mcfClass + " "
							+ dbInternalMCFProps);
				}
				mcf = (javax.resource.spi.ManagedConnectionFactory) Class
						.forName(mcfClass).newInstance();
			...
			_datasource = (DataSource) mcf
					.createConnectionFactory(_connectionManager);
		} catch (Exception ex) {
	...
	private void initEmbeddedDb() throws DatabaseConfigException {
		...
		_derbyUrl = url;
		initInternalDb(url,
				org.apache.derby.jdbc.EmbeddedDriver.class.getName(), "sa",
				null);
	...
	public BpelDAOConnectionFactoryJDBC createDaoCF()
		...
		try {
			cf = (BpelDAOConnectionFactoryJDBC) Class.forName(pClassName)
					.newInstance();
		} catch (Exception ex) {
