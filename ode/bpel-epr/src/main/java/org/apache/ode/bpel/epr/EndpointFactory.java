...
public class EndpointFactory {
	...
	private static QName WSA_ELMT_QNAME = new QName(
	...
	private static MutableEndpoint[] ENDPOINTS = new MutableEndpoint[] {
			new URLEndpoint(), new WSAEndpoint(), new WSDL11Endpoint(),
			new WSDL20Endpoint() };
	...
	public static MutableEndpoint createEndpoint(Element endpointElmt) {
		...
		__log.warn("Couldnt create any endpoint for element "
				+ DOMUtils.domToString(endpointElmt));
		return null;
	}
	 ...
	 */
	public static MutableEndpoint convert(QName targetElmtType,
			Element sourceEndpoint) {
		MutableEndpoint targetEpr;
		MutableEndpoint sourceEpr = EndpointFactory
				.createEndpoint(sourceEndpoint);
		...
		if (__log.isDebugEnabled()) {
			...
			__log.debug("Source endpoint "
					+ DOMUtils.domToString(sourceEndpoint));
			__log.debug("Destination endpoint "
					+ DOMUtils.domToString(targetEpr.toXML()));
		}
		return targetEpr;
	}
