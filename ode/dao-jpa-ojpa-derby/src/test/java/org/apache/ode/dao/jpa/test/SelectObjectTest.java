...
public class SelectObjectTest extends TestCase {
	...
	List<MessageRouteDAO> routes = corr.findRoute(new CorrelationKeySet()
		.add(key1));
	...
	for (ProcessInstanceDAO inst : insts) {
	    ...
	    MessageExchangeDAO me = ic.dequeueMessage(new CorrelationKeySet()
		    .add(key1));
	    assertNotNull(me);
	    ...
	    CorrelationSetDAO cs = childS
		    .getCorrelationSet("TestCorrelationSet1");
	    ...
	    assertEquals(cs.getProperties().get(new QName(TEST_NS, "name1")),
		    "key1");

	    // Assert PartnerLinkDAO
	    assertNotNull(spl.getMyEPR());
	    assertEquals(spl.getMyRoleName(), "MyRole1");
	    assertEquals(spl.getMyRoleServiceName(), new QName(TEST_NS,
		    "testRoleService"));
