...
public class InsertObjectTest extends TestCase {
    ...
    private MessageExchangeDAO createMessageExchange(ProcessDAO p,
	    ProcessInstanceDAO pi, PartnerLinkDAO pl) throws SAXException,
	    IOException {
    ...
    private MessageDAO createMessage(MessageExchangeDAO me, String name)
    ...
    private CorrelatorDAO createProcess(BpelDAOConnection conn, String pid,
	    String type) {
	_process = conn.createProcess(new QName(TEST_NS, pid), new QName(
		TEST_NS, type), "GUID1", 1);
	...
	QName[] names = { new QName(TEST_NS, "name1"),
		new QName(TEST_NS, "name2") };
	...
	pi
		.setFault(
			new QName(TEST_NS, "testFault"),
			"testExplanation",
			1,
			1,
			DOMUtils
				.stringToDOM("<testFaultMessage>testMessage</testFaultMessage>"));
	pi.setLastActiveTime(cal.getTime());
	pi.setState((short) 1);

	pi.createActivityRecovery("testChannel1", 3, "testReason1", cal
		.getTime(), DOMUtils
		.stringToDOM("<testData>testData1</testData>"), actions, 2);
	pi.createActivityRecovery("testChannel2", 4, "testReason2", cal
		.getTime(), DOMUtils
		.stringToDOM("<testData>testData2</testData>"), actions, 2);
	...
	PartnerLinkDAO pl1 = child1.createPartnerLink(1, "Test PartnerLink1",
		"MyRole1", "PartnerRole1");
	...
	pl1.setPartnerEPR(DOMUtils
		.stringToDOM("<testEPR>testPartnerEPR</testEPR>"));
	pl1.setPartnerSessionId("TestPartnerSessionID");

	MessageExchangeDAO mex = createMessageExchange(process, pi, pl1);

	corr.addRoute("testRoute", pi, 1, new CorrelationKeySet().add(key1),
		"one");
	corr.enqueueMessage(mex, new CorrelationKeySet().add(corrkeys[0]).add(
		corrkeys[1]));
