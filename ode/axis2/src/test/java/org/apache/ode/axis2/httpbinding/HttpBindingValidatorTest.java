...
public class HttpBindingValidatorTest extends TestCase {
	...
	private static final Log log = LogFactory
			.getLog(HttpBindingValidatorTest.class);
	...
	public void testAll() {
		for (int i = 0; i < definitions.length; i++) {
			...
			for (Iterator it = def.getBindings().entrySet().iterator(); it
					.hasNext();) {
				Map.Entry e = (Map.Entry) it.next();
				QName name = (QName) e.getKey();
				String localName = name.getLocalPart();
				Binding binding = (Binding) e.getValue();
				Element documentationElement = binding
				...
				if (!shouldFail && !shouldPass) {
					fail("Binding: "
							+ localName
							+ ", <wsdl:documentation> content must start with '"
							+ SHOULD_FAIL + "' or '" + SHOULD_PASS + "'. ");
				}
