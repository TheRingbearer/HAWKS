...
public class SoapMessageConverterTest extends TestCase {
	...
	public SoapMessageConverterTest() throws Exception {
		req1bad = DOMUtils.parse(getClass().getResourceAsStream(
				"/testRequest1Bad.xml"));
		req1 = DOMUtils.parse(getClass().getResourceAsStream(
				"/testRequest1.xml"));
		WSDLReader reader = WSDLFactory.newInstance().newWSDLReader();
		wsdl1 = reader.readWSDL(getClass().getResource("/test1.wsdl")
				.toExternalForm());
		portType = wsdl1.getPortType(portTypeName);
		op1 = portType.getOperation("getObjectId", null, null);

		wsdlHW = reader.readWSDL(getClass().getResource("/HelloWorld.wsdl")
				.toExternalForm());
		portTypeHW = wsdlHW.getPortType(new QName(wsdlHW.getTargetNamespace(),
				"HelloPortType"));
	...
	public void testBadServiceName() {
		try {
			new SoapMessageConverter(wsdl1, new QName(wsdl1tns, "foobar"),
					portName);
			fail("Should have thrown axis error.");
		} catch (AxisFault af) {
			;// expected
	...
	public void testCreateSOAPRequest() throws Exception {
		...
		portmapper.createSoapRequest(msgCtx, odeMsg,
	...
	public void testGetSoapAction() throws Exception {
		assertEquals("getObjectIdAction",
				portmapper.getSoapAction("getObjectId"));
	...
	public void testParseRequest() throws Exception {
		...
		odeMsg1.setHeaderPart("DocumentumRequestHeader", DOMUtils
				.findChildByName(req1.getDocumentElement(), new QName(
						"http://documentum.com/ws/2005/services",
		...
		Element params = DOMUtils.findChildByName(odeMsg2.getMessage(),
				new QName(null, "parameters"));
		...
		Element hdrElmt = DOMUtils.findChildByName(odeMsg2
				.getHeaderPart("DocumentumRequestHeader"), new QName(
				"http://documentum.com/ws/2005/services",
				"DocumentumSecurityToken"));
	...
	public void testHelloWorldRequest() throws Exception {
		SoapMessageConverter portmaper1 = new SoapMessageConverter(wsdlHW,
				new QName(wsdlHW.getTargetNamespace(), "HelloService"),
				"HelloPort");

		XMLStreamReader sr = XMLInputFactory.newInstance()
				.createXMLStreamReader(
						getClass().getResourceAsStream(
								"/HelloWorldRequest.soap"));
		StAXSOAPModelBuilder builder = OMXMLBuilderFactory
				.createStAXSOAPModelBuilder(new SOAP11Factory(), sr);
