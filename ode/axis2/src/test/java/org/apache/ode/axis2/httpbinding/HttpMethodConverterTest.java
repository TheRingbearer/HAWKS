...
public class HttpMethodConverterTest extends TestCase {
	...
	protected void setUp() throws Exception {
		...
		Service deliciousService = definition.getService(new QName(
				"http://ode/bpel/unit-test.wsdl", "DeliciousService"));
		...
		Service dummyService = definition.getService(new QName(
				"http://ode/bpel/unit-test.wsdl", "DummyService"));
	...
	public void testGetTag() throws Exception {
		String uri = ((HTTPAddress) deliciousPort.getExtensibilityElements()
				.get(0)).getLocationURI();
		...
		odeMex.op = deliciousBinding.getBindingOperation("getTag", null, null)
				.getOperation();
	...
	public void testHello() throws Exception {
		String uri = ((HTTPAddress) dummyPort.getExtensibilityElements().get(0))
				.getLocationURI();
		...
		MockMessageExchange odeMex = new MockMessageExchange();
		odeMex.op = dummyBinding.getBindingOperation("hello", null, null)
				.getOperation();
		odeMex.req = new MockMessage(msgEl);
		odeMex.epr = new MockEPR(uri);
		HttpMethod httpMethod = dummyBuilder.createHttpRequest(odeMex,
				new DefaultHttpParams());
		assertTrue("POST".equalsIgnoreCase(httpMethod.getName()));
		assertEquals("Generated URI does not match", expectedUri, httpMethod
				.getURI().toString());

		String b = ((StringRequestEntity) ((PostMethod) httpMethod)
	...
	class MockMessageExchange implements PartnerRoleMessageExchange {
		...
		public void replyWithFailure(FailureType type, String description,
				Element details) throws BpelEngineException {
