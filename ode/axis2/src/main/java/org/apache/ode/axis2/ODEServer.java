...
public class ODEServer {
	...
	public void init(ServletConfig config, AxisConfiguration axisConf)
	...
	public void init(String contextPath, AxisConfiguration axisConf,
		...
		if (!_appRoot.isDirectory())
			throw new IllegalArgumentException(_appRoot
					+ " does not exist or is not a directory");
		TempFileManager.setWorkingDirectory(_appRoot);

		__log.debug("Loading properties");
		String confDir = System.getProperty("org.apache.ode.configDir");
		_configRoot = confDir == null ? new File(_appRoot, "conf") : new File(
				confDir);
		if (!_configRoot.isDirectory())
			throw new IllegalArgumentException(_configRoot
					+ " does not exist or is not a directory");
		...
		try {
			...
			String errmsg = __msgs.msgOdeInstallErrorCfgNotFound(_odeConfig
					.getFile());
			__log.warn(errmsg);
		} catch (Exception ex) {
			String errmsg = __msgs.msgOdeInstallErrorCfgReadError(_odeConfig
					.getFile());
		...
		if (!_workRoot.isDirectory())
			throw new IllegalArgumentException(_workRoot
					+ " does not exist or is not a directory");
		...
		initDAO();
		EndpointReferenceContextImpl eprContext = new EndpointReferenceContextImpl(
				this);
		__log.debug("Initializing BPEL process store.");
		initProcessStore(eprContext);

		// @stmz: create GenericController instance
		GenericController gencon = GenericController.getInstance();
		gencon.setActiveMQURL(_odeConfig.getActiveMQURL());
		gencon.start(_store);
		...
		_mgtService = new ManagementService(_txMgr);
		_mgtService.enableService(_axisConfig, _bpelServer, _store,
				_appRoot.getAbsolutePath());
		
		// AH: ser fc definition into engine
		boolean success = true;
		try {
			_txMgr.begin();
			((BpelEngineImpl) _bpelServer.getEngine())
					.setFcServiceDefinition(_mgtService
							.getFcServiceDefinition());
			...
			__log.error("Exception occured while setting fcServiceDefinition.",
					e);
		...
		try {
			// @hahnml: Propagate the BPELServer object to handle the instance
			// migration
			__log.debug("Initializing Deployment Web Service");
			new DeploymentWebService().enableService(_axisConfig, _store,
					_poller, _bpelServer, _appRoot.getAbsolutePath(),
		...
		_poller.start();
		__log.info(__msgs.msgPollingStarted(_store.getDeployDir()
				.getAbsolutePath()));
		__log.info(__msgs.msgOdeStarted());
		
		// @krawczls: start the SynchronizationMiddleware
		...
		try {
			...
			if (is != null) {
				...
				try {
					...
					props.load(is);
					String deploymentPollerClass = props
							.getProperty("deploymentPoller.class");
					if (deploymentPollerClass == null) {
						...
						Class pollerClass = Class
								.forName(deploymentPollerClass);
						poller = (DeploymentPoller) pollerClass.getConstructor(
								File.class, ODEServer.class).newInstance(
								_store.getDeployDir(), this);
						__log.info("A custom deployment poller: "
								+ deploymentPollerClass
								+ " has been plugged in.");
					}
				} catch (Exception e) {
					__log.warn(
	...
	public void shutDown() throws AxisFault {
		...
		// @stmz: shutdown GenericController
		GenericController gencon = GenericController.getInstance();
		gencon.shutdown();
		...
		try {
			...
			if (idleConnectionTimeoutThread != null) {
				...
				try {
					...
					__log.error(
							"Unable to shut down Idle Connection Timeout Thread.",
							t);
				}
			}
			try {
	...
	private void initTxMgr() throws ServletException {
		...
		try {
			Class txFactClass = this.getClass().getClassLoader()
					.loadClass(txFactoryName);
			Object txFact = txFactClass.newInstance();
			_txMgr = (TransactionManager) txFactClass.getMethod(
					"getTransactionManager", (Class[]) null).invoke(txFact);
			if (__logTx.isDebugEnabled()
					&& System.getProperty("ode.debug.tx") != null)
				_txMgr = new DebugTxMgr(_txMgr);
		} catch (Exception e) {
			__log.fatal(
					"Couldn't initialize a transaction manager with factory: "
							+ txFactoryName, e);
	...
	protected void initDAO() throws ServletException {
		__log.info(__msgs.msgOdeUsingDAOImpl(_odeConfig
				.getDAOConnectionFactory()));
		try {
			_daoCF = _db.createDaoCF();
		} catch (Exception ex) {
			String errmsg = __msgs.msgDAOInstantiationFailed(_odeConfig
					.getDAOConnectionFactory());
			__log.error(errmsg, ex);
			throw new ServletException(errmsg, ex);
	...
	protected void initProcessStore(EndpointReferenceContext eprContext) {
		...
		_store.setDeployDir(_odeConfig.getDeployDir() != null ? new File(
				_odeConfig.getDeployDir()) : new File(_workRoot, "processes"));
	...
	protected ProcessStoreImpl createProcessStore(
		...
		return new ProcessStoreImpl(eprContext, ds,
				_odeConfig.getDAOConnectionFactory(), _odeConfig, false);
	}
	...
	protected Scheduler createScheduler() {
		...
		incMess.setScheduler(scheduler);
	...
	private void initBpelServer(EndpointReferenceContextImpl eprContext) {
		...
		if (_odeConfig.getThreadPoolMaxSize() == 0)
			...
			_executorService = Executors.newFixedThreadPool(
					_odeConfig.getThreadPoolMaxSize(), threadFactory);
		...
		polledRunnableProcessor
				.setPolledRunnableExecutorService(_executorService);
		...
		_bpelServer
				.setInMemDaoConnectionFactory(new BpelDAOConnectionFactoryImpl(
						_scheduler, _odeConfig.getInMemMexTtl()));
		...
		_bpelServer.setMigrationTransactionTimeout(_odeConfig
				.getMigrationTransactionTimeout());
		_bpelServer.setConfigProperties(_odeConfig.getProperties());
		_bpelServer.init();
		_bpelServer.setInstanceThrottledMaximumCount(_odeConfig
				.getInstanceThrottledMaximumCount());
		_bpelServer.setProcessThrottledMaximumCount(_odeConfig
				.getProcessThrottledMaximumCount());
		_bpelServer.setProcessThrottledMaximumSize(_odeConfig
				.getProcessThrottledMaximumSize());
		_bpelServer.setHydrationLazy(_odeConfig.isHydrationLazy());
		_bpelServer.setHydrationLazyMinimumSize(_odeConfig
				.getHydrationLazyMinimumSize());
		_bpelServer.setProcessConfLoader(_store);
	}
	...
	private void initHttpConnectionManager() throws ServletException {
		...
		int max_per_host = Integer.parseInt(_odeConfig.getProperty(
				HttpConnectionManagerParams.MAX_HOST_CONNECTIONS, ""
						+ _odeConfig.getPoolMaxSize()));
		int max_total = Integer.parseInt(_odeConfig.getProperty(
				HttpConnectionManagerParams.MAX_TOTAL_CONNECTIONS, ""
						+ _odeConfig.getPoolMaxSize()));
		if (__log.isDebugEnabled()) {
			__log.debug(HttpConnectionManagerParams.MAX_HOST_CONNECTIONS + "="
					+ max_per_host);
			__log.debug(HttpConnectionManagerParams.MAX_TOTAL_CONNECTIONS + "="
					+ max_total);
		}
		if (max_per_host < 1 || max_total < 1) {
			String errmsg = HttpConnectionManagerParams.MAX_HOST_CONNECTIONS
					+ " and "
					+ HttpConnectionManagerParams.MAX_TOTAL_CONNECTIONS
	...
	private void registerEventListeners() {
		...
		if (listenersStr != null) {
			for (StringTokenizer tokenizer = new StringTokenizer(listenersStr,
				...
				try {
					_bpelServer
							.registerBpelEventListener((BpelEventListener) Class
									.forName(listenerCN).newInstance());
					__log.info(__msgs
							.msgBpelEventListenerRegistered(listenerCN));
				} catch (Exception e) {
	...
	private void registerMexInterceptors() {
		...
		if (listenersStr != null) {
			for (StringTokenizer tokenizer = new StringTokenizer(listenersStr,
				...
				try {
					_bpelServer
							.registerMessageExchangeInterceptor((MessageExchangeInterceptor) Class
									.forName(interceptorCN).newInstance());
					__log.info(__msgs
							.msgMessageExchangeInterceptorRegistered(interceptorCN));
				} catch (Exception e) {
					__log.warn("Couldn't register the event listener "
	...
	private void handleEvent(ProcessStoreEvent pse) {
		...
		case DEPLOYED:
			if (pconf != null) {
				/*
				 * If and only if an old process exists with the same pid, the
				 * old process is cleaned up. The following line is IMPORTANT
				 * and used for the case when the deployment and store do not
				 * have the process while the process itself exists in the
				 * BPEL_PROCESS table. Notice that the new process is actually
				 * created on the 'ACTIVATED' event.
				 */
		...
		case ACTIVATED:
			...
			if (pconf != null) {
				...
				__log.debug("slighly odd: recevied event " + pse
						+ " for process not in store!");
			}
			break;
		case RETIRED:
		...
		if (pconf != null) {
			if (pse.type == ProcessStoreEvent.Type.UNDEPLOYED) {
				__log.debug("Cancelling all cron scheduled jobs on store event: "
						+ pse);
				_bpelServer.getContexts().cronScheduler.cancelProcessCronJobs(
						pse.pid, true);
			}

			// Except for undeploy event, we need to re-schedule process
			// dependent jobs
			__log.debug("(Re)scheduling cron scheduled jobs on store event: "
					+ pse);
			if (pse.type != ProcessStoreEvent.Type.UNDEPLOYED) {
				_bpelServer.getContexts().cronScheduler
	...
	private class DebugTx implements Transaction {
		...
		public void registerSynchronization(Synchronization synchronization)
				...
				SystemException {
			if (__log.isDebugEnabled()) {
				__logTx.debug("Synchronization registration on "
