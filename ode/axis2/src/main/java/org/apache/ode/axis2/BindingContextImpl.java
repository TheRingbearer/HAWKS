...
public class BindingContextImpl implements BindingContext {
    ...
    public EndpointReference activateMyRoleEndpoint(QName processId, Endpoint myRoleEndpoint) {
        try {
            ...
            ODEService service = (ODEService)_services.get(myRoleEndpoint.serviceName, myRoleEndpoint.portName);
            if (service == null) {
            	ODEService svc = createService(pconf, myRoleEndpoint.serviceName, myRoleEndpoint.portName);
            	epr = svc.getMyServiceRef();
            	_serviceEprMap.put(svc, epr);
            } else {
            	
            	AxisService axisService = ODEAxisService.createService(_server._axisConfig, pconf, myRoleEndpoint.serviceName, myRoleEndpoint.portName);
            	Iterator opIt = axisService.getOperations();
            	AxisService oldService = service.getAxisService();
            	MessageReceiver msgReceiver = ((AxisOperation)oldService.getOperations().next()).getMessageReceiver();
            	while (opIt.hasNext()) {
            		AxisOperation op = (AxisOperation)opIt.next();
            		AxisOperation oldOp = oldService.getOperation(op.getName());
            		
            		// the operation is new
            		if (oldOp == null) {

            			// Add the operation to the existing Axis service
            			oldService.addOperation(op);
            			op.setMessageReceiver(msgReceiver);
            			
            			// Add the mapping in case of doc/lit encoding
            			Definition wsdlDefinition = pconf.getDefinitionForService(myRoleEndpoint.serviceName);
            			String axisServiceName = axisService.getName();
            			List wsldOps = wsdlDefinition.getService(myRoleEndpoint.serviceName)
        						.getPort(myRoleEndpoint.portName).getBinding().getPortType().getOperations();
            			for (Object wsldOp : wsldOps) {
            				Operation wsdlOp = (Operation) wsldOp;
            				if (wsdlOp.getName().equals(op.getName().getLocalPart())) {
