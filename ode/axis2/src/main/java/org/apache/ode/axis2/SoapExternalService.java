 ...
 * maintain the WSDL description of used services.
 * 
...
public class SoapExternalService implements ExternalService {
	...
	private static final org.apache.ode.utils.wsdl.Messages msgs = Messages
	...
	public SoapExternalService(ProcessConf pconf, QName serviceName,
			String portName, ExecutorService executorService,
			AxisConfiguration axisConfig, Scheduler sched, BpelServer server,
		...
		File fileToWatch = new File(_pconf.getBaseURI().resolve(
				_serviceName.getLocalPart() + ".axis2"));
		_axisServiceWatchDog = WatchDog.watchFile(fileToWatch,
				new ServiceFileObserver(fileToWatch));
		_axisOptionsWatchDog = new WatchDog<Map, OptionsObserver>(
				new EndpointPropertiesMutable(), new OptionsObserver());
		_configContext = new ConfigurationContext(_axisConfig);
		_configContext.setProperty(
				HTTPConstants.MUTTITHREAD_HTTP_CONNECTION_MANAGER, connManager);
		...
		Element eprElmt = ODEService.genEPRfromWSDL(_definition, serviceName,
				portName);
		if (eprElmt == null)
			throw new IllegalArgumentException(msgs.msgPortDefinitionNotFound(
					serviceName, portName));
		endpointReference = EndpointFactory.convertToWSA(ODEService
				.createServiceRef(eprElmt));
	...
	public void invoke(final PartnerRoleMessageExchange odeMex) {
		...
		try {
			 ...
			 * the MessageContexgt. That allows the user to override specific
			 * options on a given message context and not affect the overall
			 * options.
			 */
			...
			String mexEndpointUrl = ((MutableEndpoint) odeMex
					.getEndpointReference()).getUrl();
			...
			if (endpointUrl.equals(mexEndpointUrl)) {
				String address = (String) client.getOptions().getProperty(
						Properties.PROP_ADDRESS);
				if (address != null) {
					if (__log.isDebugEnabled())
						__log.debug("Endpoint URL overridden by property files. "
								+ mexEndpointUrl + " => " + address);
					axisEPR.setAddress(address);
				}
			} else {
				if (__log.isDebugEnabled())
					__log.debug("Endpoint URL overridden by process. "
							+ endpointUrl + " => " + mexEndpointUrl);
			}

			axisEPR.setAddress(_clusterUrlTransformer
					.rewriteOutgoingClusterURL(axisEPR.getAddress()));
			...
			final OperationClient operationClient = client
					.createClient(isTwoWay ? ServiceClient.ANON_OUT_IN_OP
			...
			AuthenticationHelper
					.setHttpAuthentication(odeMex, operationOptions);
			...
			if (isTwoWay) {
				...
				_sched.registerSynchronizer(new Scheduler.Synchronizer() {
					public void afterCompletion(boolean success) {
						...
						// holding on the afterCompletion
						// blocks other operations that could have been listed
						// there as well.
						_executorService.submit(new Callable<Object>() {
							public Object call() throws Exception {
								try {
									...
									// need it anymore
									TransportOutDescription out = mctx
											.getTransportOut();
	...
	private ServiceClient getServiceClient() throws AxisFault {
		...
		if (client == null
				|| !client.getAxisService().getName()
						.equals(anonymousService.getName())) {
	...
	private void applySecurityPolicy(Options options) {
		if (options != null
				&& options.getProperty(Properties.PROP_SECURITY_POLICY) != null) {
			String policy = (String) options
					.getProperty(Properties.PROP_SECURITY_POLICY);
	...
	/**
	 * Extracts the action to be used for the given operation. It first checks
	 * to see if a value is specified using WS-Addressing in the portType, it
	 * then falls back onto getting it from the SOAP Binding.
	 * 
	 * @param operation
	...
	private void writeHeader(MessageContext ctxt,
		...
		if (myRoleWSAEPR != null) {
			...
			Document callbackEprXml = odeMex.getMyRoleEndpointReference()
					.toXML();
			...
			if (address != null) {
				...
				String jmsDestination = (String) options
						.getProperty(JMSConstants.PARAM_REPLY_DESTINATION);
				if (__log.isDebugEnabled()) {
				...
				if (jmsDestination == null || "".equals(jmsDestination.trim())) {
					// If the REPLY_PARAM property is not user-defined, then use
					// the default value from myRole EPR
		...
		if (replyEPR == null) {
			if (MessageExchange.MessageExchangePattern.REQUEST_RESPONSE == odeMex
					.getMessageExchangePattern()) {
				replyEPR = new EndpointReference(
						Namespaces.WS_ADDRESSING_ANON_URI);
			}
		}
		if (replyEPR != null) {
	...
	private void replyWithFailure(final String odeMexId,
			final FailureType error, final String errmsg) {
		// ODE MEX needs to be invoked in a TX.
		try {
			_sched.execTransaction(new Callable<Void>() {
				public Void call() throws Exception {
					PartnerRoleMessageExchange odeMex = (PartnerRoleMessageExchange) _server
							.getEngine().getMessageExchange(odeMexId);
	...
	private void reply(final String odeMexId, final Operation operation,
				...
				public Void call() throws Exception {
					...
					try {
						...
						if (isFault) {
							...
							Fault fault = _converter.parseSoapFault(odeMsgEl,
									reply.getEnvelope(), operation);

							if (fault != null) {
								if (__log.isWarnEnabled())
									__log.warn("Fault response: faultName="
								...
								QName faultName = new QName(_definition
										.getTargetNamespace(), fault.getName());
								...
								odeMex.replyWithFailure(FailureType.OTHER,
										reply.getEnvelope().getBody()
												.getFault().getText(), OMUtils
												.toDOM(reply.getEnvelope()
														.getBody()));
							}
						} else {
							Message response = odeMex.createMessage(odeMex
							...
							if (__log.isInfoEnabled())
								__log.info("Response:\n"
										+ (response.getMessage() != null ? DOMUtils
												.domToString(response
														.getMessage())
												: "empty"));
	...
	/**
	 * This class wraps a {@link org.apache.axis2.client.ServiceClient} and
	 * watches changes (deletions,creations,updates) on a Axis2 service config
	 * file named {service-name}.axis2.
	 * <p/>
	 * The {@link org.apache.axis2.client.ServiceClient} instance is created
	 * from the main Axis2 config instance and this service-specific config
	 * file.
	 */
	private class ServiceFileObserver extends
		...
		public void init() {
			...
			// ServiceClient
			// this service will be added to the AxisConfig so do not reuse the
			// name of the external service
			// as it could blow up if the service is deployed in the same axis2
			// instance
			...
			// set a right default action *after* operations have been added to
			// the service.
		...
		public void onUpdate() {
			...
			// if the config file has been modified (i.e added or updated),
			// re-create a ServiceClient
			// and load the new config.
			init(); // create a new ServiceClient instance
			try {
				String name = object.getName();
				AxisUtils.configureService(_configContext, object, file.toURI()
						.toURL());
				// do not allow the service.xml file to change the service name
				object.setName(name);
			} catch (Exception e) {
				if (__log.isWarnEnabled())
					__log.warn("Exception while configuring service: "
							+ _serviceName, e);
				throw new RuntimeException(
						"Exception while configuring service: " + _serviceName,
						e);
			}
	...
	private class OptionsObserver extends WatchDog.DefaultObserver<Options> {
		...
		public void onUpdate() {
			...
			// note: don't make this map an instance attribute, so we always get
			// the latest version
			final Map<String, String> properties = _pconf
					.getEndpointProperties(endpointReference);
	...
	private class EndpointPropertiesMutable implements WatchDog.Mutable<Map> {
		...
		public boolean hasChangedSince(Map since) {
			Map latest = lastModified(); // cannot be null but better be
											// prepared
