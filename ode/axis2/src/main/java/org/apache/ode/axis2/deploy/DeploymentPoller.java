...
public class DeploymentPoller {
	...
	public DeploymentPoller(File deployDir, final ODEServer odeServer) {
		...
		_systemSchedulesConf = createSystemSchedulesConfig(odeServer
				.getConfigRoot());
		_systemCronConfigWatchDog = createSystemCronConfigWatchDog(odeServer
				.getBpelServer().getContexts().cronScheduler);
	}
	...
	private void check() {
		...
		if (isDeploymentFromODEFileSystemAllowed() && files != null) {
			for (File file : files) {
				...
				try {
					Collection<QName> deployed = _odeServer.getProcessStore()
							.deploy(file);
					__log.info("Deployment of artifact " + file.getName()
							+ " successful: " + deployed);
				} catch (Exception e) {
					__log.error("Deployment of " + file.getName()
		...
		for (File file : deployed) {
			String pkg = file.getName().substring(0,
					file.getName().length() - ".deployed".length());
			File deployDir = new File(_deployDir, pkg);
			if (!deployDir.exists()) {
				Collection<QName> undeployed = _odeServer.getProcessStore()
						.undeploy(deployDir);
	...
	protected WatchDog ensureDeployXmlWatchDog(File deployFolder, File deployXml) {
		WatchDog ddWatchDog = dDWatchDogsByPath
				.get(deployXml.getAbsolutePath());
		if (ddWatchDog == null) {
			ddWatchDog = WatchDog.watchFile(deployXml, new DDWatchDogObserver(
					deployFolder.getName()));
	...
	protected void disposeDeployXmlWatchDog(File deployDir) {
		dDWatchDogsByPath.remove(new File(deployDir, "deploy.xml")
				.getAbsolutePath());
	}
	...
	@SuppressWarnings("unchecked")
	protected WatchDog createSystemCronConfigWatchDog(
		...
		return WatchDog.watchFile(_systemSchedulesConf.getSchedulesFile(),
				new WatchDog.DefaultObserver() {
					public void init() {
						cronScheduler
								.refreshSystemCronJobs(_systemSchedulesConf);
					}
				});
	}
