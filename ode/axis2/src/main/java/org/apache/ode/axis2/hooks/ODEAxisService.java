 ...
 * Allows us to build the service using a WSDL definition using our own
 * receiver.
 */
public class ODEAxisService {
	...
	public static AxisService createService(AxisConfiguration axisConfig,
			ProcessConf pconf, QName wsdlServiceName, String portName)
			throws AxisFault {
		Definition wsdlDefinition = pconf
				.getDefinitionForService(wsdlServiceName);

		if (LOG.isDebugEnabled()) {
			LOG.debug("Create AxisService:" + " service=" + wsdlServiceName
					+ " port=" + portName + " WSDL="
					+ wsdlDefinition.getDocumentBaseURI() + " BPEL="
					+ pconf.getBpelDocument());
		}

		InputStream is = null;
		try {
			URI baseUri = pconf.getBaseURI().resolve(
					wsdlDefinition.getDocumentBaseURI());
			is = baseUri.toURL().openStream();
			WSDL11ToAxisPatchedBuilder serviceBuilder = new WSDL11ToAxisPatchedBuilder(
					is, wsdlServiceName, portName);
			serviceBuilder.setBaseUri(baseUri.toString());
			serviceBuilder.setCustomResolver(new Axis2UriResolver());
			serviceBuilder
					.setCustomWSLD4JResolver(new Axis2WSDLLocator(baseUri));
			...
			for (File file : pconf.getFiles()) {
				if (file.getAbsolutePath().indexOf(
						wsdlDefinition.getDocumentBaseURI()) > 0)
			...
			URL service_file = pconf.getBaseURI()
					.resolve(wsdlServiceName.getLocalPart() + ".axis2").toURL();
			if (LOG.isDebugEnabled()) {
				LOG.debug("Looking for Axis2 service configuration file: "
						+ service_file);
			}
			try {
				...
				LOG.warn(
						"Exception while configuring service: " + service_file,
						except);
			}

			final WSDL11Endpoint endpoint = new WSDL11Endpoint(wsdlServiceName,
					portName);
			final Map<String, String> properties = pconf
	...
	private static String extractJMSDestinationName(String axisServiceName,
		...
		if (index == -1) {
			...
			index = axisServiceName.indexOf(destinationPrefix);
			return (index != -1) ? axisServiceName.substring(destinationPrefix
					.length()) : axisServiceName;
		} else {
	...
	public static AxisService createService(AxisConfiguration axisConfig,
			QName serviceQName, String port, String axisName,
			Definition wsdlDef, MessageReceiver receiver) throws AxisFault {

		WSDL11ToAxisServiceBuilder serviceBuilder = new WSDL11ToAxisServiceBuilder(
				wsdlDef, serviceQName, port);
		AxisService axisService = serviceBuilder.populateService();
	...
	private static String extractEndpointUri(ProcessConf pconf,
		...
		if (port == null) {
			throw new OdeFault("Couldn't find port " + portName
					+ " in definition " + wsdlServiceName);
		}
		...
		if (url == null) {
			throw new OdeFault(
					"Could not extract any soap:address from service WSDL definition "
							+ wsdlServiceName
	...
	private static boolean isJmsEndpoint(ProcessConf pconf,
			QName wsdlServiceName, String portName) throws AxisFault {
		String url = extractEndpointUri(pconf, wsdlServiceName, portName);
		return url.startsWith("jms:");
	}

	private static String extractServiceName(ProcessConf pconf,
			QName wsdlServiceName, String portName) throws AxisFault {
		...
		if (serviceName == null) {
			throw new OdeFault(
					"The soap:address "
							...
							+ " and port "
							+ portName
							+ " should be of the form http://hostname:port/ode/processes/myProcessEndpointName");
		}
		return serviceName;
	}

	/**
	 * Obtain the service name from the request URL. The request URL is expected
	 * to use the path "/processes/" under which all processes and their
	 * services are listed. Returns null if the path does not contain this part.
	 */
		...
		if (-1 != index) {
			...
			if (path.length() > serviceStart + 1) {
				...
				if (path.startsWith("jms:/")) {
					...
					// // allow successive slashes ("//") in the URI
	...
	private static void declarePartsElements(Definition wsdlDefinition,
			QName wsdlServiceName, String axisServiceName, String portName) {
		List wsldOps = wsdlDefinition.getService(wsdlServiceName)
				.getPort(portName).getBinding().getPortType().getOperations();
		for (Object wsldOp : wsldOps) {
			Operation wsdlOp = (Operation) wsldOp;
			Collection parts = wsdlOp.getInput().getMessage().getParts()
					.values();
			// More than one part, it's rpc/enc, no mapping needs to be declared
			if (parts.size() == 1) {
				Part part = (Part) parts.iterator().next();
				// Parts are types, it's rpc/enc, no mapping needs to be
				// declared
				if (part.getElementName() != null)
					ODEAxisOperationDispatcher.addElmtToOpMapping(
							axisServiceName, wsdlOp.getName(), part
	...
	public static class WSDL11ToAxisPatchedBuilder extends
		...
		public WSDL11ToAxisPatchedBuilder(InputStream in, QName serviceName,
				String portName) {
			super(in, serviceName, portName);
		}

		public WSDL11ToAxisPatchedBuilder(Definition def, QName serviceName,
				String portName) {
			super(def, serviceName, portName);
		}

		public WSDL11ToAxisPatchedBuilder(Definition def, QName serviceName,
				String portName, boolean isAllPorts) {
			...
			synchronized (cached) {
				...
				if (LOG.isDebugEnabled()) {
					LOG.debug("getXMLSchema identity: "
							+ System.identityHashCode(element) + " baseURI: "
							+ baseUri + " elementBaseURI: "
							+ element.getBaseURI() + " documentBaseURI:"
							+ element.getOwnerDocument().getBaseURI()
							+ " documentURI: "
	 ...
	 * ${deploy_bundleNcName}/${diagram_relativeURL}/${process_relativeURL} When
	 * a service name (local part only) is qualified (prefixed) with the above,
			...
			if (bundleName != null) {
				...
				if (pconf.getBpelDocument() != null) {
					...
					if (bpelDocumentName.indexOf(".") > 0) {
						bpelDocumentName = bpelDocumentName.substring(0,
								bpelDocumentName.indexOf("."));
					}
					baseServiceUri.append(bpelDocumentName).append("/");
					String processName = pconf.getType() != null ? pconf
							.getType().getLocalPart() : null;
					if (processName != null) {
