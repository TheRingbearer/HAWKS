...
public class HttpBindingValidator {
	...
	private static final Messages httpMsgs = Messages
			.getMessages(Messages.class);
	private static final org.apache.ode.utils.wsdl.Messages wsdlMsgs = Messages
			.getMessages(org.apache.ode.utils.wsdl.Messages.class);
	...
	public HttpBindingValidator(Binding binding) {
		...
		if (!WsdlUtils.useHTTPBinding(binding))
			throw new IllegalArgumentException(
					httpMsgs.msgHttpBindingNotUsed(binding));
	}
	...
	protected void validatePort() {
		...
		for (int i = 0; i < binding.getBindingOperations().size(); i++) {
			BindingOperation bindingOperation = (BindingOperation) binding
	...
	protected void validateOperation(BindingOperation bindingOperation) {
		...
		if (verb == null) {
			throw new IllegalArgumentException(httpMsgs.msgMissingVerb(binding,
					bindingOperation));
		}
		if (!"GET".equalsIgnoreCase(verb) && !"DELETE".equalsIgnoreCase(verb)
			...
			throw new IllegalArgumentException(
					httpMsgs.msgUnsupportedHttpMethod(binding, verb));
		}

		BindingOutput output = bindingOperation.getBindingOutput();
		MIMEContent outputContent = WsdlUtils.getMimeContent(output
				.getExtensibilityElements());
		if (outputContent != null) {
			if (StringUtils.isEmpty(outputContent.getType())) {
				throw new IllegalArgumentException(
						httpMsgs.msgEmptyContentType(binding, bindingOperation));
			}
		}
		...
		if (WsdlUtils.useMimeMultipartRelated(input)) {
			throw new IllegalArgumentException(
					httpMsgs.msgMimeMultipartRelatedUnsupported(binding,
							bindingOperation));
		}

		// only 2 content-types supported
		MIMEContent inputContent = WsdlUtils.getMimeContent(input
				.getExtensibilityElements());
	...
	protected void validateGetOrDelete(BindingOperation bindingOperation) {
		...
		if (!WsdlUtils.useUrlEncoded(input)
			...
			throw new IllegalArgumentException(
					httpMsgs.msgOnlySupportsUrlEncodedAndUrlreplacement(
	...
	protected void validateUrlReplacement(BindingOperation bindingOperation) {
		...
		BindingInput input = bindingOperation.getBindingInput();
		Map inputParts = bindingOperation.getOperation().getInput()
				.getMessage().getParts();
		...
		if (WsdlUtils.useUrlReplacement(input)) {
			...
			for (Iterator it = partNames.iterator(); it.hasNext();) {
				...
				// than once
				if (m.matches()
						&& locationUri.split("(\\(" + name + "\\))", -1).length != 2) {
					throw new IllegalArgumentException(
							httpMsgs.msgInvalidURIPattern(binding,
