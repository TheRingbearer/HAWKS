 ...
 * protocol-neutral ODE representation into a SOAP representation and vice
 * versa.
 */
public class SoapMessageConverter {
	...
	private static final Log __log = LogFactory
			.getLog(SoapMessageConverter.class);
	...
	public SoapMessageConverter(Definition def, QName serviceName,
			String portName) throws AxisFault {
		if (def == null)
			throw new NullPointerException(
					"No WSDL definition was found for service " + serviceName
							+ " and port " + portName);
		...
		if (_soapBinding.getElementType().getNamespaceURI()
				.equals(Constants.URI_WSDL11_SOAP)) {
			_soapFactory = OMAbstractFactory.getSOAP11Factory();
		} else if (_soapBinding.getElementType().getNamespaceURI()
				.equals(Constants.URI_WSDL12_SOAP)) {
			_soapFactory = OMAbstractFactory.getSOAP12Factory();
		} else {
			throw new IllegalStateException("Unsupported SOAP binding: "
					+ _soapBinding.getElementType());
		}
	}

	@SuppressWarnings("unchecked")
	public void createSoapRequest(MessageContext msgCtx,
			org.apache.ode.bpel.iapi.Message message, Operation op)
			throws AxisFault {
		if (op == null)
		...
		BindingOperation bop = _binding.getBindingOperation(op.getName(), null,
				null);

		if (bop == null)
			throw new OdeFault(__msgs.msgBindingOperationNotFound(_serviceName,
		...
		createSoapHeaders(soapEnv, getSOAPHeaders(bi), op.getInput()
				.getMessage(), message.getHeaderParts());

		SOAPBody soapBody = getSOAPBody(bi);
		if (soapBody != null) {
			org.apache.axiom.soap.SOAPBody sb = soapEnv.getBody() == null ? _soapFactory
					.createSOAPBody(soapEnv) : soapEnv.getBody();
			createSoapBody(sb, soapBody, op.getInput().getMessage(),
					message.getMessage(), op.getName());
		}

	}

	public void createSoapResponse(MessageContext msgCtx,
			org.apache.ode.bpel.iapi.Message message, Operation op)
			throws AxisFault {
		if (op == null)
			throw new NullPointerException("Null operation");
		...
		if (message.getHeaderParts().size() > 0
			...
			createSoapHeaders(soapEnv, getSOAPHeaders(bo), op.getOutput()
					.getMessage(), message.getHeaderParts());
		...
		if (soapBody != null) {
			org.apache.axiom.soap.SOAPBody sb = soapEnv.getBody() == null ? _soapFactory
					.createSOAPBody(soapEnv) : soapEnv.getBody();
			createSoapBody(sb, soapBody, op.getOutput().getMessage(),
					message.getMessage(), op.getName() + "Response");
		}
	}

	public void createSoapHeaders(SOAPEnvelope soapEnv,
			List<SOAPHeader> headerDefs, Message msgdef,
		...
		for (Node headerNode : headers.values())
			if (headerNode.getNodeType() == Node.ELEMENT_NODE) {
				if (soaphdr.getFirstChildWithName(new QName(headerNode
						.getNamespaceURI(), headerNode.getLocalName())) == null)
					soaphdr.addChild(OMUtils.toOM((Element) headerNode,
							_soapFactory));
			} else {
				throw new OdeFault(
						__msgs.msgSoapHeaderMustBeAnElement(headerNode));
			}
	}

	@SuppressWarnings("unchecked")
	private void handleSoapHeaderDef(SOAPEnvelope soapEnv,
			SOAPHeader headerdef, Message msgdef, Map<String, Node> headers)
			throws AxisFault {
		...
		if (payloadMessageHeader && msgdef.getPart(headerdef.getPart()) == null)
			throw new OdeFault(
					__msgs.msgSoapHeaderReferencesUnkownPart(headerdef
							.getPart()));
		...
		if (headers.size() > 0 && payloadMessageHeader) {
			try {
				...
				throw new OdeFault(__msgs.msgSoapHeaderMustBeAnElement(headers
						.get(headerdef.getPart())));
			}
		}
		...
		if (srcPartEl == null && payloadMessageHeader)
			throw new OdeFault(
					__msgs.msgOdeMessageMissingRequiredPart(headerdef.getPart()));
	...
	private OMElement buildSoapDetail(Element message, QName faultName,
			Operation op) throws AxisFault {
		...
		if (p == null)
			return toFaultDetail(faultName, message);
		Element partEl = DOMUtils.findChildByName(message,
	...
	private OMElement toFaultDetail(QName fault, Element message) {
		...
		if (detail == null)
			return OMUtils.toOM(firstPart, _soapFactory);
		return OMUtils.toOM(detail, _soapFactory);
	}

	public void parseSoapRequest(org.apache.ode.bpel.iapi.Message odeMessage,
		...
		SOAPBody soapBody = getSOAPBody(bi);
		if (soapBody != null)
			extractSoapBodyParts(odeMessage, envelope.getBody(), soapBody, op
					.getInput().getMessage(), op.getName());

		if (envelope.getHeader() != null)
			extractSoapHeaderParts(odeMessage, envelope.getHeader(),
					getSOAPHeaders(bi), op.getInput().getMessage());
	}
	...
	public void parseSoapResponse(org.apache.ode.bpel.iapi.Message odeMessage,
		...
		if (soapBody != null)
			extractSoapBodyParts(odeMessage, envelope.getBody(), soapBody, op
					.getOutput().getMessage(), op.getName() + "Response");
	...
	public void createSoapBody(org.apache.axiom.soap.SOAPBody sb,
			SOAPBody soapBody, Message msgDef, Element message,
			String rpcWrapper) throws AxisFault {
		OMElement partHolder = _isRPC ? _soapFactory.createOMElement(new QName(
				soapBody.getNamespaceURI(), rpcWrapper, "odens"), sb) : sb;

		List<Part> parts = msgDef.getOrderedParts(soapBody.getParts());
		for (Part part : parts) {
			Element srcPartEl = DOMUtils.findChildByName(message, new QName(
					null, part.getName()));
			if (srcPartEl == null)
				throw new OdeFault(__msgs.msgOdeMessageMissingRequiredPart(part
						.getName()));
	...
	// public Element createODEMessage(SOAPEnvelope soapEnv,Operation op) throws
	// AxisFault {
	// }
	@SuppressWarnings("unchecked")
	public void extractSoapBodyParts(org.apache.ode.bpel.iapi.Message message,
			org.apache.axiom.soap.SOAPBody soapBody, SOAPBody bodyDef,
			Message msg, String rpcWrapper) throws AxisFault {
		...
		if (_isRPC) {
			QName rpcWrapQName = new QName(bodyDef.getNamespaceURI(),
					rpcWrapper);
			OMElement partWrapper = soapBody
					.getFirstChildWithName(rpcWrapQName);
			if (partWrapper == null)
				throw new OdeFault(
						__msgs.msgSoapBodyDoesNotContainExpectedPartWrapper(
								_serviceName, _portName, rpcWrapQName));
			...
			for (Part pdef : bodyParts) {
				OMElement srcPart = partWrapper
						.getFirstChildWithName(new QName(null, pdef.getName()));
				if (srcPart == null)
					throw new OdeFault(
							__msgs.msgSOAPBodyDoesNotContainRequiredPart(pdef
									.getName()));
				message.setPart(srcPart.getLocalName(), OMUtils.toDOM(srcPart));
			}

		} else {
			// In doc-literal style, we expect the elements in the body to
			// correspond (in order) to the
			// parts defined in the binding. All the parts should be
			// element-typed, otherwise it is a mess.
			Iterator<OMElement> srcParts = soapBody.getChildElements();
			for (Part partDef : bodyParts) {
				if (!srcParts.hasNext())
					throw new OdeFault(
							__msgs.msgSOAPBodyDoesNotContainRequiredPart(partDef
									.getName()));

				OMElement srcPart = srcParts.next();
				if (partDef.getElementName() == null)
					throw new OdeFault(
							__msgs.msgBindingDefinesNonElementDocListParts());
				if (!srcPart.getQName().equals(partDef.getElementName()))
					throw new OdeFault(__msgs.msgUnexpectedElementInSOAPBody(
							srcPart.getQName(), partDef.getElementName()));
				Document doc = DOMUtils.newDocument();
				Element destPart = doc.createElementNS(null, partDef.getName());
				destPart.appendChild(doc.importNode(OMUtils.toDOM(srcPart),
						true));
	...
	public void extractSoapHeaderParts(
			org.apache.ode.bpel.iapi.Message message,
	...
	private void handleSoapHeaderPartDef(
			org.apache.ode.bpel.iapi.Message odeMessage,
			org.apache.axiom.soap.SOAPHeader header, SOAPHeader headerdef,
		...
		if (requiredHeader && header == null)
			throw new OdeFault(
					__msgs.msgSoapHeaderMissingRequiredElement(headerdef
							.getElementType()));
	...
	private String findHeaderPartName(List<SOAPHeader> headerDefs,
			QName elmtName) {
		for (SOAPHeader headerDef : headerDefs) {
			Message hdrMsg = _def.getMessage(headerDef.getMessage());
	...
	public static <T> T getFirstExtensibilityElement(ElementExtensible parent,
			Class<T> cls) {
		Collection<T> ee = CollectionsX.filter(
				parent.getExtensibilityElements(), cls);
	 ...
	 * operation definition. When WS-Addressing is being used by a service
	 * provider, the "Action" is specified in the portType->operation instead of
	 * the SOAP binding->operation.
	 * 
	 * @param operation
	...
	public String getWSAInputAction(String operation) {
		...
		if (input != null) {
			Object actionQName = input.getExtensionAttribute(new QName(
					Namespaces.WS_ADDRESSING_NS, "Action"));
	...
	public String getSoapAction(String operation) {
		...
		for (SOAPOperation soapOp : CollectionsX.filter(
				bop.getExtensibilityElements(), SOAPOperation.class))
			return soapOp.getSoapActionURI();

		return "";
	}

	public Fault parseSoapFault(Element odeMsgEl, SOAPEnvelope envelope,
		...
		Part pdef = (Part) fdef.getMessage().getParts().values().iterator()
				.next();
		Element partel = odeMsgEl.getOwnerDocument().createElementNS(null,
				pdef.getName());
		...
		if (detail.getFirstChildWithName(pdef.getElementName()) != null) {
			partel.appendChild(odeMsgEl.getOwnerDocument().importNode(
					OMUtils.toDOM(detail.getFirstChildWithName(pdef
							.getElementName())), true));
		} else {
			partel.appendChild(odeMsgEl.getOwnerDocument().importNode(
					OMUtils.toDOM(detail), true));
		}
	...
	private Fault inferFault(Operation operation, SOAPFault flt) {
		if (flt.getDetail() == null
				|| flt.getDetail().getFirstElement() == null)
			return null;
