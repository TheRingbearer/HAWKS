 ...
 * href='http://www.w3.org/TR/wsdl#_http:urlReplacement'>WSDL specification</a>. <br/>
 * Surrounding characters for parts may be parentheses '()' or braces '{}'.
 * Pattern with parentheses is look up first, if found then it's replaced with
 * the part value, else the pattern with braces is look up.
 * <p/>
...
public class UrlReplacementTransformer {
	...
	private static final Log log = LogFactory
			.getLog(UrlReplacementTransformer.class);

	private static final org.apache.ode.axis2.httpbinding.Messages httpMsgs = org.apache.ode.axis2.httpbinding.Messages
			.getMessages(org.apache.ode.axis2.httpbinding.Messages.class);
	...
	/**
	 * @param baseUri
	 *            - the base uri template containing part names enclosed within
	 *            single curly braces
	 * @param values
	 *            - a map<String, Element>, the key is a part name (without
	 *            curly braces), the value the replacement value for the part
	 *            name. If the value is not a simple type, it will be skipped.
	 * @return the encoded uri
	 * @throws java.lang.IllegalArgumentException
	 *             if a replacement value is null in the map or if a part
	...
	public String transform(String baseUri, Map<String, Element> values) {
		...
		for (Map.Entry<String, Element> e : values.entrySet()) {
			...
			if (replacementValue != null) {
				try {
					replacementValue = URIUtil
							.encodeWithinQuery(replacementValue);
	...
	private boolean replace(List<String> result, String partPattern,
			String replacementValue) {
