...
public class HttpMethodConverter {
	...
	public HttpMethodConverter(Definition definition, QName serviceName,
		...
		this.binding = definition.getService(serviceName).getPort(portName)
				.getBinding();
	...
	public HttpMethod createHttpRequest(PartnerRoleMessageExchange odeMex,
		...
		return createHttpRequest(odeMex, params,
				((MutableEndpoint) odeMex.getEndpointReference()).getUrl());
	}

	public HttpMethod createHttpRequest(PartnerRoleMessageExchange odeMex,
			HttpParams params, String baseUrl)
		...
		// extract part values into a map and check that all parts are assigned
		// a value
		Map<String, Element> partElements = extractPartElements(msgDef, message);

		// http method type
		// the operation may override the verb, this is an extension for RESTful
		// BPEL
		...
		HttpMethod method = prepareHttpMethod(bindingOperation, verb,
				partElements, odeMex.getRequest().getHeaderParts(), baseUrl,
				params);
	...
	/**
	 * create and initialize the http method. Http Headers that may been passed
	 * in the params are not set in this method. Headers will be automatically
	 * set by HttpClient. See usages of HostParams.DEFAULT_HEADERS See
	 * org.apache
	 * .commons.httpclient.HttpMethodDirector#executeMethod(org.apache.
	 * commons.httpclient.HttpMethod)
	 */
	protected HttpMethod prepareHttpMethod(BindingOperation opBinding,
		...
		HTTPOperation httpOperation = (HTTPOperation) WsdlUtils
				.getOperationExtension(opBinding);
		MIMEContent content = WsdlUtils.getMimeContent(bindingInput
		...
		if (StringUtils.isNotEmpty(relativeUri)) {
			completeUri = completeUri
					+ (completeUri.endsWith("/") || relativeUri.startsWith("/") ? ""
							: "/") + relativeUri;
		}

		if (useUrlReplacement) {
			// insert part values in the url
			completeUri = new UrlReplacementTransformer().transform(
					completeUri, partValues);
		...
		if ("GET".equalsIgnoreCase(verb) || "DELETE".equalsIgnoreCase(verb)) {
			...
			method.getParams().setDefaults(params);
			// some body-building...
			final String contentCharset = method.getParams()
					.getContentCharset();
			if (log.isDebugEnabled())
				log.debug("Content-Type [" + contentType + "] Charset ["
						+ contentCharset + "]");
			if (useUrlEncoded) {
				...
				Part part = opBinding.getOperation().getInput().getMessage()
						.getPart(content.getPart());
				...
				if (part.getElementName() == null) {
					...
					String xmlString = DOMUtils.domToString(DOMUtils
							.getFirstChildElement(partValue));
					...
					requestEntity = new StringRequestEntity(
							DOMUtils.getTextContent(partValue), contentType,
							contentCharset);
				}
			}
			...
			enclosingMethod.setContentChunked(params.getBooleanParameter(
					Properties.PROP_HTTP_REQUEST_CHUNK, false));

		} else {
			// should not happen because of HttpBindingValidator, but never say
			// never
	 ...
	 * the default headers should be overriden by any headers set from the
	 * process. Not to mention that, for a given header, HttpClient do not
	 * overwrite any previous values but simply append the default value.<br/>
	 * See {@link see
	 ...
	 * <p/>
	 * Finally go through the list of
	 * {@linkplain Namespaces.ODE_HTTP_EXTENSION_NS}{@code :header} elements
	 * included in the input binding. For each of them, set the HTTP Request
	 * Header with the static value defined by the attribute
	 * {@linkplain Namespaces.ODE_HTTP_EXTENSION_NS}{@code :value}, or the part
	 * value mentioned in the attribute
	 * {@linkplain Namespaces.ODE_HTTP_EXTENSION_NS}{@code :part}.
	 * <p/>
	 ...
	 * Meaning that message headers might get overriden by parts bound to
	 * headers.
	 * 
	 */
	public void setHttpRequestHeaders(HttpMethod method,
			BindingOperation opBinding, Map<String, Element> partValues,
			Map<String, Node> headers, HttpParams params) {
		...
		// Actually the default headers should be overriden by any headers set
		// from the process.
		// (Not to mention that, for a given header, HttpClient do not overwrite
		// any previous values but simply append the default value)
		Collection defaultHeaders = (Collection) params
		...
		for (Iterator<Map.Entry<String, Node>> iterator = headers.entrySet()
		...
		for (Iterator<UnknownExtensibilityElement> iterator = headerBindings
				.iterator(); iterator.hasNext();) {
			...
			if (StringUtils.isNotEmpty(partName)) {
				...
				if (DOMUtils.isEmptyElement(partWrapper)) {
					...
					/*
					 * The expected part value could be a simple type or an
					 * element of a simple type. So if a element is there, take
					 * its text content else take the text content of the part
					 * element itself
					 */
					Element childElement = DOMUtils
		...
		MIMEContent outputContent = WsdlUtils.getMimeContent(opBinding
	...
	protected Map<String, Element> extractPartElements(Message msgDef,
			Element message) {
		...
		for (Iterator iterator = msgDef.getParts().values().iterator(); iterator
				.hasNext();) {
			...
			if (partEl == null)
				throw new IllegalArgumentException(
						msgs.msgOdeMessageMissingRequiredPart(part.getName()));
	 ...
	 * {@link org.apache.ode.bpel.iapi.Message}. <br/>
	 * An element named with the part name will be returned. the content of this
	 * element depends on the part.
	 * <p/>
	 * If the part has a non-null element name, a new element will be created
	 * and named accordingly then the text value is inserted in this new
	 * element. <br/>
	...
	public Element createPartElement(Part part, String textContent) {
		...
		if (part.getElementName() != null) {
			Element element = doc.createElementNS(part.getElementName()
					.getNamespaceURI(), part.getElementName().getLocalPart());
	...
	/**
	 * Create the element to be associated with this part into the
	 * {@link org.apache.ode.bpel.iapi.Message}.
	 * <p/>
	 * If the part has a non-null element name, the bodyElement is simply
	 * appended. Else if the bodyElement has a text content, the value is set to
	 * the message. Else append all nodes of bodyElement to the returned
	...
	public Element createPartElement(Part part, Element receivedElement) {
		...
		if (part.getElementName() != null) {
			...
			if (DOMUtils.isEmptyElement(receivedElement)) {
				...
				// Warning! setting an empty string with setTextContent has not
				// effect. See javadoc.
				partElement.appendChild(doc.createTextNode(""));
			} else {
				// No need to make the distinction between simple and complex
				// types, importNode will handle it
	 ...
	 * {@linkplain Namespaces.ODE_HTTP_EXTENSION_NS}{@code :header} elements
	 * included in the output binding. For each of them, set the header value as
	 * the value of the message part.
	 * <p/>
	 * Then add all HTTP headers as header part in the message. The name of the
	 * header would be the part name.
	 * <p/>
	 * Finally, insert a header names 'Status-Line'. This header contains an
	 * element as returned by {@link HttpHelper#statusLineToElement(String)} .
	 * 
	 * @param odeMessage
	 * @param method
	 * @param operationDef
	 */
	public void extractHttpResponseHeaders(
			org.apache.ode.bpel.iapi.Message odeMessage, HttpMethod method,
			Operation operationDef) {
		Message messageDef = operationDef.getOutput().getMessage();

		BindingOutput outputBinding = binding.getBindingOperation(
				operationDef.getName(), operationDef.getInput().getName(),
		...
		for (Iterator<UnknownExtensibilityElement> iterator = headerBindings
			...
			if (StringUtils.isNotEmpty(partName)) {
				...
				// if the response header is not set, just skip it. no need to
				// fail.
				if (responseHeader != null) {
					odeMessage.setPart(partName,
							createPartElement(part, responseHeader.getValue()));
				}
			} else {
		...
		// add all HTTP response headers (in their condensed form) into the
		// message as header parts
		Set<String> headerNames = new HashSet<String>();
		for (Header header : method.getResponseHeaders())
			headerNames.add(header.getName());
		for (String hname : headerNames)
			odeMessage.setHeaderPart(hname, method.getResponseHeader(hname)
					.getValue());
	...
	public void parseHttpResponse(org.apache.ode.bpel.iapi.Message odeResponse,
			HttpMethod method, Operation opDef) throws SAXException,
			IOException {
		BindingOperation opBinding = binding.getBindingOperation(opDef
				.getName(), opDef.getInput().getName(), opDef.getOutput()
				.getName());
		/* process headers */
		...
		// that there is no body, so we should not fail even if a part is bound
		...
		try {
			...
			throw new RuntimeException("Unable to get the request body : "
					+ e.getMessage());
		}
		...
		if (emptyBody) {
			if (isBodyMandatory) {
				throw new RuntimeException(
						"Response body is mandatory but missing!");
			}
		} else {
			if (isBodyExpected) {
				Part partDef = opDef.getOutput().getMessage()
						.getPart(outputContent.getPart());
				Element partElement;
					...
					if (!contentTypeSet) {
						...
						if (log.isDebugEnabled())
							log.debug("Xml type was expected but non-xml type received! Expected Content-Type="
									+ outputContent.getType()
									+ " Received Content-Type=" + receivedType);
					}
				...
				// the body was not expected but we don't know how to deal with
				// it
				if (log.isDebugEnabled())
	...
	public Object[] parseFault(PartnerRoleMessageExchange odeMex,
			HttpMethod method) {
		...
		if (opDef.getFaults().isEmpty()) {
			throw new RuntimeException("Operation [" + opDef.getName()
					+ "] has no fault. This " + method.getStatusCode()
			...
			throw new RuntimeException(
					"Response Content-Type ["
							+ receivedType
							+ "] does not describe XML entities. Faults must be XML. This "
							+ method.getStatusCode()
							+ " error will be considered as a failure.");
		} else {
			...
			try {
				...
				throw new RuntimeException(
						"Unable to parse the response body as xml. This "
								+ method.getStatusCode()
								+ " error will be considered as a failure.", e);
			}
			...
			if (faultDef == null) {
				throw new RuntimeException("Unknown Fault Type [" + bodyName
						...
						+ " error will be considered as a failure.");
			} else if (!WsdlUtils.isOdeFault(opBinding.getBindingFault(faultDef
					.getName()))) {
				// is this fault bound with ODE extension?
				throw new RuntimeException("Fault [" + bodyName
						+ "] is not bound with "
						+ new QName(Namespaces.ODE_HTTP_EXTENSION_NS, "fault")
						+ ". This " + method.getStatusCode()
						+ " error will be considered as a failure.");
			} else {
				// a fault has only one part
				Part partDef = (Part) faultDef.getMessage().getParts().values()
						.iterator().next();
				...
				org.apache.ode.bpel.iapi.Message response = odeMex
						.createMessage(faultType);
