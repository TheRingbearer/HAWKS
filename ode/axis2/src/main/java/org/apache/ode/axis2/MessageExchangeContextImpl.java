 ...
 * {@link org.apache.ode.bpel.iapi.MessageExchangeContext} interface. This class
 * is used by the ODE engine to make invocation of external services using Axis.
 */
public class MessageExchangeContextImpl implements MessageExchangeContext {
	...
	public void invokePartner(
		...
		ExternalService service = (ExternalService) partnerRoleMessageExchange
				.getChannel();
		if (__log.isDebugEnabled())
			__log.debug("The service to invoke is the external service "
					+ service.getServiceName() + ":" + service.getPortName());
	...
	public void onAsyncReply(MyRoleMessageExchange myRoleMessageExchange)
		...
		// Nothing to do, no callback is necessary, the client just synchornizes
		// itself with the
