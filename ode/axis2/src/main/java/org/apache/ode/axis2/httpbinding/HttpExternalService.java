...
public class HttpExternalService implements ExternalService {
	...
	public HttpExternalService(ProcessConf pconf, QName serviceName,
			String portName, ExecutorService executorService,
			Scheduler scheduler, BpelServer server,
		...
		if (portBinding == null)
			throw new IllegalArgumentException(
					msgs.msgBindingNotFound(portName));

		// validate the http binding
		if (!WsdlUtils.useHTTPBinding(port)) {
			throw new IllegalArgumentException(
					msgs.msgNoHTTPBindingForPort(portName));
		}
		...
		httpMethodConverter = new HttpMethodConverter(definition, serviceName,
				portName);
	...
	public void invoke(PartnerRoleMessageExchange odeMex) {
		if (log.isDebugEnabled())
			log.debug("Preparing " + getClass().getSimpleName()
					+ " invocation...");
		try {
			...
			final HttpParams params = Properties.HttpClient
			...
			if (endpointUrl.equals(new URL(mexEndpointUrl))) {
				String address = (String) params
						.getParameter(Properties.PROP_ADDRESS);
				if (address != null) {
					if (log.isDebugEnabled())
						log.debug("Endpoint URL overridden by property files. "
								+ mexEndpointUrl + " => " + address);
					baseUrl = address;
				}
			} else {
				if (log.isDebugEnabled())
					log.debug("Endpoint URL overridden by process. "
			...
			final HttpMethod method = httpMethodConverter.createHttpRequest(
					odeMex, params, baseUrl);
			...
			Element authenticatePart = message == null ? null : DOMUtils
					.findChildByName(message, new QName(null,
							"WWW-Authenticate"));
			HttpHelper.configure(client, method.getURI(), authenticatePart,
					params);
			...
			if (isTwoWay) {
				...
				executionCallable = new HttpExternalService.TwoWayCallable(
						client, method, odeMex.getMessageExchangeId(),
						odeMex.getOperation());
				...
				executionCallable = new HttpExternalService.OneWayCallable(
						client, method, odeMex.getMessageExchangeId(),
						odeMex.getOperation());
				executorService.submit(executionCallable);
	...
	private class OneWayCallable implements Callable<Void> {
		...
		public OneWayCallable(HttpClient client, HttpMethod method,
				String mexId, Operation operation) {
			this.method = method;
			this.mexId = mexId;
		...
		public Void call() throws Exception {
			try {
				...
				// Actually the processResponse may happen in a separate thread
				// and
				...
				try {
					scheduler.execTransaction(new Callable<Void>() {
						public Void call() throws Exception {
							PartnerRoleMessageExchange odeMex = (PartnerRoleMessageExchange) server
									.getEngine().getMessageExchange(mexId);
					...
					String errmsg = "[Service: "
							+ serviceName
							+ ", Port: "
							+ portName
							+ ", Operation: "
							+ operation.getName()
							+ "] Error executing reply transaction; reply will be lost.";
					log.error(errmsg, e);
				}
			} finally {
		...
		public void processResponse(int statusCode) {
			...
			try {
				// log the URI since the engine may have moved on while this One
				// Way request was executing
				if (statusCode >= 400) {
					log.error("OneWay HTTP Request failed, Status-Line: "
							+ method.getStatusLine() + " for "
							+ method.getURI());
				} else {
					if (log.isDebugEnabled())
				...
				String errmsg = "[Service: "
						...
						+ ", Operation: "
						+ operation.getName()
						+ "] Exception occured while processing the HTTP response of a one-way request: "
						+ e.getMessage();
				log.error(errmsg, e);
			}
		}
				...
				scheduler.execTransaction(new Callable<Void>() {
					public Void call() throws Exception {
						try {
							...
							replyWithFailure(
									"Exception occured while processing the HTTP response of a two-way request. mexId= "
											+ mexId, e);
						}
						return null;
					}
		...
		private void _2xx_success() throws Exception {
			...
			if (log.isDebugEnabled())
				log.debug("[Service: " + serviceName + ", Port: " + portName
						...
						+ "] HTTP Status-Line: " + method.getStatusLine()
						+ " for " + method.getURI());
			if (log.isDebugEnabled())
			...
			QName outputMsgName = odeMex.getOperation().getOutput()
					.getMessage().getQName();
			...
			try {
				if (log.isInfoEnabled())
					log.info("Response: "
							+ (odeResponse.getMessage() != null ? DOMUtils
									.domToString(odeResponse.getMessage())
									: "empty"));
				odeMex.reply(odeResponse);
			} catch (Exception ex) {
				replyWithFailure(
						"Unable to process response: " + ex.getMessage(), ex);
			}
		}
		...
		void replyWithFault() {
			...
			if (log.isWarnEnabled())
				log.warn("[Service: " + serviceName + ", Port: " + portName
						+ ", Operation: " + operation.getName()
						+ "] Fault response: faultName=" + faultName
						+ " faultType=" + response.getType() + "\n"
