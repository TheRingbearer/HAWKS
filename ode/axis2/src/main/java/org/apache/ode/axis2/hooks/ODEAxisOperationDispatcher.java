...
public class ODEAxisOperationDispatcher extends AbstractDispatcher {
	...
	private static final Log log = LogFactory
			.getLog(ODEAxisOperationDispatcher.class);
	...
	public AxisOperation findOperation(AxisService service,
		...
		if (bodyFirstChild != null) {
			...
			if (index >= 0 && index + "Request".length() == localName.length()) {
				AxisOperation op = service.getOperation(new QName(localName
						.substring(0, index)));
				if (op != null)
					return op;
			}
			...
			// operation => element
			// mapping if we can find something (useful for doc/lit when people
			// have the bad idea
			// of using a different name for their operation and part element)
			String opName = (String) _elmtToOperation.get(service.getName(),
					localName);
			if (opName != null) {
	 ...
	 * Only makes sense for doc/lit services (only one part) for which the
	 * operation can't easily be guessed from the message element name.
	 * 
	 * @param axisServiceName
	 *            the service name as registered in Axis2
	 * @param operationName
	 *            operation local name
	 * @param elmtName
	 *            element local name
	 */
	public static void addElmtToOpMapping(String axisServiceName,
			String operationName, String elmtName) {
