...
public class ODEService {
	...
	public ODEService(AxisService axisService, ProcessConf pconf,
			QName serviceName, String portName, BpelServer server,
		...
		_portName = portName;
		_serviceRef = EndpointFactory
				.convertToWSA(createServiceRef(genEPRfromWSDL(_wsdlDef,
	...
	public void onAxisMessageExchange(MessageContext msgContext,
		...
		try {
			...
			__log.debug("ODE routed to portType " + odeMex.getPortType()
					+ " operation " + odeMex.getOperation() + " from service "
					+ _serviceName);
			PortType portType = odeMex.getPortType();
			Operation operation = odeMex.getOperation();

			odeMex.setProperty("isTwoWay", Boolean.toString(msgContext
					.getAxisOperation() instanceof TwoChannelAxisOperation));
			if (odeMex.getOperation() != null) {
				// Preparing message to send to ODE
				Message odeRequest = odeMex.createMessage(odeMex.getOperation()
						.getInput().getMessage().getQName());
				_converter.parseSoapRequest(odeRequest,
						msgContext.getEnvelope(), odeMex.getOperation());
				...
				// @hahnml: Read the time value to wait on instance invocation
				long waitingTime = 0L;
				OMElement timeElement = null;
				if (msgContext.getEnvelope().getHeader() != null) {
					timeElement = msgContext
							.getEnvelope()
							.getHeader()
							.getFirstChildWithName(
									new QName(
											"http://simtech.uni-stuttgart.de",
											"invokeWaitingTime", "simTech"));
					if (timeElement != null) {
				...
				// @hahnml: Set the waitingTime in the GenericController.
				GenericController.getInstance().setWaitingTime(waitingTime);
		...
		if (odeMex.getOperation().getOutput() != null) {
			...
			String mexId = null;
			try {
				mexId = responseFuture.get(getTimeout(), TimeUnit.MILLISECONDS);
				// AH: end
			} catch (Exception e) {
			...
			if (outMsgContext != null) {
				...
				try {
					...
					__log.error("Error processing response for MEX " + odeMex,
							e);
					throw new OdeFault(
	...
	public boolean respondsTo(QName serviceName, QName portTypeName) {
		...
		result = result
				&& _wsdlDef.getService(_serviceName).getPort(_portName)
						.getBinding().getPortType().getQName()
						.equals(portTypeName);
		return result;
	}
	...
	private long getTimeout() {
		String timeout = _pconf.getEndpointProperties(_serviceRef).get(
				Properties.PROP_MEX_TIMEOUT);
		if (timeout != null) {
			try {
				...
				if (__log.isWarnEnabled())
					__log.warn("Mal-formatted Property: ["
							+ Properties.PROP_MEX_TIMEOUT + "=" + timeout
							+ "] Default value ("
	...
	public Options getOptions() {
		return Properties.Axis2.translate(_pconf
				.getEndpointProperties(_serviceRef));
	}

	private void onResponse(MyRoleMessageExchange mex, MessageContext msgContext)
			throws AxisFault {
		switch (mex.getStatus()) {
		case FAULT:
			if (__log.isDebugEnabled())
				__log.debug("Fault response message: " + mex.getFault());
			SOAPFault fault = _converter.createSoapFault(mex.getFaultResponse()
					.getMessage(), mex.getFault(), mex.getOperation());
			msgContext.getEnvelope().getBody().addFault(fault);

			if (__log.isDebugEnabled())
				__log.debug("Returning fault: "
						+ msgContext.getEnvelope().toString());
			break;
		case ASYNC:
		case RESPONSE:
			_converter.createSoapResponse(msgContext, mex.getResponse(),
					mex.getOperation());
			if (__log.isDebugEnabled())
		...
		default:
			throw new OdeFault(
					"Received ODE message exchange in unexpected state: "
							+ mex.getStatus());
		}
	}
	 ...
	 */
	private void readHeader(MessageContext msgContext,
			MyRoleMessageExchange odeMex) {
		String correlationId = (String) msgContext
				.getProperty(JMSConstants.JMS_COORELATION_ID);
		...
		if (ocse != null) {
			...
			odeMex.setProperty(
					...
					endpoint.getSessionId());
			odeMex.setProperty(MessageExchange.PROPERTY_SEP_PARTNERROLE_EPR,
	 ...
	 * injection.
	 */
	private void writeHeader(MessageContext msgContext,
			MyRoleMessageExchange odeMex) {
	...
	public static Element genEPRfromWSDL(Definition wsdlDef, QName name,
		...
		if (serviceDef != null) {
			...
			if (portDef != null) {
				...
				Element service = doc.createElementNS(Namespaces.WSDL_11,
						"service");
				service.setAttribute("name", serviceDef.getQName()
						.getLocalPart());
				service.setAttribute("targetNamespace", serviceDef.getQName()
						.getNamespaceURI());
				Element port = doc.createElementNS(Namespaces.WSDL_11, "port");
				service.appendChild(port);
				port.setAttribute("name", portDef.getName());
				port.setAttributeNS("http://www.w3.org/2000/xmlns/",
						"xmlns:bindns", portDef.getBinding().getQName()
								.getNamespaceURI());
				...
				for (Object extElmt : portDef.getExtensibilityElements()) {
					if (extElmt instanceof SOAPAddress) {
						Element soapAddr = doc.createElementNS(
								Namespaces.SOAP_NS, "address");
						port.appendChild(soapAddr);
						soapAddr.setAttribute("location",
								((SOAPAddress) extElmt).getLocationURI());
					} else if (extElmt instanceof HTTPAddress) {
						Element httpAddr = doc.createElementNS(
								Namespaces.HTTP_NS, "address");
						port.appendChild(httpAddr);
						httpAddr.setAttribute("location",
								((HTTPAddress) extElmt).getLocationURI());
					} else {
						port.appendChild(doc.importNode(
								((UnknownExtensibilityElement) extElmt)
										.getElement(), true));
					}
				}
				return service;
