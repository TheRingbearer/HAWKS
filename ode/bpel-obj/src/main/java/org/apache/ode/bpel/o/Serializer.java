...
public class Serializer {
	...
	public static final byte[] MAGIC_NUMBER_OFH_20040908 = new byte[] { 0x55,
			'5', 'S', 0x00, 'O', 'F', 'H', 0x20, 0x04, 0x09, 0x08 };

	public static final byte[] MAGIC_NUMBER_OFH_20061101 = new byte[] { 0x55,
			'5', 'S', 0x00, 'O', 'F', 'H', 0x20, 0x06, 0x11, 0x01 };
	...
	public void read(InputStream is) throws IOException {
		...
		if (Arrays.equals(MAGIC_NUMBER_OFH_20040908, magic)) {
			// Old format requires us to read the OModel to get the type and
			// guid.
			...
			this.type = new QName(oprocess.targetNamespace,
	...
	static class CustomObjectOutputStream extends ObjectOutputStream {
		...
		protected Object replaceObject(Object obj) throws IOException {
			if (obj instanceof QName) {
				QName q = (QName) obj;
				return new OQName(q.getNamespaceURI(), q.getLocalPart(),
						q.getPrefix());
			}
	...
	static class CustomObjectInputStream extends ObjectInputStream {
		 ...
		 * compatibility with pre-Apache namespaces.
		 */
		@Override
		protected ObjectStreamClass readClassDescriptor() throws IOException,
			...
			if (read.getName().startsWith("com.fs.pxe.")) {
				return ObjectStreamClass.lookup(Class.forName(read.getName()
						.replace("com.fs.pxe.", "org.apache.ode.")));
			}
			if (read.getName().startsWith("com.fs.utils.")) {
				return ObjectStreamClass.lookup(Class.forName(read.getName()
						.replace("com.fs.utils.", "org.apache.ode.utils.")));
			}
			return read;
		}
