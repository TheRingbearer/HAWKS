...
public class OPartnerLink extends OBase {
	...
	/**
	 * The set of joining CorrelationSets that may be used as a match criteria,
	 * organized by {@link Operation}
	 */
	...
	// @hahnml:
	public OPartnerLink(OProcess owner, int id) {
		super(owner, id);
	}
	 ...
	 * {@link Operation}'s list of "non-initiating" correlation sets. The
	 * non-initiating correlation sets are those sets that are used (along with
	 * the operation) to "match" incoming messages. We need to know which
	 * correlation sets are used with which operation in order to pre-compute
	 * correlation keys at the time of message receipt.
	 * 
	 * @param operation
	 *            WSDL {@link Operation}
	 * @param cset
	 *            non-initiating correlation used in this operation
	 */
	public void addCorrelationSetForOperation(Operation operation,
			OScope.CorrelationSet cset, boolean isJoin) {
		addCorrelationSetForOperation(operation.getName(), cset, isJoin);
	}

	// AH: refactoring
	public void addCorrelationSetForOperation(String operation,
			OScope.CorrelationSet cset, boolean isJoin) {
		if (!isJoin) {
			Set<OScope.CorrelationSet> ret = _nonIntitiatingCorrelationSets
					.get(operation);
			if (ret == null) {
				ret = new HashSet<OScope.CorrelationSet>();
				_nonIntitiatingCorrelationSets.put(operation, ret);
			}
			ret.add(cset);
		} else {
			// serialization backward compatibility; joiningCorrelationSets
			...
			Set<OScope.CorrelationSet> ret = _joiningCorrelationSets
					.get(operation);
			if (ret == null) {
				ret = new HashSet<OScope.CorrelationSet>();
				_joiningCorrelationSets.put(operation, ret);
			}
			ret.add(cset);
		}
	}
	...
	/**
	 * Get all non-initiating correlation sets that are ever used to qualify a
	 * receive for a the given operation.
	 * 
	 * @param operation
	 *            the operation
	...
	/**
	 * Get all joining correlation sets that are ever used to qualify a receive
	 * for a the given operation.
	 * 
	 * @param operation
	 *            the operation
	 * @return all non-initiating correlation sets used in the given operation
	...
	public Set<OScope.CorrelationSet> getJoinningCorrelationSetsForOperation(
		...
		Set<OScope.CorrelationSet> ret = _joiningCorrelationSets.get(operation
				.getName());
		if (ret == null) {
			return Collections.EMPTY_SET;
		}
	...
	public Operation getPartnerRoleOperation(String name) {
		for (Operation op : (List<Operation>) partnerRolePortType
				.getOperations())
			if (op.getName().equals(name))
				return op;
		return null;
	...
	public boolean equals(Object obj) {
		...
		return (name == null && other.name == null && super.equals(obj))
				|| name.equals(other.name);
	}
	
	// AH: used to replace the CorrelationSets
	public List<Set<OScope.CorrelationSet>> getCorelationSetSets() {
		List<Set<OScope.CorrelationSet>> list = new ArrayList<Set<OScope.CorrelationSet>>();
		for (Set<OScope.CorrelationSet> set : _nonIntitiatingCorrelationSets
				.values()) {
			list.add(set);
		}
		for (Set<OScope.CorrelationSet> set : _joiningCorrelationSets.values()) {
			list.add(set);
		}
		return list;
	}

	public Map<String, Set<OScope.CorrelationSet>> getNonIntitiatingCorrelationSets() {
		return _nonIntitiatingCorrelationSets;
	}

	public Map<String, Set<OScope.CorrelationSet>> getJoiningCorrelationSets() {
		return _joiningCorrelationSets;
	}
	// AH: end
}
