 ...
 * <codE>&lt;receive&gt;</code> activities. Because the latter is essentially a
 * simplified version of the former, at run-time we do not distinguish between
 * the two.
 */
public class OPickReceive extends OActivity {
	...
	// AH:
	public boolean isReceiveActivity;
	// AH: end
	...
	// @hahnml:
	public OPickReceive(OProcess owner, OActivity parent, int id) {
	...
	public static class OnAlarm extends OBase {
		...
		// @hahnml:
		public OnAlarm(OProcess owner, int id) {
			super(owner, id);
		}
	}
	...
	public static class OnMessage extends OBase {
		...
		// left out for backward-compatibility, java serialization is lenient
		// about scope
		...
		// @hahnml:
		public OnMessage(OProcess owner, int id) {
			super(owner, id);
		}
		...
		private void readObject(ObjectInputStream in) throws IOException,
			...
			// backward compatibility; matchCorrelation could have a value if
			// read from old definition
			if (matchCorrelations == null)
				matchCorrelations = new ArrayList<OScope.CorrelationSet>();
			if (matchCorrelation != null) {
				matchCorrelations.add(matchCorrelation);
			}
			// backward compatibility; joinCorrelations could be null if read
			// from old definition
			if (joinCorrelations == null) {
				try {
					Field field = OnMessage.class
			...
			// backward compatibility; joinCorrelation could have a value if
			// read from old definition
