...
public class TreeBuilder extends Builder<String> {
	...
	public org.apache.ode.ql.tree.nodes.Node build(String query) {
		try {
			...
			// TODO create common exception which will indicate parsing
			// exception
	...
	private Query createSelection(ASTStart node) {
		Collection<Object> childs = new ArrayList<Object>(
	...
	private OrderBy createOrderBy(Node node) {
		Collection<OrderByElement> orders = new ArrayList<OrderByElement>(
				node.jjtGetNumChildren());
		for (int i = 0; i < node.jjtGetNumChildren(); i++) {
			orders.add(createOrderByElement((ASTOrderByField) node
					.jjtGetChild(i)));
		}
	...
	private In createIn(Node node) {
		...
		Collection<Value> values = new ArrayList<Value>(
				inValuesNode.jjtGetNumChildren());
	...
	private Identifier createIdentifier(Node parentNode, int index) {
		...
		if (node instanceof ASTField) {
			return new org.apache.ode.ql.tree.nodes.Field(
					((ASTField) node).name);
		}
		if (node instanceof ASTProperty) {
			return new org.apache.ode.ql.tree.nodes.Property(
					((ASTProperty) node).getName());
		}
		// TODO
		throw new IllegalArgumentException("");
	}
	...
	private Collection<LogicNode> extractLogicNodes(Node parentNode) {
		Collection<LogicNode> childs = new ArrayList<LogicNode>(
	...
	/*
	 * private static boolean checkInheritance(Object obj, Class clazz) { return
	 * clazz.isAssignableFrom(clazz.getClass()); }
	 */
	private static void checkChildsNumber(Node node, int expected) {
		int actual = node.jjtGetNumChildren();
		if (actual != expected) {
			throw new IllegalArgumentException("Expected childs cound("
					+ actual + ") differes from expected " + expected);
		}
	}
}
