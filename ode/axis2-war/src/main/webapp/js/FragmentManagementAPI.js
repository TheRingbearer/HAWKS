...
var address = baseURL + baseDirectoryName + "/processes/FragmentManagement";

var FragmentManagementService = new WebService("FragmentManagementPort");
FragmentManagementService.getAvailableFragments =

function getAvailableFragments(){

    this._options = new Array();
    isAsync = (this.getAvailableFragments.callback != null && typeof(this.getAvailableFragments.callback) == 'function');
    request = '<q0:getAvailableFragments xmlns:q0="http://www.apache.org/ode/fmapi"/>';
    if (isAsync) {
        try {
            this._call(
                    ...
                    new Array(this.getAvailableFragments.callback, this.getAvailableFragments.onError)
                    );
        } catch (e) {
            var error;
            ...
            this.getAvailableFragments.onError(error);
        }
    } else {
        try {
            response = this._call("getAvailableFragments", request);
            resultValue = response.documentElement;
            return resultValue;
        } catch (e) {
...
FragmentManagementService.getAvailableStartFragments =

function getAvailableStartFragments(){

    this._options = new Array();
    isAsync = (this.getAvailableStartFragments.callback != null && typeof(this.getAvailableStartFragments.callback) == 'function');
    request = '<q0:getAvailableStartFragments xmlns:q0="http://www.apache.org/ode/fmapi"/>';
    if (isAsync) {
        try {
            this._call(
                    "getAvailableStartFragments",
                    request,
                    function(thisRequest, callbacks) {
                        if (thisRequest.error != null) {
                            callbacks[1](thisRequest.error);
                    ...
                    new Array(this.getAvailableStartFragments.callback, this.getAvailableStartFragments.onError)
                    );
        } catch (e) {
            var error;
            if (WebServiceError.prototype.isPrototypeOf(e)) {
            ...
            this.getAvailableStartFragments.onError(error);
        }
    } else {
        try {
            response = this._call("getAvailableStartFragments", request);
            resultValue = response.documentElement;
            return resultValue;
        } catch (e) {
            if (typeof(e) == "string") throw(e);
...
FragmentManagementService.getAvailableNonStartFragments =

function getAvailableNonStartFragments(){

    this._options = new Array();
    isAsync = (this.getAvailableNonStartFragments.callback != null && typeof(this.getAvailableNonStartFragments.callback) == 'function');
    request = '<q0:getAvailableNonStartFragments xmlns:q0="http://www.apache.org/ode/fmapi"/>';
    if (isAsync) {
        try {
            this._call(
                    "getAvailableNonStartFragments",
                    request,
                    function(thisRequest, callbacks) {
                        if (thisRequest.error != null) {
                            callbacks[1](thisRequest.error);
                    ...
                    new Array(this.getAvailableNonStartFragments.callback, this.getAvailableNonStartFragments.onError)
                    );
        } catch (e) {
            var error;
            if (WebServiceError.prototype.isPrototypeOf(e)) {
            ...
            this.getAvailableNonStartFragments.onError(error);
        }
    } else {
        try {
            response = this._call("getAvailableNonStartFragments", request);
            resultValue = response.documentElement;
            return resultValue;
        } catch (e) {
            if (typeof(e) == "string") throw(e);
...
function WebService(endpointName)
{
    ...
    {
        "FragmentManagementPort": {
            "type" : "SOAP11",
            "address" : address,
            "action" : {
				"getAvailableFragments" : "http://www.apache.org/ode/fmapi/FragmentManagementPortType/getAvailableFragments",
				"getAvailableStartFragments" : "http://www.apache.org/ode/fmapi/FragmentManagementPortType/getAvailableStartFragments",
				"getAvailableNonStartFragments" : "http://www.apache.org/ode/fmapi/FragmentManagementPortType/getAvailableNonStartFragments"
            }
        }
    };
...
try {
    ...
    if (pageScheme == "http" || pageScheme == "https") {
        ...
        var endpoints = FragmentManagementService._endpointDetails;
        ...
        for (var i in endpoints) {
            ...
            if (secureEndpoint == "" && pageScheme == "https" && scheme(address) == "https") {
                secureEndpoint = i;
                FragmentManagementService.endpoint = secureEndpoint;
            }
