...
// schlieta
InstanceManagementService.finish =
function finish(/* long */ _iid)
{
    var isAsync, request, response, resultValue;

    this._options = new Array();
    isAsync = (this.finish.callback != null && typeof(this.finish.callback) == 'function');
    request =
    '<p:finish xmlns:p="http://www.apache.org/ode/pmapi">' +
    (_iid == null ? '' : '<iid>' + this._encodeXML(_iid) + '</iid>') +
    '</p:finish>';
    ...
    if (isAsync) {
        try {
            this._call(
                    ...
                    new Array(this.finish.callback, this.finish.onError)
                    );
        } catch (e) {
            var error;
            if (WebServiceError.prototype.isPrototypeOf(e)) {
            ...
            this.finish.onError(error);
        }
    } else {
        try {
            response = this._call("finish", request);
            resultValue = /* tInstanceInfo */ response.documentElement;
            return resultValue;
        } catch (e) {
            if (typeof(e) == "string") throw(e);
...
InstanceManagementService.finish.callback = null;
// end schlieta
    ...
    this._endpointDetails =
    {
        "InstanceManagementPort": {
            ...
            "action" : {
				...
				"finish" : "http://www.apache.org/ode/pmapi/InstanceManagementPortType/finishRequest",
