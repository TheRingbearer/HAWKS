...
public class HibernateInstancesQueryCompiler extends Compiler<List, Session> {
	...
	private abstract static class FieldValueEquality extends
			AbstractEqualityEvaluator<String, Criterion, Object> {
	...
	/*
	 * private final static String CORRELATION_NAME_FIELD = "name"; private
	 * final static String CORRELATION_NAMESPACE_FIELD = "namespace"; private
	 * final static String CORRELATION_NAMESPACE_FIELD = "namespace";
	 */
	// DB fields
	...
	private final static Map<String, String> nodeIdentifierToDBField = new HashMap<String, String>(
			20);
	...
	static {
		...
		nodeIdentifierToDBField.put(PROCESS_NAMESPACE_FIELD,
				PROCESS_NAMESPACE_DB_FIELD);
		nodeIdentifierToDBField.put(INSTANCE_STARTED_FIELD,
				INSTANCE_STARTED_DB_FIELD);
		nodeIdentifierToDBField.put(INSTANCE_LAST_ACTIVE_FIELD,
				INSTANCE_LAST_ACTIVE_DB_FIELD);
		nodeIdentifierToDBField.put(INSTANCE_STATUS_FIELD,
				INSTANCE_STATUS_DB_FIELD);
	}
	...
	public CommandEvaluator<List, Session> compile(final Query node) {
		...
		final OrderByEvaluator<Collection<Order>, Object> orderEvaluator = (node
				.getOrder() != null) ? compileOrderBy(node.getOrder()) : null;

		final CommandEvaluator<Criterion, Object> selectionEvaluator = node
				.getChilds().size() == 0 ? null : compileEvaluator(node
				.getChilds().iterator().next());
		...
		return new CommandEvaluator<List, Session>() {
			public List evaluate(Session session) {
				Criteria criteria = session.createCriteria(
						HProcessInstance.class).createAlias("process",
						"process");
				if (joinCorrelationSet) {
					criteria = criteria.createAlias("correlationSets",
							"property");
				}
				...
				if (sortByStatus) {
					Collections.sort(result,
							sortByStatusDesc ? StateComparator.DESC
									: StateComparator.ASC);
				}
	...
	protected ConjunctionEvaluator<Criterion, Object> compileConjunction(
			Collection<CommandEvaluator> childs) {
	...
	protected DisjunctionEvaluator<Criterion, Object> compileDisjunction(
			Collection<CommandEvaluator> childs) {
	...
	protected EqualityEvaluator<String, Criterion, Object> compileEqual(
			final Equality eq) {
			...
			return new EqualityEvaluator<String, Criterion, Object>() {
				public Criterion evaluate(Object paramValue) {
					...
					if (!StringUtils.isEmpty(property.getNamespace())) {
						conj.add(Restrictions.eq(PROPERTY_NS_DB_FIELD,
								property.getNamespace()));
					}
					conj.add(Restrictions.eq(PROPERTY_NAME_DB_FIELD,
							property.getName()));
					conj.add(Restrictions.eq(PROPERTY_VALUE_DB_FIELD, eq
							.getValue().getValue()));
				...
				return new FieldValueEquality(INSTANCE_STATUS_FIELD) {
					...
					public Criterion evaluate(Object paramValue) {
						...
						if (STATUS_ACTIVE.equals(paramValue)) {
							disj.add(Restrictions.eq(dbField,
							...
							disj.add(Restrictions.eq(dbField, noState)); // Error
							// instance
							// state
							// doesn't
							// exist
							...
							disj.add(Restrictions.eq(dbField, noState)); // Non
							// existent
							// state
						}
						return disj;
	...
	protected CommandEvaluator<Criterion, Object> compileIdentifierToValueCMP(
		...
		if (id instanceof Field) {
			...
			} else if (INSTANCE_STARTED_FIELD.equals(name)
					|| INSTANCE_LAST_ACTIVE_FIELD.equals(name)) {
				try {
					value.setValue(ISO8601DateParser.parse((String) value
							.getValue()));
		...
		if (ge.getIdentifier() instanceof Property) {
			...
			return new GEEvaluator<String, Criterion, Object>() {
				public Criterion evaluate(Object paramValue) {
					Conjunction conj = Restrictions.conjunction();
					if (!StringUtils.isEmpty(property.getNamespace())) {
						conj.add(Restrictions.ge(PROPERTY_NS_DB_FIELD,
								property.getNamespace()));
					}
					conj.add(Restrictions.ge(PROPERTY_NAME_DB_FIELD,
							property.getName()));
					conj.add(Restrictions.ge(PROPERTY_VALUE_DB_FIELD, ge
							.getValue().getValue()));
			...
			if (INSTANCE_STATUS_FIELD.equals(fieldName)) {
				throw new IllegalArgumentException("Field "
						+ INSTANCE_STATUS_FIELD + " is not supported.");
			}
	...
	protected GreaterEvaluator<String, Criterion, Object> compileGreater(
			final Greater gt) {
		if (gt.getIdentifier() instanceof Property) {
			...
			return new GreaterEvaluator<String, Criterion, Object>() {
				public Criterion evaluate(Object paramValue) {
					Conjunction conj = Restrictions.conjunction();
					if (!StringUtils.isEmpty(property.getNamespace())) {
						conj.add(Restrictions.gt(PROPERTY_NS_DB_FIELD,
								property.getNamespace()));
					}
					conj.add(Restrictions.gt(PROPERTY_NAME_DB_FIELD,
							property.getName()));
					conj.add(Restrictions.gt(PROPERTY_VALUE_DB_FIELD, gt
							.getValue().getValue()));
	...
	protected INEvaluator<String, Criterion, Object> compileIn(final In in) {
		if (in.getIdentifier() instanceof Property) {
			...
			return new INEvaluator<String, Criterion, Object>() {
				public Criterion evaluate(Object paramValue) {
					...
					for (Value value : in.getValues()) {
						...
						conj.add(Restrictions.gt(PROPERTY_NAME_DB_FIELD,
								propertyName));
						conj.add(Restrictions.gt(PROPERTY_VALUE_DB_FIELD,
								value.getValue()));
			...
			if (INSTANCE_STATUS_FIELD.equals(fieldName)) {
				...
				final Collection values = ValuesHelper
						.extract((Collection<Value>) in.getValues());

				if (values.contains(STATUS_ACTIVE)) {
					disj.add(Restrictions.eq(INSTANCE_STATUS_DB_FIELD,
							ProcessState.STATE_NEW));
					disj.add(Restrictions.eq(INSTANCE_STATUS_DB_FIELD,
							ProcessState.STATE_ACTIVE));
					disj.add(Restrictions.eq(INSTANCE_STATUS_DB_FIELD,
				...
				if (values.contains(STATUS_ERROR)) {
					disj.add(Restrictions.eq(INSTANCE_STATUS_DB_FIELD, noState)); // Error
																					// instance
																					// state
																					// doesn't
					// exist yet
				...
				if (INSTANCE_ID_FIELD.equals(fieldName)) {
					...
					try {
						for (Value value : values) {
							objValues.add(ISO8601DateParser
									.parse((String) value.getValue()));
						}
					} catch (ParseException ex) {
						// TODO
						throw new RuntimeException(ex);
					...
					objValues = ValuesHelper
							.extract((Collection<Value>) values);
				}
				final String dbField = getDBField(fieldName);
				return new INEvaluator<String, Criterion, Object>() {
					/**
		...
		if (le.getIdentifier() instanceof Property) {
			...
			return new LEEvaluator<String, Criterion, Object>() {
				public Criterion evaluate(Object paramValue) {
					Conjunction conj = Restrictions.conjunction();
					if (!StringUtils.isEmpty(property.getNamespace())) {
						conj.add(Restrictions.le(PROPERTY_NS_DB_FIELD,
								property.getNamespace()));
					}
					conj.add(Restrictions.le(PROPERTY_NAME_DB_FIELD,
							property.getName()));
					conj.add(Restrictions.le(PROPERTY_VALUE_DB_FIELD, le
							.getValue().getValue()));
	...
	protected LessEvaluator<String, Criterion, Object> compileLess(
			final Less less) {
		if (less.getIdentifier() instanceof Property) {
			...
			return new LessEvaluator<String, Criterion, Object>() {
				public Criterion evaluate(Object paramValue) {
					Conjunction conj = Restrictions.conjunction();
					if (!StringUtils.isEmpty(property.getNamespace())) {
						conj.add(Restrictions.lt(PROPERTY_NS_DB_FIELD,
								property.getNamespace()));
					}
					conj.add(Restrictions.lt(PROPERTY_NAME_DB_FIELD,
							property.getName()));
					conj.add(Restrictions.lt(PROPERTY_VALUE_DB_FIELD, less
							.getValue().getValue()));
	...
	protected LikeEvaluator<String, Criterion, Object> compileLike(
			final Like like) {
		if (like.getIdentifier() instanceof Property) {
			...
			return new LikeEvaluator<String, Criterion, Object>() {
				public Criterion evaluate(Object paramValue) {
					Conjunction conj = Restrictions.conjunction();
					if (!StringUtils.isEmpty(property.getNamespace())) {
						conj.add(Restrictions.like(PROPERTY_NS_DB_FIELD,
								property.getNamespace()));
					}
					conj.add(Restrictions.like(PROPERTY_NAME_DB_FIELD,
							property.getName()));
					conj.add(Restrictions.like(PROPERTY_VALUE_DB_FIELD, like
							.getValue().getValue()));
			...
			if (INSTANCE_STATUS_FIELD.equals(fieldName)) {
				throw new IllegalArgumentException("Field "
						...
						+ " is not supported by like operation.");
			}
			if (INSTANCE_ID_FIELD.equals(fieldName)) {
				throw new IllegalArgumentException("Field " + INSTANCE_ID_FIELD
						+ " is not supported by like operation.");
			}
	...
	protected OrderByEvaluator<Collection<Order>, Object> compileOrderBy(
			OrderBy orderBy) {
		...
		for (OrderByElement idOrder : orderBy.getOrders()) {
			if (!(idOrder.getIdentifier() instanceof Field)) {
				throw new IllegalArgumentException(
						"Only field identifier supported by order by operator.");
			}
			...
			orders.put(dbField, idOrder.getType() == null
					|| idOrder.getType() == OrderByType.ASC);
		}

		return new OrderByEvaluator<Collection<Order>, Object>() {
			public Collection<Order> evaluate(Object paramValue) {
				Collection<Order> hibernateOrders = new ArrayList<Order>(
						orders.size());
				for (Map.Entry<String, Boolean> order : orders.entrySet()) {
					hibernateOrders.add(order.getValue() ? Order.asc(order
							.getKey()) : Order.desc(order.getKey()));
				}
	...
	protected List<CommandEvaluator> evaluate(LogicExprNode exprNode) {
		ArrayList<CommandEvaluator> commandsEv = new ArrayList<CommandEvaluator>(
				exprNode.getChilds().size());
