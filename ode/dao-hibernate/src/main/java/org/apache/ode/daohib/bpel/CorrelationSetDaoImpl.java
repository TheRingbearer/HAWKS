 ...
 */
class CorrelationSetDaoImpl extends HibernateDao implements CorrelationSetDAO {

	private HCorrelationSet _correlationSet;

	public CorrelationSetDaoImpl(SessionManager sessionManager,
		...
		if (names != null) {
			if (_correlationSet.getProperties() == null
				...
				for (int m = 0; m < names.length; m++) {
					HCorrelationProperty prop = new HCorrelationProperty(
							names[m], values.getValues()[m], _correlationSet);
				...
				for (int m = 0; m < names.length; m++) {
					...
					if (prop == null)
						prop = new HCorrelationProperty(names[m],
								values.getValues()[m], _correlationSet);
					else
	...
	public CorrelationKey getValue() {
		...
		if (_correlationSet.getValue() != null)
			return new CorrelationKey(_correlationSet.getValue());
		else
			return null;
	}
