...
public class MessageExchangeDaoImpl extends HibernateDao implements
	...
	private static final Log __log = LogFactory
			.getLog(MessageExchangeDaoImpl.class);

	private HMessageExchange _hself;

	// Used when provided process and instance aren't hibernate implementations.
	// The relation
	// therefore can't be persisted. Used for in-mem DAOs so that doesn't matter
	// much.
	...
	public QName getPortType() {
		return _hself.getPortType() == null ? null : QName.valueOf(_hself
				.getPortType());
	}
	...
	public void setRequest(MessageDAO msg) {
		...
		_hself.setRequest(msg == null ? null
				: (HMessage) ((MessageDaoImpl) msg).getHibernateObj());
		update();
	}

	public void setResponse(MessageDAO msg) {
		entering("MessageExchangeDaoImpl.setResponse");
		_hself.setResponse(msg == null ? null
				: (HMessage) ((MessageDaoImpl) msg).getHibernateObj());
		update();
	}
	...
	public QName getFault() {
		return _hself.getFault() == null ? null : QName.valueOf(_hself
				.getFault());
	}
	...
	public ProcessDAO getProcess() {
		...
		if (_externalProcess != null)
			...
			return _hself.getProcess() == null ? null : new ProcessDaoImpl(_sm,
					_hself.getProcess());
	}
	...
	public void setProcess(ProcessDAO process) {
		...
		if (process == null || process instanceof ProcessDaoImpl) {
			_hself.setProcess(process == null ? null
					: (HProcess) ((ProcessDaoImpl) process).getHibernateObj());
			update();
		} else {
	...
	public void setInstance(ProcessInstanceDAO instance) {
		...
		if (instance == null || instance instanceof ProcessInstanceDaoImpl) {
			_hself.setInstance(instance == null ? null
					: (HProcessInstance) ((ProcessInstanceDaoImpl) instance)
							.getHibernateObj());
			update();
		} else {
	...
	public ProcessInstanceDAO getInstance() {
		...
		if (_externalInstance != null)
			...
			return _hself.getInstance() == null ? null
					: new ProcessInstanceDaoImpl(_sm, _hself.getInstance());
	}
	...
	public void releasePremieMessages() {
		deleteByIds(
				...
				getSession()
						.getNamedQuery(
								HCorrelatorMessage.SELECT_CORMESSAGE_IDS_BY_MEX)
						.setParameter("mex", _hself).list());
	}
