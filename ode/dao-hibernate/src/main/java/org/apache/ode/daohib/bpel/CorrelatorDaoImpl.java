...
class CorrelatorDaoImpl extends HibernateDao implements CorrelatorDAO {
	...
	private static final String FLTR_SELECTORS_SUBQUERY = ("from HCorrelatorSelector as hs where hs.processType = :processType and hs.correlatorId = "
			+ "(select hc.id from HCorrelator as hc where hc.correlatorId = :correlatorId )")
			.intern();
	...
	public List<MessageRouteDAO> findRoute(CorrelationKeySet keySet) {
		...
		Query q = getSession().createQuery(
				generateSelectorQuery(_sm.canJoinForUpdate() ? FLTR_SELECTORS
						: FLTR_SELECTORS_SUBQUERY, subSets));
		...
		// condition.
		if (__log.isDebugEnabled())
			__log.debug("Obtain record lock on " + _hobj);
		Query correlatorLockQuery = getSession().createQuery(
				"from HCorrelator as hc where id = :id");
	...
	public void enqueueMessage(MessageExchangeDAO mex,
		...
		String hdr = "enqueueMessage(mex="
				+ ((MessageExchangeDaoImpl) mex)._hobj.getId() + " keySet="
	...
	public void addRoute(String routeGroupId, ProcessInstanceDAO target,
			int idx, CorrelationKeySet correlationKeySet, String routePolicy) {
		entering("CorrelatorDaoImpl.addRoute");
		String hdr = "addRoute(" + routeGroupId + ", iid="
				+ target.getInstanceId() + ", idx=" + idx + ", ckeySet="
				+ correlationKeySet + "): ";
		...
		hsel.setInstance((HProcessInstance) ((ProcessInstanceDaoImpl) target)
				.getHibernateObj());
	...
	public boolean checkRoute(CorrelationKeySet correlationKeySet) {
		...
		Query q = getSession().getNamedQuery(
				HCorrelatorSelector.SELECT_MESSAGE_ROUTE);
	...
	public Collection<CorrelatorMessageDAO> getAllMessages() {
		...
		for (HCorrelatorMessage correlatorMessage : _hobj
