...
class CriteriaBuilder {
		...
		if (filter != null) {
			...
			if (filter.getPidFilter() != null) {
				...
				for (int m = 0; m < pids.size(); m++) {
					filters.append(" pi.process.processId ").append(cmp)
							.append(" :pid").append(m);
			...
			if (filter.getNamespaceFilter() != null) {
				...
				parameters.put("pnamespace", filter.getNamespaceFilter()
						.replaceAll("\\*", "%"));
			}
				...
				for (String propKey : props.keySet()) {
					...
					if (propKey.startsWith("{")) {
						...
						clauses.add(" csp" + i + ".name = :cspname" + i
								...
								+ i + " and csp" + i + ".value = :cspvalue" + i);

						parameters
								.put("cspname" + i, propKey.substring(
										propKey.lastIndexOf("}") + 1,
										propKey.length()));
	...
	void buildCriteria(Criteria crit, InstanceFilter filter) {
		...
		if (filter.getNameFilter() != null) {
			processCrit.add(Restrictions.like("typeName", filter
					.getNameFilter().replaceAll("\\*", "%")));
		}
		if (filter.getNamespaceFilter() != null) {
			processCrit.add(Restrictions.like("typeNamespace", filter
					.getNamespaceFilter().replaceAll("\\*", "%")));
		}

		// Specific filter for status (using a disjunction between possible
		// statuses)
		if (filter.getStatusFilter() != null) {
		...
		if (filter.getPropertyValuesFilter() != null) {
			Criteria propCrit = crit.createCriteria("correlationSets")
					.createCriteria("properties");
			for (Map.Entry<String, String> corValue : filter
					.getPropertyValuesFilter().entrySet()) {
				String propName = (String) corValue.getKey();
				if (propName.startsWith("{")) {
					String namespace = propName.substring(1,
							propName.lastIndexOf("}"));
					propName = propName.substring(
							propName.lastIndexOf("}") + 1, propName.length());
		...
		if (filter.orders != null) {
			for (String key : filter.orders) {
				...
				if ("name".equals(orderKey)) {
					if (ascending)
						processCrit
								.addOrder(Property.forName("typeName").asc());
					else
						processCrit.addOrder(Property.forName("typeName")
								.desc());
				} else if ("namespace".equals(orderKey)) {
					if (ascending)
						processCrit.addOrder(Property.forName("typeNamespace")
								.asc());
					else
						processCrit.addOrder(Property.forName("typeNamespace")
								.desc());
					...
					if (ascending)
						processCrit.addOrder(Property.forName("processId")
								.asc());
					else
						processCrit.addOrder(Property.forName("processId")
								.desc());
					...
					if (ascending)
						...
						processCrit
								.addOrder(Property.forName("version").desc());
	...
	void buildCriteria(Criteria crit, BpelEventFilter efilter) {
		if (efilter.getTypeFilter() != null)
			crit.add(Restrictions.like("type",
					efilter.getTypeFilter().replace('*', '%')));
	...
	static void addFilterOnPrefixedDate(Criteria crit, String prefixedDate,
			String dateAttribute) {
		Date realDate = null;
		try {
			realDate = parseDateExpression(getDateWithoutOp(prefixedDate));
		} catch (ParseException e) {
			// Never occurs, the deploy date format is pre-validated by the
			// filter
		}
	...
	static void addFilterOnPrefixedDate(Criteria crit, String op, Date date,
			String dateAttribute) {
		if (op.startsWith("=")) {
