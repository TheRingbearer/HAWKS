 ...
 * maintains a set of <em>selectors</em> and <em>messages</em>. The selector set
 * is a database representation of the pick/receive activities that need to be
 * matched up to a message, while the message set is the database representation
 * of the messages that have been received that need to be matched up to a
 * pcik/receive.
 * 
 * @hibernate.class table="BPEL_CORRELATOR"
 * @hibernate.query name="SELECT_CORRELATOR_IDS_BY_PROCESS"
 *                  query="select id from HCorrelator as c where c.process = :process"
 */
public class HCorrelator extends HObject {
	...
	public String toString() {
		return "{HCorrelator process=" + _process.getProcessId() + ", cid="
				+ _correlatorId + "}";
	}
}
