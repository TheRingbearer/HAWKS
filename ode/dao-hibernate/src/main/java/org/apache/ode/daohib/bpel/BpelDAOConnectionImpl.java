...
public class BpelDAOConnectionImpl implements BpelDAOConnection,
	...
	private static final Log __log = LogFactory
			.getLog(BpelDAOConnectionImpl.class);
	...
	public MessageExchangeDAO getMessageExchange(String mexid) {
		HMessageExchange mex = (HMessageExchange) getSession().get(
				HMessageExchange.class, new Long(mexid));
	...
	public Collection<ProcessInstanceDAO> instanceQuery(InstanceFilter criteria) {
		...
		Iterator<HProcessInstance> iter = _instanceQuery(getSession(), false,
				criteria);
		while (iter.hasNext()) {
	...
	public int deleteInstances(InstanceFilter criteria,
		...
		List<HProcessInstance> instances = _instanceQueryForList(getSession(),
				false, criteria);
		if (__log.isDebugEnabled())
			__log.debug("Collected " + instances.size()
					+ " instances to delete.");

		if (!instances.isEmpty()) {
			ProcessDaoImpl process = (ProcessDaoImpl) createTransientProcess(instances
					.get(0).getProcessId());
	...
	static Iterator<HProcessInstance> _instanceQuery(Session session,
			boolean countOnly, InstanceFilter filter) {
		return _instanceQueryForList(session, countOnly, filter).iterator();
	}

	@SuppressWarnings("unchecked")
	private static List<HProcessInstance> _instanceQueryForList(
			Session session, boolean countOnly, InstanceFilter filter) {
	...
	static ProcessInstanceDAO _getInstance(SessionManager sm, Session session,
			Long iid) {
		HProcessInstance instance = (HProcessInstance) session.get(
				HProcessInstance.class, iid);
	 ...
	 * @param instance
	 */
	static void _insertBpelEvent(Session sess, BpelEvent event,
			ProcessDAO process, ProcessInstanceDAO instance) {
		...
		if (process != null)
			hevent.setProcess((HProcess) ((ProcessDaoImpl) process)
					.getHibernateObj());
		if (instance != null)
			hevent.setInstance((HProcessInstance) ((ProcessInstanceDaoImpl) instance)
					.getHibernateObj());
	...
	public Collection<ProcessInstanceDAO> instanceQuery(String expression) {
		...
		final org.apache.ode.ql.tree.nodes.Node rootNode = builder
				.build(expression);

		HibernateInstancesQueryCompiler compiler = new HibernateInstancesQueryCompiler();

		CommandEvaluator<List, Session> eval = compiler
				.compile((Query) rootNode);
		List<HProcessInstance> instancesList = (List<HProcessInstance>) eval
				.evaluate(getSession());
	...
	public Map<Long, Collection<CorrelationSetDAO>> getCorrelationSets(
		...
		while (index < iids.size()) {
			List<Long> subList = iids.subList(index,
					Math.min(index + batchSize, iids.size()));
			csets.addAll(getSession()
					.getNamedQuery(HCorrelationSet.SELECT_CORSETS_BY_INSTANCES)
					.setParameterList("instances", subList).list());
			index += batchSize;
		}
	...
	public Collection<CorrelationSetDAO> getActiveCorrelationSets() {
		...
		Collection<HCorrelationSet> csets = getSession()
				.getNamedQuery(HCorrelationSet.SELECT_CORSETS_BY_PROCESS_STATES)
				.setParameter("states", ProcessState.STATE_ACTIVE).list();
		for (HCorrelationSet cset : csets)
	...
	// AH:
	public FCManagementDAO getFCManagement() {
		return null;
	}
	// AH: end
}
