 ...
 * the log category 'org.apache.ode.bpel.DAO' when entered. A typical message
 * could be "className.methodName". <br/>
...
public abstract class HibernateDao {
	...
	void entering(String msg) {
		...
		if (logDao.isDebugEnabled())
			logDao.debug("entering " + msg);
	}

	void leaving(String msg) {
		if (logDao.isDebugEnabled())
			logDao.debug("leaving " + msg);
	}
	...
	public Serializable getDHandle() {
		return new HibernateHandle(getClass(), _hobj.getClass(), getSession()
				.getIdentifier(_hobj));
	}
	...
	protected void deleteByColumn(Class entity, String column, List<Long> values) {
		if (values != null && values.size() > 0) {
			final String delete = "delete from " + entity.getName()
					+ " as e where e." + column + " in (:values)";
