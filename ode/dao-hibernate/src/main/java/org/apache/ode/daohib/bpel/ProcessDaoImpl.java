...
public class ProcessDaoImpl extends HibernateDao implements ProcessDAO,
	...
	public ProcessInstanceDAO createInstance(CorrelatorDAO correlator) {
		...
		instance.setInstantiatingCorrelator((HCorrelator) ((CorrelatorDaoImpl) correlator)
				.getHibernateObj());
	...
	public void deleteProcessAndRoutes() {
		...
		deleteByIds(
				...
				getSession()
						.getNamedQuery(
								HCorrelatorSelector.SELECT_MESSAGE_ROUTE_IDS_BY_PROCESS)
						.setParameter("process", _process).list());
		...
		deleteByIds(
				...
				getSession()
						.getNamedQuery(
								HCorrelator.SELECT_CORRELATOR_IDS_BY_PROCESS)
						.setParameter("process", _process).list());
		try {
			...
			// after this delete, we have a use case that creates the process
			// with the same procid.
			...
			__log.debug("Process: " + getProcessId()
	...
	public int deleteInstances(int transactionSize) {
		...
		Collection<HProcessInstance> instances = getSession()
				.getNamedQuery(HProcessInstance.SELECT_INSTANCES_BY_PROCESS)
				.setParameter("process", _process)
				.setMaxResults(transactionSize).list();
	...
	public int deleteInstances(Collection<HProcessInstance> instances,
			Set<CLEANUP_CATEGORY> categories) {
	...
	private void deleteProcessInstances(Collection<HProcessInstance> instances) {
		deleteByIds(
				HActivityRecovery.class,
				getSession()
						.getNamedQuery(
								HActivityRecovery.SELECT_ACTIVITY_RECOVERY_IDS_BY_INSTANCES)
	...
	private void deleteVariables(Collection<HProcessInstance> instances) {
		...
		deleteByIds(HScope.class,
				getSession()
						...
						.setParameterList("instances", instances).list());
		deleteByIds(HLink.class,
				getSession()
						.getNamedQuery(HLink.SELECT_LINK_IDS_BY_INSTANCES)
						.setParameterList("instances", instances).list());
	}
	...
	private void deleteMessages(Collection<HProcessInstance> instances) {
		...
		deleteByIds(
				HMessage.class,
				getSession()
						.getNamedQuery(HMessage.SELECT_MESSAGE_IDS_BY_INSTANCES)
						.setParameterList("instances", instances).list());
		List<Long> mex = getSession()
	...
	public Collection<ProcessInstanceDAO> getActiveInstances() {
		...
		Collection<HProcessInstance> insts = getSession()
				.getNamedQuery(
						HProcessInstance.SELECT_INSTANCES_BY_PROCESS_AND_STATES)
				.setParameter("process", _process)
