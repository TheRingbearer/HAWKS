...
/**
 * Custom Hibernate datatype that compresses (GZip) byte arrays to increase
...
public class GZipDataType implements UserType {
	...
	/**
	 * For backward compatibility with non-zipped data, prefix the gzip stream
	 * with a magic sequence
	 */
	public static final byte[] GZIP_PREFIX = new byte[] { 0x01, 0x02, 0x03,
	...
	private static volatile boolean _compressionEnabled = System.getProperty(
			"org.apache.ode.daohib.bpel.hobj.GZipDataType.enabled", "true")
	...
	public Object nullSafeGet(ResultSet rs, String[] names, Object owner)
		...
		if (names.length != 1)
			throw new IllegalStateException(
					"Expected a single column name instead of " + names.length);
		...
		if (buf != null) {
			synchronized (STATS_LOCK) {
				if (_totalBytesBefore > Integer.MAX_VALUE) {
					...
					_totalBytesAfter = _totalBytesAfter * 100
							/ _totalBytesBefore;
