 ...
 * Uses a ThreadLocal strategy for managing sessions.
 */
public class SessionManager {
	...
	private static final Map<String, TransactionManager> _txManagers = Collections
			.synchronizedMap(new HashMap<String, TransactionManager>());
	private static final Map<String, DataSource> _dataSources = Collections
	...
	public SessionManager(Properties env, DataSource ds, TransactionManager tx)
		...
		_sessionFactory = getDefaultConfiguration().setProperties(env)
				.setProperty(PROP_GUID, _uuid).buildSessionFactory();

		/*
		 * Some Hibernate dialects (like IngresDialect) do not support update
		 * for join. We need to distinguish them and explicitly define
		 * subqueries, otherwise Hibernate implicitly generates joins which
		 * causes problems during update for such DBMS. See
		 * org.apache.ode.daohib.bpel.CorrelatorDaoImpl for instance.
		 */
	...
	/**
	 * Returns flag which shows whether " where .. join ... for update" kind of
	 * queries can be used (supported by currently effective
	 * {@link org.hibernate.dialect.Dialect}. If it's {@code false} than
	 * sub-query fallback should be invoked instead.
	 * 
	 * @return currently returns false only for
	 *         {@link org.hibernate.dialect.IngresDialect}
	 */
	...
	public Configuration getDefaultConfiguration() throws MappingException {
		return new Configuration().addClass(HProcess.class)
				.addClass(HProcessInstance.class).addClass(HCorrelator.class)
				.addClass(HCorrelatorMessage.class)
				.addClass(HCorrelationProperty.class)
				.addClass(HCorrelatorSelector.class)
				.addClass(HMessageExchange.class).addClass(HMessage.class)
				.addClass(HPartnerLink.class).addClass(HScope.class)
				.addClass(HCorrelationSet.class).addClass(HXmlData.class)
				.addClass(HVariableProperty.class).addClass(HBpelEvent.class)
				.addClass(HFaultData.class).addClass(HActivityRecovery.class)
				.addClass(HMessageExchangeProperty.class)
				.addClass(HProcessInstanceMigration.class)
				.addClass(HLink.class);
	}
