...
public class TempFileManager {
	...
	private SortedSet<File> _registeredFiles = new TreeSet<File>(
	...
	private TempFileManager() {
		...
		if (__baseDir == null) {
			...
			if (tmpDir.exists()) {
				...
				throw new IllegalStateException(
						"Odd system configuration - temporary working directory "
		...
		try {
			...
			if (odeTmp.mkdir()) {
				__workDir = odeTmp;
				__log.debug("Set working directory to: "
						+ __workDir.getAbsolutePath());
				this._registerTemporaryFile(__workDir);
			} else {
				throw new IllegalStateException(
						"Unable to create temporary working directory in "
								+ __baseDir.getPath());
			}
		} catch (SecurityException se) {
			throw new IllegalStateException(
					"The security configuration is preventing the creation of a "
	 ...
	 * the singleton instance is uninitialized, and the <code>File</code> passed
	 * in must be both a directory and writable.
	 * </p>
	 * 
	 * @param f
	...
	public static synchronized void setWorkingDirectory(File f) {
		...
		// call cleanup() first.
			...
			String msg;
			if (__baseDir != null) {
				msg = "Already initialized in base directory: "
						+ __baseDir.getPath();
			} else {
	...
	/**
	 * <p>
	 * Get a temporary file, if possible, and register it for cleanup later. In
	 * the event that a temporary file cannot be created, the method will
	 * attempt to create a file in the current working directory instead.
	 * </p>
	 * 
	 * @param handle
	...
	public static synchronized File getTemporaryFile(String handle, File parent) {
		...
		try {
			tmp = File.createTempFile(
					handle + Long.toHexString(System.currentTimeMillis()),
					".tmp", parent);
	 ...
	 * 
	 * @param handle
	 *            a prefix to use in naming the directory.
	...
	/**
	 * <p>
	 * Clear out the temporary working directory. This can be called by, e.g., a
	 * commandline tool or other client when it is known that all temporary
	...
	private synchronized void _cleanup() {
		try {
			// collect all subdirectory contents that still exist, ordered
			// files-first
			SortedSet<File> allFiles = new TreeSet(
					Collections.reverseOrder(null));
			...
			for (File f : allFiles) {
				...
				if (f.exists() && !f.delete()) {
					__log.error("Unable to delete file "
							+ f.getAbsolutePath()
