...
public class Messages extends MessageBundle {
	...
	public String msgMultipleBindingsForOperation(String operationName) {
		return format("Operation {0} has multiple binding elements",
				operationName);
	}
	...
	public String msgNoBinding(QName bindingName) {
		return format("No <http:binding> or <soap:binding> for binding {0}",
				bindingName);
	}
	...
	public Throwable msgUndefinedFault(QName serviceName, String portName,
			String opname, QName faultName) {
		String s = format(
				"Undefined fault: service {0} port {1} operation {2} fault {3}.",
				serviceName, portName, opname, faultName);
		return new IllegalArgumentException(s);
	}

	public Throwable msgOdeMessagePartMissingRequiredElement(QName serviceName,
			String portName, String opname, QName elementName) {
		String s = format(
				"Message part is missing required element: service {0} port {1} operation {2} element {3}.",
	...
	public Throwable msgUnexpectedElementInSOAPBody(QName name,
			QName elementName) {
		String s = format(
				"Unexpected element in SOAP body: message {0} element {1}.",
				name, elementName);
		return new IllegalArgumentException(s);
	}
	...
	public Throwable msgSoapBodyDoesNotContainExpectedPartWrapper(
			QName serviceName, String portName, QName rpcWrapQName) {
		String s = format(
				"SOAP body does not contain expected part wrapper: service {0} port {1} wrapper {2}",
	...
	public Throwable msgUnexpectedBindingClass(Class passedClass) {
		String s = format(
				"Unexpected class: {0}! Must be passed javax.wsdl.extensions.soap.SOAPBinding or javax.wsdl.extensions.http.HTTPBinding",
				passedClass);
		return new IllegalArgumentException(s);
	}

	public String msgPortDefinitionNotFound(QName serviceName, String portName) {
		return format("Port definition not found: service {0} port {1}.",
				serviceName, portName);
	}

	public Throwable msgBindingOperationNotFound(QName serviceName,
			String portName, String name) {
		String s = format(
				"Binding operation not found: service {0} port {1} name {2}.",
				serviceName, portName, name);
		return new IllegalArgumentException(s);
	}

	public Throwable msgBindingInputNotFound(QName serviceName,
			String portName, String name) {
		String s = format(
				"Binding input not found: service {0} port {1} name {2}.",
				serviceName, portName, name);
		return new IllegalArgumentException(s);
	}

	public Throwable msgBindingOutputNotFound(QName serviceName,
			String portName, String name) {
		String s = format(
				"Binding output not found: service {0} port {1} name {2}.",
				serviceName, portName, name);
		return new IllegalArgumentException(s);
	}
