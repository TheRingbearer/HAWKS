...
public class DbIsolation {
	...
	static {
		try {
			_isolationLevel = Integer.parseInt(System.getProperty(
					"ode.connection.isolation", "2"));
		} catch (Throwable t) {
			__log.error(
					"Error while reading 'ode.connnection.isolation' property",
					t);
		}
	}
	...
	public static void setIsolationLevel(Connection c) throws SQLException {
		try {
			if (_isolationLevel != 0
					&& c.getTransactionIsolation() != _isolationLevel) {
