 ...
 * transformation is then delegated to a Transformer. Supports both XSL 1.0 and
 * XSL 2.0 depending on the version attribute provided in the XSL stylesheet
 * (see http://www.w3.org/TR/xslt20/#backwards - 3.8 Backwards-Compatible
 * Processing). <br/>
 * The transform handler also implements a simple cache to avo??d multiple
 * pre-compilation of the same XSL sheet.
 */
public class XslTransformHandler {

	private static final Log __log = LogFactory
			.getLog(XslTransformHandler.class);
	...
	/**
	 * Always parses the provided stylesheet and stores it in cache from its
	 * URI.
	 * 
	 * @param uri
	 ...
	 *            of the XSL document
	 * @param resolver
	 *            used to resolve includes and imports
	 */
	public void parseXSLSheet(QName processQName, URI uri, String body,
			URIResolver resolver) {
		Templates tm;
		try {
			_transformerFactory.setURIResolver(resolver);
			tm = _transformerFactory.newTemplates(new StreamSource(
	...
	/**
	 * Parses the provided stylesheet and stores it in cache only if it's not
	 * there already.
	 * 
	 * @param uri
	 *            referencing the stylesheet
	 ...
	 *            used to resolve includes and imports
	 */
	public void cacheXSLSheet(QName processQName, URI uri, String body,
			URIResolver resolver) {
		Templates tm;
		synchronized (_templateCache) {
	 ...
	 * referenced by the provided URI. The stylesheet MUST have been parsed
	 * previously.
	 * 
	 * @param uri
	 *            referencing the stylesheet
	...
	public Object transform(QName processQName, URI uri, Source source,
		...
		if (tm == null)
			throw new XslTransformException("XSL sheet" + uri
					+ " has not been parsed before transformation!");
		try {
			Transformer tf = tm.newTransformer();
			tf.setURIResolver(resolver);
			if (parameters != null) {
				for (Map.Entry<QName, Object> param : parameters.entrySet()) {
					tf.setParameter(param.getKey().getLocalPart(),
							param.getValue());
				}
			}
			...
			if (method == null || "xml".equals(method)) {
				...
				if (__log.isDebugEnabled())
					__log.debug("Returned node: type=" + node.getNodeType()
