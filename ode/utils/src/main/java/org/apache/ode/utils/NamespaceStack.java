...
/**
 * <p>
 * This is a utility for a SAX <code>ContentHandler</code> implementation to use
 * in tracking namespace declarations during a parse. The assumption is that the
 ...
 * e.g., from attribute values or element content, to <code>QName</code>
 * objects.
 * </p>
 */
public class NamespaceStack {
	 ...
	 * <code>xmlns</code>, <code>xml</code>, and empty prefixes. Note that the
	 * empty prefix is bound to the empty (non-<code>null</code>) URI.
	 * </p>
	 */
	...
	/**
	 * <p>
	 * Push a fresh context onto the stack. This method should be called
	 * somewhere in the body of a <code>startElement()</code>, as it represents
	 * the namespace resolution context for the events that occur between that
	 * event and the corresponding <code>endElement()</code>.
	 * </p>
	 * 
	...
	/**
	 * <p>
	 * Pop a context from the stack. This method should be called somewhere in
	 * the body of an <code>endElement</code>, as it clears the context that was
	 * used for namespace resolution within the body of the corresponding
	 * element.
	 * </p>
	 * 
	 * @see org.xml.sax.ContentHandler
	...
	/**
	 * <p>
	 * Declare a new prefix binding. This binding will supercede a binding with
	 * the same prefix in the same scope. As a crutch, <code>null</code>
	 * arguments may be passed and will be interpreted as
	 * <code>&quot;&quot;</code>. Note that binding a non-empty prefix to an
	 * empty URI is not permitted in XML 1.0 but is not flagged as an error by
	 * the method.
	 * </p>
	 * 
	 * @param prefix
	...
	public void declarePrefix(String prefix, String uri) {
		_current.declarePrefix(prefix == null ? "" : prefix, uri == null ? ""
				: uri);
	}

	/**
	 * <p>
	 * Retrieve the URI bound to the supplied prefix or <code>null</code> if no
	 * URI is bound to the supplied prefix. As a crutch, a <code>null</code>
	 * argument may be passed and will be interpreted as the empty prefix (
	 * <code>&quot;&quot;</code>).
	 * </p>
	 * 
	 ...
	 * 
	 * @param ch
	 *            the target <code>ContentHandler</code>
	 * @throws SAXException
	 *             if the target method does.
	 */
	...
	/**
	 * <p>
	 * Allocate and declare a new namespace prefix for the current context that
	 * uses the supplied &quot;hint&quot; as a start. The algorithm used will
	 * defer to an existing binding, then try the hint, then use a variant of
	 * the hint until it finds an available prefix.
	 * </p>
	 * 
	 * @param hint
	 *            a hint as to the desired prefix or <code>null</code> if any
	 *            prefix will do.
	 * @param uri
	...
	/**
	 * <p>
	 * Derference the prefix on a QName in <code>String</code> form and return a
	 * Java <code>QName</code> object.
	 * </p>
	 * 
	 * @param qname
	 *            the QName in string form.
	 * @return the dereferenced <code>QName</code>.
	 * @throws IllegalArgumentException
	 *             if a <code>null</code> argument is passed, a malformed
	 *             argument (e.g., <code>:foo</code> or <code>foo:</code>) is
	 *             passed, or if the prefix cannot be resolved to a URI.
	 */
	public QName dereferenceQName(String qname) {
		if (qname == null) {
			throw new IllegalArgumentException(
					"Unable to dereference <null> as a QName.");
		}
		int pos = qname.indexOf(':');
		QName qn;
		if (pos == qname.length() - 1 || pos == 0) {
			throw new IllegalArgumentException("\"" + qname
					+ "\" is a malformed QName.");
		} else if (pos == -1) {
	...
	private class Frame {
		...
		String getNamespaceURI(String prefix) {
			...
			if (uri == null) {
				uri = _parent == null ? null
						: (_parent.getNamespaceURI(prefix));
			}
			return uri;
		}
