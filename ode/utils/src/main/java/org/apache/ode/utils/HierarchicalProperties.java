 ...
 * feature is that property can be chained in three levels. Then when querying
 * for a property, if it's not found in the deepest level, the parent will be
 * queryed and so on.
 * <p/>
 * A prefix must be defined to discriminate the property name and the level-1,
 * level-2 names. The default prefix is {@link #ODE_PREFFIX}.
 * <p/>
 ...
 * and ports. <br/>
 * Level 0 would be: values common to all services and ports. <br/>
 * Level 1: values common to a given service. <br/>
 ...
 * the associated value is a path and as such is resolved against the path of
 * the file it was loaded from.
 * </p>
 * Assigned properties must not start with 'system.' or 'env.'. These prefix are
 * reserved to access system properties and environment variables.
 * 
...
public class HierarchicalProperties {
	...
	private static final Log log = LogFactory
			.getLog(HierarchicalProperties.class);
	 ...
	 * (qualified) service and/or port they are associated with. All ChainedMap
	 * instances has a common parent. The ChainedMap instances are chained to
	 * each others so that if a property is not found for [service, port], the
	 * ChainedMap associated to [service] will be queried, and if still not
	 * found, then the common parent.
	 * 
	 * The ChainedMap instance common to all services and ports is associated to
	 * the [null, null] key. ChainedMap instance common to all ports of a given
	 * service is associated to [service, null]. ChainedMap instance of a given
	 * service, port couple is associated to [service, port].
	 * 
	...
	/**
	 * @param files
	 *            the property file to be loaded. The file may not exist. But if
	 *            the file exists it has to be a file (not a directory),
	 *            otherwhise an IOException is thrown. Files will be loaded in
	 *            the given order.
	 * @param prefix
	 *            the property prefix
	 * @throws IOException
	 */
	public HierarchicalProperties(File[] files, String prefix)
			throws IOException {
		this.files = files;
		this.prefix = prefix;
	...
	private void processProperties(Properties props, File file)
			throws IOException {
		...
		for (Iterator it = props.entrySet().iterator(); it.hasNext();) {
			...
			if (nsalias != null) {
				qname = new QName(
						nsByAlias.get(nsalias) != null ? nsByAlias.get(nsalias)
								: nsalias, service);
			}
			...
			if (targetedProperty.endsWith(".file")
				...
				if (log.isDebugEnabled())
					log.debug("path: " + value + " resolved into: "
							+ absolutePath);
				value = absolutePath;
			}
	...
	private Map<String, String> collectAliases(Properties props, File file) {
		...
		for (Iterator it = props.entrySet().iterator(); it.hasNext();) {
			...
			if (key.startsWith("alias.")) {
				...
				final String alias = key.substring("alias.".length(),
						key.length());
				if (log.isDebugEnabled())
					log.debug("Alias found: " + alias + " -> " + value);
				if (nsByAlias.containsKey(alias)
						&& value.equals(nsByAlias.get(alias)))
					throw new RuntimeException(
							"Same alias used twice for 2 different namespaces! file="
									+ file + ", alias=" + alias);
	...
	private void validatePropertyNames(Properties props, File file) {
		...
		if (!invalids.isEmpty()) {
			throw new IllegalArgumentException(
					"Property files cannot define properties starting with 'system.' nor 'env.' File="
							+ file
							+ ". Invalid names="
							+ StringUtils.join(invalids, ","));
		}
	}

	private void replacePlaceholders() {
		Pattern systemProperty = Pattern.compile("\\$\\{system\\.([^\\}]+)\\}");
		Pattern environmentVariable = Pattern
				.compile("\\$\\{env\\.([^\\}]+)\\}");
		...
		for (Iterator it = hierarchicalMap.values().iterator(); it.hasNext();) {
			...
			for (Iterator it1 = properties.entrySet().iterator(); it1.hasNext();) {
				...
				// /!\ replacement values themselves might contain placeholders.
				// So always retrieve the value from the map entry
				e.setValue(SystemUtils.replaceProperties((String) e.getValue(),
						localPlaceholder, getRootMap().child));
				e.setValue(SystemUtils.replaceProperties((String) e.getValue(),
						systemProperty, System.getProperties()));
				e.setValue(SystemUtils.replaceProperties((String) e.getValue(),
						environmentVariable, System.getenv()));
			}
		}
	}
	...
	public Map getProperties(String serviceNamespaceURI,
			String serviceLocalPart, String port) {
		return getProperties(new QName(serviceNamespaceURI, serviceLocalPart),
				port);
	}

	/**
	 * Return a map containing all the properties for the given port. The map is
	 * an immutable snapshot of the properties. Meaning that futur changes to
	 * the properties will NOT be reflected in the returned map.
	 * 
	 * @param service
	 * @param port
	...
	public String getProperty(String serviceNamespaceURI,
			String serviceLocalPart, String property) {
	...
	public String getProperty(String serviceNamespaceURI,
			String serviceLocalPart, String port, String property) {
		return getProperty(new QName(serviceNamespaceURI, serviceLocalPart),
				port, property);
	}
	...
	private String[] parseProperty(String property) {
		...
		if (index <= 0) {
			...
			res[3] = property.substring(index + dotted_prefix.length()); // targeted
			// property
			...
			if (t.length != 2 && t.length != 3) {
				throw new IllegalArgumentException("Invalid property name:"
						+ property
						+ " Expected pattern: [nsalias.service.[port.]"
						+ prefix + ".]property");
			}
			if (t.length >= 2) {
				res[0] = t[0]; // ns alias
	...
	/**
	 * Link two Maps instances in a parent-child relation. Meaning that if a key
	 * is looked up but not found on the child, then the key will be looked up
	 * on the parent map. <br/>
	 * The raison d'etre of this class is to the {@link #keySet()} method. This
	 * methods returns a set of <strong>all</strong> the keys contained in the
	 * child and the parent. That's the main reason to not used the
	 * {@link java.util.Properties} class (which offers access to child keys
	 * only).
	 * <p/>
	 * The child has an immutable view of the parent map. Methods
	 * {@link #clear()} and {@link #remove(Object)} throw
	 * {@link UnsupportedOperationException}. Methods
	 * {@link #put(Object, Object)} and {@link #putAll(java.util.Map)} impacts
	 * only the child map. <br/>
	 * Methods {@link #clearLocally()}
	 * <p/>
	 * This class does NOT implement the {@link java.util.Map} interface because
	 * methods {@link java.util.Map#entrySet()} , {@link java.util.Map#values()}
	 * and {@link java.util.Map#keySet()} would NOT be backed by the Map itself.
	 * <br/>
	...
	private static class ChainedMap {
		...
		/**
		 * @return true if the child map is empty AND the parent map is null or
		 *         empty as well.
		 * 
		 *         <pre>
		...
		/**
		 * @return true if the child map contains this key OR the parent map is
		 *         not null and contains this key.
		 * 
		 *         <pre>
		 ...
		 *         and parent maps. <strong>The returned set is not backed by
		 *         the maps.</strong> Any references to the returned sets are
		 *         hold at the holder's own risks. This breaks the general
		 *         {@link java.util.Map#entrySet()} contract.
		 */
		public Set keySet() {
