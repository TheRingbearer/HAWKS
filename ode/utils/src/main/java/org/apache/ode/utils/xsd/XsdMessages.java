...
public class XsdMessages extends MessageBundle {
	 ...
	 *         {0}: [{2} @ L{3}:C{4}]: {1}
	 */
	public String msgXsdMessage(String type, String msg, String systemId,
			int line, int col) {
		return this.format("{0}: [{2} @ L{3}:C{4}]: {1}", type, msg, systemId,
				line, col);
	}
	 ...
	 */
	public String msgXsdExceptionMessage(String message, String systemId,
			int lineNumber, int columnNumber) {
		return this.format(
				"Unable to process XML Schema from {1} [@L{2}:C{3}]: {0}",
				message, systemId, lineNumber, columnNumber);
	}

	/** An unknown error occured processing schema at {0}" */
	public String msgXsdUnknownError(String systemId) {
		return this.format("An unknown error occured processing schema at {0}",
				systemId);
	}
