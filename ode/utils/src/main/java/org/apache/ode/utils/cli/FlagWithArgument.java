...
public class FlagWithArgument implements CommandlineFragment {
	 ...
	 * 
	 * @param name
	 *            the name of the flag, as it will be used on the commandline
	 * @param argName
	 *            the name of the argument, as it will be used to generate usage
	 *            and help
	 * @param description
	 *            the description of the flag
	 * @param optional
	 *            whether or not this flag is optional
	 */
	public FlagWithArgument(String name, String argName, String description,
			boolean optional) {
		_isOptional = false;
	...
	public List<String> consume(List<String> s)
		...
		if (!s.contains(_name)) {
			if (!_isOptional) {
				throw new CommandlineSyntaxException("The " + _name
						+ " flag with an argument is required.");
			} else {
				return s;
			}
		}
