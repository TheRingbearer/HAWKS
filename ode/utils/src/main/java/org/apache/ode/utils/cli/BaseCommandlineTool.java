...
public abstract class BaseCommandlineTool {
	...
	protected static final Fragments HELP = new Fragments(
			new CommandlineFragment[] { HELP_FLAG });
	...
	private static final CommandLineMessages __msgs = MessageBundle
			.getMessages(CommandLineMessages.class);
	...
	public static void outputHeader() {
		...
		System.out.println(__msgs.msgCliHeader(getProgramName(),
				Version.getVersionName(), Version.getBuildDate()));
	}
	...
	protected static void initLogFile(int level) throws IOException {
		FileAppender appender = new FileAppender(new PatternLayout(
				LOGGING_PATTERN), System.getProperty("ode.home")
				+ "/logs/ode.log");
