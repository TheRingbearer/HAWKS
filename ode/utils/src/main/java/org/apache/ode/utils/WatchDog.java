 ...
 * invocations of {@link #check()} (i.e wihtout having to use a thread) while
 * preserving time checking.
 * <p/>
 * Now two use cases coexist:
 * <ol>
 * <li>Pass an instance of {@link WatchDog} to a new thread ({@link WatchDog} is
 * a {@link Runnable}). So that {@link WatchDog# check ()} will be called
 * automatically every {@code delay} milliseconds.</li>
 * <li>Invoke {@link WatchDog# check ()} only when you feel like it. If the
 * expiration date previously set is lower than NOW then event callback methods
 * will be invoked accordingly.</li>
 * </ol>
 * 
...
public class WatchDog<T, C extends WatchDog.Observer> implements Runnable {
	...
	public final void check() {
		...
		if (expire <= now) {
			/*
			 * get a lock on the observer right now It would be overkilled to
			 * lock before testing the dates. By locking after the comparaison
			 * the worst scenario is that 2 threads get in the "if", thread A
			 * gets the lock, thread B waits for it. Once the lock is released,
			 * thread B acquires it and do the checks on the mutable. So this
			...
			try {
				if (log.isDebugEnabled())
					log.debug("[" + mutable + "]" + " check for changes");
				if (mutable.exists()) {
					existedBefore = true;
					if (lastModif == null || mutable.hasChangedSince(lastModif)) {
						...
						if (log.isInfoEnabled())
							log.info("[" + mutable + "]" + " updated");
						warnedAlready = false;
					} else {
						if (log.isDebugEnabled())
							log.debug("[" + mutable + "]" + " has not changed");
					}
				} else if (!observer.isInitialized()) {
					// no resource and first time
					observer.init();
					if (log.isInfoEnabled())
						log.info("[" + mutable + "]" + " initialized");
				} else {
					if (existedBefore) {
						existedBefore = false;
						lastModif = null;
						observer.onDelete();
						if (log.isInfoEnabled())
							log.info("[" + mutable + "]" + " deleted");
					}
					if (!warnedAlready) {
						warnedAlready = true;
						if (log.isInfoEnabled())
							log.info("[" + mutable + "]" + " does not exist.");
					}
				}
			} catch (Exception e) {
				if (log.isDebugEnabled())
			...
			if (log.isTraceEnabled())
				log.trace("[" + mutable + "]" + " wait period is not over");
		}
	}

	public static <C extends Observer> WatchDog<Long, C> watchFile(File file,
			C handler) {
		return new WatchDog<Long, C>(new FileMutable(file), handler);
	}

	public static <C extends Observer> WatchDog<Map<File, Long>, C> watchFiles(
			List<File> files, C handler) {
		return new WatchDog<Map<File, Long>, C>(new FileSetMutable(files),
				handler);
	}
	...
	static public class FileMutable implements WatchDog.Mutable<Long> {
		...
		public boolean hasChangedSince(Long since) {
			// do use 'greater than' to handle file deletion. The timestamp of a
			// non-exising file is 0L.
	...
	static public class FileSetMutable implements
	...
	public interface Observer<A> {
		...
		/**
		 * Called by {@link WatchDog#check()} if the underlying object is not
		 * {@link #isInitialized initialized} and the
		 * {@link WatchDog.Mutable#exists()} resource does not exist}. <br/>
		 ...
		 * exist. <br/>
		 * The default implementation invokes {@link #init()} .
		 * 
		 * @throws Exception
		 */
		void onDelete();
		 ...
		 * {@link WatchDog.Mutable#lastModified()} timestamp has changed
		 * (greater than the previous value). <br/>
		 * The default implementation invokes {@link #init()} .
		 * 
		 * @throws Exception
		 */
	 ...
	 * invoke the #init method which satifies most use cases. So subclasses may
	 * simply override the #init method to fit their own needs.
	 */
