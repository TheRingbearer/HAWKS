...
/**
 * <p>
 * A multiple argument commandline fragment that greedily consumes what's left
 * on the commandline as arguments.
 * </p>
 */
public class MultiArgument implements CommandlineFragment {
	...
	public List<String> consume(List<String> s)
		...
		if (s.size() == 0 && !_isOptional) {
			throw new CommandlineSyntaxException("The " + _argName
		...
		if (s.size() != 0) {
			...
			for (int i = 0; i < _values.length; ++i) {
				if (_values[i].startsWith(COMMAND_PREFIX)) {
					throw new CommandlineSyntaxException("The flag "
							+ _values[i] + " was found where an argument <"
							+ _argName + "> was expected.");
				}
			}
		}
