...
public abstract class MessageBundle extends ResourceBundle {
	...
	public static <T extends MessageBundle> T getMessages(
			Class<T> bundleInterface) {
		return getMessages(bundleInterface, Locale.getDefault());
	}

	@SuppressWarnings("unchecked")
	public static <T extends MessageBundle> T getMessages(
			Class<T> bundleInterface, Locale locale) {
		// TODO maybe catch MissingResourceException here
		return (T) ResourceBundle.getBundle(bundleInterface.getName(), locale,
				bundleInterface.getClassLoader());
	}
