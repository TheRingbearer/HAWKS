...
public class CollectionsX {
	...
	public static <T> void apply(Collection<T> coll, UnaryFunctionEx<T, ?> f)
			throws Exception {
		apply(coll.iterator(), f);
	}

	public static <T> void apply(Iterator<T> i, UnaryFunctionEx<T, ?> f)
			throws Exception {
		while (i.hasNext()) {
			f.apply(i.next());
		}
	 ...
	 * given in the form of a unary function which returns a non-
	 * <code>false</code> value when the condition is satisfied. The first
	 * object in the collection matching the condition is returned.
	 * 
	 * @param coll
	 ...
	 *         condition (f)
	 */
	public static <T> T find_if(Collection<T> coll,
	...
	public static <T> Collection<T> insert(Collection<T> coll,
			Enumeration<? extends T> e) {
	 ...
	 */
	public static <T> Collection<T> remove_if(Collection<T> coll,
	 ...
	 */
	public static <C extends Collection<T>, T, V extends T, E> C transform(
			C dest, Collection<E> src, UnaryFunction<E, V> f) {
	...
	public static <C extends Collection<T>, T, V extends T, E> C transformEx(
			C dest, Collection<E> src, UnaryFunctionEx<E, V> f)
			throws Exception {
		Iterator<E> i = src.iterator();
	...
	public static <C extends Collection<T>, T, V extends T, E> C transform(
			C dest, Enumeration<E> i, UnaryFunction<E, V> f) {
	...
	public static <C extends Collection<T>, T, S extends T> C filter(C dest,
			Collection<S> source, MemberOfFunction<S> function) {
		return filter(dest, source.iterator(), function);
	}

	public static <C extends Collection<T>, T, S extends T> C filter(C dest,
			Iterator<S> source, MemberOfFunction<S> function) {
