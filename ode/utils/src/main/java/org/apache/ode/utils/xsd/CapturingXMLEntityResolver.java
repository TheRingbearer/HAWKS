...
public class CapturingXMLEntityResolver implements XMLEntityResolver {
	...
	public XMLInputSource resolveEntity(XMLResourceIdentifier resourceIdentifier)
		...
		if (is == null) {
			...
			throw new IOException("Unable to locate resource for namespace "
		...
		try {
			URI systemId = new URI(FileUtils.encodePath(resourceIdentifier
					.getLiteralSystemId() == null ? resourceIdentifier
					.getNamespace() : resourceIdentifier.getLiteralSystemId()));

			__log.debug("Captured: " + systemId);
			_capture.put(systemId, data);
		} catch (URISyntaxException use) {
			__log.error("Invalid URI: "
