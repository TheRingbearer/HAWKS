...
public class Properties {
	...
	/**
	 * Property used to define how long (in milliseconds) the message will wait
	 * for a response. Default value is {@link #DEFAULT_MEX_TIMEOUT}
	 */
	public static final String PROP_MEX_TIMEOUT = "mex.timeout";

	/**
	 * Property used to define how long (in milliseconds) the message will wait
	 * for a response for process-to-process invocations.
	 */
	...
	public static final String PROP_HTTP_PROXY_HOST = PROP_HTTP_PROXY_PREFIX
			+ "host";
	public static final String PROP_HTTP_PROXY_PORT = PROP_HTTP_PROXY_PREFIX
			+ "port";
	...
	public static final String PROP_HTTP_PROXY_USER = PROP_HTTP_PROXY_PREFIX
			+ "user";
	public static final String PROP_HTTP_PROXY_PASSWORD = PROP_HTTP_PROXY_PREFIX
			+ "password";
	/**
	 * @deprecated use org.apache.commons.httpclient.params.HttpMethodParams#
		...
		for (Map.Entry<String, String> e : properties.entrySet()) {
			...
			if (k.startsWith(PROP_HTTP_HEADER_PREFIX)) {
				...
				if (PROP_HTTP_PROXY_HOST.equals(k))
					...
					log.warn("Unknown proxy properties [" + k + "]. "
							+ PROP_HTTP_PROXY_PREFIX
	...
	public static class Axis2 {
		...
		public static Options translate(Map<String, String> properties,
				Options options) {
			if (log.isDebugEnabled())
				log.debug("Translating Properties for Axis2");
			 ...
			 * value) are automatically handled (i.e no translation needed)
			 */
			for (Map.Entry<String, String> e : properties.entrySet()) {
			...
			if (properties.containsKey(PROP_HTTP_CONNECTION_TIMEOUT)) {
				final String value = properties
						.get(PROP_HTTP_CONNECTION_TIMEOUT);
				try {
					...
					if (log.isWarnEnabled())
						log.warn("Mal-formatted Property: ["
								...
								+ "="
								+ value
								+ "]. Integer expected. Property will be skipped.");
				}
			}
			if (properties.containsKey(PROP_HTTP_SOCKET_TIMEOUT)) {
				...
				try {
					...
					if (log.isWarnEnabled())
						log.warn("Mal-formatted Property: ["
								+ Properties.PROP_HTTP_SOCKET_TIMEOUT
								+ "="
								+ value
								+ "]. Integer expected. Property will be skipped.");
				}
			...
			if (properties.containsKey(PROP_JMS_REPLY_TIMEOUT)) {
				...
				// if (log.isWarnEnabled())
				// log.warn("Mal-formatted Property: [" +
				// Properties.PROP_JMS_REPLY_TIMEOUT + "=" + value +
				// "]. Long expected. Property will be skipped.");
				// }
			}
			if (properties.containsKey(PROP_JMS_DESTINATION_TYPE)) {
				String value = properties.get(PROP_JMS_DESTINATION_TYPE);
				try {
					options.setProperty(JMSConstants.PARAM_DEST_TYPE,
							Long.valueOf(value));
				} catch (NumberFormatException e) {
					if (log.isWarnEnabled())
						log.warn("Mal-formatted Property: ["
								+ Properties.PROP_JMS_DESTINATION_TYPE + "="
								+ value
								+ "]. Long expected. Property will be skipped.");
				}
			}
			...
			if (properties.containsKey("ws-adddressing.headers")) {
				if (log.isWarnEnabled())
					log.warn("Deprecated property: ws-adddressing.headers (Mind the 3 d's). Use ws-addressing.headers");
			...
			ArrayList<Header> headers = (ArrayList<Header>) o[1]; // /!\ Axis2
			// requires
			// an
			// ArrayList
			// (not a
			...
			if (JavaUtils.isTrueExplicitly(options
					.getProperty(HTTPConstants.REUSE_HTTP_CLIENT))) {
				if (log.isWarnEnabled())
					log.warn("This property cannot be overidden, and must always be false. "
	...
	public static class HttpClient {
		...
		public static HttpParams translate(Map<String, String> properties,
				HttpParams p) {
			if (log.isDebugEnabled())
				log.debug("Translating Properties for HttpClient. Properties size="
						+ properties.size());
			if (properties.isEmpty())
				return p;
			...
			if (properties.containsKey(PROP_HTTP_CONNECTION_TIMEOUT)) {
				final String value = properties
						.get(PROP_HTTP_CONNECTION_TIMEOUT);
				try {
					p.setParameter(HttpConnectionParams.CONNECTION_TIMEOUT,
							Integer.valueOf(value));
				} catch (NumberFormatException e) {
					if (log.isWarnEnabled())
						log.warn("Mal-formatted Property: ["
								+ Properties.PROP_HTTP_CONNECTION_TIMEOUT + "="
								+ value + "] Property will be skipped.");
				}
			}
			if (properties.containsKey(PROP_HTTP_SOCKET_TIMEOUT)) {
				final String value = properties.get(PROP_HTTP_SOCKET_TIMEOUT);
				try {
					p.setParameter(HttpMethodParams.SO_TIMEOUT,
							Integer.valueOf(value));
				} catch (NumberFormatException e) {
					if (log.isWarnEnabled())
						log.warn("Mal-formatted Property: ["
			...
			// HttpMethodParams.HTTP_CONTENT_CHARSET accepts a string and we use
			...
			if (properties.containsKey(PROP_HTTP_PROTOCOL_VERSION)) {
				try {
					p.setParameter(HttpMethodParams.PROTOCOL_VERSION,
							HttpVersion.parse(properties
									.get(PROP_HTTP_PROTOCOL_VERSION)));
			...
			if (properties.containsKey(PROP_HTTP_REQUEST_CHUNK)) {
				// see
				// org.apache.commons.httpclient.methods.EntityEnclosingMethod.setContentChunked()
				p.setBooleanParameter(PROP_HTTP_REQUEST_CHUNK, Boolean
						.parseBoolean(properties.get(PROP_HTTP_REQUEST_CHUNK)));
			}
			if (properties.containsKey(PROP_HTTP_REQUEST_GZIP)) {
				if (log.isWarnEnabled())
					log.warn("Property not supported by HTTP External Services: "
							+ PROP_HTTP_REQUEST_GZIP);
			}
			...
			if (Boolean.parseBoolean(properties.get(PROP_HTTP_ACCEPT_GZIP))) {
				...
				// ((Collection)
				// p.getParameter(HostParams.DEFAULT_HEADERS)).add(new
				// Header("Accept-Encoding", "gzip"));
				if (log.isWarnEnabled())
					log.warn("Property not supported by HTTP External Services: "
							+ PROP_HTTP_ACCEPT_GZIP);
			}

			if (properties.containsKey(PROP_HTTP_MAX_REDIRECTS)) {
				final String value = properties.get(PROP_HTTP_MAX_REDIRECTS);
				try {
					p.setParameter(HttpClientParams.MAX_REDIRECTS,
							Integer.valueOf(value));
				} catch (NumberFormatException e) {
					if (log.isWarnEnabled())
						log.warn("Mal-formatted Property: ["
								+ Properties.PROP_HTTP_MAX_REDIRECTS + "="
								+ value + "] Property will be skipped.");
				}
			}
			...
			if (headers != null && !headers.isEmpty())
				((Collection) p.getParameter(HostParams.DEFAULT_HEADERS))
						.addAll(headers);
			if (proxy != null)
				p.setParameter(PROP_HTTP_PROXY_PREFIX, proxy);
