...
public class TempFileManagerTest extends TestCase {
	...
	public TempFileManagerTest(String s) throws Exception {
		super(s);
		DEFAULT_TEMPDIR_PATH = new File(SystemUtils.javaTemporaryDirectory())
				.getCanonicalPath();
	}
	...
	public void testSetWorkingDirectoryTwice() throws Exception {
		...
		assertTrue(managedDir.getCanonicalPath().contains(
	...
	public void testSetWorkingDirectory() throws Exception {
		File tmpDir = File.createTempFile(TempFileManagerTest.class.getName(),
				"test");
		...
		assertTrue(managedFile.getCanonicalPath().contains(
				tmpDir.getCanonicalPath()));
		File managedDir = TempFileManager.getTemporaryDirectory("fooDir-");
		assertTrue(managedDir.getCanonicalPath().contains(
				tmpDir.getCanonicalPath()));
	...
	public void testNestedTempDirs() throws Exception {
		...
		assertTrue(dir2.getParentFile().getCanonicalPath()
				.equals(dir1.getCanonicalPath()));
		File fileInDir2 = TempFileManager.getTemporaryFile("file-", dir2);
		assertTrue(fileInDir2.exists());
		assertTrue(fileInDir2.getParentFile().getCanonicalPath()
				.equals(dir2.getCanonicalPath()));
		TempFileManager.cleanup();
		assertTrue(!fileInDir2.exists());
		assertTrue(!dir1.exists());
		assertTrue(!dir2.exists());
