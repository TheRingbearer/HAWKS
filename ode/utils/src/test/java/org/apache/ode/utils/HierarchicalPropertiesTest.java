...
public class HierarchicalPropertiesTest extends TestCase {
	...
	protected void setUp() throws Exception {
		...
		File[] files = new File[] {
				new File(getClass().getResource("/hierarchical-2.properties")
						.toURI()),
				new File(getClass().getResource("/hierarchical-1.properties")
						.toURI()) };
		hp = new HierarchicalProperties(files);
	}

	private void checkEnv() {
		// Java does not let us set ENV variables. The test avriable must be set
		// by the process running this test.
	...
	public void testGetProperty() {
		...
		assertEquals(msg, "40000",
				hp.getProperty("http://foo.com", "film-service", "timeout"));
		...
		assertEquals(msg, "4", hp.getProperty("a_namespace_with_no_alias",
				"a_service", "poolsize"));
		assertEquals(
				"If the same property is set by two different files, the last loaded file must take precedence",
				"50000", hp.getProperty("http://foo.com", "film-service",
						"port-of-cannes", "timeout"));
		assertEquals(
				"The prefix could be use without interfering",
				"so green or red?",
	...
	public void testCachedGetProperties() {
		assertSame("Snapshot maps should be cached!",
				hp.getProperties("foo", "film-service"),
				hp.getProperties("foo", "film-service"));
		assertSame("Snapshot maps should be cached!",
				hp.getProperties("foo", "film-service", "port-of-cannes"),
				hp.getProperties("foo", "film-service", "port-of-cannes"));
		assertSame("Snapshot maps should be cached!",
				hp.getProperties("bla", "unknown-service"),
				hp.getProperties("bla", "unknown-service"));
	}

	public void testPathHandling() {
		assertTrue(
				"If the property name ends with '.file' or '.path' its value might be resolved against the file path",
				FileUtils.isAbsolute(hp.getProperty("http://foo.com",
						"film-service", "port-of-cannes", "p1.file")));
		assertTrue(
				"If the property name ends with '.file' or '.path' its value might be resolved against the file path",
				FileUtils.isAbsolute(hp.getProperty("http://foo.com",
						"film-service", "port-of-cannes", "p1.path")));
		assertEquals("An absolute path should not be altered",
				"/home/ode/hello.txt", hp.getProperty("http://foo.com",
						"film-service", "port-of-cannes", "p2.path"));
	...
	public void testReservedNames() {
		...
		try {
			HierarchicalProperties f = new HierarchicalProperties(new File(
					getClass().getResource("/hierarchical-bad.properties")
							.toURI()));
			fail(s);
		} catch (Exception e) {
	...
	public void testReplaceSystemProperty() {
		// one config step happens in setUp()
		assertEquals(
				"${system.*} must be replaced by the corresponding system property.",
				System.getProperty("TestSystemProperty"), hp.getProperty(
						"http://bar.com", "brel-service", "sys.property"));
	}

	public void testReplaceEnvVariable() {
		checkEnv();
		assertEquals(
				"${env.*} must be replaced by the corresponding environment variable.",
				System.getenv("TEST_DUMMY_ENV_VAR"), hp.getProperty(
						"http://bar.com", "brel-service",
						"environment.property"));
	}

	public void test_placeholder() {
		assertEquals("A placeholder must be replaced by its value",
				"placeholder1-value", hp.getProperty("test.placeholder1"));
	}

	public void test_placeholder_that_uses_sys_prop_and_env_var() {
		checkEnv();
		assertEquals(
				"A placeholder can use system properties and/or environment variables",
				System.getProperty("TestSystemProperty") + "#"
						+ System.getenv("TEST_DUMMY_ENV_VAR"),
				hp.getProperty("test.placeholder2"));
	}

	public void test_placeholder_defined_in_2_files() {
		assertEquals(
				"Last loaded placeholder value must overridden any previous values",
				"placeholder3-value", hp.getProperty("http://foo.com",
						"film-service", "port-of-cannes", "test.placeholder3"));
	}
