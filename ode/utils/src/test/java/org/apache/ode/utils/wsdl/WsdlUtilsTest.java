...
public class WsdlUtilsTest extends TestCase {
	...
	protected void setUp() throws Exception {
		...
		dummyService = definition.getService(new QName(
				"http://axis2.ode.apache.org", "DummyService"));
	}

	public void testNoBinding() {
		Port noBindingPort = dummyService
				.getPort("DummyService_port_with_no_binding");
	...
	public void testEmptyBinding() {
		Port noBindingPort = dummyService
				.getPort("DummyService_port_with_empty_binding");
	...
	public void testGetOperationExtension() {
		...
		for (Port port : ports) {
			BindingOperation bindingOperation = port.getBinding()
					.getBindingOperation("hello", null, null);
			ExtensibilityElement operationExtension = WsdlUtils
					.getOperationExtension(bindingOperation);
	...
	public void testUseUrlEncoded() {
		for (Object o : dummyService.getPorts().entrySet()) {
			...
			if (binding == null)
				continue; // some bindings intentionally missing
			BindingOperation bindingOperation = binding.getBindingOperation(
					"hello", null, null);
			if (bindingOperation == null)
				continue; // some bindings intentionally empty
			if ("DummyServiceHttpport_urlEncoded".equals(portName)) {
				assertTrue(WsdlUtils.useUrlEncoded(bindingOperation
						.getBindingInput()));
			} else {
				assertFalse(WsdlUtils.useUrlEncoded(bindingOperation
						.getBindingInput()));
			}
		}
	}
	...
	public void testUseUrlReplacement() {
		for (Iterator it = dummyService.getPorts().entrySet().iterator(); it
				.hasNext();) {
			...
			if ("DummyServiceHttpport_urlReplacement".equals(portName)) {
				assertTrue(WsdlUtils.useUrlReplacement(bindingOperation
						.getBindingInput()));
			} else {
				assertFalse(WsdlUtils.useUrlReplacement(bindingOperation
						.getBindingInput()));
			}
		}
	}
	...
	public void testUseMimeMultipartRelated() {
		for (Iterator it = dummyService.getPorts().values().iterator(); it
				.hasNext();) {
			...
			for (int i = 0; i < binding.getBindingOperations().size(); i++) {
				BindingOperation operation = (BindingOperation) binding
						.getBindingOperations().get(i);
				assertFalse(WsdlUtils.useMimeMultipartRelated(operation
						.getBindingInput()));
			}
		}
	}
	...
	public void testGetAddresExtgension() {
		for (Iterator it = dummyService.getPorts().entrySet().iterator(); it
			...
			if ("DummyService_port_with_empty_binding".equals(port.getName())
					|| "DummyService_port_with_no_binding".equals(port
							.getName())) {
				continue;
			}

			if (WsdlUtils.useHTTPBinding(port)) {
				HTTPAddress add = (HTTPAddress) WsdlUtils
						.getAddressExtension(port);
				assertNotNull("Address expected", add);
				assertNotNull("Non-null Location expected",
						add.getLocationURI());
				assertTrue("Non-empty Location expected", add.getLocationURI()
						.length() > 0);
			} else if (WsdlUtils.useHTTPBinding(port)) {
				SOAPAddress add = (SOAPAddress) WsdlUtils
						.getAddressExtension(port);
				assertNotNull("Address expected", add);
				assertNotNull("Non-null Location expected",
						add.getLocationURI());
	...
	public void testGetMimeContentType() {
		Binding binding = definition.getBinding(new QName(
				"http://axis2.ode.apache.org", "DummyServiceHttpBinding"));
		BindingOperation operation = binding.getBindingOperation("hello", null,
				null);
		MIMEContent mimeContent = WsdlUtils.getMimeContent(operation
		...
		binding = definition.getBinding(new QName(
				"http://axis2.ode.apache.org", "DummyServiceSOAP11Binding"));
		operation = binding.getBindingOperation("hello", null, null);
		mimeContent = WsdlUtils.getMimeContent(operation.getBindingInput()
	...
	public void testGetHeaders() {
		...
		BindingOperation operation = binding.getBindingOperation("hello", null,
				null);
		Collection headers = WsdlUtils.getHttpHeaders(operation
