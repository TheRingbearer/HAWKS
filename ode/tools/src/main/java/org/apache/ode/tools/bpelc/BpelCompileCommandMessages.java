...
public class BpelCompileCommandMessages extends MessageBundle {
	...
	public String msgIoErrWritingCbp(String file, String reason) {
		return this.format("Unable to write compiled process to {0}: {1}",
				file, reason);
	}
	...
	public String msgInvalidRrDirectory(String name) {
		return this.format("The location {0} is not a resource repository.",
				name);
	}
	...
	String strBpelcUsage() {
		return this
				.format("[-cp addclasspath] [-o outfile] [-rr rrfile] [-sar [-sdd sdd-url]]"
						+ "'{'wsdl-url '}' '{' bpel-url, ... '}'");
	}
