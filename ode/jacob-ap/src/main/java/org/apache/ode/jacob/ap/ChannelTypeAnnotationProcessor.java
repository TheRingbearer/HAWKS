...
class ChannelTypeAnnotationProcessor implements AnnotationProcessor {
	...
	ChannelTypeAnnotationProcessor(AnnotationTypeDeclaration atd,
	...
	public void process() {
		Collection<InterfaceDeclaration> channels = DeclarationFilter
				.getFilter(InterfaceDeclaration.class).filter(
						_env.getDeclarationsAnnotatedWith(_atd),
						InterfaceDeclaration.class);
		for (InterfaceDeclaration c : channels) {
			PrintWriter pw = null;
			try {
				pw = _env.getFiler().createSourceFile(
						c.getQualifiedName() + "Channel");
				writeChannelClass(pw, c);
			} catch (IOException e) {
				_env.getMessager().printError(c.getPosition(),
			...
			try {
				pw = _env.getFiler().createSourceFile(
						c.getQualifiedName() + "ChannelListener");
	...
	private void writeChannelClass(PrintWriter pw, InterfaceDeclaration c) {
		...
		if (supers.isEmpty()) {
			...
			for (InterfaceType s : supers)
				pw.println("            "
						+ s.getDeclaration().getQualifiedName() + "Channel, ");
	...
	private void writeChannelListenerClass(PrintWriter pw,
		...
		pw.println(" */");
		pw.println("public abstract class " + c.getSimpleName()
				+ "ChannelListener");
