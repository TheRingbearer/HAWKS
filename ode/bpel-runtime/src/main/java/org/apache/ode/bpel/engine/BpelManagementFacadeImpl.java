...
/**
 * Implementation of the instance/process management interaction. This class
 * implements the methods necessary to support process debugging. It also
 * implements all the methods in the newer Process/Instance Management interface
 * (pmapi).
 */
public class BpelManagementFacadeImpl extends ProcessAndInstanceManagementImpl
	...
	public Long getProcessInstance(String pid, final CorrelationKey ckey)
		...
		return dbexec(_db.getProcessDb(QName.valueOf(pid)).new Callable<Long>() {
			public Long run(BpelDAOConnection session) throws Exception {
				Iterator<ProcessInstanceDAO> i = getProcessDAO().findInstance(
						ckey).iterator();
	...
	public EventInfoListDocument getEvents(final Long iid, final int startIdx,
			final int count) throws ManagementException {
		...
		timeline = timeline.subList(startIdx,
				Math.min(timeline.size(), startIdx + count));
	...
	public String getVariable(final Long iid, final Long scopeId,
	...
	public Long[] getScopeInstancesForScope(final Long iid,
			final String scopeName) throws ManagementException {
	...
	public void setCorrelation(final Long iid, final Long scopeId,
			final String correlationSet, final QName[] propertyNames,
			final CorrelationKey values) throws ManagementException {
		...
		dbexec(new BpelDatabase.Callable<Object>() {
			public Object run(BpelDAOConnection session) throws Exception {
				...
				CorrelationSetDAO correlationSetDAO = instance
						.getCorrelationSet(correlationSet);
	...
	public CorrelationKey getCorrelation(final Long iid, final Long scopeId,
		...
		return dbexec(new BpelDatabase.Callable<CorrelationKey>() {
			public CorrelationKey run(BpelDAOConnection session)
					throws Exception {
				ProcessInstanceDAO instance = session.getInstance(iid);
				if (instance == null) {
					throw new ManagementException("InstanceNotFound:" + iid);
	...
	public OProcess getProcessDef(String procid) throws ManagementException {
		if (_server._engine == null)
			throw new ProcessingException(
					"ServiceProvider required for debugger operation.");

		BpelProcess process = _server._engine._activeProcesses.get(QName
				.valueOf(procid));
		if (process == null)
			throw new InvalidRequestException(
					"The process \""
							+ procid
							+ "\" is not available. Please make sure it is deployed and encompassing System is activated.");
	...
	public Breakpoint[] getGlobalBreakpoints(String procId)
			throws ManagementException {
	...
	public void removeGlobalBreakpoint(String procid, Breakpoint sp)
			throws ManagementException {
		DebuggerSupport debuggerSupport = getDebugger(QName.valueOf(procid));
		debuggerSupport.removeGlobalBreakpoint(sp);
	}

	public void removeBreakpoint(Long iid, Breakpoint sp)
			throws ManagementException {
		DebuggerSupport debuggerSupport = getDebugger(iid);
	...
	public ActivityBreakpoint addGlobalActivityBreakpoint(String procid,
		...
		ActivityBreakpointImpl bp = new ActivityBreakpointImpl(
				_uuidGen.nextUUID(), activity);
	...
	public VariableModificationBreakpoint addVariableModificationBreakpoint(
			Long iid, String scopename, String variable) {
		throw new UnsupportedOperationException();
	}
	...
	private CorrelationSetDAO findCorrelationSetDAO(
			ProcessInstanceDAO instance, Long scopeId, String correlationSet)
			throws ManagementException {
		ScopeDAO scope = instance.getScope(scopeId);
