...
/**
 * <p>
 * A facade for accessing all the BPEL functionality that is not implemented
 ...
 * some sort), the communication activities pick/receive/invoke (i.e. there are
 * no JACOB channels representing partnerLink links), and correlation (i.e. the
 * JACOB objects do not match message to process instances, this happens in this
 * <em>native</em> layer).
 * </p>
 */
public interface BpelRuntimeContext {
	 ...
	 * 
	 * @param parentScopeId
	 *            _id of parent scope (null if root scope)
	 * @param scopeType
	...
	/**
	 * Creates a new LinkDAO for a given link.
	 * 
	 * @param link to create a new LinkDAO
	 */
	void createLinkInstance(OLink link);
	...
	void initializePartnerLinks(Long parentScopeId,
			Collection<OPartnerLink> partnerLinks, ScopeFrame sFrame, String actXPath);
	...
	Node initializeVariable(VariableInstance var, ScopeFrame scopeFrame,
	...
	/**
	 * Check if a channel is registered at the IMAManager.
	 * 
	 * @author hahnml
	 * 
	 * @param channelId of the channel to check if it is registered at the IMAManager.
	 * @return If the channel is registered or not.
	 */
	boolean isIMAChannelRegistered(String channelId);
	 ...
	 * @param oElement
	 *            Wait activity which has created the timer.
	 */
	 ...
	 */
	ValueReferencePair writeExtVar(Variable variable, Node reference, Node value)
			throws ExternalVariableModuleException;
