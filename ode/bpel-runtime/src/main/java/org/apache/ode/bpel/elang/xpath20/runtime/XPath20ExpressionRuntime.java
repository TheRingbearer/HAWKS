...
public class XPath20ExpressionRuntime implements ExpressionLanguageRuntime {
	...
	private static final Log __log = LogFactory
			.getLog(XPath20ExpressionRuntime.class);
	...
	public Node evaluateNode(OExpression cexp, EvaluationContext ctx)
		...
		if (retVal.size() == 0 || retVal.size() > 1) {
			StringBuffer msg = new StringBuffer(
					(retVal.size() == 0) ? "No results for expression: '"
							: "Multiple results for expression: '");
	...
	private Object evaluate(OExpression cexp, EvaluationContext ctx, QName type)
		...
		try {
			...
			JaxpVariableResolver varResolver = new JaxpVariableResolver(ctx,
					oxpath20, ((XPathFactoryImpl) _xpf).getConfiguration());
			...
			if (oxpath20.insertMissingData) {
				XPath20ExpressionModifier modifier = new XPath20ExpressionModifier(
						oxpath20.namespaceCtx, ((XPathFactoryImpl) _xpf)
			...
			throw new EvaluationException(
					"Error while executing an XPath expression: "
			...
			throw new EvaluationException(
					"Error while executing an XPath expression: ", t);
		}
	}
}
