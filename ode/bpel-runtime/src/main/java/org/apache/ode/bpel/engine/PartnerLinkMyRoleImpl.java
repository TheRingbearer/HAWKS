...
import org.apache.ode.deployapi.Deployer;
...
public class PartnerLinkMyRoleImpl extends PartnerLinkRoleImpl {
	...
	PartnerLinkMyRoleImpl(BpelProcess process, OPartnerLink plink,
			Endpoint endpoint) {
		super(process, plink);
		_endpoint = endpoint;
	}
	...
	public List<RoutingInfo> findRoute(MyRoleMessageExchangeImpl mex) {
		...
		if (__log.isTraceEnabled()) {
			__log.trace(ObjectPrinter.stringifyMethodEnter(this
					+ ":inputMsgRcvd", new Object[] { "messageExchange", mex }));
		}
		...
		// is anyone waiting for it. Get the correlator, a persisted
		...
		CorrelatorDAO correlator = _process.getProcessDAO().getCorrelator(
				correlatorId);
		...
		try {
			...
			__log.debug(
					"Unable to evaluate correlation keys, invalid message format. ",
					ime);
			mex.setFailure(MessageExchange.FailureType.FORMAT_ERROR,
					ime.getMessage(), null);
			return null;
		}

		String mySessionId = mex
				.getProperty(MessageExchange.PROPERTY_SEP_MYROLE_SESSIONID);
		String partnerSessionId = mex
		...
		if (messageRoutes != null && messageRoutes.size() > 0) {
			for (MessageRouteDAO messageRoute : messageRoutes) {
				if (__log.isDebugEnabled()) {
					__log.debug("INPUTMSG: " + correlatorId + ": ckeySet "
							+ messageRoute.getCorrelationKeySet()
							+ " route is to " + messageRoute);
				}
				routingInfos.add(new RoutingInfo(messageRoute, messageRoute
						.getCorrelationKeySet(), correlator, keySet));
			}
		}
	...
	public static class RoutingInfo {
		...
		public RoutingInfo(MessageRouteDAO messageRoute,
				CorrelationKeySet matchedKeySet, CorrelatorDAO correlator,
	...
	public void invokeNewInstance(MyRoleMessageExchangeImpl mex,
		...
		QName root = FragmentCompositionUtil.getRootProcessQName(_process
				.getConf());
		...
		if (_process.isProcessFragment()) {
			...
			try {
				...
				for (QName id : ids) {
					...
					if (rootProcess._pid.equals(currentRoot)) {
						newProcess = process;
						break;
					}
				}
			...
			/*
			 * _process.getEngine().getFragmentDeploymentJobExecuter().addJob(new
			 * Runnable(){ // to prevent deadlock on setting the process model
			 * as retired public void run() { //Deployer d = new Deployer();
			 * //QName id = d.deployNewProcessVersion(files, name); }
			 * 
			 * });
			 */
		}
		...
		if (newProcess != null) {
			...
			NewProcessInstanceEvent evt = new NewProcessInstanceEvent(
					new QName(newProcess.getOProcess().targetNamespace,
							newProcess.getOProcess().getName()), newProcess
							.getProcessDAO().getProcessId(),
					newInstance.getInstanceId(), newProcess._pconf.getVersion());
	...
	public void invokeInstance(MyRoleMessageExchangeImpl mex,
		...
		ProcessInstanceDAO instanceDao = routing.messageRoute
				.getTargetInstance();
		BpelProcess process2 = _process._engine._activeProcesses
				.get(instanceDao.getProcess().getProcessId());

		// Reload process instance for DAO.
		BpelRuntimeContextImpl instance = process2.createRuntimeContext(
		...
		CorrelationMatchEvent evt = new CorrelationMatchEvent(new QName(
				process2.getOProcess().targetNamespace, process2.getOProcess()
						.getName()), process2.getProcessDAO().getProcessId(),
	...
	public void noRoutingMatch(MyRoleMessageExchangeImpl mex,
			List<RoutingInfo> routings) {
		if (!mex.isAsynchronous()) {
			mex.setFailure(MessageExchange.FailureType.NOMATCH,
					"No process instance matching correlation keys.", null);
		} else {
			// enqueue message with the last message route, as per the comments
			// in caller (@see BpelProcess.invokeProcess())
			RoutingInfo routing = (routings != null && routings.size() > 0) ? routings
					.get(routings.size() - 1) : null;
			if (routing != null) {
				if (__log.isDebugEnabled()) {
					__log.debug("INPUTMSG: "
							+ routing.correlator.getCorrelatorId()
							+ ": SAVING to DB (no match) ");
				}

				// send event
				CorrelationNoMatchEvent evt = new CorrelationNoMatchEvent(mex
						.getPortType().getQName(),
						mex.getOperation().getName(),
						mex.getMessageExchangeId(), routing.wholeKeySet);

				evt.setProcessId(_process.getProcessDAO().getProcessId());
				evt.setProcessName(new QName(
						_process.getOProcess().targetNamespace, _process
								.getOProcess().getName()));
	...
	private CorrelationKeySet computeCorrelationKeys(
		...
		Set<OScope.CorrelationSet> csets = _plinkDef
				.getNonInitiatingCorrelationSetsForOperation(operation);
		for (OScope.CorrelationSet cset : csets) {
			CorrelationKey key = computeCorrelationKey(cset,
					_process.getOProcess().messageTypes.get(msgDescription
							.getQName()), msg);
			keySet.add(key);
		}
		...
		for (Iterator j = cset.properties.iterator(); j.hasNext(); ++jIdx) {
			...
			if (alias == null) {
				...
				throw new IllegalArgumentException(
						"No alias matching property '" + property.name
			...
			try {
				...
				String emsg = __msgs.msgPropertyAliasDerefFailedOnMessage(
