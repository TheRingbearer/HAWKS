...
public class JdbcExternalVariableModule implements ExternalVariableModule {
	...
	private static final Log __log = LogFactory
			.getLog(JdbcExternalVariableModule.class);

	public static final String JDBC_NS = "http://ode.apache.org/externalVariables/jdbc";

	/**
	 * Unique QName for the engine, this should be the element used for the
	...
	public void configure(QName pid, String extVarId, Element config)
			throws ExternalVariableModuleException {
		EVarId evarId = new EVarId(pid, extVarId);
		DataSource ds = null;

		Element jndiDs = DOMUtils.findChildByName(config, new QName(JDBC_NS,
				"datasource-jndi"));
		Element jndiRef = DOMUtils.findChildByName(config, new QName(JDBC_NS,
				"datasource-ref"));
		Element initMode = DOMUtils.findChildByName(config, new QName(JDBC_NS,
				"init-mode"));
		if (jndiRef != null) {
			...
			if (ds == null)
				throw new ExternalVariableModuleException(
						"Data source reference \""
								+ refname
								+ "\" not found for external variable "
								+ evarId
			...
			try {
				...
				throw new ExternalVariableModuleException(
						"Unable to access JNDI context for external variable "
								+ evarId, ex);
			}
			...
			try {
				...
				throw new ExternalVariableModuleException(
						"Lookup of data source for " + evarId + "  failed.", ex);
			} finally {
				try {
					ctx.close();
			...
			if (dsCandidate == null)
				throw new ExternalVariableModuleException("Data source \""
						+ name + "\" not found in JNDI!");

			if (!(dsCandidate instanceof DataSource))
				throw new ExternalVariableModuleException("JNDI object \""
						+ name + "\" does not implement javax.sql.DataSource");
		...
		if (ds == null) {
			throw new ExternalVariableModuleException(
					"No valid data source configuration for JDBC external varible "
							+ evarId);
		}
		...
		try {
			...
			throw new ExternalVariableModuleException(
					"Unable to open database connection for external variable "
							+ evarId, ex);
		}
		...
		try {
			...
			if (initMode != null)
				try {
					dbev._initType = InitType.valueOf(initMode.getTextContent()
							.trim());
				} catch (Exception ex) {
					throw new ExternalVariableModuleException(
							"Invalid <init-mode> value: "
									+ initMode.getTextContent().trim());
				}

			Element tableName = DOMUtils.findChildByName(config, new QName(
					JDBC_NS, "table"));
			if (tableName == null
					|| tableName.getTextContent().trim().equals(""))
				throw new ExternalVariableModuleException(
						"Must specify <table> for external variable " + evarId);
			...
			if (tables.next()) {
				...
				throw new ExternalVariableModuleException("Table \"" + table
						+ "\" not found in database.");

			tables.close();

			List<Element> columns = DOMUtils.findChildrenByName(config,
			...
			for (Element col : columns) {
				...
				try {
					...
					throw new ExternalVariableModuleException(
							"External variable " + evarId + " column \"" + name
									+ "\" generator type \"" + gentype
									+ "\" is unknown.");

				}

				if (gtype == GenType.expression
				...
				Column c = dbev.new Column(name, colname,
						key.equalsIgnoreCase("yes"), gtype, expression);
				ResultSet cmd = metaData.getColumns(null, dbev.schema,
						dbev.table, colname);
				try {
					if (cmd.next()) {
						...
						throw new ExternalVariableModuleException(
								"External variable " + evarId + " referenced "
	...
	public Value writeValue(QName varType, Value newval)
			throws ExternalVariableModuleException {
		...
		RowVal val = evar
				.parseXmlRow(evar.new RowVal(), (Element) newval.value);
		if (__log.isDebugEnabled())
			__log.debug("JdbcExternalVariable.writeValue() RowKey: " + key
					+ " RowVal: " + val);

		if (!key.missingValues() && evar._initType == InitType.delete_insert) {
			// do delete...
			throw new ExternalVariableModuleException(
					"Delete not implemented. "); // todo
		}

		// should we try an update first? to do this we need to have all the
		// required keys
		// and there should be some keys
		boolean tryupdatefirst = (evar._initType == InitType.update || evar._initType == InitType.update_insert)
				&& !evar._keycolumns.isEmpty()
				&& !key.missingDatabaseGeneratedValues();

		boolean insert = evar._initType != InitType.update;

		if (__log.isDebugEnabled())
			__log.debug("tryUpdateFirst: " + tryupdatefirst + " insert: "
					+ insert + " initType: " + evar._initType
	...
	public Value readValue(QName varType, Locator locator)
			throws ExternalVariableModuleException {
	...
	/**
	 * Manually register a data source. Handy if you don't want to use JNDI to
	 * look these up.
	 * 
	 * @param dsName
	 * @param ds
	...
	int execUpdate(DbExternalVariable dbev, RowKey key, RowVal values)
			throws SQLException {
				...
				if (rs.next()) {
					for (Column c : dbev._columns) {
						...
						if (c.isDate())
							...
							val = rs.getObject(i);
						if (__log.isDebugEnabled())
							__log.debug("Result column index " + c.idx + ": "
									+ val);
						ret.set(c.idx, val);
					}
				} else
	...
	RowKey execInsert(DbExternalVariable dbev, Locator locator, RowKey keys,
			RowVal values) throws SQLException {
		Connection conn = dbev.dataSource.getConnection();
		PreparedStatement stmt = null;
		try {
			if (__log.isDebugEnabled()) {
				...
				__log.debug("missingDatabaseGeneratedValues: "
						+ keys.missingDatabaseGeneratedValues());
				__log.debug("_autoColNames: "
						+ ObjectPrinter.stringifyNvList(dbev._autoColNames));
			}

			stmt = keys.missingDatabaseGeneratedValues() ? conn
					.prepareStatement(dbev.insert, dbev._autoColNames) : conn
			...
			if (keys.missingDatabaseGeneratedValues()) {
				// With JDBC 3, we can get the values of the key columns (if the
				// db supports it)
				ResultSet keyRS = stmt.getGeneratedKeys();
				try {
					if (keyRS == null)
						throw new SQLException(
								"Database did not return generated keys");
					keyRS.next();
					for (Column ck : keys._columns) {
						Object value = keyRS.getObject(ck.idx + 1);
						if (__log.isDebugEnabled())
							__log.debug("Generated key " + ck.name + ": "
									+ value);
