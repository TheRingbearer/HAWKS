...
public class CronScheduler {
	...
	public void shutdown() {
		...
		for (Collection<TerminationListener> listeners : _terminationListenersByPid
				.values()) {
	...
	public void cancelProcessCronJobs(QName pid, boolean undeployed) {
		...
		synchronized (_terminationListenersByPid) {
			Collection<TerminationListener> listeners = _terminationListenersByPid
					.get(pid);
			if (listeners != null) {
	...
	public void scheduleProcessCronJobs(QName pid, ProcessConf pconf) {
		...
		synchronized (pid) {
			...
			for (final CronJob job : pconf.getCronJobs()) {
				if (__log.isDebugEnabled())
					__log.debug("Scheduling PROCESS CRON job: "
							+ job.getCronExpression() + " for: " + pid);
					...
					public void run() {
						...
						for (JobDetails details : job.getRunnableDetailList()) {
							try {
								...
								__log.error(
										"Error during runtime data cleanup from a PROCESS CRON: "
												+ details
												+ "; check your cron settings in deploy.xml.",
										re);
								// don't sweat.. the rest of the system and
								// other cron jobs still should work
							}
						}
					}
				};
				newListeners.add(schedule(job.getCronExpression(), runnable,
						null, null));
			}
		}

		// make sure the pid does not get into the terminationListener map if no
		// cron is setup
		if (!newListeners.isEmpty()) {
			synchronized (_terminationListenersByPid) {
				Collection<TerminationListener> oldListeners = _terminationListenersByPid
						.get(pid);
	...
	public void refreshSystemCronJobs(SystemSchedulesConfig systemSchedulesConf) {
		...
		synchronized (_systemTerminationListeners) {
			...
			try {
				// if error thrown on reading the schedules.xml, do not cancel
				// existing cron jobs
				List<CronJob> systemCronJobs = systemSchedulesConf
						.getSystemCronJobs();
				...
				for (final CronJob job : systemCronJobs) {
					...
					Runnable runnable = new Runnable() {
						public void run() {
							for (JobDetails details : job
									.getRunnableDetailList()) {
								try {
									// for now, we have only runtime data
									// cleanup cron job defined
									// for each clean up for the scheduled time
									RuntimeDataCleanupRunnable cleanup = new RuntimeDataCleanupRunnable();
									synchronized (_terminationListenersByPid) {
										if (!_terminationListenersByPid
												.isEmpty()) {
											details.getDetailsExt().put(
													"pidsToExclude",
													_terminationListenersByPid
															.keySet());
										}
									}
									cleanup.restoreFromDetails(details);
									...
									__log.error(
											"Error running a runtime data cleanup from a SYSTEM CRON job: "
													+ details
													+ "; check your system cron setup.",
											e);
								}
							}
						}
					};
					_systemTerminationListeners.add(schedule(
							job.getCronExpression(), runnable, null, null));
				}
			} catch (Exception e) {
				__log.error("Error during refreshing SYSTEM CRON schedules: ",
						e);
			}
		}
	}
	...
	public TerminationListener schedule(final CronExpression cronExpression,
		...
		final Date nextScheduleTime = cronExpression
				.getNextValidTimeAfter(new Date(System.currentTimeMillis()
						+ MIN_INTERVAL));
		final CronScheduledJob job = new CronScheduledJob(nextScheduleTime,
				runnable, runnableDetails, cronExpression, terminationListener);
		if (__log.isDebugEnabled())
			__log.debug("CRON will run in "
		...
		try {
			_schedulerTimer.schedule(new TimerTask() {
				@Override
				public void run() {
					__log.debug("Cron scheduling timer kicked in: "
							+ cronExpression);
					// run only if the current node is the coordinator,
					// with the SimpleScheduler, the node is always the
					// coordinator
					if (!(_contexts.scheduler instanceof ClusterAware)
							|| ((ClusterAware) _contexts.scheduler)
									.amICoordinator()) {
						// do not hold the timer thread too long, submit the
						// work to an executorService
	...
	private class CronScheduledJob implements Callable<TerminationListener> {
		...
		public CronScheduledJob(Date nextScheduleTime, Runnable runnable,
				JobDetails runnableDetails, CronExpression cronExpression,
				TerminationListener terminationListener) {
		...
		public TerminationListener call() throws Exception {
			try {
				...
				if (_shuttingDown) {
					__log.info("A cron job threw an Exception during ODE shutdown: "
							+ e.getMessage() + ", you can ignore the error.");
				} else if (e instanceof RuntimeException) {
					throw e;
				} else {
					throw new RuntimeException(
							"Exception during running cron scheduled job: "
									+ runnable, e);
				}
			} finally {
				if (!_shuttingDown && !terminated) {
					schedule(cronExpression, runnable, runnableDetails,
							terminationListener);
				}
			}
