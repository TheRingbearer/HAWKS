...
public class JaxpVariableResolver implements XPathVariableResolver {
	...
	private static final Log __log = LogFactory
			.getLog(JaxpVariableResolver.class);
	...
	public Value convertSimpleTypeToSaxon(QName type, String value) {
		...
		if (type2 == null || !type2.isAtomicType()) {
			...
			try {
				AtomicValue value2 = StringValue.convertStringToAtomicType(
						value, (AtomicType) type2, null).asAtomic();
				if (__log.isDebugEnabled()) {
	...
	public Object getSimpleContent(Node simpleNode, QName type) {
		...
		if (__log.isDebugEnabled()) {
			__log.debug("getSimpleContent for "
					+ DOMUtils.domToString(simpleNode) + " " + type
					+ " returned " + o);
		}
		return o;
	}
