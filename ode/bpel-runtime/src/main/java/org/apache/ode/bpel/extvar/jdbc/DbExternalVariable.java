...
class DbExternalVariable {
	private static final Log __log = LogFactory
			.getLog(DbExternalVariable.class);
	 ...
	 */
	RowKey keyFromLocator(Locator locator)
	...
	<T extends RowSubset> Element renderXmlRow(Locator locator, QName varType,
		...
		Element el = doc.createElementNS(varType.getNamespaceURI(),
				varType.getLocalPart());
		doc.appendChild(el);
		if (value != null) {
			...
			for (Column c : _columns) {
				Object data = c.getValue(c.name, keys, new RowVal(),
						locator.iid);
				addElement(el, varType, c, data);
			}
		}
	...
	class Column {
		...
		public boolean isDatabaseGenerated() {
			return isGenerated()
					&& (genType.equals(GenType.sequence) || genType
							.equals(GenType.expression));
		}

		public String toString() {
			return "Column {idx=" + idx + ",name=" + name + ",colname="
					+ colname + ",key=" + key + ",genType=" + genType + ")";
		}
	}
