...
/**
 * A helper utility that modifies XPath Expression in-place. This is meant to be
 * reusable across the XPath and XQuery runtimes.
 */
public class XPath20ExpressionModifier {
	...
	/**
	 * Insert nodes into the specified XPath expression wherever required To be
	 * precise, an node is added to its parent if: a) the node is an element...
	 * b) that corresponds to an step... c) that has a child axis... d) whose
	 * parent had no children with its name... e) and all preceding steps are
	 * element name tests.
	 * 
	 * @param xpathExpr
	 * @param namePool
	...
	public void insertMissingData(XPathExpression xpathExpr, Node contextNode)
		...
		Expression expression = ((XPathExpressionImpl) xpathExpr)
				.getInternalExpression();
		Iterator<Expression> subExpressions = (Iterator<Expression>) expression
		...
		while (step != null) {
			if (step instanceof AxisExpression) {
				...
				if (Axis.CHILD == axisExpr.getAxis()) {
					if (NodeKindTest.ELEMENT.getNodeKindMask() != nameTest
							.getNodeKindMask()) {
						break;
					}

					NodeList children = ((Element) contextNode)
							.getElementsByTagNameNS(
									childName.getNamespaceURI(),
									childName.getLocalPart());
					if ((children == null) || (children.getLength() == 0)) {
						Node child = document.createElementNS(
								childName.getNamespaceURI(),
					...
					if (NodeKindTest.ATTRIBUTE.getNodeKindMask() != nameTest
							.getNodeKindMask()) {
						break;
					}

					Attr attribute = ((Element) contextNode)
							.getAttributeNodeNS(childName.getNamespaceURI(),
									childName.getLocalPart());
					if (attribute == null) {
	...
	/**
	 * Create the QName by running the given finger print against the given
	 * context
	 * 
	 * @param fingerprint
	 * @param namePool
