...
/**
 * Class adapting the {@link javax.xml.namespace.NamespaceContext} interface to
 * the JAXEN {@link NamespaceContext} interface.
 * 
 * @pattern Adapter
 */
class JaxenNamespaceContextAdapter implements NamespaceContext {
	private static final Log __log = LogFactory
			.getLog(JaxenNamespaceContextAdapter.class);
	...
	public String translateNamespacePrefixToUri(String prefix) {
		...
		if (__log.isDebugEnabled()) {
			__log.debug("translateNamespacePrefixToUri(" + prefix + ")="
					+ namespaceURI);
		}
		return namespaceURI;
	}
