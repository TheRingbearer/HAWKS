...
/**
 * This class intercepts invocations on BpelRuntimeContextImpl and substitutes
 * them as necessary during replaying. For exaple when INVOKE activity calls
 * invoke on BpelRuntimeContextImpl then ReplayerBpelRuntimeContextImpl
 * intercepts this call and provides specific answer.
 * 
 * @author Rafal Rusin
 * 
 */
public class ReplayerBpelRuntimeContextImpl extends BpelRuntimeContextImpl {
	private static final Log __log = LogFactory
			.getLog(ReplayerBpelRuntimeContextImpl.class);
	...
	public void checkInvokeExternalPermission() {
		throw new IllegalStateException(
				"Invoking external services is disabled during replaying");
	}
	...
	public String invoke(int aid, PartnerLinkInstance partnerLink,
		...
		__log.debug("invoke");
		AnswerResult answerResult = replayerContext.answers
				.fetchAnswer(
						partnerLink.partnerLink.partnerRolePortType.getQName(),
						operation.getName(), outgoingMessage,
						getCurrentEventDateTime());

		if (answerResult.isLive) {
			return super.invoke(aid, partnerLink, operation, outgoingMessage,
					channel);
		} else {
			...
			mexDao.setCreateTime(new Date(
					getCurrentEventDateTime().getTime() + 1));
			...
			mexDao.setInstance(_dao);
			{
				...
				// } catch (Exception e) {
				// throw new FaultException(new QName("replayer", "replayer"),
				// e);
				// }
			...
			if (mexDao.getPattern().equals(
					MessageExchangePattern.REQUEST_RESPONSE.toString())) {
				if (answer.isSetFault()) {
					MessageDAO response = mexDao.createMessage(new QName(
							"replayer", "replayer"));
					try {
					...
					mexDao.setFaultExplanation(answer.getFault()
							.getExplanation());
					...
					mexDao.setFaultExplanation(answer.getFailure()
							.getExplanation());
					mexDao.setStatus(Status.FAILURE.toString());
				} else {
					// We don't have output for in-out operation - resulting
					// with
					// replayer error to the top
					throw new IllegalStateException(
							"I don't have response for invoke " + answer);
				}
	...
	@Override
	public void registerTimer(final TimerResponseChannel timerChannel,
			final Date timeToFire, OBase oElement) {
		...
		if (timeToFire.before(replayerContext.replayStartDate)) {
			...
			super.registerTimer(timerChannel, timeToFire, oElement);
		}
	}
	...
	public void registerActivityForRecovery(ActivityRecoveryChannel channel,
		...
		super.registerActivityForRecovery(channel, activityId, reason,
	...
	public void reply(PartnerLinkInstance plinkInstnace, String opName,
		...
		if (mexRef == null) {
			throw new FaultException(
					_bpelProcess.getOProcess().constants.qnMissingRequest);
		}
		...
		if (pipedId != null) {
			...
			super.reply2(plinkInstnace, opName, mexId, msg, fault, false,
					mexRef);
		} else {
	...
	public static MyRoleMessageExchangeImpl createMyRoleMex(Exchange e,
			BpelEngineImpl engine) throws Exception {
		MyRoleMessageExchangeImpl mex = (MyRoleMessageExchangeImpl) engine
				.createMessageExchange(new GUID().toString(), e.getService(),
						e.getOperation());
	...
	public static void assign(MessageDAO m, XmlObject o) throws Exception {
		NodeList nodes = DOMUtils
				.parse(o.newInputStream(new XmlOptions().setSaveOuter()))
	...
	public void handleIncomingRequest(final MyRoleMessageExchangeImpl mex,
			final Date currentEventDateTime) {
		...
		_bpelProcess.invokeProcess(mex, new BpelProcess.InvokeHandler() {
			public boolean invoke(PartnerLinkMyRoleImpl target,
				...
				if (routing.messageRoute == null && createInstance) {
					...
					throw new IllegalStateException(
							"Mock type M mex caused creation of new instance "
									+ mex);
				} else if (routing.messageRoute != null) {
					if (!routing.messageRoute.getTargetInstance()
							.getInstanceId().equals(_dao.getInstanceId())) {
						throw new IllegalStateException(
								"Routed target instance is not equal to replayed instance");
					}
					...
					routing.correlator.removeRoutes(
							routing.messageRoute.getGroupId(), _dao);
