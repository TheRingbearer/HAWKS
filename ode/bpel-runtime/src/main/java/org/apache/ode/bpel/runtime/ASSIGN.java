...
//krawczk: TODO -update TerminationChannel in every activity
	...
	public void run() {
		...
		LinkStatusChannelListener receiver = new LinkStatusChannelListener(
			...
			public void linkStatus(boolean value) {
				...
				{
					ASSIGN.this.Activity_Completed();
				}
			}
	...
	public void execute() {
		...
		if (faultData != null) {
			...
			ActivityFaulted evt2 = new ActivityFaulted(_self.o.name, _self.o.getId(),
					_self.o.getXpath(), _self.aId, sFrame.oscope.getXpath(),
					sFrame.scopeInstanceId, process_name, process_ID,
					_self.o.getArt(), false, signal, faultData);
			getBpelRuntimeContext().getBpelProcess().getEngine()
					.fireEvent(evt2);
		} else {
			// Waiting
	...
	private void copy(OAssign.Copy ocopy) throws FaultException,
		...
		if ((ocopy.to instanceof VariableRef && ((VariableRef) ocopy.to)
			...
			if ((ocopy.to instanceof VariableRef && ((VariableRef) ocopy.to)
				...
				// @hahnml: Fire the new VariableModificationAtAssignEvent with
				// the index number of the copy statement
				se = new VariableModificationAtAssignEvent(
						lval.declaration.name, _self.o.getXpath(), _self.aId,
						sFrame.oscope.getXpath(), lval.declaration.getXpath(),
						sFrame.scopeInstanceId, 
						getOAsssign().operations.indexOf(ocopy) + 1);
				((VariableModificationAtAssignEvent) se).setNewValue(lvalue);
				_scopeFrame.fillEventInfo(se);
				getBpelRuntimeContext().sendEvent(se);
			} else {
				// This really should have been caught by the compiler.
				__log.fatal("Message/Non-Message Assignment, should be caught by compiler:"
						+ ocopy);
				throw new FaultException(
			...
			if (ocopy.to instanceof OAssign.PartnerLinkRef) {
				...
				se = new PartnerLinkModificationEvent(
						((OAssign.PartnerLinkRef) ocopy.to).partnerLink
								.getName(),
						((OAssign.PartnerLinkRef) ocopy.to).partnerLink
								.getXpath(), rvalue, _self.o.getXpath(),
						sFrame.oscope.getXpath(), sFrame.scopeInstanceId);
				_scopeFrame.fillEventInfo(se);
				getBpelRuntimeContext().sendEvent(se);
			} else {
	...
	private Element replaceElement(Element lval, Element ptr, Element src,
		...
		for (int i = 0; i < attrs.getLength(); ++i) {
			...
			if (!attr.getName().startsWith("xmlns")) {
				replacement.setAttributeNodeNS((Attr) doc.importNode(
						attrs.item(i), true));
	...
	private void invokeExtensionAssignOperation(
		...
		try {
			ExtensionOperation ea = getBpelRuntimeContext()
					.createExtensionActivityImplementation(eao.extensionName);
			if (ea == null) {
				for (OProcess.OExtension oe : eao.getOwner().mustUnderstandExtensions) {
					if (eao.extensionName.getNamespaceURI().equals(
							oe.namespaceURI)) {
