...
/**
 * A very simple, in-memory implementation of the {@link CorrelatorDAO}
 * interface.
 */
class CorrelatorDaoImpl extends DaoBaseImpl implements CorrelatorDAO {
	...
	public MessageExchangeDAO dequeueMessage(CorrelationKeySet instanceKeySet) {
		if (__log.isDebugEnabled()) {
			__log.debug("dequeueEarliest: MATCHING correlationKey="
					+ instanceKeySet);
		}
	...
	public void addRoute(String routeId, ProcessInstanceDAO target, int idx,
			CorrelationKeySet keySet, String routePolicy) {
		if (__log.isDebugEnabled()) {
			__log.debug("addRoute: target=" + target + " correlationKeySet="
					+ keySet);
		}

		final MessageRouteDaoImpl mr = new MessageRouteDaoImpl(
				(ProcessInstanceDaoImpl) target, routeId, keySet, idx,
				routePolicy);
