...
/**
 * Migrates the database from using single correlations to multiple correlations
 * support.
 */
public class CorrelationKeySetMigration implements Migration {
	private static final Log __log = LogFactory
			.getLog(CorrelationKeySetMigration.class);
	...
	public boolean migrate(Set<BpelProcess> registeredProcesses,
		...
		for (BpelProcess process : registeredProcesses) {
			ProcessDAO processDao = connection.getProcess(process.getConf()
					.getProcessId());
			Collection<ProcessInstanceDAO> pis = processDao
					.getActiveInstances();

			// Migrate the correlation key stored in the jacob state of the
			// instance
			for (ProcessInstanceDAO instance : pis) {
				__log.debug("Migrating correlation key in jacob for instance "
						+ instance.getInstanceId());
				OProcess oproc = findOProcess(registeredProcesses, instance
	...
	private boolean updateV1Key(ProcessInstanceDAO instance, OProcess oproc) {
		...
		try {
			OutstandingRequestManager orm = (OutstandingRequestManager) soup
					.getGlobalData();
			for (OutstandingRequestManager.Entry entry : orm._byChannel
					.values()) {
				...
				for (Object selector : entry.selectors) {
					...
					if (selCKey != null) {
						...
						__log.debug("   Changing V1 key "
								+ old.toCanonicalString());

						CorrelationKeySet newKeySet = new CorrelationKeySet();
						newKeySet.add(new CorrelationKey("" + old.getCSetId(),
								old.getValues()));
						Selector newSelector = new Selector(sel.idx,
								sel.plinkInstance, sel.opName, sel.oneWay,
								sel.messageExchangeId, newKeySet, "one");
						newSelector.correlationKey = new CorrelationKey(""
								+ old.getCSetId(), old.getValues());
	...
	private boolean updateV2Key(ProcessInstanceDAO instance, OProcess oproc) {
		...
		for (OutstandingRequestManager.Entry entry : orm._byChannel.values()) {
			...
			for (Object selector : entry.selectors) {
				...
				if (selCKey != null) {
					__log.debug("   Changing V2 key "
							+ selCKey.toCanonicalString());

					CorrelationKeySet newKeySet = new CorrelationKeySet();
					newKeySet.add(new CorrelationKey(""
							+ selCKey.getCorrelationSetName(), selCKey
							.getValues()));
					Selector newSelector = new Selector(sel.idx,
							sel.plinkInstance, sel.opName, sel.oneWay,
							sel.messageExchangeId, newKeySet, "one");
					newSelector.correlationKey = new CorrelationKey(""
							+ selCKey.getCorrelationSetName(),
							selCKey.getValues());
					newSelectors[index++] = newSelector;
				}
			}
	...
	private ExecutionQueueImpl readOldState(ProcessInstanceDAO instance,
			OProcess oprocess, ClassLoader cl, boolean changeKey) {
		...
		try {
			...
			if (changeKey) {
				osc = ObjectStreamClass
						.lookup(Class
								.forName(
										"org.apache.ode.bpel.engine.migration.OldCorrelationKey",
										true, cl));
				ExecutionQueueImpl._classDescriptors.put(
						"org.apache.ode.bpel.common.CorrelationKey", osc);
			}
			osc = ObjectStreamClass.lookup(Class.forName(
					"org.apache.ode.bpel.engine.migration.OldSelector", true,
					cl));
			ExecutionQueueImpl._classDescriptors.put(
					"org.apache.ode.bpel.runtime.Selector", osc);
			osc = ObjectStreamClass.lookup(Class.forName(
					"[Lorg.apache.ode.bpel.engine.migration.OldSelector;",
					true, getClass().getClassLoader()));
			ExecutionQueueImpl._classDescriptors.put(
					"[Lorg.apache.ode.bpel.runtime.Selector;", osc);
	...
	private void writeOldState(ProcessInstanceDAO instance,
			ExecutionQueueImpl soup) {
		try {
	...
	private OProcess findOProcess(Set<BpelProcess> registeredProcesses,
			QName name) {
		for (BpelProcess process : registeredProcesses) {
