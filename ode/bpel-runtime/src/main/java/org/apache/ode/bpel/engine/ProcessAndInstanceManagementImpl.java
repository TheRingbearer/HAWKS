...
import org.apache.ode.bpel.pmapi.PartnerLinkInfoDocument;
...
import org.apache.ode.bpel.pmapi.SnapshotInfoDocument;
import org.apache.ode.bpel.pmapi.SnapshotInfoListDocument;
import org.apache.ode.bpel.pmapi.SnapshotNotFoundException;
import org.apache.ode.bpel.pmapi.SnapshotPartnerLinksDocument;
import org.apache.ode.bpel.pmapi.SnapshotVariablesDocument;
...
import org.apache.ode.bpel.pmapi.TPartnerLinkInfo;
...
import org.apache.ode.bpel.pmapi.TSnapshotInfo;
import org.apache.ode.bpel.pmapi.TSnapshotInfoList;
import org.apache.ode.bpel.pmapi.TSnapshotPartnerLinks;
import org.apache.ode.bpel.pmapi.TSnapshotVariables;
import org.apache.ode.bpel.pmapi.TSnapshotVersion;
...
public class ProcessAndInstanceManagementImpl implements InstanceManagement,
	...
	protected static final Messages __msgs = MessageBundle
			.getMessages(Messages.class);

	protected static Log __log = LogFactory
			.getLog(BpelManagementFacadeImpl.class);
	...
	public ProcessInfoListDocument listProcessesCustom(String filter,
			String orderKeys, final ProcessInfoCustomizer custom) {
		ProcessInfoListDocument ret = ProcessInfoListDocument.Factory
				.newInstance();
	...
	public ProcessInfoDocument getProcessInfoCustom(final QName pid,
		...
		try {
			...
			throw new ManagementException(
					"Exception in getProcessInfoCustom(): " + ex.toString());
		}
	}
	...
	public ProcessInfoDocument setRetired(QName pid, boolean retired)
	...
	// @hahnml: Added the method to the WSDL
	public boolean setPackageRetired(String packageName, boolean retired)
			throws ManagementException {
	...
	public ProcessInfoDocument setProcessPropertyNode(final QName pid,
			final QName propertyName, final Node value)
			throws ManagementException {
		...
		try {
			_db.exec(new BpelDatabase.Callable<Object>() {
				public Object run(BpelDAOConnection conn) throws Exception {
					...
					fillProcessInfo(pi, proc, new ProcessInfoCustomizer(
							ProcessInfoCustomizer.Item.PROPERTIES));
			...
			throw new ProcessingException(
					"Exception while setting process property: " + e.toString());
		}
		return ret;
	}

	public ProcessInfoDocument setProcessProperty(final QName pid,
			final QName propertyName, final String value)
			throws ManagementException {
		ProcessInfoDocument ret = ProcessInfoDocument.Factory.newInstance();
		final TProcessInfo pi = ret.addNewProcessInfo();
		try {
	...
	public InstanceInfoListDocument listInstances(String filter, String order,
		...
		final InstanceFilter instanceFilter = new InstanceFilter(filter, order,
				limit);
		try {
			_db.exec(new BpelDatabase.Callable<Object>() {
				public Object run(BpelDAOConnection conn) {
					Collection<ProcessInstanceDAO> instances = conn
							.instanceQuery(instanceFilter);
					for (ProcessInstanceDAO instance : instances) {
						fillInstanceInfo(infolist.addNewInstanceInfo(),
								instance);
					}
					return null;
				}
	...
	public InstanceInfoListDocument listInstancesSummary(String filter,
		...
		try {
			_db.exec(new BpelDatabase.Callable<Object>() {
				public Object run(BpelDAOConnection conn) {
					Collection<ProcessInstanceDAO> instances = conn
							.instanceQuery(instanceFilter);
					Map<Long, Collection<CorrelationSetDAO>> icsets = conn
							.getCorrelationSets(instances);
					for (ProcessInstanceDAO instance : instances) {
						TInstanceInfo info = infolist.addNewInstanceInfo();
						fillInstanceSummary(info, instance);
						Collection<CorrelationSetDAO> csets = icsets
	...
	public InstanceInfoDocument getInstanceInfo(final Long iid)
		...
		try {
			...
			throw new ProcessingException(
					"Exception while retrieving instance info: " + e.toString());
		}
	}
	...
	public ScopeInfoDocument getScopeInfoWithActivity(final String siid,
			final boolean includeActivityInfo) {
		try {
			return _db.exec(new BpelDatabase.Callable<ScopeInfoDocument>() {
				public ScopeInfoDocument run(BpelDAOConnection conn)
						throws Exception {
			...
			throw new ProcessingException(
					"Exception while retrieving scope info: " + e.toString());
		}
	}

	public VariableInfoDocument getVariableInfo(final String scopeId,
			final String varName) throws ManagementException {
	...
	// INSTANCE ACTIONS
	//
	public InstanceInfoDocument fault(Long iid, QName faultname,
			Element faultData) {
		// TODO: Implement
	...
	// Added by Bo Ning
	// @hahnml: Extended with a version parameter
	public InstanceInfoDocument reexecute(final Long iid, String xpath,
			String snapshotXPath, final Long version)
			throws ManagementException {

		getDebugger(iid).reexecute(iid, xpath, snapshotXPath, version);

		return getInstanceInfo(iid);
	}

	// @hahnml
	public InstanceInfoDocument reexecuteExt(final Long iid, String xpath,
			String snapshotXPath, final Long version,
			TVariableRefList variables, TPartnerLinkRefList partnerLinks)
			throws ManagementException {

		getDebugger(iid).reexecuteExt(iid, xpath, snapshotXPath, version,
	...
	// @hahnml
	public InstanceInfoDocument iterateExt(final Long iid, String xpath,
			String snapshotXPath, final Long version,
			TVariableRefList variables, TPartnerLinkRefList partnerLinks)
			throws ManagementException {

		getDebugger(iid).iterateExt(iid, xpath, snapshotXPath, version,
				variables, partnerLinks);
	...
	// @hahnml
	public InstanceInfoDocument jumpToActivity(final Long iid, String xpath)
			throws ManagementException {

		getDebugger(iid).jumpToActivity(iid, xpath);
	...
	public InstanceInfoDocument recoverActivity(final Long iid, final Long aid,
			final String action) {
		try {
			_db.exec(new BpelDatabase.Callable<QName>() {
				public QName run(BpelDAOConnection conn) throws Exception {
					...
					if (instance == null)
						return null;
					for (ActivityRecoveryDAO recovery : instance
							.getActivityRecoveries()) {
						if (recovery.getActivityId() == aid) {
							BpelProcess process = _server._engine._activeProcesses
									.get(instance.getProcess().getProcessId());
							if (process != null) {
								process.recoverActivity(instance,
										recovery.getChannel(), aid, action,
										null);
								break;
							}
						}
	...
	public Collection<Long> delete(String filter) {
		...
		try {
			_db.exec(new BpelDatabase.Callable<Object>() {
				public Object run(BpelDAOConnection conn) {
					Collection<ProcessInstanceDAO> instances = conn
							.instanceQuery(instanceFilter);
					for (ProcessInstanceDAO instance : instances) {
						ProcessConf proc = _store
								.getProcessConfiguration(instance.getProcess()
										.getProcessId());
						if (proc == null)
			...
			throw new ProcessingException(
					"Exception during instance deletion: " + e.toString());
		}
		return ret;
	}
	...
	// EVENT RETRIEVAL
	//
	public List<String> getEventTimeline(String instanceFilter,
			String eventFilter) {
		final InstanceFilter ifilter = new InstanceFilter(instanceFilter, null,
				0);
	...
	public EventInfoListDocument listEvents(String instanceFilter,
			String eventFilter, int maxCount) {
		final InstanceFilter ifilter = new InstanceFilter(instanceFilter, null,
				0);
	...
	public ActivityExtInfoListDocument getExtensibilityElements(QName pid,
			Integer[] aids) {
		ActivityExtInfoListDocument aeild = ActivityExtInfoListDocument.Factory
				.newInstance();
		TActivitytExtInfoList taeil = aeild.addNewActivityExtInfoList();
		OProcess oprocess = _server._engine.getOProcess(pid);
		if (oprocess == null)
			throw new ProcessNotFoundException("The process \"" + pid
					+ "\" does not exist.");
		...
		for (int aid : aids) {
			...
			if (obase != null && obase.debugInfo != null
				...
				for (Map.Entry<QName, Object> entry : obase.debugInfo.extensibilityElements
						.entrySet()) {
					...
					if (extValue instanceof Element)
						taei.getDomNode().appendChild(
								taei.getDomNode().getOwnerDocument()
										.importNode((Element) extValue, true));
					else if (extValue instanceof String) {
						Element valueElmt = taei
								.getDomNode()
								.getOwnerDocument()
								.createElementNS(
										entry.getKey().getNamespaceURI(),
										entry.getKey().getLocalPart());
						valueElmt.appendChild(taei.getDomNode()
								.getOwnerDocument()
								.createTextNode((String) extValue));
	 ...
	 * @throws ManagementException
	 */
	protected final DebuggerSupport getDebugger(QName procid)
			throws ManagementException {
	...
	protected final DebuggerSupport getDebugger(final Long iid) {
		...
		try {
			processId = _db.exec(new BpelDatabase.Callable<QName>() {
				public QName run(BpelDAOConnection conn) throws Exception {
					ProcessInstanceDAO instance = conn.getInstance(iid);
					return instance == null ? null : instance.getProcess()
							.getProcessId();
				}
			});
		} catch (Exception e) {
			...
			throw new ProcessingException(
					"Exception during instance retrieval: " + e.toString());
		}
	 ...
	 * @throws ManagementException
	 */
	protected <T> T dbexec(BpelProcessDatabase.Callable<T> runnable)
			throws ManagementException {
		try {
			...
			throw new ManagementException(
					"Exception during database operation: " + e.toString());
		}
	}
	...
	protected <T> T dbexec(BpelDatabase.Callable<T> callable)
		...
		try {
			...
			throw new ManagementException("Exception during database operation"
					+ ex.toString());
		}
	}
	...
	private ProcessInfoDocument genProcessInfoDocument(BpelDAOConnection conn,
		...
		if (procid == null) {
			throw new InvalidRequestException(
					"Valid QName as process id expected.");
		}
		ProcessInfoDocument ret = ProcessInfoDocument.Factory.newInstance();
		...
		try {
			...
			throw new ProcessingException(
					"Exception while retrieving process information: "
							+ e.toString());
		}
	 ...
	 */
	private InstanceInfoDocument genInstanceInfoDocument(
			BpelDAOConnection conn, Long iid) {
		if (iid == null)
	...
	private ScopeInfoDocument genScopeInfoDocument(BpelDAOConnection conn,
		...
		if (siid == null)
			throw new InvalidRequestException(
					"Must specifiy scope instance id.");
	...
	private void fillProcessInfoSummary(BpelDAOConnection conn,
		...
		if (custom.includeInstanceSummary()) {
			...
			Map<InstanceSummaryKey, Long> m = conn.getProcessManagement()
					.countInstancesSummary(pids);
			Map<String, FailedSummaryValue> f = conn.getProcessManagement()
					.findFailedCountAndLastFailedDateForProcessIds(pids);
	...
	private void fillProcessInfo(TProcessInfo info, ProcessConf pconf,
		...
		if (custom.includeProcessProperties()) {
			for (Map.Entry<QName, Node> propEntry : pconf
					.getProcessProperties().entrySet()) {
				TProcessProperties.Property tprocProp = properties
						.addNewProperty();
				tprocProp.setName(new QName(propEntry.getKey()
						.getNamespaceURI(), propEntry.getKey().getLocalPart()));
				Node propNode = tprocProp.getDomNode();
				Document processInfoDoc = propNode.getOwnerDocument();
				Node node2append = processInfoDoc.importNode(
						propEntry.getValue(), true);
		...
		if (custom.includeEndpoints() && oprocess != null) {
			for (OPartnerLink oplink : oprocess.getAllPartnerLinks()) {
				if (oplink.hasPartnerRole() && oplink.initializePartnerRole) {
					...
					EndpointReference pepr = _server._engine._activeProcesses
							.get(pconf.getProcessId())
							.getInitialPartnerRoleEPR(oplink);

					if (pepr != null) {
						TEndpointReferences.EndpointRef epr = eprs
								.addNewEndpointRef();
						Document eprNodeDoc = epr.getDomNode()
								.getOwnerDocument();
						epr.getDomNode().appendChild(
								eprNodeDoc.importNode(pepr.toXML()
	...
	private void fillProperties(TInstanceInfo info,
			ProcessInstanceDAO instance, Map<QName, String> props) {
		TInstanceInfo.CorrelationProperties corrProperties = info
				.addNewCorrelationProperties();
		for (Map.Entry<QName, String> property : props.entrySet()) {
			TCorrelationProperty tproperty = corrProperties
	...
	private void fillInstanceSummary(TInstanceInfo info,
		...
		if (instance.getActivityFailureCount() > 0) {
			...
			failures.setDtFailure(toCalendar(instance
					.getActivityFailureDateTime()));
			failures.setCount(instance.getActivityFailureCount());
		}
	}

	private void fillInstanceInfo(TInstanceInfo info,
			ProcessInstanceDAO instance) {
		fillInstanceSummary(info, instance);
		...
		ProcessInstanceDAO.EventsFirstLastCountTuple flc = instance
		...
		if (!instance.getCorrelationSets().isEmpty()) {
			TInstanceInfo.CorrelationProperties corrProperties = info
					.addNewCorrelationProperties();
			for (CorrelationSetDAO correlationSetDAO : instance
					.getCorrelationSets()) {
				for (Map.Entry<QName, String> property : correlationSetDAO
	...
	private void fillScopeInfo(TScopeInfo scopeInfo, ScopeDAO scope,
		...
		if (!scope.getCorrelationSets().isEmpty()) {
			TScopeInfo.CorrelationSets correlationSets = scopeInfo
					.addNewCorrelationSets();
			for (CorrelationSetDAO correlationSetDAO : scope
					.getCorrelationSets()) {
				TScopeInfo.CorrelationSets.CorrelationSet correlationSet = correlationSets
						.addNewCorrelationSet();
				correlationSet.setCsetid(""
						+ correlationSetDAO.getCorrelationSetId());
				correlationSet.setName(correlationSetDAO.getName());
				for (Map.Entry<QName, String> property : correlationSetDAO
						.getProperties().entrySet()) {
					TCorrelationProperty tproperty = correlationSet
							.addNewCorrelationProperty();
					tproperty.setCsetid(""
							+ correlationSetDAO.getCorrelationSetId());
					tproperty.setPropertyName(property.getKey());
		...
		if (includeActivityInfo) {
			Collection<ActivityRecoveryDAO> recoveries = scope
					.getProcessInstance().getActivityRecoveries();
			...
			for (ActivityInfoDocument ai : b.getActivities()) {
				for (ActivityRecoveryDAO recovery : recoveries) {
					if (String.valueOf(recovery.getActivityId()).equals(
							ai.getActivityInfo().getAiid())) {
						TFailureInfo failure = ai.getActivityInfo()
								.addNewFailure();
		...
		if (plinks.size() > 0) {
			...
			for (PartnerLinkDAO plink : plinks) {
				if (plink.getPartnerRoleName() != null
					...
					TEndpointReferences.EndpointRef ref = refs
							.addNewEndpointRef();
					ref.setPartnerLink(plink.getPartnerLinkName());
					ref.setPartnerRole(plink.getPartnerRoleName());
					if (plink.getPartnerEPR() != null) {
						Document eprNodeDoc = ref.getDomNode()
								.getOwnerDocument();
						ref.getDomNode().appendChild(
								eprNodeDoc.importNode(plink.getPartnerEPR(),
										true));
					}
				}
			}
	...
	private void fillVariableRef(TVariableRef ref, XmlDataDAO i) {
		ref.setIid(i.getScopeDAO().getProcessInstance().getInstanceId()
				.toString());
	...
	private void fillEventInfo(TEventInfo info, BpelEvent event) {
		...
		if (event instanceof ProcessInstanceStartedEvent) {
			info.setRootScopeId(((ProcessInstanceStartedEvent) event)
					.getRootScopeId());
			info.setRootScopeDeclarationId(((ProcessInstanceStartedEvent) event)
					.getScopeDeclarationId());
		}
		...
		if (event instanceof VariableModificationEvent) {
			info.setNewValue(DOMUtils
					.domToString(((VariableModificationEvent) event)
							.getNewValue()));
		}
	}
	...
	public InstanceInfoListDocument queryInstances(final String query) {
		...
		try {
			...
			throw new ProcessingException(
					"Exception while querying instances: " + e.toString());
		}
		return ret;
	}
	...
	Collection<ProcessConf> processQuery(ProcessFilter filter) {
		...
		if (filter != null && filter.getNamespaceFilter() != null) {
			final Pattern pattern = Pattern.compile(filter.getNamespaceFilter()
					.replace("*", ".*"));
			CollectionsX.remove_if(pids, new MemberOfFunction<QName>() {
				@Override
				public boolean isMember(QName o) {
					String ns = o.getNamespaceURI() == null ? "" : o
							.getNamespaceURI();
		...
		if (filter != null) {
			...
			if (filter.getDeployedDateFilter() != null) {
				for (final String ddf : filter.getDeployedDateFilter()) {
					final Date dd;
					try {
						dd = ISO8601DateParser.parse(Filter
								.getDateWithoutOp(ddf));
					...
					CollectionsX.remove_if(confs,
							new MemberOfFunction<ProcessConf>() {
								@Override
								public boolean isMember(ProcessConf o) {
									...
									if (ddf.startsWith("<="))
										return o.getDeployDate().getTime() > dd
												.getTime();
			...
			if (filter.getOrders() != null) {
				...
				for (String key : filter.getOrders()) {
					...
					if ("name".equals(orderKey))
						...
						c = new Comparator<ProcessConf>() {
							public int compare(ProcessConf o1, ProcessConf o2) {
								String ns1 = o1.getProcessId()
										.getNamespaceURI() == null ? "" : o1
										.getProcessId().getNamespaceURI();
								String ns2 = o2.getProcessId()
										.getNamespaceURI() == null ? "" : o2
										.getProcessId().getNamespaceURI();
						...
						c = new Comparator<ProcessConf>() {
							public int compare(ProcessConf o1, ProcessConf o2) {
								return o1.getDeployDate().compareTo(
										o2.getDeployDate());
							}
	...
	public ReplayResponseDocument replay(final Replay request)
			throws ManagementException {
		final Throwable[] e = new Throwable[1];
		try {
			ReplayResponseDocument response = _db
					.exec(new BpelDatabase.Callable<ReplayResponseDocument>() {
						public ReplayResponseDocument run(BpelDAOConnection conn)
								throws Exception {
							try {
								ReplayResponse response = ReplayResponse.Factory
										.newInstance();
								Replayer replayer = new Replayer();
								List<Long> iids = replayer.replayInstances(
										request, _server.getEngine(), conn);
								for (Long iid : iids) {
									response.addRestoredIID(iid);
								}
								ReplayResponseDocument responseDoc = ReplayResponseDocument.Factory
										.newInstance();
	...
	public GetCommunicationResponseDocument getCommunication(
			final GetCommunication request) throws ManagementException {
		final Throwable[] e = new Throwable[1];
		try {
			GetCommunicationResponseDocument response = _db
					.exec(new BpelDatabase.Callable<GetCommunicationResponseDocument>() {
						public GetCommunicationResponseDocument run(
								BpelDAOConnection conn) throws Exception {
							try {
								Replayer replayer = new Replayer();

								GetCommunicationResponseDocument responseDoc = GetCommunicationResponseDocument.Factory
										.newInstance();
								responseDoc.setGetCommunicationResponse(replayer
										.getCommunication(request, conn));
								return responseDoc;
							} catch (Throwable e2) {
								e[0] = e2;
	...
	private static void failIfSQLException(Exception e) throws Exception {
		...
		for (int i = 0; i < fatal.length; i++) {
			if (e.getClass().getName().startsWith(fatal[i]))
				throw e;
		}
	}

	// @hahnml
	public PartnerLinkInfoDocument getPartnerLinkInfo(final String scopeId,
			final String partnerLinkName) throws ManagementException {
		PartnerLinkInfoDocument ret = PartnerLinkInfoDocument.Factory
				.newInstance();
		final TPartnerLinkInfo plInf = ret.addNewPartnerLinkInfo();
		final TPartnerLinkRef sref = plInf.addNewSelf();
		dbexec(new BpelDatabase.Callable<Object>() {
			public Object run(BpelDAOConnection session) throws Exception {
				ScopeDAO scope = session.getScope(new Long(scopeId));
				if (scope == null) {
				...
				sref.setIid(scope.getProcessInstance().getInstanceId()
						.toString());
				sref.setName(partnerLinkName);

				PartnerLinkDAO pl = null;
				Iterator<PartnerLinkDAO> plItr = scope.getPartnerLinks()
						.iterator();
				while (pl == null && plItr.hasNext()) {
					PartnerLinkDAO tmp = plItr.next();
					if (tmp.getPartnerLinkName().equals(partnerLinkName)) {
						pl = tmp;
					}
				}

				if (pl == null) {
					throw new InvalidRequestException("PartnerLinkNotFound:"
							+ partnerLinkName);
				}
				
				sref.setModelId(String.valueOf(pl.getPartnerLinkModelId()));

				// Fill the simple elements
				plInf.setMyRoleName(pl.getMyRoleName());
				plInf.setMyRoleServiceName(pl.getMyRoleServiceName());
				plInf.setMySessionId(pl.getMySessionId());
				plInf.setPartnerRoleName(pl.getPartnerRoleName());
				plInf.setPartnerSessionId(pl.getPartnerSessionId());

				// Fill the EPR elements
				Element myEPR = pl.getMyEPR();
				if (myEPR != null) {
					TPartnerLinkInfo.MyEPR epr = plInf.addNewMyEPR();
					epr.getDomNode().appendChild(
							epr.getDomNode().getOwnerDocument()
									.importNode(myEPR, true));
				}
				
				Element partnerEPR = pl.getPartnerEPR();
				if (partnerEPR != null) {
					TPartnerLinkInfo.PartnerEPR epr = plInf.addNewPartnerEPR();
					epr.getDomNode().appendChild(
							epr.getDomNode().getOwnerDocument()
									.importNode(partnerEPR, true));
				}
	...
	// @hahnml
	public SnapshotInfoListDocument getSnapshots(final Long iid,
			final String activityXPath) throws SnapshotNotFoundException {
		SnapshotInfoListDocument ret = SnapshotInfoListDocument.Factory
				.newInstance();
		final TSnapshotInfoList list = ret.addNewSnapshotInfoList();
		...
		try {
			_db.exec(new BpelDatabase.Callable<Object>() {
				public Object run(BpelDAOConnection conn) {
					...
					// Check if the given xpath corresponds to an available
					// snapshot
					if (!ReexecutionHandler.isValidSnapshotXPath(
							instance.getSnapshotDAO(), activityXPath)) {
						BpelProcess process = _server._engine._activeProcesses
								.get(instance.getProcess().getProcessId());
						// Get the nearest valid snapshot xpath's
						targetXPaths = ReexecutionHandler
								.calculateNearestSnapshotXPaths(
										instance.getSnapshotDAO(),
										activityXPath, process);
					} else {
						targetXPaths.add(activityXPath);
					}

					for (String targetXPath : targetXPaths) {
						TSnapshotInfo info = list.addNewSnapshotInfo();
						info.setActivityXPath(targetXPath);
						info.setIid(iid.toString());
						info.setProcessId(instance.getProcess().getProcessId());

						for (SnapshotDAO dao : instance
								.getSnapshots(targetXPath)) {
							TSnapshotVersion version = info
									.addNewSnapshotVersion();

							version.setCreated(toCalendar(dao.getCreationTime()));
							version.setSnapshotId(dao.getSnapshotID()
									.toString());
							version.setVersion(dao.getVersion().toString());
						}
					}
			...
			__log.error("Exception while collecting snapshots", e);
			throw new ProcessingException(
					"Exception while collecting snapshots: " + e.toString());
		}

		return ret;
	}

	public SnapshotInfoDocument getSnapshotVersions(final Long iid,
			final String activityXPath) throws SnapshotNotFoundException {
		SnapshotInfoDocument ret = SnapshotInfoDocument.Factory.newInstance();
		final TSnapshotInfo info = ret.addNewSnapshotInfo();
		...
		try {
			_db.exec(new BpelDatabase.Callable<Object>() {
				public Object run(BpelDAOConnection conn) {
					...
					String targetXPath = activityXPath;
					...
					if (!ReexecutionHandler.isValidSnapshotXPath(
						...
						BpelProcess process = _server._engine._activeProcesses
								.get(instance.getProcess().getProcessId());
						// Get the nearest valid snapshot xpath
						targetXPath = ReexecutionHandler
								.calculateNearestSnapshotXPath(
										instance.getSnapshotDAO(),
										activityXPath, process);
					}
					...
					for (SnapshotDAO dao : instance.getSnapshots(targetXPath)) {
						TSnapshotVersion version = info.addNewSnapshotVersion();

						version.setCreated(toCalendar(dao.getCreationTime()));
						version.setSnapshotId(dao.getSnapshotID().toString());
						version.setVersion(dao.getVersion().toString());
					}
	...
	public SnapshotVariablesDocument getSnapshotVariables(final Long iid,
			final Long snapshotId) throws SnapshotNotFoundException {
		SnapshotVariablesDocument ret = SnapshotVariablesDocument.Factory
				.newInstance();
		final TSnapshotVariables snapshotVars = ret.addNewSnapshotVariables();
		...
		try {
			_db.exec(new BpelDatabase.Callable<Object>() {
				public Object run(BpelDAOConnection conn) {
					...
					SnapshotDAO dao = instance.getSnapshot(snapshotId);

					if (dao != null) {
						for (SnapshotVariableDAO variable : dao.getVariables()) {
							TVariableInfo variableInfo = snapshotVars
									.addNewVariableInfo();

							TVariableRef varRef = variableInfo.addNewSelf();
							varRef.setIid(iid.toString());
							varRef.setSiid(variable.getScopeInstanceId()
									.toString());
							varRef.setName(variable.getName());

							Node nval = variable.get();
							if (nval != null) {
								TVariableInfo.Value val = variableInfo
										.addNewValue();
								val.getDomNode().appendChild(
										val.getDomNode().getOwnerDocument()
												.importNode(nval, true));
			...
			__log.error("Exception while collecting snapshot variables", e);
			throw new ProcessingException(
					"Exception while collecting snapshot variables: "
							+ e.toString());
		}
	...
	public SnapshotPartnerLinksDocument getSnapshotPartnerLinks(final Long iid,
			final Long snapshotId) throws SnapshotNotFoundException {
		SnapshotPartnerLinksDocument ret = SnapshotPartnerLinksDocument.Factory
				.newInstance();
		final TSnapshotPartnerLinks snapshotPLs = ret.addNewSnapshotPartnerLinks();
		...
		try {
			_db.exec(new BpelDatabase.Callable<Object>() {
				public Object run(BpelDAOConnection conn) {
					...
					if (dao != null) {
						for (SnapshotPartnerlinksDAO partnerLink : dao.getPartnerLinks()) {
							TPartnerLinkInfo plInfo = snapshotPLs
									.addNewPartnerLinkInfo();

							TPartnerLinkRef plRef = plInfo.addNewSelf();
							plRef.setIid(iid.toString());
							plRef.setSiid(partnerLink.getScopeInstanceId()
									.toString());
							plRef.setName(partnerLink.getPartnerLinkName());
							plRef.setModelId(String.valueOf(partnerLink.getPartnerLinkModelId()));

							// Fill the simple elements
							plInfo.setMyRoleName(partnerLink.getMyRoleName());
							plInfo.setMyRoleServiceName(partnerLink.getMyRoleServiceName());
							plInfo.setMySessionId(partnerLink.getMySessionId());
							plInfo.setPartnerRoleName(partnerLink.getPartnerRoleName());
							plInfo.setPartnerSessionId(partnerLink.getPartnerSessionId());

							// Fill the EPR elements
							Element myEPR = partnerLink.getMyEPR();
							if (myEPR != null) {
								TPartnerLinkInfo.MyEPR epr = plInfo.addNewMyEPR();
								epr.getDomNode().appendChild(
										epr.getDomNode().getOwnerDocument()
												.importNode(myEPR, true));
							}
							
							Element partnerEPR = partnerLink.getPartnerEPR();
							if (partnerEPR != null) {
								TPartnerLinkInfo.PartnerEPR epr = plInfo.addNewPartnerEPR();
								epr.getDomNode().appendChild(
										epr.getDomNode().getOwnerDocument()
												.importNode(partnerEPR, true));
							}
			...
			__log.error("Exception while collecting snapshot partnerlinks", e);
			throw new ProcessingException(
					"Exception while collecting snapshot partnerlinks: "
							+ e.toString());
		}
