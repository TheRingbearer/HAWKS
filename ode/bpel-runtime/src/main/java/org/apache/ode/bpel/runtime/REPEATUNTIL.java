...
public class REPEATUNTIL extends ACTIVITY {
	...
	public REPEATUNTIL(ActivityInfo self, ScopeFrame scopeFrame,
			LinkFrame linkFrame) {
	...
	public void run() {
		...
		if (first_time) {
			...
			LinkStatusChannelListener receiver = new LinkStatusChannelListener(
					signal) {
				private static final long serialVersionUID = 9873875L;

				public void linkStatus(boolean value) {
					if (value) // Incoming Event Start_Activity received
					{
						REPEATUNTIL.this.execute();
					} else // Incoming Event Complete_Activity received
					{
						REPEATUNTIL.this.Activity_Completed(
			...
			TerminationChannelListener termChan = new TerminationChannelListener(
					_self.self) {
				private static final long serialVersionUID = 789563346L;
			...
			ActivityReady evt = new ActivityReady(_self.o.name, _self.o.getId(),
					...
					_self.o.getArt(), false, signal, _self.self,
					REPEATUNTIL.this);
	...
	private boolean checkCondition() throws FaultException {
		try {
			return getBpelRuntimeContext().getExpLangRuntime()
					.evaluateAsBoolean(getORepeatUntil().untilCondition,
	...
	private class WAITER extends BpelJacobRunnable {
		...
		public void Iteration_Complete() {
			...
			LinkStatusChannelListener receiver = new LinkStatusChannelListener(
					signal) {
				private static final long serialVersionUID = 3464676565157675L;

				public void linkStatus(boolean value) {
					WAITER.this.Check_Condition();
				}
		...
		public void Check_Condition() {
			...
			if (!condResult) // LoopConditionTrue
			{
				...
				LinkStatusChannelListener receiver = new LinkStatusChannelListener(
						signal) {
					private static final long serialVersionUID = 5688347675L;

					public void linkStatus(boolean value) {
						if (value) {
							instance(REPEATUNTIL.this);
						} else {
							REPEATUNTIL.this.Activity_Completed(_compHandlers,
									false);
						}
					}
				...
				getBpelRuntimeContext().getBpelProcess().getEngine()
						.fireEvent(loop_evt);
			} else // LoopConditionFalse
			{
				LinkStatusChannel signal = newChannel(LinkStatusChannel.class);
				LinkStatusChannelListener receiver = new LinkStatusChannelListener(
						signal) {
					private static final long serialVersionUID = 56699547675L;
	...
	public void Activity_Faulted(FaultData fault,
		...
		if (term) {
			...
			LinkStatusChannelListener receiver = new LinkStatusChannelListener(
					signal) {
				private static final long serialVersionUID = 46765157675L;
	...
	public void Activity_Completed(Set<CompensationHandler> compens,
		...
		if (term) {
			...
			LinkStatusChannelListener receiver = new LinkStatusChannelListener(
					signal) {
				private static final long serialVersionUID = 6572448855L;
			...
			TerminationChannelListener termChan = new TerminationChannelListener(
					_self.self) {
				private static final long serialVersionUID = 856457457L;
