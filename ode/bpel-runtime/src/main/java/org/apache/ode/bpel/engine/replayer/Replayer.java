...
public class Replayer {
		...
		try {
			...
			contexts = new ArrayList<ReplayerContext>();
			{
				...
				for (Long iid : request.getUpgradeInstanceList()) {
					toDelete.add(iid);
					toRestore.add(CommunicationType.Factory
							.parse(getCommunication(iid, conn).toString()));
				}
	...
	public GetCommunicationResponse getCommunication(GetCommunication request,
			BpelDAOConnection conn) throws Exception {
		GetCommunicationResponse response = GetCommunicationResponse.Factory
				.newInstance();
			...
			try {
				...
				if (status == Status.FAULT) {
					...
					if (mexDao.getResponse() != null) {
						f.setMessage(XmlObject.Factory.parse(mexDao
								.getResponse().getData()));
					}
				} else if (status == Status.FAILURE) {
					e.addNewFailure().setExplanation(
							mexDao.getFaultExplanation());
				} else {
					if (mexDao.getResponse() != null) {
						e.setOut(XmlObject.Factory.parse(mexDao.getResponse()
								.getData()));
					}
				}
			} catch (XmlException e1) {
	...
	public void handleJobDetails(JobDetails jobDetail, final Date when) {
		...
		if (we.getType() == JobType.INVOKE_INTERNAL) {
			final BpelProcess p = engine._activeProcesses
					.get(we.getProcessId());
			final ProcessDAO processDAO = p.getProcessDAO();
			final MyRoleMessageExchangeImpl mex = (MyRoleMessageExchangeImpl) engine
					...
					new BpelProcess.InvokeHandler() {
						public boolean invoke(PartnerLinkMyRoleImpl target,
							...
							if (routing.messageRoute == null && createInstance) {
								__log.debug("creating new instance via live communication mex:"
										+ mex);
								ProcessInstanceDAO newInstance = processDAO
										.createInstance(routing.correlator);

								ReplayerContext context = new ReplayerContext(
										null);
								context.bpelEngine = (BpelEngineImpl) engine;
								contexts.add(context);

								ReplayerBpelRuntimeContextImpl runtimeContext = new ReplayerBpelRuntimeContextImpl(
								...
								long iid = routing.messageRoute
										.getTargetInstance().getInstanceId();
								ReplayerContext ctx = findReplayedInstance(iid);
								if (ctx == null) {
									throw new IllegalStateException(
											"Trying to hit existing instance via live communication, but there's no such instance mex:"
													+ mex + " iid:" + iid);
								}
								__log.debug("hitting existing instance via live communication mex:"
										+ mex + " iid:" + iid);

								ctx.runtimeContext.inputMsgMatch(
										routing.messageRoute.getGroupId(),
			...
			assert ctx != null;
			ctx.runtimeContext.invocationResponse(we.getMexId(),
					we.getChannel());
			ctx.runtimeContext.execute();
		}
	}
