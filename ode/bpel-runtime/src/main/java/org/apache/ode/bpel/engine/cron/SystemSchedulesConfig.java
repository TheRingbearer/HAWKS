...
public class SystemSchedulesConfig {
	private final static Log __log = LogFactory
			.getLog(SystemSchedulesConfig.class);
	...
	public SystemSchedulesConfig(File configRoot) {
		String scheduleConfigFile = System
				.getProperty(SCHEDULE_CONFIG_FILE_PROP_KEY);
		if (scheduleConfigFile != null) {
			...
			if (!new File(scheduleConfigFile).exists()) {
				__log.warn("A custom location for schedules has been set. However, the file does not exist at the location: "
						+ schedulesFile.getAbsolutePath()
						+ ". The file will be read when one gets created.");
			}
		} else {
			assert configRoot != null;
			schedulesFile = new File(configRoot, "schedules.xml");
		}
		__log.info("SYSTEM CRON configuration: "
				+ schedulesFile.getAbsolutePath());
	}
	...
	/**
	 * Returns the list of cron jobs configured for all processes. This call
	...
	public List<CronJob> getSystemCronJobs() {
		...
		if (schedulesFile != null && schedulesFile.exists()) {
			for (TSchedule schedule : getSystemSchedulesDocument()
					.getSchedules().getScheduleList()) {
				...
				try {
					...
					for (final TCleanup aCleanup : schedule.getCleanupList()) {
						...
						__log.info("SYSTEM CRON configuration added a runtime data cleanup: "
								+ runnableDetails);
					}
					jobs.add(job);
		...
		if (__log.isDebugEnabled())
			__log.debug("SYSTEM CRON configuration found cron jobs: " + jobs);
		return jobs;
	}
	...
	private SchedulesDocument getSystemSchedulesDocument() {
		...
		try {
			...
			throw new ContextException(
					"Couldn't read schedule descriptor at location "
