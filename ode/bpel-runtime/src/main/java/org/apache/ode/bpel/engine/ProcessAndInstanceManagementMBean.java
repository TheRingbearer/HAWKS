...
public class ProcessAndInstanceManagementMBean implements DynamicMBean {
	...
	public ProcessAndInstanceManagementMBean(BpelServer server,
			ProcessStore store) {
	...
	public ProcessAndInstanceManagementMBean(ProcessAndInstanceManagementImpl pm) {
		...
		MBeanOperationInfo[] operations = new MBeanOperationInfo[exposedOperations
				.size()];
		operations = (MBeanOperationInfo[]) exposedOperations
				.toArray(operations);
		MBeanNotificationInfo[] notifications = new MBeanNotificationInfo[] {};

		_mbeanInfo = new MBeanInfo(getClass().getName(),
				"Process and Instance Management", attributes, constructors,
	...
	private Class[] findTypes(ClassLoader loader, String[] signature)
