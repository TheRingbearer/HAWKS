...
/**
 * Class for converting status codes from external (PMAPI) to internal (DAO)
 * representation.
 */
class ProcessStatusConverter {

	/**
	 * A mapping from the interface status codes, to the internal ODE status
	 * codes.
	 */
	...
	ProcessStatusConverter() {
		...
		for (int i = 0; i < ProcessState.ALL_STATES.length; ++i) {
			...
			BitSet bset = __interfaceStatusCodeToInternalStatusCodeMap
					.get(intstc);
			if (bset == null) {
				bset = new BitSet();
	...
	/**
	 * Convert instance status from the internal database representation to the
	 * process management API enumerations.
	 * 
	 * @param instancestate
	 *            internal database state code
	...
	TInstanceStatus.Enum cvtInstanceStatus(short instancestate) {
		...
		throw new ProcessingException("Encountered unexpected instance state: "
				+ instancestate);
	}
	 ...
	 * 
	 * @param status
	 *            instance status (interface representation)
	 * @return internal states corresponding to the requested interface
	 *         representation.
	 */
