...
public class RuntimeDataCleanupRunnable implements MapSerializableRunnable,
		ContextsAware {
	private final Log _log = LogFactory
			.getLog(RuntimeDataCleanupRunnable.class);
	...
	public void restoreFromDetails(JobDetails details) {
		...
		_transactionSize = (Integer) details.getDetailsExt().get(
				"transactionSize");
		_pid = (QName) details.getDetailsExt().get("pid");
		_pidsToExclude = (Set<QName>) details.getDetailsExt().get(
				"pidsToExclude");
	}
	...
	int cleanInstances(String filter, final Set<CLEANUP_CATEGORY> categories,
		...
		try {
			if (_contexts.scheduler != null) {
				return _contexts.scheduler
						.execTransaction(new Callable<Integer>() {
							public Integer call() throws Exception {
								BpelDAOConnection con = _contexts.dao
										.getConnection();
								if (con instanceof FilteredInstanceDeletable) {
									return ((FilteredInstanceDeletable) con)
											.deleteInstances(instanceFilter,
													categories);
								}
								return 0;
							}
