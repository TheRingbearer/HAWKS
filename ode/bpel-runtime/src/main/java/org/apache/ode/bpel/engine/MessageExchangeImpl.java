...
abstract class MessageExchangeImpl implements MessageExchange {
	...
	private static final Log __log = LogFactory
			.getLog(MessageExchangeImpl.class);
	 ...
	 */
	MessageExchangeImpl(BpelEngineImpl engine, MessageExchangeDAO dao,
			MessageExchangePattern pattern, String opname, EndpointReference epr) {
		_engine = engine;
		_dao = dao;
		_epr = epr;
	...
	void setPattern(MessageExchangePattern pattern) {
		if (__log.isTraceEnabled())
			__log.trace("Mex[" + getMessageExchangeId() + "].setPattern("
					+ pattern + ")");
		getDAO().setPattern(pattern.toString());
	}
	...
	void setFault(QName faultType, Message outputFaultMessage)
	...
	void setFailure(FailureType type, String reason, Element details)
			throws BpelEngineException {
	...
	public EndpointReference getEndpointReference() throws BpelEngineException {
		...
		return _epr = _engine._contexts.eprContext
				.resolveEndpointReference(getDAO().getEPR());
	}
