...
public class ScopeFrame implements Serializable {
	...
	ScopeFrame(OScope scopeDef, Long scopeInstanceId, ScopeFrame parent,
	...
	public ScopeFrame(OScope scopeDef, Long scopeInstanceId, ScopeFrame parent,
			Set<CompensationHandler> visibleCompensationHandlers) {
...
//		// AH: end
	...
	public CorrelationSetInstance resolve(OScope.CorrelationSet cset) {
		return new CorrelationSetInstance(
				find(cset.declaringScope).scopeInstanceId, cset);
	}

	public PartnerLinkInstance resolve(OPartnerLink partnerLink) {
		return new PartnerLinkInstance(
				find(partnerLink.declaringScope).scopeInstanceId, partnerLink);
	}
	...
	//
	// Move all the external variable stuff in here so that it can be used by
	// the expr-lang evaluation
	// context.
	//

	// @hahnml: Changed to public to use with extension activities
	public Node fetchVariableData(BpelRuntimeContext brc,
			VariableInstance variable, boolean forWriting)
		...
		if (variable.declaration.extVar != null) {
			...
			Element reference = (Element) fetchVariableData(brc,
					resolve(variable.declaration.extVar.related), false);
			try {
				Node ret = brc.readExtVar(variable.declaration, reference);
				if (ret == null) {
					throw new FaultException(
							oscope.getOwner().constants.qnUninitializedVariable,
				...
				// This indicates that the external variable needed to be
				// written do, put has not been.
				...
				throw new FaultException(
						oscope.getOwner().constants.qnUninitializedVariable,
						"The extenral variable \"" + variable.declaration.name
			...
			Node data = brc.readVariable(variable.scopeInstance,
					variable.declaration.name, forWriting);
			if (data == null) {
				...
				throw new FaultException(
						oscope.getOwner().constants.qnUninitializedVariable,
						"The variable " + variable.declaration.name
	...
	Node fetchVariableData(BpelRuntimeContext brc, VariableInstance var,
			OMessageVarType.Part part, boolean forWriting)
			throws FaultException {
	...
	// @hahnml: Changed to public to use with extension activities
	public Node initializeVariable(BpelRuntimeContext context,
			VariableInstance var, Node value)
		...
		if (var.declaration.extVar != null) /* external variable */{
			...
			try {
				...
				// In this context this is not necessarily a problem, since the
				// assignment may re-init the related var
			}
			if (reference != null)
			...
			if (__log.isDebugEnabled())
				__log.debug("Initialize variable: name=" + var.declaration
						+ " value=" + DOMUtils.domToString(value));
			return context.writeVariable(var, value);
		}
	}

	Node commitChanges(BpelRuntimeContext context, VariableInstance var,
			Node value) throws ExternalVariableModuleException {
		return writeVariable(context, var, value);
	}

	Node writeVariable(BpelRuntimeContext context, VariableInstance var,
			Node value) throws ExternalVariableModuleException {
		if (var.declaration.extVar != null) /* external variable */{
			if (__log.isDebugEnabled())
			...
			ValueReferencePair vrp = context.writeExtVar(var.declaration,
					reference, value);
