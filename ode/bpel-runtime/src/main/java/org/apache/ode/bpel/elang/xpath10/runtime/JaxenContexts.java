...
class JaxenContexts implements FunctionContext, VariableContext {
	...
	private static final FunctionContext __defaultXPathFunctions = XPathFunctionContext
			.getInstance();

	private static final QName BOOLEAN = new QName(
	...
	public JaxenContexts(OXPath10Expression oxpath, Map extensionFunctions,
	...
	public Object getVariableValue(String namespaceURI, String prefix,
		...
		if (!(_oxpath instanceof OXPath10ExpressionBPEL20)) {
			throw new IllegalStateException(
					"XPath variables not supported for bpel 1.1");
		}
			...
			try {
				...
				if (variableNode == null)
					throw new WrappedFaultException.JaxenUnresolvableException(
							new FaultException(
									variable.getOwner().constants.qnSelectionFailure,
									"Unknown variable " + localName));
				OVarType type = variable.type;
				if (type instanceof OMessageVarType) {
					OMessageVarType.Part typePart = ((OMessageVarType) type).parts
							.get(partName);
					if (typePart == null) {
						throw new WrappedFaultException.JaxenUnresolvableException(
								new FaultException(
										variable.getOwner().constants.qnSelectionFailure,
										"Unknown part " + partName
												+ " for variable " + localName));
					}
					type = typePart.type;
				}

				if (_xpathEvalCtx.narrowTypes()
						&& type instanceof OXsdTypeVarType
			...
			try {
				...
				if (e.getCause() instanceof FaultException) {
					...
					throw new FunctionCallException(
							"SubLanguageException: Unable to evaluate query expression",
							e);
				}
			}
		}
	...
	class BpelVariablePropertyFunction implements Function {
		public Object call(Context context, List args)
			...
			try {
				...
				__log.error("bpws:getVariableProperty(" + args
						+ ") threw FaultException", e);
				throw new WrappedFaultException.JaxenFunctionException(e);
			}
		}
	...
	class GetLinkStatusFunction implements Function {
		public Object call(Context context, List args)
			...
			try {
				...
				__log.error("bpws:getLinkStatus(" + args
						+ ") threw FaultException", e);
				throw new WrappedFaultException.JaxenFunctionException(e);
			}
		}
	...
	class DoXslTransformFunction implements Function {
		public Object call(Context context, List args)
			...
			if (!(_oxpath instanceof OXPath10ExpressionBPEL20)) {
				throw new IllegalStateException(
						"XPath function bpws:doXslTransform not supported in "
								+ "BPEL 1.1!");
			}
