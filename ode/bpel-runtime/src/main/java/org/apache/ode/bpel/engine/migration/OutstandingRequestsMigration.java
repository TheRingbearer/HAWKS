...
public class OutstandingRequestsMigration implements Migration {
	private static Log __log = LogFactory
			.getLog(OutstandingRequestsMigration.class);
			...
			for (ProcessInstanceDAO instance : pis) {
				...
				try {
					if (instance.getExecutionState() == null) {
						...
						ExecutionQueueImpl soup = new ExecutionQueueImpl(this
								.getClass().getClassLoader());
						soup.setReplacementMap(process
								.getReplacementMap(processDao.getProcessId()));
						soup.read(new ByteArrayInputStream(instance
								.getExecutionState()));
