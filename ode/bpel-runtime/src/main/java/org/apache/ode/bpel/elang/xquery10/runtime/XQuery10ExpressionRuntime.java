...
public class XQuery10ExpressionRuntime implements ExpressionLanguageRuntime {
	...
	private static final Log __log = LogFactory
			.getLog(XQuery10ExpressionRuntime.class);
	...
	public Node evaluateNode(OExpression cexp, EvaluationContext ctx)
		...
		if (retVal.size() == 0) {
			throw new FaultException(
					cexp.getOwner().constants.qnSelectionFailure,
	...
	public Calendar evaluateAsDate(OExpression cexp, EvaluationContext context)
		...
		try {
			...
			throw new FaultException(
					cexp.getOwner().constants.qnInvalidExpressionValue, errmsg);
		}
	}
	...
	private Object evaluate(OExpression cexp, EvaluationContext ctx, QName type)
		...
		try {
			...
			for (QName variable : exp.getAllUnboundExternalVariables()) {
				...
				if (value instanceof Value) {
					...
					try {
						...
						if (item == null) {
							exp.bindSequence(variable, xqconn
									.createSequence(Collections.EMPTY_LIST
											.iterator()));
						} else {
					...
					if (value instanceof Date) {
						Date d = (Date) value;
						value = org.apache.ode.utils.ISO8601DateParser
								.format(d);
					}
			...
			// Set context node
			Node contextNode = (ctx.getRootNode() == null) ? DOMUtils
					.newDocument() : ctx.getRootNode();
			contextNode.setUserData(
					XQuery10BpelFunctions.USER_DATA_KEY_FUNCTION_RESOLVER,
					funcResolver, null);
			exp.bindItem(
					XQConstants.CONTEXT_ITEM,
					xqconn.createItemFromNode(contextNode,
			...
			throw new EvaluationException(
					"Error while executing an XQuery expression: "
	 ...
	 *             XQException
	 */
	private Object getResultValue(QName type, XQResultSequence result)
			throws XQException {
		...
		if (XPathConstants.NODESET.equals(type)) {
			...
			while (result.next()) {
				...
				if (itemValue instanceof Document) {
					itemValue = DOMUtils.cloneNode(document,
							((Document) itemValue).getDocumentElement());
			...
			if (item != null) {
				...
				if (resultValue instanceof Node) {
					resultValue = DOMUtils.cloneNode(document,
							(Node) resultValue);
				}
			}
