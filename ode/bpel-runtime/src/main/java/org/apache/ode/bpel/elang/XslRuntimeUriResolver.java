...
/**
 * Used to give the Xsl processor a way to access included XSL sheets by using
 * the maps of sheets pre-processed at compilation time and stored in the
 * OXPath10Expression.
 */
public class XslRuntimeUriResolver implements URIResolver {

	private static final Log __log = LogFactory
			.getLog(XslRuntimeUriResolver.class);
	...
	/**
	 * Given a URI this function will attempt to retrieve the resource declared
	 * at that URI location as a stream. This URI can be defined as being
	 * relative to the executing process instance's physical file location or
	 * can point to an HTTP(S) resource.
	 * 
	 * @param docUri
	 *            - the URI to resolve
