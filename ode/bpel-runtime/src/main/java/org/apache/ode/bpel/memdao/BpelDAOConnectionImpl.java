...
/**
 * A very simple, in-memory implementation of the {@link BpelDAOConnection}
 * interface.
 */
class BpelDAOConnectionImpl implements BpelDAOConnection {
	...
	BpelDAOConnectionImpl(Map<QName, ProcessDaoImpl> store,
			Scheduler scheduler, long mexTtl) {
		_store = store;
	...
	public Collection<ProcessInstanceDAO> instanceQuery(InstanceFilter filter) {
		...
		selectionCompleted: for (ProcessDaoImpl proc : _store.values()) {
			...
			if (filter.getNameFilter() != null
					&& !equalsOrWildcardMatch(filter.getNameFilter(), proc
							.getProcessId().getLocalPart()))
				pmatch = false;
			if (filter.getNamespaceFilter() != null
					&& !equalsOrWildcardMatch(filter.getNamespaceFilter(), proc
	...
	public Collection<ProcessDAO> processQuery(ProcessFilter filter) {
		throw new UnsupportedOperationException(
				"Can't query process configuration using a transient DAO.");
	}
	...
	private int compareInstanceUsingKey(String key,
			ProcessInstanceDAO instanceDAO1, ProcessInstanceDAO instanceDAO2) {
		String s1 = null;
		String s2 = null;
		...
		ProcessDAO process1 = getProcess(instanceDAO1.getProcess()
				.getProcessId());
		ProcessDAO process2 = getProcess(instanceDAO2.getProcess()
				.getProcessId());
	...
	public void insertBpelEvent(BpelEvent event,
			ProcessDAO processConfiguration, ProcessInstanceDAO instance) {
		_events.add(event);
	}
	...
	public List<Date> bpelEventTimelineQuery(InstanceFilter ifilter,
		...
		CollectionsX.transform(dates, _events,
