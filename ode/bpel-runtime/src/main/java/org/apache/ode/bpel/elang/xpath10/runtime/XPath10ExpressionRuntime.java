...
public class XPath10ExpressionRuntime implements ExpressionLanguageRuntime {
	...
	private static final Log __log = LogFactory
			.getLog(XPath10ExpressionRuntime.class);
	...
	public String evaluateAsString(OExpression cexp, EvaluationContext ctx)
		...
		try {
			return compile((OXPath10Expression) cexp).stringValueOf(
					createContext((OXPath10Expression) cexp, ctx));
	...
	public boolean evaluateAsBoolean(OExpression cexp, EvaluationContext ctx)
			throws FaultException, EvaluationException {
		try {
			return compile((OXPath10Expression) cexp).booleanValueOf(
					createContext((OXPath10Expression) cexp, ctx));
		} catch (JaxenException e) {
			handleJaxenException(e);
		}
	...
	public Number evaluateAsNumber(OExpression cexp, EvaluationContext ctx)
			throws FaultException, EvaluationException {
		try {
			return compile((OXPath10Expression) cexp).numberValueOf(
					createContext((OXPath10Expression) cexp, ctx));
		} catch (JaxenException e) {
			handleJaxenException(e);
		}
	...
	private Context createContext(OXPath10Expression oxpath,
		...
		ContextSupport support = new ContextSupport(
				new JaxenNamespaceContextAdapter(oxpath.namespaceCtx),
				bpelSupport, bpelSupport, new BpelDocumentNavigator(
						ctx.getRootNode()));
	...
	private void handleJaxenException(JaxenException je)
			throws EvaluationException, FaultException {
