...
public class FaultData implements Serializable {
	...
	// @stmz: so that has not to be part of a OVarType, when throwing a fault
	// from outside
	private QName messType = null;

	// @stmz: so that has not to be part of a OVarType, when throwing a fault
	// from outside
	private QName elemType = null;
	...
	public FaultData(QName fault, Element faultMsg, OVarType faultVarType,
			OBase location) {
		this(fault, location, null);
		assert faultMsg != null;
		assert faultVarType != null;
		assert faultVarType instanceof OMessageVarType
				|| faultVarType instanceof OElementVarType;
		_faultMsg = new SerializableElement(faultMsg);
		_faultVarType = faultVarType;
	}

	// @stmz: new constructor for faults from outside
	public FaultData(QName fault, Element faultMsg, QName messageType,
			QName elementType, OBase location) {
		this(fault, location, null);
		if (faultMsg != null) {
			_faultMsg = new SerializableElement(faultMsg);
		}
