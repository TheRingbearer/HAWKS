...
/**
 * Manager for external variable instances; used by {@link BpelProcess} to
...
public class ExternalVariableManager {
	...
	private static final Log __log = LogFactory
			.getLog(ExternalVariableManager.class);

	/** Mapping of engine names to engine. */
	private Map<QName, ExternalVariableModule> _engines;

	/**
	 * Mapping between external variable identifiers and the engines that are
	...
	public ExternalVariableManager(QName pid, ExternalVariableConf evconf,
			Map<QName, ExternalVariableModule> engines, OProcess oprocess)
		...
		for (ExternalVariableConf.Variable var : _extVarConf.getVariables()) {
			EVar evar = new EVar(var.extVariableId,
					_engines.get(var.engineQName), var.configuration);
			if (evar._engine == null) {
				__log.error("External variable engine \"" + var.engineQName
						+ "\" referenced by external variable \""
						+ var.extVariableId + "\" not registered.");
				fatal = true;
				continue;
			}
			...
			try {
				...
				__log.error("External variable subsystem configuration error.",
						eve);
				throw new BpelEngineException(
						"External variable subsystem configuration error.", eve);
			}
			if (_externalVariables.containsKey(var.extVariableId)) {
				__log.warn("Duplicate external variable configuration for \""
						+ var.extVariableId + "\" will be ignored!");
			}
		...
		for (OBase child : oprocess.getChildren()) {
			...
			for (OScope.Variable var : oscope.variables.values()) {
				...
				EVar evar = _externalVariables
						.get(var.extVar.externalVariableId);
				if (evar == null) {
					__log.error("The \""
							+ oscope.name
							+ "\" scope declared an unknown external variable \""
							+ var.extVar.externalVariableId
	 ...
	 */
	public Value read(Variable variable, Node reference, Long iid)
			throws ExternalVariableModuleException {
		EVar evar = _externalVariables.get(variable.extVar.externalVariableId);
		if (evar == null) {
			// Should not happen if constructor is working.
			throw new BpelEngineException(
					"InternalError: reference to unknown external variable "
							+ variable.extVar.externalVariableId);
		}

		Locator locator = new Locator(variable.extVar.externalVariableId, _pid,
				iid, reference);
		Value newval;
		newval = evar._engine.readValue(
				((OElementVarType) variable.type).elementType, locator);
		if (newval == null)
			return null;
		return newval;
	}

	public Value write(Variable variable, Node reference, Node val, Long iid)
			throws ExternalVariableModuleException {
		EVar evar = _externalVariables.get(variable.extVar.externalVariableId);
		if (evar == null) {
			// Should not happen if constructor is working.
		...
		newval = evar._engine.writeValue(
				((OElementVarType) variable.type).elementType, newval);
