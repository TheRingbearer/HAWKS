 ...
 */
//AH: added ProcessRegistry
public class BpelServerImpl implements BpelServer, ProcessRegistry,
	...
	public final static String DEFERRED_PROCESS_INSTANCE_CLEANUP_DISABLED_NAME = "org.apache.ode.disable.deferredProcessInstanceCleanup";

	private static boolean DEFERRED_PROCESS_INSTANCE_CLEANUP_DISABLED = Boolean
			.getBoolean(DEFERRED_PROCESS_INSTANCE_CLEANUP_DISABLED_NAME);

	/**
	 * Set of processes that are registered with the server. Includes hydrated
	 * and dehydrated processes. Guarded by _mngmtLock.writeLock().
	 */
	...
	static {
		...
		try {
			...
			if (__log.isDebugEnabled()) {
				__log.debug(
						"Could not parse ode.process.maxage environment variable.",
						t);
			} else {
	...
	public void start() {
		...
		try {
			...
			BpelEventListenerImpl tmp = new BpelEventListenerImpl();
			registerBpelEventListener(tmp);
	...
	public void init() throws BpelEngineException {
		...
		try {
			...
			// AH:
			fcEventBroker = new FragmentCompositionEventBroker();

			// AH: end

			_engine = createBpelEngineImpl(_contexts);
			// AH:
			_engine.setProcessRegistry(this);
			_engine.setBpelDatabase(_db);
			_engine.setFragmentCompositionEventBroker(fcEventBroker);
			// AH: end
	...
	public BpelEngine getEngine() {
		...
		try {
			...
			// If we failed to register the synchro,then there was an
			// ex/throwable; we need to unlock now.
			if (!registered)
				_mngmtLock.readLock().unlock();
		}
		return _engine;
	...
	public void reregister(ProcessConf conf) {
		QName processId = conf.getProcessId();
		_mngmtLock.readLock().unlock();
		_mngmtLock.writeLock().lock();
		try {
			if (processModelOk(processId)) {
				_engine._activeProcesses.remove(processId);
				register(conf);
			}
		} finally {
			_mngmtLock.readLock().lock();
	...
	private boolean processModelOk(QName processId) {
		return _engine._activeProcesses.containsKey(processId);
	}
	...
	public void register(ProcessConf conf) {
		if (conf == null)
			throw new NullPointerException(
					"must specify non-null process configuration.");
		...
		try {
			...
			if (_engine.isProcessRegistered(conf.getProcessId())) {
				__log.debug("skipping doRegister" + conf.getProcessId()
			...
			process._classLoader = Thread.currentThread()
		...
		try {
			...
			if (_engine != null) {
				...
				if (p != null) {
					...
					XslTransformHandler.getInstance().clearXSLSheets(
							p.getProcessType());
	...
	private boolean deleteProcessDAO(BpelDAOConnection conn, QName pid) {
		...
		if (proc != null) {
			// delete routes
			if (__log.isDebugEnabled())
				__log.debug("Deleting only the process " + pid + "...");
			proc.deleteProcessAndRoutes();
			if (__log.isInfoEnabled())
				__log.info("Deleted only the process " + pid + ".");
			...
			if (proc instanceof DeferredProcessInstanceCleanable
				...
				_engine._contexts.scheduler.scheduleMapSerializableRunnable(
						new ProcessCleanUpRunnable(
								((DeferredProcessInstanceCleanable) proc)
										.getId()), new Date());
			...
			try {
				while (true) {
					...
					try {
						...
						// synchronization
						// problems and a potential mess if a policy modifies
						// the list
						List<BpelProcess> candidates = new ArrayList<BpelProcess>(
								_registeredProcesses);
						CollectionsX.remove_if(candidates,
						...
						List<BpelProcess> ripped = _dehydrationPolicy
								.markForDehydration(candidates);
						// Bye bye
						for (BpelProcess process : ripped) {
							__log.debug("Dehydrating process "
									+ process.getPID());
							process.dehydrate();
						}
					} finally {
	 ...
	 * 
	 */
	public static class PolledRunnableProcessor implements
			Scheduler.JobProcessor {
		...
		public void setPolledRunnableExecutorService(
				ExecutorService polledRunnableExecutorService) {
		...
		public void onScheduledJob(final Scheduler.JobInfo jobInfo)
			...
			synchronized (resultsByJobId) {
				PolledRunnableResults results = resultsByJobId
						.get(jobInfo.jobName);
				if (results != null) {
				...
				if (statusOfPriorTry == JOB_STATUS.PENDING
						|| statusOfPriorTry == JOB_STATUS.FAILED) {
					resultsByJobId.put(jobInfo.jobName,
							new PolledRunnableResults(JOB_STATUS.IN_PROGRESS,
									null));
					toRetry = true;
				}
			}
			...
			if (toRetry) {
				...
				_polledRunnableExec.submit(new Runnable() {
					public void run() {
						try {
							MapSerializableRunnable runnable = (MapSerializableRunnable) jobInfo.jobDetail
									.getDetailsExt().get("runnable");
							runnable.restoreFromDetails(jobInfo.jobDetail);
							if (runnable instanceof ContextsAware) {
								((ContextsAware) runnable)
										.setContexts(_contexts);
							}
							runnable.run();
							synchronized (resultsByJobId) {
								resultsByJobId.put(jobInfo.jobName,
										new PolledRunnableResults(
												JOB_STATUS.COMPLETED, null));
							}
						} catch (Exception e) {
							__log.error("", e);
							synchronized (resultsByJobId) {
			...
			if (exceptionThrownOnPriorTry != null) {
				throw new Scheduler.JobProcessorException(
						exceptionThrownOnPriorTry, true);
			}
		}
	...
	// @hahnml
	public void migrateProcessInstanceMetaData(final QName newProcessID,
			final Long instanceID, final Long currentVersion) {
		...
		try {
			_db.exec(new BpelDatabase.Callable<Object>() {
				public Object run(BpelDAOConnection conn) throws Exception {
					...
					if (instance != null) {
						BpelProcess old_process = _engine.getProcess(instance
								.getProcess().getProcessId());

						MigrationDatabaseHelper helper = new MigrationDatabaseHelper(
								_contexts);

						Integer InstanceID = instanceID.intValue();
						Integer NewProcessID = helper
								.getNewProcessID(currentVersion);
						Integer CorrID = helper.getCorrID(NewProcessID);

						helper.setOdeEvent(InstanceID, NewProcessID);
						helper.setOdeMessageExchange(InstanceID, NewProcessID);
						helper.setOdeProcessInstance(InstanceID, NewProcessID,
								CorrID);

						// Get the instance migration DAO and set the migration
						// flag
						instance.getInstanceMigrationDAO().setMigrated(true);

						// Copy the activity lists from the old to the new
						// process model
						BpelProcess new_process = _engine
								.getProcess(newProcessID);

						new_process._timerToIgnoreForIteration = old_process._timerToIgnoreForIteration;
						new_process._activityChannelMap = old_process._activityChannelMap;

						// Handle the case that a iteration/reexecution or
						// jumpTo was executed before the migration.
						// This has to be done because the migration destroys
						// the activity which is built by the
						// iteration/reexecution or jumpTo processing.
						if (instance.getInstanceMigrationDAO().isIterated()) {
							// Start the iteration again with the saved job
							// details.
							new_process.handleJobDetails(instance
									.getInstanceMigrationDAO()
									.getIterationJobDetails());
						}
					}
			...
			__log.error("Exception during instance migration", e);
		}
	}

	// @hahnml
	public OProcess getProcessModel(final Long instanceID) {
		OProcess process = null;

		try {
			process = _db.exec(new BpelDatabase.Callable<OProcess>() {
				public OProcess run(BpelDAOConnection conn) throws Exception {
					ProcessInstanceDAO instance = conn.getInstance(instanceID);
					BpelProcess process = null;

					if (instance != null) {
						process = _engine.getProcess(instance.getProcess()
								.getProcessId());
					}
			...
			__log.error("Exception during process model query", e);
		}
	...
	public void setProcessConfLoader(ProcessConfLoader loader) {
		_engine.setProcessConfLoader(loader);
	}
	// AH: end
}
