...
public class EventContextImpl implements EventContext {
	 ...
	 * @param __scope
	 *            Scope Object used in this scope instance.
	 * @param __scopeInstanceId
	 *            Scope Instance ID
	 * @param __runtimeContext
	 *            BPEL Runtime Context
	 */
	public EventContextImpl(OScope __scope, Long __scopeInstanceId,
	 ...
	 * @param varName
	 *            Variable Name
	 * @return DOM Node as XML String. If no value exists or variable not
	 *         initialized, NULL will be returnrd.
	 */
	public String getVariableData(String varName) {
		String value = null;
		try {
			Variable var = __scope.getVisibleVariable(varName);
			VariableInstance varInstance = new VariableInstance(
					__scopeInstanceId, var);
			Node varNode = __runtimeContext.readVariable(
					varInstance.scopeInstance, varInstance.declaration.name,
					false);
			value = DOMUtils.domToString(varNode);
		} catch (Throwable e) {
			// Don't throw any exception back to the caller. Just return null as
			// value.
		}
		return value;
	}
	 ...
	 * 
	 * @return Array of Variable Names. If no variable(s) exists, null will be
	 *         returned.
	 */
