...
class MockBpelServer {
	...
	public MockBpelServer() {
		try {
			...
			_store = new ProcessStoreImpl(_eprContext, _dataSource, "hib",
					new OdeConfigProperties(storeProps, ""), true);
	...
	public void invoke(QName serviceName, String opName, Element body)
			throws Exception {
		try {
			...
			if (mex.getOperation() == null)
				throw new Exception("Did not find operation " + opName
						+ " on service " + serviceName);
			Message request = mex.createMessage(mex.getOperation().getInput()
					.getMessage().getQName());
			Element wrapper = body.getOwnerDocument().createElementNS("",
					"main");
			wrapper.appendChild(body);
			Element message = body.getOwnerDocument().createElementNS("",
					"message");
	...
	protected DataSource createDataSource() throws Exception {
		...
		PoolingSupport poolingSupport = new SinglePool(10, 0, 1000, 1, true,
				false, false);

		ConnectionManager connectionManager = new GenericConnectionManager(
				transactionSupport, poolingSupport, null, connectionTracker,
				(RecoverableTransactionManager) _txManager, getClass()
	...
	protected BpelDAOConnectionFactory createDAOConnection() throws Exception {
		...
		//
		// BpelDAOConnectionFactoryJDBC daoCF = new
	...
	protected EndpointReferenceContext createEndpointReferenceContext() {
		_eprContext = new EndpointReferenceContext() {
			...
			public EndpointReference convertEndpoint(QName qName,
					Element element) {
				return null;
			}
	...
	protected BindingContext createBindingContext() {
		_bindContext = new BindingContext() {
			public EndpointReference activateMyRoleEndpoint(QName processId,
				...
				serviceRef.appendChild(doc
						.createTextNode(myRoleEndpoint.serviceName.toString()));
			...
			public PartnerRoleChannel createPartnerRoleChannel(QName processId,
					PortType portType, final Endpoint initialPartnerEndpoint) {
				final EndpointReference epr = new EndpointReference() {
					public Document toXML() {
						Document doc = DOMUtils.newDocument();
						Element serviceRef = doc.createElementNS(
								EndpointReference.SERVICE_REF_QNAME
										.getNamespaceURI(),
								EndpointReference.SERVICE_REF_QNAME
										.getLocalPart());
						serviceRef
								.appendChild(doc
										.createTextNode(initialPartnerEndpoint.serviceName
												.toString()));
						doc.appendChild(serviceRef);
						return doc;
					}
				};
				_endpoints.put(initialPartnerEndpoint.serviceName.toString(),
						epr);
	...
	private class SchedulerWrapper implements Scheduler {
		...
		public String scheduleVolatileJob(boolean transacted,
			...
			String jobId = _scheduler.scheduleVolatileJob(transacted,
					jobDetail, when);
