...
public class JdbcExternalVariableEngineTest extends TestCase {
	...
	public void setUp() throws Exception {
		...
		s.execute("create table extvartable1 (" + "id1 VARCHAR PRIMARY KEY,"
				+ "_id2_ VARCHAR," + "pid VARCHAR, " + "iid INT,"
				+ "cts DATETIME," + "uts DATETIME," + "foo VARCHAR,"
				+ "bar VARCHAR );");
		...
		_el1 = DOMUtils
				.stringToDOM("<foobar><id2>ignored</id2><foo>foo</foo><bar>bar</bar></foobar>");
	}

	@Test
	public void testConfigurationParsing() throws Exception {
		Document deploydoc = DOMUtils.parse(getClass().getResourceAsStream(
				"evardeploy.xml"));
		NodeList nl = deploydoc.getElementsByTagNameNS(
				ExternalVariableConf.EXTVARCONF_ELEMENT.getNamespaceURI(),
	...
	public void testInitWriteValue() throws Exception {
		...
		assertTrue(DOMUtils.domToString((Element) value.locator.reference)
				.indexOf("id1") != -1);
		assertTrue(DOMUtils.domToString((Element) value.locator.reference)
				.indexOf("id2") != -1);
	}
	...
	public void testWriteUpdate() throws Exception {
		...
		Value newvalue = new Value(value.locator, DOMUtils.stringToDOM(domstr
				.replaceAll("<bar>bar</bar>", "<bar>boohoo</bar>")), null);
	...
	public void testRead() throws Exception {
		...
		assertEquals(2, DOMUtils.countKids((Element) readVal.locator.reference,
				Node.ELEMENT_NODE));
		assertEquals(DOMUtils.domToString(value.locator.reference),
				DOMUtils.domToString(readVal.locator.reference));
