...
public class CombineUrlTest extends TestCase {
	...
	public void testCombineUrl() throws XPathFunctionException {
		...
		m.put("tutorial1/2.html",
				"http://WebReference.com/html/tutorial1/2.html");
		...
		/*
		 * m.put("../../../", "http://WebReference.com/"); m.put("../../../",
		 * "http://WebReference.com/../../");
		 * 
		 * This case is considered as an error by rfc2396. but several options
		 * are left to the implementation. So the result is implementation
		 * specific. see section 5.2, step 6.g
		 * 
		 * "Implementations may handle this error by retaining these components
		 * in the resolved path (i.e., treating them as part of the final URI),
		 * by removing them from the resolved path (i.e., discarding relative
		...
		for (Iterator it = m.entrySet().iterator(); it.hasNext();) {
			...
			assertEquals(e.getValue(),
					f.evaluate(Arrays.asList(baseURL, e.getKey())));
		}
	}
