...
public class ActivityRecoveryTest extends MockObjectTestCase {
	...
	static {
		...
		System.setProperty(
				BpelServerImpl.DEFERRED_PROCESS_INSTANCE_CLEANUP_DISABLED_NAME,
				"true");
	}

	/**
	 * The process calls the failing service, simulated by a call to invoke. The
	 * method returns true if the call succeeded, false for failure. If the
	 * process completes, it calls the completed method.
	 */
	...
	public void testFailureWithManualRecovery() throws Exception {
		// Recovery required after three failures. Only one attempt made after
		// recovery.
	...
	public void testFailureWithFaultAction() throws Exception {
		...
		assertTrue(OFailureHandling.FAILURE_FAULT_NAME
				.equals(lastInstance(null).getFaultInfo().getName()));
		assertNoFailures();
	}
	...
	public void testFailureWithCancelAction() throws Exception {
		// Recovery required after three failures. Only one attempt made after
		// recovery.
		// Use the last failure to cancel the activity, allowing the process to
		// complete.
		_testService.expects(exactly(4)).method("invoke").will(failTheFirst(4));
		_testService.expects(once()).method("completed").after("invoke");
	...
	public void testImmediateFailure() throws Exception {
		// This process does not attempt to retry, entering recovery
		// immediately.
		_testService.expects(exactly(1)).method("invoke")
	...
	public void _testInstanceSummary() throws Exception {
		...
		_testService.expects(once()).method("completed").after("invoke");
		_server.invoke(_processQName, "instantiate", DOMUtils.newDocument()
				.createElementNS(NAMESPACE, "tns:RequestElement"));
		...
		// Failing the first three times, we can then leave it waiting for
		// recovery.
		_testService.expects(exactly(3)).method("invoke").will(failTheFirst(3));
		_server.invoke(_processQName, "instantiate", DOMUtils.newDocument()
				.createElementNS(NAMESPACE, "tns:RequestElement"));
		...
		TInstanceSummary summary = _management.getProcessInfo(_processId)
				.getProcessInfo().getInstanceSummary();
	...
	protected void setUp() throws Exception {
		...
		// We use one partner to simulate failing service and receive message
		// upon process completion.
		final Mock partner = mock(MessageExchangeContext.class);
		// Some processes will complete, but not all.
		partner.expects(atMostOnce()).match(invokeOnOperation("respond"))
				.will(new CustomStub("process completed") {
		...
		partner.expects(atLeastOnce()).match(invokeOnOperation("invoke"))
				.will(new CustomStub("invoke failing service") {
					public Object invoke(Invocation invocation) {
						PartnerRoleMessageExchange mex = (PartnerRoleMessageExchange) invocation.parameterValues
								.get(0);
						if (((TestService) _testService.proxy()).invoke()) {
							Message response = mex.createMessage(mex
									.getOperation().getOutput().getMessage()
									.getQName());
							response.setMessage(DOMUtils.newDocument()
									.createElementNS(NAMESPACE,
											"tns:ResponseElement"));
							mex.reply(response);
						} else {
							mex.replyWithFailure(
		...
		partner.expects(atMostOnce()).method("onAsyncReply")
				.will(new CustomStub("async reply") {
					public Object invoke(Invocation invocation) {
						return null;
					}
				});
		...
		_server.deploy(new File(new URI(this.getClass()
				.getResource("/recovery").toString())));
		_management = new BpelManagementFacadeImpl(_server._server,
				_server._store);
	}
	...
	/**
	 * Returns a stub that will fail (return false) for the first n number of
	 * times, and on the last call succeed (return true).
	 */
	...
	protected InvocationMatcher invokeOnOperation(final String opName) {
		// Decides which method to call the TestService mock based on the
		// operation.
		return new StatelessInvocationMatcher() {
			public boolean matches(Invocation invocation) {
				return invocation.invokedMethod.getName().equals(
						"invokePartner")
						&& invocation.parameterValues.size() == 1
						&& ((PartnerRoleMessageExchange) invocation.parameterValues
								.get(0)).getOperation().getName()
								.equals(opName);
			}

			public StringBuffer describeTo(StringBuffer buffer) {
				return buffer.append("check that the operation ")
						.append(opName).append(" is invoked");
			}
		};
	}

	/**
	 * Call this to execute the process so it fails the specified number of
	 * times. Returns when the process has either completed, or waiting for
	 * recovery to happen.
	 */
	...
	protected void assertNoFailures() throws Exception {
		...
		failures = _management.getProcessInfo(_processId).getProcessInfo()
				.getInstanceSummary().getFailures();
		assertTrue(failures == null || failures.getCount() == 0);
	}
	 ...
	 * @throws Exception
	 */
	protected TInstanceInfo lastInstance(TInstanceStatus.Enum expected)
			throws Exception {
		int counter = 0;
		do {
			TInstanceInfo info = getInstanceInfo();
			if (info != null
					&& (expected == null || info.getStatus() == expected)) {
				return info;
			}
			if (counter * DELAY > MAX_WAIT) {
				throw new Exception(
						"Timed out wait for instance to reach "
								+ expected
								+ " status. Actual status: "
								+ (info == null ? "missing instance" : info
										.getStatus()));
			}
			counter++;
			Thread.sleep(DELAY);
	...
	private TInstanceInfo getInstanceInfo() {
		TInstanceInfoList instances = _management.listInstances("", "", 1000)
				.getInstanceInfoList();
		int size = instances.sizeOfInstanceInfoArray();
		if (size > 0) {
			return instances.getInstanceInfoArray(instances
					.sizeOfInstanceInfoArray() - 1);
		}
		return null;
	}
	...
	protected void assertRecovery(int invoked, String[] actions)
		...
		while (true) {
			count--;
			if (count <= 0)
				throw new AssertionError("No failures info, which are required");
			Thread.sleep(sleep);
			sleep = 1000;
			TFailuresInfo failures = instance.getFailures();
			if (!(failures != null && failures.getCount() == 1))
				continue;
			ProcessInfoDocument m = _management.getProcessInfo(_processId);
			if (!m.getProcessInfo().isSetInstanceSummary())
				continue;
	...
	/**
	 * Blocks for until there are recoveries available or throws an exception if
	 * MAX_WAIT exceeded
	 */
	private ArrayList<TActivityInfo> getRecoveries() throws Exception {
		...
		do {
			...
			if (instance != null) {
				ArrayList<TActivityInfo> recoveries = getRecoveriesInScope(
						instance, null, null);
	...
	protected ArrayList<TActivityInfo> getRecoveriesInScope(
			TInstanceInfo instance, TScopeInfo scope,
			ArrayList<TActivityInfo> recoveries) throws Exception {
		if (scope == null)
			scope = _management.getScopeInfoWithActivity(
					instance.getRootScope().getSiid(), true).getScopeInfo();
		if (recoveries == null)
		...
		for (TScopeRef ref : scope.getChildren().getChildRefList()) {
			TScopeInfo child = _management.getScopeInfoWithActivity(
					ref.getSiid(), true).getScopeInfo();
			if (child != null)
