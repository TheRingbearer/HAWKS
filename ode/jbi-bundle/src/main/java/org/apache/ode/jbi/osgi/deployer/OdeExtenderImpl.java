...
public class OdeExtenderImpl implements OdeExtender, InitializingBean,
	...
	private static final Logger LOG = Logger.getLogger(OdeExtenderImpl.class
			.getName());
	private static final String SU_ACTIVATOR_DEPRECATED = ServiceUnitActivator.class
			.getName();
	...
	public void afterPropertiesSet() throws Exception {
		// Create our executor.
		executor = Executors
				.newSingleThreadExecutor(new OdeExtenderThreadFactory(
						"ODE Extender"));
		...
		this.tracker = new BundleTracker(this.bundleContext, stateMask,
				new ODEBundleTrackerCustomizer());
		this.tracker.open();
	}
		...
		if (entry != null) {
			...
			if (bpelFiles != null && bpelFiles.hasMoreElements()) {
				...
				if (bundle.getSymbolicName() != null) {
					...
					if (headers == null
							|| (activator = headers
									.get(Constants.BUNDLE_ACTIVATOR)) == null
						...
						LOG.warning("Ignoring ODE bundle "
								+ bundle.getSymbolicName()
								+ " which has custom activator: " + activator);
				} else
	...
	private class ODEBundleTrackerCustomizer implements BundleTrackerCustomizer {
		...
		public Object addingBundle(Bundle bundle, BundleEvent event) {
			...
			if (isBPELBundle(bundle)) {
				...
				if (deployer == null)
					bundles.put(bundle.getBundleId(),
							deployer = new OdeDeployedBundle(bundle,
									OdeExtenderImpl.this));
				result = deployer;
		...
		public final void run() {
			...
			try {
				...
				LOG.log(Level.WARNING, "Could not perform '"
						...
						+ "' operation on bundle: " + target.getName(), e);
			} finally {
