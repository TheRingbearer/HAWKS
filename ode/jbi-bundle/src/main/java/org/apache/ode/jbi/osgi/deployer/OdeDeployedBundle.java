...
public class OdeDeployedBundle {
	private static final Logger LOG = Logger.getLogger(OdeDeployedBundle.class
			.getName());
	...
	private File getRootDir() {
		if (this.rootDir == null && this.bundle.getBundleContext() != null)
			this.rootDir = this.bundle.getBundleContext().getDataFile(
					"bpelData/" + getDUName());
		return this.rootDir;
	}
	...
	void doStart() throws Exception {
		...
		// Do deploy.
		this.deploy(this.bundle, this.odeRegistrationService.getOdeComponent()
				.getServiceUnitManager(), forceDeploy, needUpdate);
	}

	void doStop() throws Exception {
		LOG.info("Stopping ODE service unit: " + getName());
		this.shutdown(this.bundle, this.odeRegistrationService
				.getOdeComponent().getServiceUnitManager());
	}
	...
	void doUninstall() throws Exception {
		...
		this.undeploy(this.bundle, this.odeRegistrationService
				.getOdeComponent().getServiceUnitManager());
	}
	...
	void doUpdate() throws Exception {
		...
		// We simply mark the update state of this bundle so that on next start,
		// we do a redeploy
		updated = true;
	}

	private void deploy(Bundle bundle, ServiceUnitManager suM,
			boolean forceDeploy, boolean undeploy) throws Exception {
		// Do deployment.
		...
		try {
			...
			Thread.currentThread().setContextClassLoader(
					new BundleClassLoader(suL, bundle));
			try {
				...
				LOG.log(Level.WARNING,
						"Deploy failed; could not deploy/start this bundle: "
								+ this.getName(), e);
				throw e;
			}
		} finally {
	...
	private boolean isDeployed(String processName) {
		...
		try {
			...
			LOG.log(Level.WARNING,
					"Could not determine deployment state for process: "
							+ processName, e);
		}
		return Result;
	}
	...
	private void shutdown(Bundle bundle, ServiceUnitManager suM)
			throws Exception {
		String duName = getDUName();
		if (suM != null) {
			suM.stop(duName);
	...
	private void undeploy(Bundle bundle, ServiceUnitManager suM)
			throws Exception {
		String duName = getDUName();
		if (suM != null) {
	...
	private void waitAvailable() throws InterruptedException {
		 ...
		 * initializing. This would be much cleaner if we could simply call
		 * "isStarted" on OdeLifeCycle, which maintains a started state.
		 * 
		...
		while (this.odeRegistrationService.getOdeComponent()
	...
	private boolean isStarted(Component odeComponent) {
		...
		try {
			...
			LOG.log(Level.WARNING,
					"Could not determine started state for ode component", e);
		}
		return Result;
	}
}
