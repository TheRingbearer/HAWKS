 ...
 * Java. In particular, parallel composition is modelled as sequential Java
 * invocation and if-else are modelled using Java's if-else statement. Note also
 * that the scoping rules for channel names are simply the Java object
 * visibility rules.
 */
public interface JacobThread {
	 ...
	 * ChannelListener)</code> corresponds to the process term
	 * <code> x ? { ChannelListener }</code>; if in the same expression the
	 * initial <code>replicate</code> parameter were instead set to
	 * <code>true</code>, corresponding term would be
