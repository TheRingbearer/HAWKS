...
public final class JacobVPU {
	 ...
	 */
	private static final JacobMessages __msgs = MessageBundle
			.getMessages(JacobMessages.class);

	/**
	 * Thread-local for associating a thread with a VPU. Needs to be stored in a
	 * stack to allow reentrance.
	 */
	 ...
	 */
	public ExecutionQueue _executionQueue;
	 ...
	 *            previously saved execution context
	 */
	 ...
	 * <code>P</code> is the previous process context and <code>Q</code> is the
	 * injected process. This method is equivalent to the parallel operator, but
	 * is intended to be used from outside of an active {@link JacobThread}.
	 */
	...
	private class JacobThreadImpl implements Runnable, JacobThread {
		...
		JacobThreadImpl(Continuation rqe) {
			...
			if (__log.isDebugEnabled()) {
				StringBuffer buf = new StringBuffer(_methodBody.getClass()
						.getName());
				buf.append('.');
		...
		public Channel message(Channel channel, Method method, Object[] args) {
			if (__log.isTraceEnabled()) {
				__log.trace(_cycle + ": " + channel + " ! " + method.getName()
						+ "(" + stringify(args) + ")");
			}
			...
			if (method.getReturnType() != void.class) {
				if (method.getReturnType() != SynchChannel.class) {
					throw new IllegalStateException(
							"ChannelListener method can only return SynchChannel: "
									+ method);
				}
		...
		/*
		 * UNUSED private DebugInfo fillDebugInfo() { // Some of the debug
		 * information is a bit lengthy, so lets not put // it in all the
		 * time... eh. DebugInfo frame = new DebugInfo();
		 * frame.setCreator(_source); Exception ex = new Exception();
		 * StackTraceElement[] st = ex.getStackTrace(); if (st.length > 2) {
		...
		public void run() {
			...
			assert _method != null;
			assert _method.getDeclaringClass().isAssignableFrom(
					_methodBody.getClass());
