...
public abstract class JacobObject implements Serializable {
	...
	// @stmz
	protected Boolean runner = false;
	 ...
	 * <code>Channel x = newChannel(XChannel.class) ...</code> is equivalent to
	 * <code>(new x) ... </code> in the process calculus.
	 */
	@SuppressWarnings("unchecked")
	...
	protected static void object(boolean replication,
			Set<ChannelListener> methodLists) {
		JacobVPU.activeJacobThread().object(replication,
				methodLists.toArray(new ChannelListener[methodLists.size()]));
	}

	protected static <T extends Channel> T replication(
			ChannelListener<T> methodList) {
