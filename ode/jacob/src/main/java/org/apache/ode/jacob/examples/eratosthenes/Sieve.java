...
/**
 * Sieve of Eratosthenes prime number generator. This class represents the
...
public class Sieve extends JacobRunnable {
	...
	/**
	 * A counter, aka integer stream generator; represents the following process
	 * term:
	 * 
	 * <pre>
	 * <em>
	...
	private static class Counter extends JacobRunnable {
		...
		public void run() {
			_out.val(_n, object(new SynchChannelListener(
	...
	/**
	 * Head extractor, takes the first element from a stream; this represent the
	 * process term:
	 * 
	 * <pre>
	 * <em>
	...
	private static final class Head extends JacobRunnable {
		...
		public Head(NaturalNumberStreamChannel in,
		...
		public void run() {
			object(new NaturalNumberStreamChannelListener(_in) {
				...
				public void val(final int n, final SynchChannel ret) {
					_primes.val(n, object(new SynchChannelListener(
							newChannel(SynchChannel.class)) {
	...
	/**
	 * A prime filter, filters out the nubmer in an input stream that are
	 * multiple of a prime. This represents the following process term:
	 * 
	 * <pre>
	 * <em>
		...
		public void run() {
			object(true, new NaturalNumberStreamChannelListener(_in) {
				...
				public void val(int n, final SynchChannel ret) {
					if (n % _prime != 0)
						_out.val(n, object(new SynchChannelListener(
								newChannel(SynchChannel.class)) {
	...
	public static void main(String args[]) {
		if (args.length != 1) {
			System.err
					.println("JACOB Sieve of Eratosthenes Prime Number Generator Demonstration");
			System.err.println("usage: java " + Sieve.class.getName()
					+ " requested-prime");
			System.err
					.println("  requested-prime = which prime to show (0->inf)");
			System.exit(1);
		} else {
