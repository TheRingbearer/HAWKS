...
public abstract class ChannelListener<CT extends Channel> extends JacobObject {
	...
	protected ChannelListener(CT channel) throws IllegalStateException {
		assert getClass().getSuperclass().getSuperclass() == ChannelListener.class : "Inheritance in ChannelListener classes not allowed!";
		if (channel == null) {
	...
	public Set<Method> getImplementedMethods() {
		if (_implementedMethods == null) {
			...
			getImplementedMethods(implementedMethods, getClass()
					.getSuperclass());
			_implementedMethods = Collections
					.unmodifiableSet(implementedMethods);
		}
