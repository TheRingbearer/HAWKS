...
public class ExecutionQueueImpl implements ExecutionQueue {
	...
	private static final Log __log = LogFactory
			.getLog(ExecutionQueueImpl.class);

	private ClassLoader _classLoader;

	public static ConcurrentHashMap<String, ObjectStreamClass> _classDescriptors = new ConcurrentHashMap<String, ObjectStreamClass>();

	/**
	 * Cached set of enqueued {@link Continuation} objects (i.e. those read
	 * using {@link #enqueueReaction(org.apache.ode.jacob.soup.Continuation)}).
	 ...
	 */
	// changed by Bo Ning (from private to public)
	public Set<Continuation> _reactions = new HashSet<Continuation>();
	...
	public void add(CommChannel channel) {
		...
		verifyNew(channel);
		ChannelFrame cframe = new ChannelFrame(channel.getType(),
				++_objIdCounter, channel.getType().getName(), channel
						.getDescription());
	...
	public void enqueueReaction(Continuation continuation) {
		if (__log.isTraceEnabled())
			__log.trace(ObjectPrinter.stringifyMethodEnter("enqueueReaction",
	...
	public Continuation dequeueReaction() {
		...
		//@hahnml: Changed while loop to enable the privileged scheduling of "executeImmediately" runnables
		Continuation temp_Cont = null;
		Continuation continuation = null;
		if (!_reactions.isEmpty()) {
			Iterator it = _reactions.iterator();
			while (it.hasNext()) {
				Continuation tmp = (Continuation) it.next();
				if (!tmp.getClosure().getRunner()) {
					temp_Cont = tmp;
					if (tmp.getClosure().getExecuteImmediately()) {
						continuation = tmp;
						break;
					}
					if (continuation == null) {
						continuation = temp_Cont;
					}
				}
				if (temp_Cont == null) {
					//@hahnml: Buffer first runner
					temp_Cont = tmp;
				}
			}
			
			//@hahnml: If only runners are in the reactions, take the buffered first one
			if (continuation == null) {
				continuation = temp_Cont;
			}
			
			_reactions.remove(continuation);
		}
		return continuation;
	}

	public boolean hasChannels() {
		for (Iterator<ChannelFrame> i = _channels.values().iterator(); i
				.hasNext();) {
			return true;
		}
		return false;
			...
			if (comm instanceof CommSend) {
				if (chnlFrame.replicatedSend) {
					...
					throw new IllegalStateException(
							"Send attempted on channel containing replicated send! Channel= "
									+ comm.getChannel());
				}
				...
				MessageFrame mframe = new MessageFrame(commGroupFrame,
						chnlFrame, commSend.getMethod().getName(), commSend
								.getArgs());
				...
				if (chnlFrame.replicatedRecv) {
					// TODO: JACOB "bad-process" ex
					throw new IllegalStateException(
							"Receive attempted on channel containing replicated receive! Channel= "
									+ comm.getChannel());
				}
				if (group.isReplicated())
					chnlFrame.replicatedRecv = true;
				CommRecv commRecv = (CommRecv) comm;
				ObjectFrame oframe = new ObjectFrame(commGroupFrame, chnlFrame,
	...
	public String createExport(CommChannel channel) {
		if (__log.isTraceEnabled())
			__log.trace(ObjectPrinter.stringifyMethodEnter("createExport",
	...
	public CommChannel consumeExport(String exportId) {
		if (__log.isTraceEnabled()) {
			__log.trace(ObjectPrinter.stringifyMethodEnter("consumeExport",
					new Object[] { "exportId", exportId }));
		}
	...
	public void read(InputStream iis) throws IOException,
	...
	private void matchCommunications(CommChannel channel) {
		if (__log.isTraceEnabled()) {
			__log
					.trace(ObjectPrinter.stringifyMethodEnter(
							"matchCommunications", new Object[] { "channel",
									channel }));
		}
		ChannelFrame cframe = _channels.get(channel.getId());
		while (cframe != null && !cframe.msgFrames.isEmpty()
				&& !cframe.objFrames.isEmpty()) {
			...
			Continuation continuation = new Continuation(oframe._continuation,
					oframe._continuation.getMethod(mframe.method), mframe.args);
			if (__log.isInfoEnabled()) {
				continuation.setDescription(channel + " ? {...} | " + channel
						+ " ! " + mframe.method + "(...)");
			}
	 ...
	 */
	private void verifyNew(ExecutionQueueObject so)
			throws IllegalArgumentException {
		if (so.getId() != null)
			throw new IllegalArgumentException("The object " + so
					+ " is not new!");
	}
	...
	private static class ObjectFrame extends CommFrame implements
		...
		public ObjectFrame(CommGroupFrame commGroupFrame,
				ChannelFrame channelFrame, ChannelListener continuation) {
	...
	private static class MessageFrame extends CommFrame implements
		...
		public MessageFrame(CommGroupFrame commFrame,
				ChannelFrame channelFrame, String method, Object[] args) {
	...
	private class ExecutionQueueOutputStream extends ObjectOutputStream {
		...
		public ExecutionQueueOutputStream(OutputStream outputStream)
				throws IOException {
		...
		protected Object replaceObject(Object obj) throws IOException {
			...
			if (obj instanceof org.apache.ode.jacob.Channel) {
				CommChannel commChannel = (CommChannel) ChannelFactory
						.getBackend((Channel) obj);
				_serializedChannels.add(commChannel.getId());
				return new ChannelRef(commChannel.getType(),
						(Integer) commChannel.getId());
	...
	public class ExecutionQueueInputStream extends ObjectInputStream {
		...
		protected Object resolveObject(Object obj) throws IOException {
			...
			if (obj instanceof ChannelRef) {
				...
				resolved = ChannelFactory.createChannel(channel, channel
						.getType());
			} else if (_replacementMap != null
					&& _replacementMap.isReplacement(obj)) {
				resolved = _replacementMap.getOriginal(obj);
				if (__log.isDebugEnabled()) {
					__log.debug("ReplacementMap: getOriginal(" + obj + ") = "
							+ resolved);
				}
			} else {
				resolved = obj;
