...
/**
 * JUnit {@link TestCase} of the ODE BPEL compiler. This test case provides is
 * not very complex, it simply ensures that the given BPEL input compiles
...
class GoodCompileTCase extends TestCase implements CompileListener {
	...
	protected void setUp() throws Exception {
		...
		_compiler.setOutputStream(new ByteArrayOutputStream(
				StreamUtils.DEFAULT_BUFFER_SIZE));
		_errors.clear();
	}

	public void runTest() throws Exception {
		try {
			File bpelFile = new File(getClass().getResource(_bpel).toURI()
					.getPath());
