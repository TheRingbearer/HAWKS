...
class ReceiveGenerator extends PickReceiveGenerator {
	...
	public void compile(OActivity output, Activity src) {
		...
		OPickReceive.OnMessage onMessage = compileOnMessage(
				rcvDef.getVariable(), rcvDef.getPartnerLink(),
				rcvDef.getOperation(), rcvDef.getMessageExchangeId(),
				rcvDef.getPortType(), rcvDef.isCreateInstance(),
				rcvDef.getCorrelations(), rcvDef.getRoute(), opick.getXpath()
						+ "/onMessage[1]");

		// @hahnml
		String xpath = onMessage.getXpath() + "/empty[1]";
		if (ProcessModelChangeRegistry.getRegistry().isModelChanged()) {
			int id = ProcessModelChangeRegistry.getRegistry().getCorrectID(
					xpath);
			onMessage.activity = new OEmpty(_context.getOProcess(), opick, id);
		} else {
			onMessage.activity = new OEmpty(_context.getOProcess(), opick);
		}
		onMessage.activity.setXpath(xpath);

		// @stmz: empty activity is artificially created
		onMessage.activity.setArt(true);
		opick.onMessages.add(onMessage);
		
		// AH:
		opick.isReceiveActivity = true;
		// AH: end
	}
