...
/**
 * Basic implementation of the {@link ResourceFinder} interface. Resolves URIs
 * relative to a base URI specified at the time of construction.
 * 
...
public class DefaultResourceFinder implements ResourceFinder {
	private static final Log __log = LogFactory
			.getLog(DefaultResourceFinder.class);

	private File _relativeDir;
	private File _absoluteDir;

	/**
	 * Default constructor: resolve relative URIs against current working
	 * directory.
	 */
	...
	private InputStream openFileResource(URI uri) throws MalformedURLException,
		...
		if (__log.isDebugEnabled()) {
			__log.debug("openResource: uri=" + uri + " relativeDir="
					+ _relativeDir + " absoluteDir=" + _absoluteDir);
		}
