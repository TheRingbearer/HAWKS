...
class JaxenBpelHandler extends JaxenHandler {
	...
	JaxenBpelHandler(String bpelNsUri, OXPath10Expression out,
			NSContext nsContext, CompilerContext cctx) {
   ...
   */
	public void endFunction() {
		...
		if (ns == null) {
			throw new CompilationException(__msgs.errUndeclaredFunctionPrefix(
					prefix, c.getFunctionName()));
		} else if (isBpelNamespace(ns)) {
			if (Constants.EXT_FUNCTION_GETVARIABLEDATA.equals(c
					.getFunctionName())) {
				compileGetVariableData(c);
			} else if (Constants.EXT_FUNCTION_GETVARIABLEPROPRTY.equals(c
					.getFunctionName())) {
				compileGetVariableProperty(c);
			} else if (Constants.EXT_FUNCTION_GETLINKSTATUS.equals(c
					.getFunctionName())) {
				compileGetLinkStatus(c);
			} else if (Constants.EXT_FUNCTION_DOXSLTRANSFORM.equals(c
					.getFunctionName())) {
				compileDoXslTransform(c);
			} else {
				throw new CompilationException(__msgs.errUnknownBpelFunction(c
						.getFunctionName()));
			}
		}
	}
	...
	private void compileGetLinkStatus(FunctionCallExpr c)
		...
		if (params.size() != 1) {
			throw new CompilationException(__msgs.errInvalidNumberOfArguments(c
					.getFunctionName()));
		}
	...
	/**
	 * Compile a
	 * <code>bpws:getVariableData(...)</em> function call. Note that all arguments
	 ...
	 * 
	 * @param c
	 *            {@link FunctionCallExpr} for this invocation
	 * @throws CompilationException
	 */
	private void compileGetVariableData(FunctionCallExpr c)
		...
		String partname = params.size() > 1 ? getLiteralFromExpression((Expr) params
				.get(1)) : null;
		String locationstr = params.size() > 2 ? getLiteralFromExpression((Expr) params
				.get(2)) : null;

		OScope.Variable var = _cctx.resolveVariable(varname);
		OMessageVarType.Part part = partname != null ? _cctx.resolvePart(var,
				partname) : null;
		...
		// @hahnml
		OXPath10Expression.OSigGetVariableData vdat = null;
		String xpath = BpelCompiler.getXPath("OXPath10Expression/OSigGetVariableData", "");
		if (ProcessModelChangeRegistry.getRegistry().isModelChanged()) {
			int id = ProcessModelChangeRegistry.getRegistry().getCorrectID(
					xpath);
			vdat = new OXPath10Expression.OSigGetVariableData(
					_cctx.getOProcess(), var, part, location, id);
		} else {
			vdat = new OXPath10Expression.OSigGetVariableData(
		...
		_out.addGetVariableDataSig(varname, partname, locationstr, vdat);
	...
	private void compileDoXslTransform(FunctionCallExpr c)
			throws CompilationException {
		List params = c.getParameters();
	...
	private String getLiteralFromExpression(Expr expr)
		...
		throw new CompilationException(
				__msgs.errLiteralExpected(expr.getText()));
	}
