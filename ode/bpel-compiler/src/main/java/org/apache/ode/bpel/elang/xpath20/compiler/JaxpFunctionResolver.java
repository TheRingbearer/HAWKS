...
public class JaxpFunctionResolver implements XPathFunctionResolver {
	...
	private static final XPathMessages __msgs = MessageBundle
	...
	public JaxpFunctionResolver(CompilerContext cctx,
			OXPath20ExpressionBPEL20 out, NSContext nsContext, String bpelNS) {
		_cctx = cctx;
		_bpelNS = bpelNS;
	...
	public XPathFunction resolveFunction(QName functionName, int arity) {
		if (functionName.getNamespaceURI() == null) {
			throw new WrappedResolverException("Undeclared namespace for "
					+ functionName);
			...
			if (Constants.EXT_FUNCTION_GETVARIABLEPROPERTY.equals(localName)) {
				...
				throw new WrappedResolverException(
						__msgs.errUnknownBpelFunction(localName));
			}
			...
			if (Constants.NON_STDRD_FUNCTION_SPLIT_TO_ELEMENTS
				...
				return new CombineUrl();
					...
					.equals(localName)
					|| Constants.NON_STDRD_FUNCTION_EXPAND_TEMPLATE
							.equals(localName)
	...
	public class DoXslTransform implements XPathFunction {
		public Object evaluate(List params) throws XPathFunctionException {
			...
			try {
				XslTransformHandler.getInstance().parseXSLSheet(
						_cctx.getOProcess().getQName(), xslSheet.uri,
						xslSheet.sheetBody,
						new XslCompileUriResolver(_cctx, _out));
			} catch (Exception e) {
				throw new CompilationException(__msgs.errXslCompilation(xslUri,
						e.toString()));
			}
