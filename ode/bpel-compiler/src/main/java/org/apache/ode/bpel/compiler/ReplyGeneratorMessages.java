...
public class ReplyGeneratorMessages extends CompilationMessageBundle {
	...
	public CompilationMessage errOutputVariableMustBeSpecified() {
		return this
				.formatCompilationMessage("The <reply> must specify a reply message.");
	}
	 ...
	 * "{1}".
	 */
	public CompilationMessage errUndeclaredFault(String faultName,
			String operationName) {
		return this.formatCompilationMessage(
				"The <reply> activity has an undeclared fault"
						+ "\"{0}\" for operation \"{1}\".", faultName,
				operationName);
	}
