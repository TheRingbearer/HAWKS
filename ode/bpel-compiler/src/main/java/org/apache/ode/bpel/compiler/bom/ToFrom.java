...
public class ToFrom extends BpelObject {
	...
	public VariableVal getAsVariableVal() {
		if (getAttribute("variable", null) != null) {
			if (is11())
				return new VariableVal11(getElement());
			else
	...
	/**
	 * Cast this tofrom to an "extension" to/from. This is NOT part of the BPEL
	 * spec, and is used to provide access to custom extensions (for example
	 * reading/writing SOAP message headers)... Yes. it's evil.
	 * 
	 * @author mszefler
	 * @return the object cast to {@link ExtensionVal} if appropriate, null
	 *         otherwise.
	 */
	...
	/**
	 * Test whether this to/from is an "extension" to-from (i.e. does it have
