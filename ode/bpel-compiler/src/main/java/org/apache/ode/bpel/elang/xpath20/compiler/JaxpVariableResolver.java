 ...
 * It always returns an empty string which allows execution of XPath expressions
 * even if we don't have any values yet. This way we can easily rule out invalid
...
public class JaxpVariableResolver implements XPathVariableResolver {
	...
	public Object resolveVariable(QName variableName) {
		...
		if ("ode".equals(variableName.getPrefix())
				|| Namespaces.ODE_EXTENSION_NS.equals(variableName
						.getNamespaceURI())) {
	...
	private Object extractValue(OScope.Variable var, OVarType varType) {
		if (varType instanceof OXsdTypeVarType) {
			...
			// MR That's an ugly hack but otherwise, xpath compilation doesn't
			// work
			if (((OMessageVarType) varType).parts.size() == 0)
				throw new WrappedResolverException(
						__msgs.errExpressionMessageNoPart(var.name));
			return extractValue(var, ((OMessageVarType) varType).parts.values()
					.iterator().next().type);
		}
		return "";
	}
