...
class AssignGenerator extends DefaultActivityGenerator {
	...
	private static final AssignGeneratorMessages __cmsgs = MessageBundle
			.getMessages(AssignGeneratorMessages.class);
	...
	public OActivity newInstance(Activity src) {
		...
		if (ProcessModelChangeRegistry.getRegistry().isModelChanged()) {
			int id = ProcessModelChangeRegistry.getRegistry().getCorrectID(
					xpath);
			assign = new OAssign(_context.getOProcess(), _context.getCurrent(),
					id);
		} else {
			assign = new OAssign(_context.getOProcess(), _context.getCurrent());
		}
	...
	public void compile(OActivity dest, Activity source) {
		...
		for (AssignOperation operation : ad.getOperations()) {
			if (operation instanceof Copy) {
				...
				OAssign.Copy ocopy = null;

				// @hahnml
				if (ProcessModelChangeRegistry.getRegistry().isModelChanged()) {
					int id = ProcessModelChangeRegistry.getRegistry()
							.getCorrectID(
									BpelCompiler.getXPath(scopy.getElement()));
					ocopy = new OAssign.Copy(_context.getOProcess(), id);
				} else {
					ocopy = new OAssign.Copy(_context.getOProcess());
				}

				ocopy.keepSrcElementName = scopy.isKeepSrcElement();
				ocopy.ignoreMissingFromData = scopy.isIgnoreMissingFromData();
				ocopy.ignoreUninitializedFromVariable = scopy
				...
				ocopy.debugInfo = new DebugInfo(_context.getSourceLocation(),
						scopy.getLineNo(), source.getExtensibilityElements());
				ocopy.setXpath(BpelCompiler.getXPath(scopy.getElement()));
				try {
					if (scopy.getTo() == null)
						throw new CompilationException(__cmsgs
								.errMissingToSpec().setSource(scopy));
					Object[] toResultType = new Object[1];
					ocopy.to = compileTo(scopy.getTo(), toResultType);

					if (scopy.getFrom() == null)
						throw new CompilationException(__cmsgs
								.errMissingFromSpec().setSource(scopy));
				...
				OAssign.ExtensionAssignOperation oext = null;
				String xpath = BpelCompiler.getXPath(sop.getElement());
				// @hahnml
				if (ProcessModelChangeRegistry.getRegistry().isModelChanged()) {
					int id = ProcessModelChangeRegistry.getRegistry()
							.getCorrectID(xpath);
				...
				oext.debugInfo = new DebugInfo(_context.getSourceLocation(),
	...
	private void verifyCopy(OAssign.Copy ocopy) {
		...
		if (ocopy.to instanceof OAssign.VariableRef
			...
			if (!tlvalue.equals(trvalue))
				throw new CompilationException(
						__cmsgs.errMismatchedMessageAssignment(lvar, tlvalue,
								rvar, trvalue));
		...
		// If Message->Non-Message copy
				...
				&& ((OAssign.VariableRef) ocopy.from).isMessageRef()
				&& (!(ocopy.to instanceof OAssign.VariableRef) || !((OAssign.VariableRef) ocopy.to)
						.isMessageRef())) {
			String rval = ((OAssign.VariableRef) ocopy.from).variable.name;
			throw new CompilationException(
					__cmsgs.errCopyFromMessageToNonMessage(rval));
		...
		// If Non-Message->Message copy
			...
			throw new CompilationException(
					__cmsgs.errCopyToMessageFromNonMessage(lval));
		}
		...
		// If *->Partner Link copy
				...
				&& !((OAssign.PartnerLinkRef) ocopy.to).partnerLink
						.hasPartnerRole()) {
			String lval = ((OAssign.PartnerLinkRef) ocopy.to).partnerLink
					.getName();
			throw new CompilationException(
					__cmsgs.errCopyToUndeclaredPartnerRole(lval));
		}

		// If Partner Link->* copy
		else if (ocopy.from instanceof OAssign.PartnerLinkRef) {
			if (((OAssign.PartnerLinkRef) ocopy.from).isMyEndpointReference
					&& !((OAssign.PartnerLinkRef) ocopy.from).partnerLink
							.hasMyRole()) {
				String lval = ((OAssign.PartnerLinkRef) ocopy.from).partnerLink
						.getName();
				throw new CompilationException(
						__cmsgs.errCopyFromUndeclaredPartnerRole(lval, "myRole"));
			}
			if (!((OAssign.PartnerLinkRef) ocopy.from).isMyEndpointReference
					&& !((OAssign.PartnerLinkRef) ocopy.from).partnerLink
							.hasPartnerRole()) {
	...
	private OAssign.RValue compileFrom(From from, Object requestedResultType) {
		...
		try {
			if (from.isExtensionVal()) {
				...
				OAssign.PropertyRef pref = null;

				// @hahnml; Use the XPath of the from XML element to identify
				// this OBase object
				String xpath = BpelCompiler.getXPath(from.getElement());
				if (ProcessModelChangeRegistry.getRegistry().isModelChanged()) {
					int id = ProcessModelChangeRegistry.getRegistry()
							.getCorrectID(xpath);
					pref = new OAssign.PropertyRef(_context.getOProcess(), id);
				} else {
					pref = new OAssign.PropertyRef(_context.getOProcess());
				}
				pref.setXpath(xpath);

				PropertyVal pval = from.getAsPropertyVal();
				pref.variable = _context.resolveVariable(pval.getVariable());
				pref.propertyAlias = _context.resolvePropertyAlias(
						pref.variable, pval.getProperty());
				return pref;
			} else if (from.isVariableVal()) {
				VariableVal vv = from.getAsVariableVal();
				OAssign.VariableRef vref = null;
				...
				if (ProcessModelChangeRegistry.getRegistry().isModelChanged()) {
					int id = ProcessModelChangeRegistry.getRegistry()
							.getCorrectID(xpath);
					vref = new OAssign.VariableRef(_context.getOProcess(), id);
				} else {
					vref = new OAssign.VariableRef(_context.getOProcess());
				}
				vref.setXpath(xpath);
				...
				if (vv.getHeader() != null) {
					...
					if (vref.headerPart == null) {
						// @hahnml
						xpath = vref.getXpath() + "/headerPart";
						if (ProcessModelChangeRegistry.getRegistry()
								.isModelChanged()) {
							int id = ProcessModelChangeRegistry.getRegistry()
									.getCorrectID(xpath);
							vref.headerPart = new OMessageVarType.Part(
									_context.getOProcess(), vv.getHeader(), null, id);
						} else {
							vref.headerPart = new OMessageVarType.Part(
									_context.getOProcess(), vv.getHeader(), null);
						}
						vref.headerPart.setXpath(xpath);
					}
				...
				OAssign.PartnerLinkRef plref = null;
				...
				if (ProcessModelChangeRegistry.getRegistry().isModelChanged()) {
					int id = ProcessModelChangeRegistry.getRegistry()
							.getCorrectID(xpath);
					plref = new OAssign.PartnerLinkRef(_context.getOProcess(),
							id);
				} else {
					plref = new OAssign.PartnerLinkRef(_context.getOProcess());
				}
				plref.setXpath(xpath);

				plref.partnerLink = _context.resolvePartnerLink(plv
						.getPartnerLink());
				...
				OAssign.Expression asExpression = null;
				...
				if (ProcessModelChangeRegistry.getRegistry().isModelChanged()) {
					int id = ProcessModelChangeRegistry.getRegistry()
							.getCorrectID(xpath);
					asExpression = new OAssign.Expression(
							_context.getOProcess(), _context.compileExpr(
									from.getAsExpression(), null,
									requestedResultType, new Object[1]), id);
				} else {
					asExpression = new OAssign.Expression(
							_context.getOProcess(), _context.compileExpr(
									from.getAsExpression(), null,
				...
				asExpression.setXpath(xpath);

				return asExpression;
			}

			throw new CompilationException(__cmsgs.errUnkownFromSpec()
					.setSource(from));
	...
	private RValue compileExtensionVal(ExtensionVal extVal) {
		OAssign.DirectRef dref = null;

		// @hahnml; Use the XPath of the from/to XML element to identify this
		// OBase object
		String xpath = BpelCompiler.getXPath(extVal.getElement()
				.getParentNode());
		if (ProcessModelChangeRegistry.getRegistry().isModelChanged()) {
			int id = ProcessModelChangeRegistry.getRegistry().getCorrectID(
					xpath);
			dref = new OAssign.DirectRef(_context.getOProcess(), id);
		} else {
			dref = new OAssign.DirectRef(_context.getOProcess());
		}
		dref.setXpath(xpath);
	...
	private OAssign.RValue compileLiteral(LiteralVal from) {
		...
		// @hahnml; Use the XPath of the from/to XML element to identify this
		// OBase object
		String xpath = BpelCompiler.getXPath(literal.getParentNode());
		OAssign.Literal oLiteral = null;
		if (ProcessModelChangeRegistry.getRegistry().isModelChanged()) {
			int id = ProcessModelChangeRegistry.getRegistry().getCorrectID(
					xpath);
			oLiteral = new OAssign.Literal(_context.getOProcess(), newDoc, id);
		} else {
			oLiteral = new OAssign.Literal(_context.getOProcess(), newDoc);
		}
		oLiteral.setXpath(xpath);
	...
	private OAssign.LValue compileTo(To to, Object[] resultType) {
		...
		try {
			if (to.isPropertyVal()) {
				...
				// @hahnml; Use the XPath of the from XML element to identify
				// this OBase object
				String xpath = BpelCompiler.getXPath(to.getElement());
				if (ProcessModelChangeRegistry.getRegistry().isModelChanged()) {
					int id = ProcessModelChangeRegistry.getRegistry()
							.getCorrectID(xpath);
					pref = new OAssign.PropertyRef(_context.getOProcess(), id);
				...
				pref.variable = _context.resolveVariable(to.getAsPropertyVal()
						.getVariable());
				pref.propertyAlias = _context.resolvePropertyAlias(
						pref.variable, to.getAsPropertyVal().getProperty());
				return pref;
					...
					if (vref.headerPart == null) {
						...
						if (ProcessModelChangeRegistry.getRegistry()
							...
							int id = ProcessModelChangeRegistry.getRegistry()
									.getCorrectID(xpath);
							vref.headerPart = new OMessageVarType.Part(
									_context.getOProcess(), to.getAsVariableVal()
											.getHeader(), null, id);
						} else {
							vref.headerPart = new OMessageVarType.Part(
									_context.getOProcess(), to.getAsVariableVal()
				...
				plref.partnerLink = _context.resolvePartnerLink(to
						.getAsPartnerLinkVal().getPartnerLink());
				return plref;
			} else if (to.getAsExpression() != null) {
				OAssign.LValueExpression lValueExpr = null;
				...
				if (ProcessModelChangeRegistry.getRegistry().isModelChanged()) {
					int id = ProcessModelChangeRegistry.getRegistry()
							.getCorrectID(xpath);
					lValueExpr = new OAssign.LValueExpression(
							_context.getOProcess(), _context.compileLValueExpr(
									to.getAsExpression(), null, null,
									resultType), id);
				} else {
					lValueExpr = new OAssign.LValueExpression(
							_context.getOProcess(), _context.compileLValueExpr(
				...
				lValueExpr.setXpath(xpath);

				return lValueExpr;
			}

			throw new CompilationException(__cmsgs.errUnknownToSpec()
					.setSource(to));
		} catch (CompilationException ce) {
			if (ce.getCompilationMessage().source == null)
