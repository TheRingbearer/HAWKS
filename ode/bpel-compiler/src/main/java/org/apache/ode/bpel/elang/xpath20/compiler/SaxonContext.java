...
/**
 * Hooks on Saxon StandaloneContext to be notified when the compilation finds
 * some variables and functions. This allows us to prepare the OXpathExpression
 * with variable references and all the things needed at runtime.
 * 
...
public class SaxonContext extends JAXPXPathStaticContext {
	...
	public SaxonContext(Configuration config, JaxpVariableResolver varResolver,
		...
		for (Object lib : libList) {
			...
			if (flib instanceof XPathFunctionLibrary)
				xpathLib = (XPathFunctionLibrary) flib;
		}
		if (xpathLib != null)
			libList.remove(xpathLib);
		OdeXPathFunctionLibrary oxpfl = new OdeXPathFunctionLibrary(
				funcResolver);
	...
	// The following methods don't apply to the JAXPXPathStaticContext
	// interface.

	// public Variable declareVariable(String qname, Object initialValue) throws
	// XPathException {
	// String prefix;
	// String localName;
