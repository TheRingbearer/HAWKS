...
public class BpelObject implements SourceLocation {
	...
	public static final QName ATTR_LINENO = new QName(
			"urn:org.apache.ode.bpel.compiler", "lineno");
	...
	public Map<QName, Object> getExtensibilityElements() {
		// We consider anything that is not in the namespace of this element to
		// be an
		...
		for (BpelObject child : getChildren()) {
			if (child.getType().getNamespaceURI() != null
					&& !child.getType().getNamespaceURI()
							.equals(getType().getNamespaceURI()))
		...
		for (int i = 0; i < nnm.getLength(); ++i) {
			...
			if (n.getNamespaceURI() != null
					&& !n.getNamespaceURI().equals(getType().getNamespaceURI()))
				ee.put(new QName(n.getNamespaceURI(), n.getLocalName()),
						n.getTextContent());
		}
		return ee;
	...
	public Element getFirstExtensibilityElementElement() {
		...
		for (int i = 0; i < nl.getLength(); ++i) {
			...
			if (node.getNodeType() == Node.ELEMENT_NODE
					&& !getType().getNamespaceURI().equals(
	...
	public boolean is11() {
		return getType().getNamespaceURI() != null
				&& (getType().getNamespaceURI().equals(
						Bpel11QNames.NS_BPEL4WS_2003_03) || getType()
	...
	public boolean is20Draft() {
		return getType().getNamespaceURI() != null
				&& (getType().getNamespaceURI().equals(
						Bpel20QNames.NS_WSBPEL2_0) || getType()
						.getNamespaceURI().equals(
	...
	protected List<BpelObject> getChildren(final QName type) {
		return CollectionsX.filter(new ArrayList<BpelObject>(), getChildren(),
				new MemberOfFunction<BpelObject>() {
					@Override
	...
	protected BpelObject getFirstChild(final QName type) {
		return CollectionsX.find_if(getChildren(),
				new MemberOfFunction<BpelObject>() {
					@Override
					public boolean isMember(BpelObject o) {
						return o.getType().equals(type);
	...
	protected String getAttribute(QName name, String dflt) {
		String val = _element.getAttributeNS(name.getNamespaceURI(),
				name.getLocalPart());
	...
	protected <T> T getAttribute(String attrName,
			Map<String, T> suppressJoinFailure, T notset) {
	...
	private void initNSContext(Element el) {
		if (el.getParentNode() != null
				&& el.getParentNode().getNodeType() == Node.ELEMENT_NODE)
	...
	public String getTextValue() {
		...
		for (Node n = getElement().getFirstChild(); n != null; n = n
				.getNextSibling())
