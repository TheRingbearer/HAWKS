...
public class AssignGeneratorMessages extends CompilationMessageBundle {
	 ...
	 */
	public CompilationMessage errMismatchedMessageAssignment(String lvar,
			QName tlvalue, String rval, QName trvalue) {
		return this
				.formatCompilationMessage(
						"Copy to message variable {0} of type {1} from message {2}"
								+ " of type {3} is not permitted (the message types do not match).",
						lvar, tlvalue, rval, trvalue);
	}

	/**
	 * Copy to partner link {0} requires the partnerRole to be defined on
	 * partner link.
	 */
	...
	/**
	 * Copy from partner link {0} with enpoint reference {1} requires the
	 * corresponding role to be defined on partner link.
	 */
	public CompilationMessage errCopyFromUndeclaredPartnerRole(String rval,
			String epr) {
		return this
				.formatCompilationMessage(
	...
	public CompilationMessage errUnkownFromSpec() {
		return this
				.formatCompilationMessage("From-spec format is unrecognized.");
	}
	...
	public CompilationMessage errMissingExtensionAssignOperationElement() {
		return this
				.formatCompilationMessage("Extensibility element in <extensionAssignOperation> is missing.");
	}

	/** ExtensionAssignOperation's nested element missing */
	public CompilationMessage errUndeclaredExtensionAssignOperation() {
		return this
				.formatCompilationMessage("Extension namespace of <extensionAssignOperation> has not been declared.");
	}

	/** Draft extensibleAssign is not supported. */
	public CompilationMessage errExtensibleAssignNotSupported() {
		return this
				.formatCompilationMessage("ExtensibleAssign is not supported, please upgrade to BPEL 2.0 final.");
	}
}
