...
/**
 * <p>
 * Wrapper for {@link org.apache.ode.bpel.compiler.BpelCompiler}
 * implementations, providing basic utility methods and auto-detection of BPEL
 * version.
 * </p>
 * s
 */
public class BpelC {
	...
	private void invalidate() {
		...
		XslTransformHandler.getInstance().setErrorListener(
				new net.sf.saxon.StandardErrorListener());
	}
	...
	/**
	 * Configures the compiler to run a dry compilation, doesn't generate the
	 * produced compiled process.
	 * 
	 * @param dryRun
	 */
	...
	/**
	 * <p>
	 * Tell the compiler how to locate WSDL imports for a BPEL process. Setting
	 * this to <code>null</code> will cause the default behavior.
	 * </p>
	 * 
	 * @param finder
	 *            the {@link ResourceFinder} implementation to use.
	 */
	...
	/**
	 * Register a "global" WSDL import for compilation. This is used to specify
	 * WSDL imports for BPEL 1.1 processes that do not support the
	 * <code>&lt;import&gt;</code> BPEL construct.
	 * 
	 * @param wsdl
	 *            the WSDL URI (resolvable against the resource repository)
	 */
	...
	/**
	 * Set the output stream to which the compiled representation will be
	 * generated.
	 * 
	 * @param os
	 *            compiled representation output stream
	 */
	...
	public void setBaseDirectory(File baseDir) {
		...
		if (!baseDir.exists())
			throw new IllegalArgumentException("Directory " + baseDir
					+ " does not exist");
		_suDir = baseDir;
	}
	 ...
	 * 
	 * @throws IOException
	 *             if one occurs while processing (e.g., getting imports) or
	 *             writing output.
	...
	public void compile(final Process process, String outputPath, long version)
		...
		if (_wsdlFinder != null) {
			...
			wf = new DefaultResourceFinder(_bpelFile.getAbsoluteFile()
					.getParentFile(), suDir.getAbsoluteFile());
		}

		CompileListener clistener = new CompileListener() {
			public void onCompilationMessage(
		...
		try {
			...
			case BPEL20:
				...
				if (_bpel11wsdl != null) {
					CompilationMessage cmsg = __cmsgs
							.warnWsdlUriIgnoredFor20Process();
			...
			case BPEL11:
				...
				compiler.setResourceFinder(wf);
				if (_bpel11wsdl != null) {
					compiler.addWsdlImport(new URI(_bpelFile.getName()),
							_bpel11wsdl, null);
				} else {
			...
			if (_compileProperties != null) {
				if (_compileProperties.get(PROCESS_CUSTOM_PROPERTIES) != null)
					compiler.setCustomProperties((Map<QName, Node>) _compileProperties
							.get(PROCESS_CUSTOM_PROPERTIES));
			}
		...
		try {
			...
			// @hahnml: Register the Compiler at the CompilerRegistry to reuse it during runtime to compile breakpoint conditions
			compiler.isCompileTime = false;
			CompilerRegistry.getInstance().registerCompiler(compiler);
		...
		if (!_dryRun) {
			if (outputPath != null) {
				...
				if (__log.isDebugEnabled()) {
					__log.debug("Writing compilation results to "
							+ _outputStream.getClass().getName());
				}
			} else {
				throw new IllegalStateException(
						"must setOutputStream() or setOutputDirectory()!");
			}

			try {
				Serializer fileHeader = new Serializer(
	...
	/**
	 * <p>
	 * Compile a BPEL process from a file. This method uses a
	 * {@link BpelObjectFactory} to parse the XML and then calls
	 * {@link #compile(Process,String)}.
	 * </p>
	 * 
	 * @param bpelFile
	 ...
	 */
	public void compile(File bpelFile, long version)
			throws CompilationException, IOException {
		if (__log.isDebugEnabled()) {
		...
		try {
			...
			CompilationMessage cmsg = __cmsgs.errBpelParseErr().setSource(
					new SourceLocationImpl(bpelFile.toURI()));
			this.invalidate();
