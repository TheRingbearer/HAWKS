 ...
 */
public class BpelExtensionSerializer implements ExtensionDeserializer,
		ExtensionSerializer {
	...
	public void marshall(Class arg0, QName arg1, ExtensibilityElement arg2,
			PrintWriter arg3, Definition arg4, ExtensionRegistry arg5)
	 ...
	 *      javax.wsdl.Definition, javax.wsdl.extensions.ExtensionRegistry)
	 */
	public ExtensibilityElement unmarshall(Class clsType, QName eltype,
		...
		BpelObject4WSDL obj;
		try {
			obj = (BpelObject4WSDL) _factory.createBpelObject(el,
					new URI(def.getDocumentBaseURI()));
	...
	/**
	 * Dereference a qualified name given in the "ns:name" form using the
	 * namespace context of a given element.
	 * 
	 * @param prefixedQNameStr
	 *            qualified name, represented as a prefixed string
	 * @param context
	 *            context element
	...
	protected static QName derefQName(String prefixedQNameStr, Element context)
		...
		return new QName(uri, prefixedQNameStr.substring(idx + 1,
				prefixedQNameStr.length()));
	}

	/**
	 * Ensure that a top-level extensibility element does not occur before other
	 * WSDL declarations (messages, port types, etc.).
	 * 
	 * @param el
	 *            the extensibility element
	 * @throws javax.wsdl.WSDLException
	 *             if the requirements of the WSDL schema are violated.
	 */
	static void validateExtensibilityElementContext(Element el)
			throws WSDLException {
		Node n = el.getParentNode();
		if (n == null || n.getNodeType() != Node.ELEMENT_NODE) {
			WSDLException we = new WSDLException(WSDLException.OTHER_ERROR,
					__msgs.msgCannotBeDocumentRootElement(DOMUtils
							.getNodeQName(el).toString()));
			we.setLocation(XPathUtils.getXPathExprFromNode(el));
			throw we;
		}
		Element def = (Element) n;
		if (def.getNamespaceURI() == null
				|| !def.getNamespaceURI().equals(DOMUtils.WSDL_NS)
				|| !def.getLocalName().equals(DOMUtils.WSDL_ROOT_ELEMENT)) {
			WSDLException we = new WSDLException(WSDLException.OTHER_ERROR,
					__msgs.msgMustBeChildOfDef(DOMUtils.getNodeQName(el)
							.toString()));
			we.setLocation(XPathUtils.getXPathExprFromNode(el));
			throw we;
		}
		/*
		 * NOTE: This check was originally put in to match the W3C version of
		 * the WSDL schema, but the WS-I version is more permissive. Leaving
		 * this check out complies with the WS-I version, which is preferable.
		 */
		// n = el.getNextSibling();
		// while (n != null) {
		// if (n.getNamespaceURI() != null &&
		// n.getNamespaceURI().equals(DOMUtils.WSDL_NS)) {
