...
class WaitGenerator extends DefaultActivityGenerator {
	...
	private WaitGeneratorMessages _msgs = MessageBundle
			.getMessages(WaitGeneratorMessages.class);
	...
	public OActivity newInstance(Activity src) {
		...
		if (ProcessModelChangeRegistry.getRegistry().isModelChanged()) {
			int id = ProcessModelChangeRegistry.getRegistry().getCorrectID(
					xpath);
			wait = new OWait(_context.getOProcess(), _context.getCurrent(), id);
		} else {
			wait = new OWait(_context.getOProcess(), _context.getCurrent());
		}
		wait.setXpath(xpath);
	...
	public void compile(OActivity output, Activity src) {
		...
		if (waitDef.getFor() != null && waitDef.getUntil() == null) {
			...
			throw new CompilationException(_msgs
					.errWaitMustDefineForOrUntilDuration().setSource(waitDef));
		}
	}
}
