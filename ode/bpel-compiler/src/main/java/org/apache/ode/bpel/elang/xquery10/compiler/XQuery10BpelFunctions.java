...
/**
 * XQuery-Friendly definition of BPEL functions, which delegates calls to the
 * JAXP-based function resolver
 */
public class XQuery10BpelFunctions {
	public static final String USER_DATA_KEY_FUNCTION_RESOLVER = "function-resolver";
	private static final QName WSBPEL_GET_VARIABLE_PROPERTY = new QName(
			Namespaces.WSBPEL2_0_FINAL_EXEC,
			Constants.EXT_FUNCTION_GETVARIABLEPROPERTY);
	private static final QName WSBPEL_GET_VARIABLE_DATA = new QName(
			Namespaces.WSBPEL2_0_FINAL_EXEC,
			Constants.EXT_FUNCTION_GETVARIABLEDATA);
	private static final QName WSBPEL_GET_LINK_STATUS = new QName(
			Namespaces.WSBPEL2_0_FINAL_EXEC,
			Constants.EXT_FUNCTION_GETLINKSTATUS);
	private static final QName WSBPEL_DO_XSL_TRANSFORM = new QName(
			Namespaces.WSBPEL2_0_FINAL_EXEC,
	...
	private static XPathFunction resolveFunction(XPathContext context,
		...
		if (item instanceof NodeWrapper) {
			...
			if (node != null) {
				funcResolver = (JaxpFunctionResolver) node
						.getUserData(USER_DATA_KEY_FUNCTION_RESOLVER);
			}
	...
	private static Object evaluate(XPathFunction function, List arguments)
		...
		if (function == null) {
			throw new XPathFunctionException(
					"Unable to locate function in library");
		}
