	...
	private OExpression _compile(Expression xpath, boolean isJoinCondition)
		...
		if (ProcessModelChangeRegistry.getRegistry().isModelChanged()) {
			int id = ProcessModelChangeRegistry.getRegistry().getCorrectID(
					location);
			oexp = new OXPath10ExpressionBPEL20(
					_compilerContext.getOProcess(), _qnFnGetVariableData,
					_qnFnGetVariableProperty, _qnFnGetLinkStatus,
					_qnDoXslTransform, isJoinCondition, id);
		} else {
			oexp = new OXPath10ExpressionBPEL20(
					_compilerContext.getOProcess(), _qnFnGetVariableData,
					_qnFnGetVariableProperty, _qnFnGetLinkStatus,
					_qnDoXslTransform, isJoinCondition);
		}
