 ...
 * {@link ResourceFinder} interface for loading resources. This class is used
 * for XSD-Schema capture which uses the Xerces schema model.
 * 
...
public class WsdlFinderXMLEntityResolver implements XMLEntityResolver {
	 ...
	 * be returned so that a "default" resolution mechanism can be used;
	 * however, we don't necessarily want to use this default mechanism.
	 */
	 ...
	 * 
	 * @param finder
	 *            {@link ResourceFinder} implementation.
	 * @param baseURI
	 *            the base URI against which all relative URIs are to be
	 *            resolved; typically this is the system URI of the WSDL
	 *            containing an embedded schema
	 */
	public WsdlFinderXMLEntityResolver(ResourceFinder finder, URI baseURI,
			Map<URI, byte[]> internalSchemas, boolean failIfNotFound) {
		_wsdlFinder = finder;
		_baseURI = baseURI;
	...
	public XMLInputSource resolveEntity(XMLResourceIdentifier resourceIdentifier)
		...
		if (resourceIdentifier.getLiteralSystemId() == null) {
			...
			if (__log.isDebugEnabled())
				__log.debug("resolveEntity: no schema location for "
						+ resourceIdentifier.getNamespace());
			try {
				if (_internalSchemas.get(new URI(resourceIdentifier
						.getNamespace())) != null) {
					src.setByteStream(new ByteArrayInputStream(_internalSchemas
							.get(new URI(resourceIdentifier.getNamespace()))));
					return src;
				}
			} catch (URISyntaxException e) {
				__log.debug("resolveEntity: no schema location an no known namespace for: "
						+ resourceIdentifier.getNamespace());
			}
			return null;
		} else {
			location = _wsdlFinder.resolve(
					URI.create(resourceIdentifier.getBaseSystemId()),
					URI.create(resourceIdentifier.getLiteralSystemId()));
		}

		if (__log.isDebugEnabled())
			__log.debug("resolveEntity: Expecting to find "
					+ resourceIdentifier.getNamespace() + " at " + location);
		...
		try {
			...
			throw new IOException("Unexpected error loading resource: "
					+ location);
		}
		return src;
	}
