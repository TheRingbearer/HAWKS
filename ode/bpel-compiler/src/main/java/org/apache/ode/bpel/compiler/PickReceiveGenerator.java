...
abstract class PickReceiveGenerator extends DefaultActivityGenerator {
	...
	protected static final PickGeneratorMessages __cmsgs = MessageBundle
			.getMessages(PickGeneratorMessages.class);
	 ...
	 * @return
	 */
	protected OPickReceive.OnMessage compileOnMessage(String varname,
		...
		if (ProcessModelChangeRegistry.getRegistry().isModelChanged()) {
			int id = ProcessModelChangeRegistry.getRegistry().getCorrectID(
					xPath);
			onMessage = new OPickReceive.OnMessage(_context.getOProcess(), id);
		} else {
			onMessage = new OPickReceive.OnMessage(_context.getOProcess());
		}
		...
		onMessage.operation = _context.resolveMyRoleOperation(
		...
		if (onMessage.operation.getInput() != null
			...
			onMessage.variable = _context.resolveMessageVariable(varname,
					onMessage.operation.getInput().getMessage().getQName());
