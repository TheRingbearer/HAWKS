...
public class ExpressionValidatorFactory {
	private static final Log __log = LogFactory
			.getLog(ExpressionValidatorFactory.class);
	...
	public ExpressionValidatorFactory(Properties config) {
		...
		if (!validatorClass.equals("")) {
			try {
				_validator = (ExpressionValidator) getClass().forName(
	...
	private static class EmptyValidator implements ExpressionValidator {
		public void bpelImportsLoaded(Process source,
				CompilerContext compilerContext) throws CompilationException {
		}

		public void bpelCompilationCompleted(Process source)
				throws CompilationException {
		}

		public Object validate(Expression source, OVarType rootNodeType,
				Object requestedResultType) throws CompilationException {
			return null;
		}
	}
}
