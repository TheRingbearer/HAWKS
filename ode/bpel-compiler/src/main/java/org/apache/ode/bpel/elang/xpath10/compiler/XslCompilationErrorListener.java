...
/**
 * Reports errors that occured during Xsl sheets processing. This implementation
 * isn't built to be thread safe in case multiple compilations occur parrallely,
 * however this shouldn't occur.
 */
public class XslCompilationErrorListener implements ErrorListener {

	private static final Log __log = LogFactory
			.getLog(XslCompilationErrorListener.class);
	...
	private void recover(short severity, TransformerException exception) {
		...
		SourceLocation loc = exception.getLocator() != null ? new SourceLocatorWrapper(
