...
/**
 * JDBC-based implementation of the {@link DatabaseDelegate} interface. Should
 * work with most reasonably behaved databases.
 * 
...
public class JdbcDelegate implements DatabaseDelegate {
	...
	private static final String SAVE_JOB = "insert into ODE_JOB "
			...
			+ "mexId," + "processId," + "type," + "channel," + "correlatorId,"
			+ "correlationKeySet," + "retryCount," + "inMem," + "detailsExt"
			+ ") values(?, ?, ?, ?, ?," + "?," + "?," + "?," + "?," + "?,"
			+ "?," + "?," + "?," + "?," + "?" + ")";
	...
	public List<String> getNodeIds() throws DatabaseException {
		...
		try {
			con = getConnection();
			ps = con.prepareStatement(GET_NODEIDS, ResultSet.TYPE_FORWARD_ONLY,
	...
	private Long asLong(Object o) {
		if (o == null)
			...
			throw new IllegalStateException("Can't convert to long "
					+ o.getClass());
	}
	...
	private Integer asInteger(Object o) {
		if (o == null)
			...
			throw new IllegalStateException("Can't convert to integer "
					+ o.getClass());
	}
	...
	public List<Job> dequeueImmediate(String nodeId, long maxtime, int maxjobs)
		...
		try {
			...
			while (rs.next()) {
				...
				details.correlationKeySet = (String) rs
						.getObject("correlationKeySet");
				...
				if (rs.getObject("detailsExt") != null) {
					try {
						ObjectInputStream is = new ObjectInputStream(
								rs.getBinaryStream("detailsExt"));
						details.detailsExt = (Map<String, Object>) is
								.readObject();
						is.close();
					} catch (Exception e) {
						throw new DatabaseException(
								"Error deserializing job detailsExt", e);
					}
				}

				{
					...
					if (detailsExt.get("ckey") != null) {
						details.correlationKeySet = (String) detailsExt
								.get("ckey");
					}
					...
					if (detailsExt.get("correlatorId") != null) {
						details.correlatorId = (String) detailsExt
								.get("correlatorId");
					}
					if (detailsExt.get("retryCount") != null) {
						details.retryCount = Integer
								.parseInt((String) detailsExt.get("retryCount"));
					}
				}

				Job job = new Job(rs.getLong("ts"), rs.getString("jobid"),
						asBoolean(rs.getInt("transacted")), details);
				ret.add(job);
			}
			rs.close();
			ps.close();
	...
	private Dialect guessDialect() {
		...
		try {
			...
			if (metaData != null) {
				...
				__log.info("Using database " + dbProductName
						+ " major version " + dbMajorVer);
