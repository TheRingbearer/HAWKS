 ...
 * without loss of reliability while supporting distributed/clustered
 * configurations.
 * 
 * The design is based around three time horizons: "immediate", "near future",
 * and "everything else". Immediate jobs (i.e. jobs that are about to be up) are
 * written to the database and kept in an in-memory priority queue. When they
 * execute, they are removed from the database. Near future jobs are placed in
 * the database and assigned to the current node, however they are not stored in
 * memory. Periodically jobs are "upgraded" from near-future to immediate
 * status, at which point they get loaded into memory. Jobs that are further out
 * in time, are placed in the database without a node identifer; when they are
 * ready to be "upgraded" to near-future jobs they are assigned to one of the
 * known live nodes. Recovery is rather straighforward, with stale node
 * identifiers being reassigned to known good nodes.
 * 
...
public class SimpleScheduler implements Scheduler, TaskRunner {
	...
	/**
	 * Jobs scheduled with a time that is between [now, now+immediateInterval]
	 * will be assigned to the current node, and placed directly on the todo
	 * queue.
	 */
	long _immediateInterval = 30000;

	// @stmz
	private ZZBool zzbool;
	private ZJobList jobList;

	/**
	 * Jobs scheduled with a time that is between
	 * (now+immediateInterval,now+nearFutureInterval) will be assigned to the
	 * current node, but will not be placed on the todo queue (the promoter will
	 * pick them up).
	 */
	...
	/**
	 * Duration used to log a warning if a job scheduled at a date D is queued
	 * at D'>D+_warningDelay
	 */
	long _warningDelay = 5 * 60 * 1000;

	/**
	 * Estimated sustained transaction per second capacity of the system. e.g.
	 * 100 means the system can process 100 jobs per seconds, on average This
	 * number is used to determine how many jobs to load from the database at
	 * once.
	 */
	int _tps = 100;
	 ...
	 * dequeued or dispatched yet. Used to avoid cases where a job would be
	 * dispatched twice if the server is under high load and does not fully
	 * process a job before it is reloaded from the database.
	 */
	private ConcurrentHashMap<String, Long> _outstandingJobs = new ConcurrentHashMap<String, Long>();
	/**
	 * Set of Jobs processed since the last LoadImmediate task. This prevents a
	 * race condition where a job is processed twice. This could happen if a
	 * LoadImediate tasks loads a job from the db before the job is processed
	 * but puts it in the _outstandingJobs map after the job was processed . In
	 * such a case the job is no longer in the _outstandingJobs map, and so it's
	 * queued again.
	 */
	...
	private long _pollIntervalForPolledRunnable = Long.getLong(
			"org.apache.ode.polledRunnable.pollInterval", 10 * 60 * 1000);
	...
	public void setPollIntervalForPolledRunnable(
			long pollIntervalForPolledRunnable) {
		...
		return _exec.submit(new Callable<T>() {
			public T call() throws Exception {
				try {
					...
					__log.error(
							"An exception occured while executing an isolated transaction, "
									+ "the transaction is going to be abandoned.",
							e);
					return null;
				}
			}
	...
	public void registerSynchronizer(final Synchronizer synch)
	...
	public String schedulePersistedJob(final JobDetails jobDetail, Date when)
			throws ContextException {
		...
		return schedulePersistedJob(new Job(when.getTime(), true, jobDetail),
				when, ctime);
	}
	...
	public String scheduleMapSerializableRunnable(
		...
		return schedulePersistedJob(new Job(when.getTime(), true, jobDetails),
				when, ctime);
	}

	private String schedulePersistedJob(Job job, Date when, long ctime)
			throws ContextException {
		boolean immediate = when.getTime() <= ctime + _immediateInterval;
		boolean nearfuture = !immediate
				&& when.getTime() <= ctime + _nearFutureInterval;
		try {
			if (immediate) {
				...
				// Near future, assign the job to ourselves (why? -- this makes
				// it very
				// unlikely that we
				...
				// Not the near future, we don't assign a node-id, we'll assign
				// it
				// later.
	...
	public synchronized void stop() {
		...
		// graceful shutdown; any new submits will throw
				...
				if (job.transacted) {
					...
					try {
							...
							public Void call() throws Exception {
								if (job.persisted)
									if (!_db.deleteJob(job.jobId, _nodeId))
										throw new JobNoLongerInDbException(
												job.jobId, _nodeId);
								try {
									processor.onScheduledJob(jobInfo);

									// @hahnml CHECK if this is the right
									// position
									...
									// the next job
									// occurence
									if (job.detail.getDetailsExt().get(
											"runnable") != null
											&& !"COMPLETED"
													.equals(String
															.valueOf(jobInfo.jobDetail
																	.getDetailsExt()
																	.get("runnable_status")))) {
										// the runnable is still in progress,
										// schedule checker to 10
										// mins later
						...
						__log.error("Error while processing a "
								+ (job.persisted ? "" : "non-")
								+ "persisted job"
								+ (needRetry[0] && job.persisted ? ": "
										: ", no retry: ") + job, ex);

						// We only get here if the above execTransaction fails,
						// so that
						// transaction got
						// rollbacked already
						if (job.persisted) {
								...
								public Void call() throws Exception {
									if (needRetry[0]) {
										...
										if (retry <= 10) {
											...
											long delay = (long) (Math.pow(5,
													retry));
											job.schedDate = System
													.currentTimeMillis()
													+ delay * 1000;
											_db.updateJob(job);
											__log.error("Error while processing job, retrying in "
													+ delay + "s");
										} else {
											_db.deleteJob(job.jobId, _nodeId);
											__log.error("Error while processing job after 10 retries, no more retries:"
													+ job);
										}
									} else {
										_db.deleteJob(job.jobId, _nodeId);
					...
					processor.onScheduledJob(jobInfo);
					// @hahnml CHECK if this is the right position
					// @stmz: job was executed. next one can be executed.
					zzbool.setCanRun(true);
				}
				return null;
			} finally {
				// the order of these 2 actions is crucial to avoid a race
				// condition.
	...
	protected void runJob(final Job job) {
		...
		final Boolean bool = (Boolean) job.detail.detailsExt.get("bool");

		// @stmz: we can not execute the job right now
		// add this job to the list in ZJobList and kill the DB entry (if it
		// exists)
			...
			if (job.transacted) {
				if (job.persisted) {
					try {
						...
						__log.debug("Error while adding job to jobList: " + job);
					}
				}
			}
			_outstandingJobs.remove(job.jobId);
	 ...
	 * dispatches the runnable to a runnable delegate thread and schedules
	 * itself to a later time.</li>
	 * <li>2. The runnable is running; the poller job re-schedules itself every
	 * time it sees the runnable is not completed.</li>
	 * <li>3. The runnable failed; the poller job passes the exception thrown on
	 * the runnable down, and the standard scheduler retries happen.</li>
	 * <li>4. The runnable completes; the poller persistent does not re-schedule
	 * itself.</li>
	 * <li>5. System powered off and restarts; the poller job does not know what
	 * the status of the runnable. This is handled just like the case #1.</li>
	 * </ul>
	 * <p/>
	 * There is at least one re-scheduling of the poller job. Since, the
	 * runnable's state is not persisted, and the same runnable may be tried
	 * again after system failure, the runnable that's used with this polling
	...
	public boolean isTransacted() {
		...
		try {
			...
			throw new ContextException(
					"Internal Error: Could not obtain transaction status.");
		}
	}
	...
	boolean doLoadImmediate() {
		...
		try {
			...
			final int batch = Math.min(
					(int) (_immediateInterval * _tps / 1000), _todoLimit
							- _outstandingJobs.size());

			// jobs might have been enqueued by #addTodoOnCommit meanwhile
			if (batch <= 0) {
				if (__log.isDebugEnabled())
					__log.debug("Max capacity reached: "
							+ _outstandingJobs.size()
			...
			if (__log.isDebugEnabled())
				__log.debug("loading " + batch + " jobs from db");
			jobs = execTransaction(new Callable<List<Job>>() {
				public List<Job> call() throws Exception {
					return _db.dequeueImmediate(_nodeId,
							System.currentTimeMillis() + _immediateInterval,
							batch);
				}
			});
			if (__log.isDebugEnabled())
				__log.debug("loaded " + jobs.size() + " jobs from db");
			...
			if (delayedCount > 0) {
				__log.warn("Dispatching jobs with more than "
						+ (_warningDelay / 60000)
						+ " minutes delay. Either the server was down for some time or the job load is greater than available capacity");
			}
	...
	void enqueue(Job job) {
		// @hahnml: CHECK if this change works. The idea is that we check if the
		// job was already added to the jobList or not.
		// In some cases it is possible that the LoadImmediateTask and
		// addTodoOnCommit() want to add the job both to the jobList. This
		// happens if the job
		// was scheduledPersisted and will be deleted from DB and added to
		// jobList (in runJob() with bool=false) while a LoadImmediateTask is
		// running parallel in background and load the job from DB to enqueue
		// it.
		if (_processedSinceLastLoadTask.get(job.jobId) == null
				&& !jobList.containsHistory(job.jobId)) {
			...
			if (__log.isDebugEnabled())
				__log.debug("Job " + job.jobId
						+ " is being processed (processed since last load)");
		}
	}
	...
	void recoverStaleNode(final String nodeId) {
		...
		try {
			...
			// Force a load-immediate to catch anything new from the recovered
			// node.
			doLoadImmediate();
	...
	private class LoadImmediateTask extends SchedulerTask {
		...
		public void run() {
			...
			try {
				...
				if (success)
					_todo.enqueue(new LoadImmediateTask(System
							.currentTimeMillis()
							+ (long) (_immediateInterval * .90)));
				else
					_todo.enqueue(new LoadImmediateTask(System
	...
	/**
	 * Upgrade jobs from far future to immediate future (basically, assign them
	 * to a node).
	 * 
	 * @author mszefler
	 * 
	...
	private class UpgradeJobsTask extends SchedulerTask {
		...
		public void run() {
			...
			// We could be too early, this can happen if upgrade gets delayed
			// due to
			// another
			// node
			if (_nextUpgrade.get() > System.currentTimeMillis()) {
				__log.debug("UPGRADE skipped -- wait another "
						+ (ntime - ctime) + "ms");
			...
			try {
				...
				long future = System.currentTimeMillis()
						+ (success ? (long) (_nearFutureInterval * .50) : 1000);
