...
public class GeronimoDelegateSupport extends DelegateSupport {
	...
	protected void initialize(TransactionManager txm) throws Exception {
		_ds = createGeronimoDataSource(txm,
				"jdbc:hsqldb:mem:" + new GUID().toString(),
				"org.hsqldb.jdbcDriver", "sa", "");
		setup();
		_del = new JdbcDelegate(_ds);
	}

	private DataSource createGeronimoDataSource(TransactionManager txm,
		...
		_connectionManager = new GenericConnectionManager(transactionSupport,
				...
				(RecoverableTransactionManager) txm, getClass().getName(),
