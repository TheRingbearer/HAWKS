...
public class SimpleSchedulerTest extends TestCase implements JobProcessor {
	...
	public void testConcurrentExec() throws Exception {
		...
		for (int i = 0; i < 10; i++) {
			...
			try {
				int jobs = _jobs.size();
				jobId = _scheduler.schedulePersistedJob(newDetail("123"),
						new Date(System.currentTimeMillis() + 200));
				Thread.sleep(100);
				// we're using transacted jobs which means it will commit at the
				// end
				// if the job is scheduled, the following assert is not valid
				// @seanahn
	...
	public void testNearFutureScheduling() throws Exception {
		...
		_txm.begin();
		try {
			_scheduler.schedulePersistedJob(newDetail("123"),
					new Date(System.currentTimeMillis() + 7500));
		} finally {
			_txm.commit();
		}
	...
	public void testRecovery() throws Exception {
		...
		_txm.begin();
		try {
			_scheduler.schedulePersistedJob(newDetail("immediate"), new Date(
					System.currentTimeMillis()));
			_scheduler.schedulePersistedJob(newDetail("near"),
					new Date(System.currentTimeMillis() + 1100));
			_scheduler.schedulePersistedJob(newDetail("far"),
					new Date(System.currentTimeMillis() + 2500));
		} finally {
			_txm.commit();
		}
	...
	public void onScheduledJob(final JobInfo jobInfo)
