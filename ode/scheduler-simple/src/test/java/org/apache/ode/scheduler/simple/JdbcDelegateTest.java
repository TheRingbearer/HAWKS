...
public class JdbcDelegateTest extends TestCase {
	...
	public void testUpgrade() throws Exception {
		for (int i = 0; i < 200; ++i)
			_del.insertJob(
					new Job(i, "j" + i, true, new Scheduler.JobDetails()),
					null, false);
	...
	public void testMigration() throws Exception {
		...
		j1.getDetailsExt().put("pid",
				new QName("http://test1", "test2").toString());
		...
		Scheduler.JobDetails j2 = _del.dequeueImmediate("m", 10000L, 1000).get(
				0).detail;
		...
		assertEquals(j2.getCorrelationKeySet().toCanonicalString(),
				(Object) "@2[some~001~002]");
