...
class DeploymentUnitDir {
	...
	// @hahnml:
	private volatile ChangeDocument _mc;
	...
	private static final FileFilter _endpointFilter = new FileFilter() {
		public boolean accept(File path) {
			// endpoint-configuration.properties is deprecated, keep it for
			// backward compatibility
			return (path.getName().endsWith(".endpoint") || path.getName()
					.equals("endpoint-configuration.properties"))
					&& path.isFile();
		}
	};
	...
	DeploymentUnitDir(File dir) {
		...
		try {
			...
			__log.info("Can't initialize beans.xml application context " + e
					+ " for " + _name);
		}
	 ...
	 * If we don't, starts compilation.
	 */
	void compile() {
	...
	void scan() {
		...
		for (TDeployment.Process p : getDeploymentDescriptor().getDeploy()
				.getProcessList()) {
	...
	private void compile(final File bpelFile) {
		...
		// BPEL 1.1 does not suport the <import> element, so "global" WSDL needs
	...
	public boolean equals(Object obj) {
		if (!(obj instanceof DeploymentUnitDir))
			return false;
		return ((DeploymentUnitDir) obj).getDeployDir().getAbsolutePath()
				.equals(getDeployDir().getAbsolutePath());
	}
	 ...
	 * directory and its subdirectories. Files are ordered lexicographically but
	 * for each directory, files come before its sudirectories.
	 * <p>
	...
	public List<File> getEndpointConfigFiles() {
		return FileUtils
				.directoryEntriesInPath(getDeployDir(), _endpointFilter);
	}
		...
		if (_dd == null) {
			...
			try {
				...
				throw new ContextException(
						"Couldn't read deployment descriptor at location "
	...
	public DocumentRegistry getDocRegistry() {
		if (_docRegistry == null) {
			...
			WSDLFactory4BPEL wsdlFactory = (WSDLFactory4BPEL) WSDLFactoryBPEL20
					.newInstance();
			...
			for (File file : wsdls) {
				...
				try {
					_docRegistry.addDefinition((Definition4BPEL) r
							.readWSDL(new WSDLLocatorImpl(rf, uri)));
				} catch (WSDLException e) {
					throw new ContextException(
							"Couldn't read WSDL document at " + uri, e);
				}
			}
		}
		return _docRegistry;
	...
	private Map<String, Object> prepareCompileProperties(File bpelFile) {
		List<Process> plist = getDeploymentDescriptor().getDeploy()
				.getProcessList();
		for (Process process : plist) {
			...
			if (bpelFile.getName().equals(process.getFileName())) {
				Map<QName, Node> props = ProcessStoreImpl
						.calcInitialProperties(_properties, process);
	...
	private File findBpel11Wsdl(File bpelFile) {
		...
		for (Process process : plist) {
			...
			if (!bpelFile.getName().equals(process.getFileName()))
				continue;
			if (process.getBpel11WsdlFileName() == null
					|| "".equals(process.getBpel11WsdlFileName()))
				return null;

			return new File(bpelFile.getParentFile(),
					process.getBpel11WsdlFileName());
		}
		return null;
	}
	...
	// @stmz: returns a list of wsdl files
	...
	public ChangeDocument getChangeDocument() {
		if (_mc == null) {
			File mcLocation = new File(_duDirectory, "modelChanges.xml");
			...
			if (mcLocation.exists()) {
				...
				try {
					...
					_mc = ChangeDocument.Factory.parse(mcLocation, options);
				} catch (Exception e) {
					throw new ContextException(
							"Couldn't read model change document at location "
									+ mcLocation.getAbsolutePath(), e);
				}
