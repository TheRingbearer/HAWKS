 ...
 * Hibernate's {@link org.hibernate.Session} interface.
 * 
...
public class ConfStoreConnectionHib implements ConfStoreConnection {
	...
	private static final Log __log = LogFactory
			.getLog(ConfStoreConnectionHib.class);
	...
	public ProcessConfDAO getProcess(QName pid) {
		try {
			return (ProcessConfDaoImpl) _session.get(ProcessConfDaoImpl.class,
					pid.toString());
	...
	public DeploymentUnitDAO getDeploymentUnit(String name) {
		try {
			DeploymentUnitDaoImpl du = (DeploymentUnitDaoImpl) _session.get(
					DeploymentUnitDaoImpl.class, name);
			return du;
		} catch (HibernateException e) {
			__log.error("DbError", e);
			throw e;
	...
	public long getNextVersion() {
		VersionTrackerDAOImpl vt = (VersionTrackerDAOImpl) _session
				.createQuery("from VersionTrackerDAOImpl v ").uniqueResult();
		if (vt == null)
			return 1;
		else
