...
/**
 * This is a simple MessageExchangeContext implementation that only supports a
 * set of "well known" portTypes used for testing.
 * 
 * 
 */
public class MessageExchangeContextImpl implements MessageExchangeContext {
	...
	private void invokeProbeService(PartnerRoleMessageExchange prmx) {
		...
		if (elm1 != null && elm2 != null) {
			...
			final Message response = prmx.createMessage(prmx.getOperation()
	...
	private void invokeFaultService(PartnerRoleMessageExchange prmx) {
		...
		String ind1 = prmx.getRequest().getPart("faultIndicator1")
				.getTextContent();
		String ind2 = prmx.getRequest().getPart("faultIndicator2")
				.getTextContent();
		String inputData = prmx.getRequest().getPart("faultData")
				.getTextContent();

		StringBuffer faultData = new StringBuffer(
				"<message><errorID>FA-1</errorID><errorText>");
		faultData.append(inputData);
		faultData.append("</errorText></message>");

		StringBuffer responseData = new StringBuffer(
				"<message><faultName>FA-NoFault</faultName><faultData>");
		...
		// TODO: Question - how does one set parts that are of a simple xsd
		// type?
		...
		if (ind1.equals("yes")) {
			...
			if (ind2.equals("yes")) {
				prmx.replyWithFault(new QName(FAULT_NS, "FaultMessage2"),
						faultMsg);
			} else {
				prmx.replyWithFault(new QName(FAULT_NS, "UnKnownFault"),
						faultMsg);
			}
		}
