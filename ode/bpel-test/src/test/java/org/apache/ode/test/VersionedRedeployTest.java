...
public class VersionedRedeployTest extends BPELTestAbstract {
	...
	public void testInstancePersistence() throws Throwable {
		...
		Assert.assertEquals(1, _cf.getConnection().getProcess(qName1)
				.getNumInstances());
		...
		Assert.assertEquals(1, _cf.getConnection().getProcess(qName1)
				.getNumInstances());
		Assert.assertEquals(1, _cf.getConnection().getProcess(qName2)
				.getNumInstances());
		...
		Assert.assertEquals(1, _cf.getConnection().getProcess(qName2)
				.getNumInstances());
		Assert.assertEquals(1, _cf.getConnection().getProcess(qName3)
				.getNumInstances());
	}
