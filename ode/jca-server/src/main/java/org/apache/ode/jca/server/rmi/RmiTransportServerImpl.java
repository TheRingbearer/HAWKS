...
public class RmiTransportServerImpl implements OdeRemote {
	private static final Log __log = LogFactory
			.getLog(RmiTransportServerImpl.class);
	...
	public synchronized void start() throws RemoteException {
		...
		try {
			...
			__log.debug("Could not create registry on port " + _port
		...
		__log.debug("Bound JCA server as \"" + _id + "\" on registry port "
				+ _port);
	}
	...
	public synchronized OdeTransportPipeRemote newPipe() throws RemoteException {
		RmiPipeServerImpl pipe = new RmiPipeServerImpl(this,
				_connProvider.createConnectionObject(),
				_connProvider.getConnectionIntefaces());
		OdeTransportPipeRemote remote = (OdeTransportPipeRemote) UnicastRemoteObject
				.exportObject(pipe, 0);
		pipe.remote = remote;
		_pipes.add(pipe);
		return remote;
