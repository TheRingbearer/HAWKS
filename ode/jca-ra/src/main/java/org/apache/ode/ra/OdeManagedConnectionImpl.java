...
class OdeManagedConnectionImpl implements ManagedConnection {
	...
	public OdeManagedConnectionImpl(OdeTransportPipe pipe, Subject subject,
		...
		try {
			...
			throw new ResourceException(
					"Unable to obtain interface names from server.", e1);
		}
		...
		for (int i = 0; i < classNames.length; ++i)
			try {
				...
				throw new ResourceException("Connection class " + classNames[i]
	...
	public Object getConnection(Subject subject,
			ConnectionRequestInfo connectionRequestInfo)
			throws ResourceException {
		OdeConnectionImpl conn = new OdeConnectionImpl(subject,
				connectionRequestInfo);
		_connections.add(conn);
		_activeConnection = conn;
		conn.associate(this);
		return Proxy.newProxyInstance(getClass().getClassLoader(),
				_connectionClasses, conn);
	}
	...
	void connectionClosed(OdeConnectionImpl odeConnection) {
		_eventListenerSupport.connectionClosed(new ConnectionEvent(this,
